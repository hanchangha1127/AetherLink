...
The current non-security G6/G7 lane builds an isolated unsealed Release app and
matching dSYM from one stable source snapshot. It stages the app, dSYM, and a
canonical source receipt together, then atomically replaces the three-
entry `dist/unsealed-package-only` generation. An independent reader binds the
receipt to the current source and ledger before reading the closed physical
output trees. The reader reports each generation's app/dSYM sizes, closed-tree
SHA-256 identities, thin arm64 architecture, shared UUID, and ledger version
`1.0.0+24`. This generic output gate deliberately does not freeze a future
generation's linked identities because a later clean local link may produce a
new UUID; the producer-bound current-run lifecycle successor below instead
derives and validates one observed generation inside that same run. Stable no-
follow reads cover the receipt and source on both sides of the app/dSYM
readback, and reject drift or partial generations. The app has no outer
`_CodeSignature`/`CodeResources`; this is not a claim that the Mach-O lacks a
linker-generated signature. It is local build/readback evidence, not bit-for-
bit build reproducibility, signing, notarization, installation, launch,
device/network, hosted CI, canonical G6/G7 exit, RC/GA, or a V1 production
release.
...
Android 13+ app-language startup now treats a supported nonempty
`LocaleManager.applicationLocales` value as authoritative. A legacy explicit
language carried forward from API 26 through 32 is the one exception when the
platform list is initially empty: current source durably records a pending
migration, writes that exact locale, reads the platform value back, and only
then marks the migration complete. A retry after process interruption reuses
the pending value without another persistence write. Once complete, an empty
platform list means Follow system and cannot resurrect the legacy language.
API 26 through 32 neither start the marker nor write the platform list.
Explicit user language actions remain the ordinary OS-write path, and explicit
English remains distinct from empty/system English. The first localized frame
uses the reconciled persisted/platform snapshot. Three focused regressions plus
three API 32/33/36 production-lifecycle regressions and the 45-test Android
product selector pass. The API 33 lifecycle now covers legacy migration,
recreation and cold launch, external clear without resurrection, and an
external Korean override. The complete app JVM suite also passes 1,226/1,226 through an
exact 19-class `--rerun-tasks` run. A pre-run marker snapshots every declared
Android input path, byte stream, and mode; the result gate requires the exact
19-report set, 1,226 unique nonempty test cases, and testcase-manifest SHA-256
`cc3ea9e2d72ca96e7f937b22a893d8cdaf38c409564ac8baecc5b947b8aa1b78`,
then writes and independently reads a canonical binding over the marker and
every report byte. Equality is rejected at the source/report, marker/report,
and report/binding freshness boundaries. The drawer regression resets each
locale to the app title and
traverses `top -> header -> detail -> header` inside the actual history
viewport above the fixed Settings footer, re-reading geometry at each step and
preserving the exact merged accessibility summary. On a disposable API 36.1 emulator,
external Korean/Japanese, in-app French, explicit English, Follow system,
repeated cold launches, the real camera denial/recovery/settings path, and 200%
font-scale reachability all passed without a FATAL or ANR. The migration proof
is JVM/Robolectric evidence, not a physical OS-upgrade run. The emulator work
is not physical optical-camera, TalkBack, OEM, signing, or production-release
proof.
...
An earlier post-Build 24 source snapshot also passed an isolated offline
strict-lock local Release qualification. Clean Android `assembleRelease`, `bundleRelease`,
...
and the 10,684,471-byte AAB has SHA-256
`736f4debe24ada3bdbfd51055a56dcd4d5ccae103568d92183d9e74c696bb62f`.
The 253-file source snapshot stayed
...
then passed independent 29-member readback. Its 167,578,488-byte ZIP has
SHA-256
...
a duplicate CAMERA request. A dedicated G5 qualification class now reports
three independent results at exactly 100%, 150%, and 200%, exercising scanner,
drawer, Chat, Settings pairing, Memory, and chat history across all five app
locales with full state coverage in English and Korean. QR-scanner close,
flashlight, cancel, and permission actions now hold a 48 dp minimum. The exact
Android product selector therefore passes 45/45 across twelve result classes.
The result checker resolves focused contracts by class name and rejects JUnit
XML older than the workflow, checker, Android build inputs, production sources,
or complete app test-source graph.
This is post-Build 24
...
policy, the physical permission dialog, camera, optical QR, physical/OEM
typography, TalkBack, or release installation.
...
python3 -B script/check_release_artifact_archive.py --android-build-outputs
```

The second command independently opens the current APK, AAB, output metadata,
baseline profiles, R8 mapping set, SDK-dependency report, and native-library
intermediates. It rechecks the final APK/AAB manifests and bundle resources
with aapt2 and bundletool and does not create a release archive or require a
macOS package.
...
contract plus the 19-test A/B contract. Pull requests assemble the Android
Release APK and AAB once, run release lint, and independently read those build
outputs with strict dependency locks. The macOS
lane also validates the checked-in offline compliance catalog and runs the
exact 22-test deterministic SPDX/license render and independent reconstruction
manifest with zero skips; it never invokes the network refresh command. Pushes to `main`
additionally compile the macOS Release app and execute the exact
19-class complete Android app unit lane through its pre-run source marker and
post-run result binding/readback. Android then executes two source-bound,
offline forced Release graphs, independently reads their typed A/B result, and
directly reads the final B outputs before diagnostics.
The macOS job also runs 54 clean-checkout-safe current-unsealed lifecycle
contract tests on pull requests and `main`: 35 runner tests, 11 portable
evidence-checker snapshot/schema tests, and eight portable current-run checker
tests. They cover dynamic running-code CDHash binding, post-reap log
revalidation, held-source cross-binding, and same-byte replacement during
snapshot acquisition without reading ignored local lifecycle evidence. On
`main`, the fresh unsealed package readback is followed by the macOS Release
diagnostics producer/readback, then the two-observation lifecycle producer and
independent current-run readback in a private `.build` directory.
The macOS job first lists the current test graph and requires the focused
filter to resolve to the exact reviewed 217-test identity manifest before it
runs those tests. The macOS job uses the supported `macos-26` image and the
exact Xcode 26.6 toolchain used by the current local verification. The workflow
does not use a device, emulator, live provider, external-network smoke,
credential, signing step, artifact upload, release publication, or
...
The standalone full Android result contract uses this order:

```bash
python3 -B script/check_product_ci.py --prepare-android-full-test-run
...
`script/check_no_device_quality.sh` owns the same exact marker, 19-class
runner, binding, and readback sequence. The focused 45-test result contract
remains separate and requires the exact twelve-report set plus testcase
identity manifest; a same-count method substitution is rejected.
...
readback remains an explicit local full-checker operation.
The focused Swift result contract snapshots only the 27 explicit
`Package.swift` target roots (216 source files), excludes sibling build
outputs, and publishes the serial console only after exit-zero, exact testcase,
and marker/source freshness validation. The retained run passes 217/217, and
subprocess mutation tests prove failed or malformed runs cannot replace the
canonical log.
...
follow-ups. The current 217-test Swift identity preflight and execution,
45-test focused Android exact-manifest result, and 1,226-test complete Android
marker/binding/readback have local evidence only. Test selectors limit which
