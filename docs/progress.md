...
- Android first-run pairing now keeps local discovery out of the primary QR onboarding surface. Discovery appears only while resolving an identity-only QR route or when a trusted runtime already exists, with the full diagnostics path still available under Settings.
...
- The macOS Remote Relay panel now blocks saving loopback, private-network, or `.local` relay hosts from the GUI, because those addresses cannot serve as the mutually reachable relay address for different-network client devices. Environment/CLI relay settings remain available for local smoke tests and controlled development runs.
...
- The current relay smoke test confirms the relay path can carry authenticated pairing, challenge-response auth, runtime health, model list, streaming chat, and cancel generation. If a real different-Wi-Fi device still cannot connect, the first checks are whether the relay host is reachable from both networks, whether the runtime shows connected/waiting/failed in Remote Relay status, and whether the client scanned a fresh relay QR containing the current `relay_id` and `relay_secret`.
- Android can now treat a fresh relay QR from the same pinned runtime identity as a route refresh for an already trusted runtime. This lets an existing trusted device save the relay host/id/secret and reconnect through the relay without depending on the stale same-network private IP, while still keeping model access behind AetherLink Runtime.
