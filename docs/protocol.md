...
Android correlates each memory-summary list, generate, approve, and dismiss exchange to the exact client-generated request id, channel identity, connection generation, and authenticated runtime-authority generation. A required or manual list refresh encountered behind pending actions is retained and emitted once after the last action terminates through success, malformed result, protocol error, or send failure. Authentication revocation, successful reauthentication, channel replacement, disconnect, and receive failure clear all pending correlations, the retained refresh, and action UI state. Correlated `error` payloads apply the closed error shape before authentication handling, and unknown metadata is `invalid_payload`, not an authentication transition. Client-generated namespaced summary request ids allow arbitrarily delayed errors to be ignored after bounded list tombstones are evicted; this namespace is correlation metadata only and grants no protocol capability.
