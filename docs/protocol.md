...
The historical libjuice v1.7.2 candidate failed its mandatory Phase A source audit before compilation. Regular nomination and RFC 7675 cadence mechanisms exist, but the source fails the profile's authenticated role/error handling, entropy, redirect, pre-I/O numeric authorization, teardown, parser strictness, logging, and pre-identity payload boundaries. No library behavior was executed to make that determination.

Later one-shot authorities acquired exact official libnice 0.1.23 and GLib 2.64.2 artifacts for bounded read-only static audit. Decision-v6, progress-v8, and handoff-v9 reject `libnice-0.1.23-glib-c-abi` before compilation after four P1 source blockers; no networking library is selected. The remaining dependency set was identified but not acquired. No P2P/NAT wire message, capability, candidate exchange, ICE/STUN/TURN runtime path, socket, or application payload route is activated. Source execution, compiler/archive/linker use, runtime or harness networking, Phase B, production authority, and deployment remain separate and closed.
...
One exact-current terminal consumes each pending mutation and cancels its local deadline before payload publication or error handling. This includes a valid result or error, a malformed or unknown-metadata terminal, an operation/target/expected-field mismatch, and an exact request-bound send failure. The production receive loop forwards its captured source channel and connection generation through envelope dispatch unchanged; it never re-stamps ingress with current authority. Late or duplicate terminals, stale request ids, wrong-channel frames, old connection generations, prior-authentication frames, and delayed failures from a replaced authority are inert: they cannot consume a newer mutation, change authentication, mutate memory or device storage, publish an error, or trigger reconciliation.

Mutations for the same logical target are serialized while one is pending; a repeated create with the same normalized content and update/enable/delete operations for the same entry id do not dispatch concurrently. Independent targets may remain pending concurrently and close independently. The production client applies a 15-second local timeout to each mutation. Timeout consumes only that exact pending mutation, publishes the bounded local timeout error, marks fresh unqueried `memory.list` reconciliation as required, and does not automatically retry the mutation. Independent sibling mutations defer that reconciliation until the final sibling closes. If a later mutation boundary invalidates an in-flight required reconciliation, the requirement is preserved and a fresh request is reissued after all siblings close. A late terminal after timeout is inert, while a later explicit user mutation receives a fresh request id and authority record.

Mutation dispatch invalidates any older pending `memory.list` correlation. A pre-dispatch list response delivered while the mutation remains pending is therefore inert. A distinct full-list request started while a mutation remains pending may publish first, after which the mutation result applies its exact delta; if the mutation terminal wins the ordering, it invalidates that distinct list correlation before publishing success or error. A delayed full-list response therefore cannot erase a completed add, revert an enable change, or resurrect a completed delete.

Disconnect, receive-failure connection closure, successful reauthentication, authentication revocation, and ViewModel clear cancel all mutation deadlines and remove all pending mutation authority. This is Android-local acceptance and lifecycle hardening only: it changes no protocol message, JSON schema or field, capability, host mutation behavior, provider route, socket, or network authority. Focused no-device coverage passes the five-test set `memoryUpsertResultRejectsUnknownMetadataBeforeMemoryMutation`, `memoryDeleteResultRejectsUnknownMetadataBeforeMemoryMutation`, `memoryMutationResultsRequireExactCurrentAuthorityAndExpectedPayload`, `memoryMutationErrorsRequireExactCurrentAuthorityAndConsumeOnce`, and `memoryMutationSendFailureAndLifecycleCleanupRequireExactAuthority`.

That no-device proof does not establish physical Android behavior, optical QR, peer receipt or exactly-once host mutation, real lost-response recovery beyond the deterministic local timeout, live-provider or live-network behavior, production relay/P2P/NAT, controlled-spike Phase B, or deployment. Current verification records all 600 `RuntimeClientViewModelTest` tests and all 1,074 Android app JVM tests passing. The successful 12,101-line default aggregate in `build/qa/check-no-device-quality-memory-mutation-authority-final-reviewed-20260717.log` includes separate generated-JUnit execution proofs for the canonical five authority tests and the production 15-second deadline test.

## Android `runtime.health` Current-Request Authority

Android sends each authenticated `runtime.health` request with a client-generated `runtime-health-<UUID>` request id. The request payload and response schema are unchanged. Before dispatch, the ViewModel records the request id together with the exact channel object, connection generation, and authenticated runtime-authority generation. A later health request supersedes the earlier pending request instead of allowing both to publish.

A `runtime.health` result or request-specific `error` may act only when all four correlation values still match the latest pending request and the session remains authenticated. Stale request ids, duplicate terminals, wrong-channel frames, old connection generations, and prior-authentication frames are inert before payload validation, error publication, authentication revocation, runtime-state mutation, or refresh fanout. Namespaced late errors and delayed send failures are also inert after supersession.

The exact current result is terminal even when its payload is malformed or contains unsupported metadata. A valid current result clears correlation before publishing health state and requesting model, chat-session, memory, memory-summary, and research refreshes once. The exact current protocol error or send failure also clears correlation before publishing its bounded error. Disconnect, authentication revocation, successful reauthentication, receive-failure connection closure, and ViewModel clear remove pending health authority.

This is Android-local acceptance hardening. It adds no message, JSON field, capability, provider route, client execution path, socket/network authority, controlled-spike Phase B, production networking, or deployment authority. Current proof is Android JVM no-device evidence, not physical-device, optical QR, peer receipt, live-provider, external-network, production relay/P2P, or NAT-traversal proof.

## Memory-Summary Durable Terminal Decision Linearization

For one owner-scoped current v2 memory-summary draft, approval and dismissal are mutually exclusive terminal decisions. The production JSONL memory store acquires the shared canonical-path lock and OS coordination-file write lock, rereads that owner's complete event history in physical append order, decides the existing terminal state, and appends at most one approval upsert or dismissal event before releasing the lock. Independent store instances and processes therefore cannot both commit opposite decisions for the same owner and draft, and wall-clock rollback cannot reorder a later edit, disable, or delete before the approval it followed.

Repeating the committed decision is idempotent. A repeated dismissal returns the original `dismissed_at` and does not append another event. A repeated approval returns the current surviving approved entry and does not overwrite later user edits or a later disabled state. If that approved entry was deleted or no longer retains the matching source-bound draft provenance, approval retry returns `memory_summary_draft_unavailable` instead of recreating it. Attempting approval after dismissal, dismissal after approval, or either action against a legacy log containing contradictory terminal records also returns `memory_summary_draft_unavailable` without mutation.

The approval response remains the existing `memory.summary.draft.approve` shape, and dismissal remains the existing `memory.summary.draft.dismiss` shape. A memory-store implementation that does not explicitly provide atomic approval fails closed, and non-terminal persistence failures return sanitized `memory_store_unavailable`; neither path falls back to generic upsert or exposes the raw store error. This storage linearization adds no message, JSON field, capability, permission, source access, provider route, or network authority.

## Memory-Summary Source-Bound Draft Identity And Decision Commit Contract

The existing memory-summary `draft_id` string is opaque. Newly computed long-inactivity drafts use `long-inactivity:v2:<64 lowercase hex>`, where the digest is a host-computed, domain-separated and length-framed SHA-256 identity over session id/title/model, exact last activity, session message count, selected source count/range, every ordered source-pointer field, and deterministic summary preview. Naturally advancing `inactive_seconds` is not part of this identity. Clients must echo the id exactly and must not parse it, reconstruct it, or infer source authority from its components. This changes no message type, JSON field, schema branch, or capability.

Approve and dismiss first resolve the exact current owner-scoped v2 draft, then recompute it inside the chat store's source-current transaction while performing the memory-store mutation. If the source changed before that transaction, the decision returns `memory_summary_draft_stale` or, when the old id is no longer present at initial lookup, `memory_summary_draft_unavailable`; it does not mutate memory or dismissal state.

Legacy session/time/count ids remain readable only inside historical persisted records. They do not suppress or overlay a current v2 draft and cannot authorize generate, approve, or dismiss because the old id does not prove title, model, ordered source pointers, or preview identity. Draft lookup and stale guards run before installed-model resolution, so requests carrying a v1 id fail before provider model-catalog access, backend dispatch, or memory mutation. The runtime returns the deterministic v2 draft for explicit regeneration and review.

Any committed title change, including automatic title generation scheduled after `chat.done`, rotates the v2 id even when message count and transcript content are unchanged. A client holding the older id must treat `memory_summary_draft_unavailable` as a stale review, list again, and present the replacement draft before generation or decision. This is intentional source-identity behavior rather than an expiry extension or compatibility fallback.

## Android Memory-Summary Request Deadlines

Android applies a 15-second local deadline to `memory.summary.drafts.list`, `memory.summary.draft.approve`, and `memory.summary.draft.dismiss` control requests created by the production ViewModel factory. `memory.summary.draft.generate` instead uses a 75-second local deadline because the host owns a 60-second whole-generation deadline; the remaining 15-second margin permits host timeout/error delivery. These deadlines are not wire fields. Each Job is keyed by request id and may act only while the same pending correlation still owns the exact channel object, connection generation, and authenticated runtime-authority generation.

Timeout closes only that request's pending/UI state and permits a new request. When multiple draft actions are pending, earlier deadlines retain the deferred list barrier and the last timed-out action sends one deferred list refresh exactly once. Successful, malformed, protocol-error, and send-failure terminals cancel their exact Job. Late result/error frames after timeout are inert and cannot mutate drafts or memory, replace the timeout error, send another refresh, or change authentication.

A valid generation terminal at 59,999 milliseconds is accepted after the control-request boundary and cancels the exact retained 75-second Job. Timeout-enabled protocol-error, malformed-result, and send-failure terminals also cancel their exact retained Jobs before closing local correlation. Android's generation deadline is a local fallback after the host contract, not permission to extend host generation or a new cancellation message.

Reauthentication, authentication revocation, disconnect, receive failure, and ViewModel clear cancel every outstanding memory-summary deadline and correlation. The replacement authority starts independent Jobs, so an old deadline cannot consume its requests. This is Android-local lifecycle hardening and does not change JSON schema, capabilities, host behavior, or authorization.

## Android Memory-Summary Terminal Correlation And Source Identity

Android treats `memory.summary.drafts.list`, `memory.summary.draft.generate`, `memory.summary.draft.approve`, and `memory.summary.draft.dismiss` result and error frames as correlated terminals. Request id, channel object identity, connection generation, and authenticated runtime-authority generation must all equal the pending operation. Wrong-channel and late-duplicate result/error frames are inert: they do not consume correlation, mutate review or memory state, request reconciliation, publish an error, or change authentication.

Generated draft source comparison binds draft id; session id, title, model, last activity, and message count; source message count and range; and exact source-pointer count, order, session id, message index, role, timestamp, and excerpt. Approved `entry.source` additionally binds canonical source kind and exact summary method. Only `inactive_seconds` is excluded from source identity because it advances with wall time. Generated summary method, generation time, and generated model id are output metadata and remain exact under the separate generation-result contract.

This tightens Android acceptance only and does not change the JSON schema or grant a capability. The proof is no-device Android JVM state-machine coverage, not physical Android, peer receipt, live-provider, external-network, production relay/P2P, NAT traversal, measurement, or deployment evidence.
...
The research instruction is the first bundled entry in the host-local `RuntimePromptSkillRegistry`: immutable id `research_brief_v1`, effect `prompt_only`, and revision `004a2e575e7c453853ee53521b45b4865c7caa7540c0a58786d49460199f3418`. Registry construction requires canonical bounded UTF-8 strings, an exact domain-separated SHA-256 revision, unique ids and revisions, and the sole allowlisted non-executing effect. The router resolves this compile-time id and exact revision before any research backend call and fails closed with `runtime_prompt_skill_unavailable` if unavailable. The injected text remains backend-only and is absent from chat storage, titles, history, notebook metadata, client payloads, and audit fields. This registry does not activate `prompt_skill_id`, `skills.*`, `permission.*`, `approval.*`, `audit.*`, or any tool, file, terminal, Python, network, MCP, web-search, or backend-control path.

The separate host-local `RuntimePermissionPolicyRegistry` contains no prompt or executor. Its sole bundled action is `models_pull_ollama_v1`, effect `provider_artifact_install`, decision `host_explicit_approval`, audit requirement `durable_redacted_required`, and revision `5969f34082e579a4e393bded6ce62706382e7376258b364c3afed0dbbcb163d3`. An internal `RuntimeHostApprovalCoordinator` owns only the bounded review queue, dual-clock expiry, exact registered-policy check, durable reservation, one-use terminal-commit capability, recovery, and storage-degraded state. The model-pull adapter owns `ModelPullDispatching`, Ollama result redaction, and preparation of an authority-bound publication closure. The coordinator invokes that closure with its one-use terminal commit; the closure must revalidate authority and permission, commit, and send while holding the same authority lock. This internal lifecycle core does not activate generic permission discovery, a generic executor registry, standing grants, client-selected action ids, a second production action, or any `permission.*`, `approval.*`, or `audit.*` wire message. The schema-v2 model-pull store and its legacy digest prefix remain unchanged.
...
20. Current clients keep uninstalled model rows disabled and do not advertise or send `models.pull`.
21. A legacy authenticated `models.pull` request enters the macOS-host-local review only when the durable broker is active; absent or unapproved work returns `model_pull_approval_required` with no provider call.
...
```json
{
  ...
  "payload": {
    "device_id": "client-device-id",
    "device_name": "Client Device",
    "client_capabilities": ["chat", "streaming", "attachments", "chat.source_attributions.v1", "chat.source_attribution.resolve.v1", "research.notebooks.v1", "research.notebooks.authoritative_sync.v1", "auth.challenge.runtime_capabilities.v1"]
  }
}
```

`hello.payload` accepts only `device_id`, `device_name`, `client_capabilities`, and optional transport-owned `transport_binding`. Only `device_id` is required, and it must be a non-blank string. When present, `device_name` must be a non-blank string, and `client_capabilities` must be an array of at most 64 unique non-blank strings. Malformed allowed fields return `invalid_payload` before challenge creation. Clients must not send challenge or response fields such as `nonce`, `signature`, `runtime_signature`, backend URLs, provider URLs, backend credentials, route tokens, relay secrets, workspace IDs, permission grants, source paths, source-control state, or direct-provider route material in this payload.

`auth.challenge.runtime_capabilities.v1` is an opt-in compatibility signal: it says the client can parse the optional closed `runtime_capabilities` array in `auth.challenge`. A runtime must omit that response field when the client did not advertise the signal. The current returned feature `memory.summary.approval_method.v1` means the client may send the optional `expected_summary_method` stale-review guard on `memory.summary.draft.approve`. These capability names grant no model, source, permission, network, or persistence authority.
...
Installed Ollama models are derived from the AetherLink Runtime calling Ollama `/api/tags` on the runtime host, with optional `/api/show` capability checks to classify chat vs embedding models. Installed LM Studio local models are derived from the AetherLink Runtime calling LM Studio native `GET /api/v1/models`, with fallback to OpenAI-compatible `GET /v1/models` only after native 404, 405, or 501. Local models are the main path. The normal client chat picker should show installed runtime-host-local chat models only. Ollama cloud/source metadata may remain in protocol data for compatibility, but it is not a default, recommendation, or normal chat selection path. Running status may be derived from backend metadata when available. The runtime does not invent recommended/default model cards when backend model lists are empty.

Provider catalog metadata is validated before it can populate this existing response shape. Ollama and LM Studio catalog/detail reads use true streaming ingestion with a 4 MiB response ceiling; an oversized positive `Content-Length` is rejected before reading and a chunked or inaccurate response stops at limit plus one. Ollama rejects malformed or duplicate-key `/api/tags`, `/api/ps`, and `/api/show` data, exact or canonical `:latest` duplicate identities, disagreeing `name`/`model` aliases, and invalid or conflicting direct/`model_info`/`parameters` context aliases. `/api/tags` and `/api/ps` admit at most 256 rows and their combined unique identity set authorizes at most 256 `/api/show` calls; cancellation propagates through that detail loop. A structurally untrusted or oversized show response excludes the model, while a context-only error omits the context value and preserves validated capabilities. Provider JSON numeric context fields decode as `Decimal`; the shared validator uses `NSDecimalRound` and exact equality to require a mathematically integral positive value no greater than 16,777,216 before integer conversion. Thus exact integral decimal and scientific forms remain valid, while `16777215.9999999999`, `16777216.0000000001`, and precision-rounded alias bypasses fail. LM Studio applies the same precision-safe rule plus strict duplicate-key and exact/NFC identity checks to native and fallback catalogs and requires byte-exact `key`/`id` agreement. Its provider-internal `loaded_instances` state is not published in this wire shape and is capped at 256 unique nonblank identifiers of at most 512 code points before unload dispatch and during polling. Fallback to `GET /v1/models` occurs only after native 404, 405, or 501, not after an oversized or malformed response, transport/auth failure, or another 4xx/5xx response. Native chat fallback is separate, occurs only after a pre-stream HTTP 400, 404, 405, 422, or 501 response, and never follows malformed native output. A successful native stream requires explicit `chat.end` observed in the main loop or returned by `parser.finish()`; malformed partial streams and clean EOF after a delta fail without OpenAI-compatible redispatch.

The response contains at most 256 models and is never truncated to fit. `id`, `name`, `provider_model_id`, and optional `remote_model` contain at most 512 Unicode code points; `qualified_id` contains at most 522. These strings must contain at least one code point outside the fixed shared blank/invisible set. `capabilities` contains at most 32 byte-exact-after-decoding unique values with the same content rule, each at most 128 Unicode code points; NFC/NFD-distinct values remain distinct. `size_bytes` is an exact integer in 0...9,223,372,036,854,775,807. The complete encoded plaintext must be at most 1,048,560 bytes so the relay's 16-byte authentication tag remains inside the 1 MiB frame. The shared schema, macOS publication boundary, and Android decoder enforce the same field limits, and the runtime enforces the aggregate frame limit. Any final catalog that violates them is rejected as `bad_backend_response` before partial wire publication.
...
- `context_window_tokens`: optional runtime-host-derived model context window hint. When present, it has passed the runtime's exact positive-`Int` validation and is no greater than 16,777,216. Ollama may omit a context-only invalid `/api/show` value while retaining otherwise trusted model metadata. LM Studio rejects invalid context catalog metadata, and a non-nil invalid value reaching final router publication rejects the whole catalog. The runtime may use a validated value to choose backend-side compaction budgets; clients should treat it as metadata and must not call provider APIs directly to derive or enforce it.
...
Runtime dispatch resolves an installed request to an exact provider and exact `provider_model_id` before a backend call. A direct provider receives that raw id; an aggregate backend receives the canonical provider-qualified form. Qualified aggregate routing then matches only that selected provider and exact raw id, not display `id`, `name`, or `:latest` aliases. A provider-native id that itself parses as `ollama:*` or `lm_studio:*` is ambiguous and invalid for both chat and embedding dispatch. The originally requested model remains the protocol-visible identity and is not rewritten to the provider-native dispatch id.
...
Direction: Legacy Client -> Runtime; Runtime -> Client legacy result or structured `error` after a macOS-host decision.

`models.pull` is a recognized legacy command routed through a macOS-host-local approval broker. Android does not advertise this capability, persist an uninstalled selection, or send this command. A valid authenticated request becomes an ephemeral host review and receives no immediate success. The runtime may call Ollama `/api/pull` only after the host explicitly confirms the review, the exact trust/authentication generation and transport binding are revalidated, and a durable redacted one-time `dispatch_reserved` audit transition commits. A client-side confirmation alone is not an authorization boundary. LM Studio downloads remain host-managed through LM Studio or `lms`.

Before creating that review, the runtime constructs a host-local permission claim for exact action `models_pull_ollama_v1` and revision `5969f34082e579a4e393bded6ce62706382e7376258b364c3afed0dbbcb163d3`. A length-framed, domain-separated digest binds the policy effect/decision/audit requirement, connection and request, authentication generation, trusted device and key, optional transport binding, resource kind, and authorized model. The broker requires the registered claim before durable intake and rechecks it before reservation and outcome preparation. The reservation capability is one-use and is invalidated synchronously when authorization returns, so retaining the callback cannot create a later dispatch reservation. A wrong, repeated, or concurrent receipt fails immediately without waiting for the in-flight persistence adapter. That violation invalidates the capability; if durability cannot be proven, the coordinator clears all pending in-memory reviews, blocks approve/dismiss, and enters fail-closed recovery before any later work. Persistence returns an explicit `expiredTerminalized` result when expiry was durably recorded; a thrown expiry without that proof is storage ambiguity and enters the same recovery state. Policy drift terminalizes as local `permission_changed` without provider dispatch. Schema v2 audit rows persist only the action id and policy revision in addition to existing redacted metadata; schema-v1 migration never restores unfinished execution.

`models.pull.payload` accepts only `model` and optional legacy `backend`. `model` must be NFC canonical printable ASCII, untrimmed, nonblank, Ollama-bound, and no more than 256 characters/UTF-8 bytes; legacy `backend`, when present, must be `ollama`. The shared JSON Schema, macOS router, and Android `ModelPullPayload` DTO enforce the same ASCII boundary, keeping the character ceiling identical to the runtime byte ceiling even though current Android clients do not send this command. It must not carry backend URLs, provider URLs, backend credentials, route tokens, relay secrets, requested route tokens, workspace IDs, permission grants, approval IDs/tokens, audit metadata, or direct-provider route material. Provider routing, credentials, approval state, and audit state remain runtime-host concerns.

The broker keeps at most 32 pending reviews for five minutes and performs at most one active provider dispatch. The current-authority `dispatch_reserved` commit is the irreversible provider-dispatch claim: dismissal, expiry, disconnect, trust removal, reauthentication, key replacement, transport-binding drift, duplicate request binding, concurrent approval, or durable-store failure that linearizes before that commit cannot start a provider call. A change after the claim cannot undo already authorized provider work, but the second authority check suppresses its wire result and records `result_suppressed`. Restart recovery terminalizes pending work and outcome-unknown reserved work without retry. The wire does not expose an approval-control namespace or approval identifier.

The shared schema retains the old `models.pull` result shape for wire compatibility with older runtime captures. After an approved successful dispatch, the runtime emits the authorized request model with fixed `status=completed`, `installed=true`, `backend=ollama`, and `provider=ollama`; provider-controlled success strings are never copied to the wire. Provider failures are reduced to a generic redacted structured error. Before result publication, the adapter calls `prepareOutcomePublication` to produce a closure without sending. The coordinator passes that closure a one-use terminal-commit capability. The required `RuntimeMessageSink` transport-context transaction serializes current binding validation, exact lifecycle/authentication/trust/permission checks, capability invocation, and send against binding mutation; the relay sink implements this transaction with its dedicated context lock. The capability rechecks wall and monotonic deadlines and commits the matching durable terminal audit with a timestamp no earlier than the reservation commit. Preparation failure, terminal-audit failure, stale authority or transport, duplicate commit use, publication delay across either deadline, or an expired review suppresses the wire result and records or recovers only a redacted local audit outcome. The current Android treats unsolicited results as stale and does not correlate this command.
...
Dismissed, expired, unavailable-broker, or otherwise unapproved result:
...
```json
{
  ...
  "payload": {
    "code": "model_pull_approval_required",
    "message": "Model downloads require approval on the AetherLink Runtime host.",
    "retryable": false
  }
}
```

No provider call occurs for the error path above. Approved success preserves the legacy result shape, while duplicate or stale completion remains one-shot and may be suppressed. Clients must keep the previous installed selection and only send `chat.send` with models where `installed` is `true`. `permission.*`, `approval.*`, `audit.*`, `skills.*`, and other reserved execution namespaces remain inactive.
...
For this calculation, "reports" means the provider-derived value survived strict catalog parsing, alias agreement, the shared 16,777,216-token ceiling, and router revalidation. A genuinely absent value, or an Ollama `/api/show` value omitted after context-only validation failure, is not a context budget and uses the conservative legacy fallback described below. Malformed, conflicting, nonpositive, non-integral, or over-ceiling LM Studio catalog metadata rejects that catalog; a non-nil invalid value reaching router publication rejects the whole catalog. This tightens trust in the existing optional field; it adds no protocol field or client/provider route.

Adaptive compaction inserts a fixed runtime-owned `system` provenance message and an adjacent `assistant` historical summary explicitly labeled as untrusted conversation data. User-controlled historical text never becomes generated system instructions. After the deterministic fallback fits the hard budget, the runtime may ask the same selected runtime-host model for a bounded backend-only summary prepass. That prepass resolves the exact current host-local `chat_compaction_summary_v1` definition before cache access and uses its prompt exactly; the prompt body remains registry-owned and absent from protocol, event, and cache rows. A missing or drifted current compaction skill skips cache lookup and the LLM prepass without failing chat, leaving the deterministic fallback as the primary context. Reasoning is discarded, the derived generation is cancelled with the parent request, and blank, failed, oversized, or non-fitting output leaves the deterministic fallback unchanged. Only the connection that owns the active request id may cancel its primary or derived generation; the ownership check and cancellation claim occur atomically before any backend cancellation call. The active runtime context persists durable cancellation intent even when neither backend generation is registered during the prepass-to-primary handoff, preventing later primary dispatch. The primary backend stream registration is atomic with its final terminal-state check. Derived-prepass registration uses the same atomic in-progress state plus a final terminal-state check: cancellation before derived registration prevents dispatch, cancellation during registration forces the returned derived generation to be cancelled, and no successful cancellation can race a later primary dispatch. The aggregate backend reserves the generation before async provider routing and rechecks cancellation while registering the selected provider stream. Primary and derived generation ids share one active reservation namespace, so a colliding active id is rejected before it can replace the original runtime context or reach backend chat.

The generated summary remains backend-only and outside the chat event log, transcript, and FTS index. After the primary request reaches a successfully stored `done`, the production runtime may retain the generated text in an owner-only SQLite sidecar. Exact reuse binds normalized owner, session, the bounded prepass source under `sha256-length-framed-chat-compaction-summary-source-v1`, its UTF-8 byte count, the full compacted-prefix lineage under `sha256-length-framed-chat-compaction-summary-lineage-v1`, the compacted turn count, the actually resolved provider-qualified model id, `llm_prepass_with_incremental_lineage_v2`, and the exact prompt-skill identifier and revision. On an exact miss, the cache may reuse only the newest smaller lineage whose digest exactly matches the corresponding prefix of the current storage-safe conversation and the same prompt binding. The runtime then presents the previous generated summary and only the newly compacted whole-turn delta as separately labeled untrusted input to the bounded same-model prepass. Edit, reorder, deletion, owner/session/model/policy/prompt-revision mismatch, malformed lineage, or a non-strict extension fails closed to an independent full-source prepass. Cache lookup or commit failure also degrades to a fresh prepass without failing chat. Cancellation, primary error, or a non-fitting generated summary never commits a row, including after a successful incremental prepass. The current prompt binding is revalidated before and during cache commit. Session deletion purges the owner/session-derived rows before the authoritative lifecycle mutation; a purge failure blocks that mutation, while a later lifecycle failure may conservatively discard only the derived cache. The previous cache schema is derived data and is dropped rather than migrated if the required lineage or prompt-binding columns are absent. New adaptive request events use `adaptive_backend_only_summary_v3`. Its separate request-bound source pointer includes `source_fingerprint_algorithm = sha256-length-framed-chat-compaction-source-v1`, a lowercase 64-hex `source_fingerprint`, and `source_canonical_byte_count`. The length-framed digest binds the pointer identity and turn ranges to the exact storage-safe compacted user/assistant prefix, including exact UTF-8 role/content and ordered attachment fields after inline attachment data has been removed for storage. Event validation recomputes the digest and byte count from the stored request before append and after reopen. This request-bound integrity binding is distinct from the reusable summary lineage; neither is a signature, external trust proof, tokenizer result, source excerpt, or replacement for the transcript. Existing `adaptive_backend_only_summary_v2` and `backend_only_summary_v1` records remain readable without fingerprint fields.
...
The macOS runtime host may derive a local calibration acceptance report from a bounded, revalidated store window. This is not a protocol message, negotiated capability, Android payload, or runtime command. The report considers at most the newest 1,000 fully eligible records and 32 newest exact provider/model/wire/estimator groups, exposes only aggregate relation counts, and uses 20 samples solely as a `ready_for_review` floor. Any `exceeded_input_budget` observation takes warning precedence. Prompt/messages, owner/session/request/event ids, timestamps, source pointers, and summary text are absent. Store scan-ceiling exhaustion, missing exact request binding, duplicate terminal, deterministic estimate mismatch, or malformed calibration produces a host-local report failure and no wire fallback. The report does not make a provider request, change compaction decisions, grant acceptance authority, or establish tokenizer parity.
...
If the requested model is not installed on the AetherLink Runtime, the AetherLink Runtime returns `error` with `code = "model_not_installed"`. Android clients do not initiate downloads. A legacy authenticated `models.pull` request enters the active runtime-host-local review only when the durable approval broker is available; absent broker, dismissal, expiry, or otherwise unapproved work returns `error` with `code = "model_pull_approval_required"` without provider dispatch.
...
The request fields are strict shape and stale-client guards, not title-generation authority. The AetherLink Runtime uses the existing active runtime-owned placeholder session, its stored runtime model, the first completed runtime-owned visible user/final-assistant turn, and the exact current `chat_title_v1` prompt binding. It resolves that stored model to an exact provider-native dispatch id; the requested model remains unchanged in the request/result correlation. Provider disappearance or same-provider alias reuse after resolution cannot redirect title work and instead uses the bounded deterministic runtime-transcript fallback. Locale is canonical runtime policy and is sent as JSON `null`; client `messages` do not replace the stored transcript. The runtime returns a single non-streaming result and does not emit `chat.delta` or normal assistant output for title requests. A missing session returns `chat_session_not_found`; an archived, deleted, already titled, source-drifted, or model-drifted session returns an empty result without stale publication.
...
Android treats every bulk lifecycle terminal as authority-bound. Each bulk batch uses a `chat-sessions-bulk-` request-id namespace so prior-batch and completed-operation errors are discarded before generic error handling. A bulk success must match the exact pending request id, lifecycle operation type, receiving channel object, connection generation, and authenticated authority generation before batch counters, local archive/delete persistence, progress state, or reconciliation can change. Malformed and ordinary error frames must match the same request, channel, connection, and authentication authority before they can consume the operation, publish an error, reconcile, or revoke authentication. Asynchronous send failures additionally bind the captured dispatch channel and operation type. Wrong-channel, old-connection, prior-authentication, prior-batch, duplicate, and completed-operation terminals are inert.
...
`error` result/response payloads accept only `code`, `message`, and `retryable`. `code` must be one of the canonical error codes below, `message` must be a string when present, and `retryable` must be a boolean when present. Error payloads must not carry backend URLs, provider URLs, backend credentials, route tokens, relay secrets, requested route tokens, workspace IDs, permission grants, source paths, source-control state, source approval handles, citations, trusted-source review data, audit handles, direct-store metadata, or direct-provider route material. Android clients reject unsupported error metadata before active stream termination, route/auth state mutation, or device storage mutation. Correlation closure is request-specific: an exact-current namespaced `memory.list` error consumes only its request correlation before publishing `invalid_payload`, permits a fresh replacement, and makes late errors for the closed id inert; an active chat-stream error preserves its correlation for a canonical same-id retry.
...
Long-inactivity memory summary drafts are review artifacts. `memory.summary.drafts.list` identifies active, owner-scoped, sufficiently old chats and returns bounded visible-transcript source pointers plus either a deterministic preview or a previously cached generated summary. Listing itself does not call a model, write memory, inject memory into `chat.send`, or include assistant reasoning, runtime-only system context, archived/deleted sessions, or another trusted device's sessions. `memory.summary.draft.generate` is an explicit review action: the runtime revalidates the owner-scoped current draft and exact stale guards, resolves an installed runtime-host local chat model to an exact provider and exact `provider_model_id` plus the exact current host-local `memory_summary_draft_v1` prompt-skill revision, sends only bounded visible user/final-assistant excerpts to that model, requires strict bounded JSON output terminated by an explicit backend `done` event, and stops at the first `done`. It never auto-approves memory. The host applies a 60-second whole-request deadline from installed-model discovery through source-current transport enqueue/publication intent and accepts at most 16,384 raw UTF-8 bytes across answer and explicit reasoning deltas. This boundary does not include or prove peer receipt or end-to-end delivery. Backend generation uses a host-private `memory-summary-generation-` identifier rather than the client request id. Timeout, last-waiter cancellation, and output overflow request cancellation of that exact backend id at most once without waiting for a blocking provider cancel call; cancellation dispatch is concurrent across exact generation keys. Repeated generation or reopening reuses a persistent cache only when owner, draft/source identity, exact requested model, exact `provider_qualified_model_id`, and exact current prompt-skill binding all match; a legacy row without provider identity remains readable but cannot satisfy this new reuse contract. Changing the model, resolved provider artifact, or prompt revision performs a new generation and does not coalesce with incompatible in-flight work. The shared worker returns only a generated candidate and tracks individual waiters. Each waiter has its own deadline, so expiration of one waiter preserves an in-flight worker still owned by another. Cancellation of the last waiter retires that flight even if its worker already completed, while only unfinished work is cancelled. A later same-key waiter receives a fresh identity-guarded flight only after the exact-key worker/cancellation permit is released; unrelated exact model keys retain concurrent generation. Delayed old cleanup may remove only the exact currently registered flight when it is still retired and empty. The first still-authorized waiter materializes one candidate in a bounded 256-entry in-process cache and invokes `RuntimeMessageSink.send` while the chat store holds the exact visible source stable: JSONL uses its instance and cross-instance file locks, and SQLite uses `BEGIN IMMEDIATE`. A one-shot deadline authority linearizes timeout against that source-current enqueue transaction. Production local and relay sinks report Network.framework `contentProcessed`; only successful completion allows a concurrent utility dispatcher to attempt the conditional derived JSONL cache write. Each materialized draft identity is pinned while publication or persistence is in flight, and each publication owns a unique reservation ID so duplicate completion cannot release another waiter. A cache-derived worker result may reserve only its exact still-present identity and cannot reinsert a superseded snapshot. Insertion evicts only unpinned identities and fails closed if all 256 slots are pinned. The first successful completion starts persistence. One concurrent or later success can request one retry only when the store declares generated-draft caching idempotent; consuming that retry permanently closes the identity's budget at two total store calls, and a non-idempotent store remains terminal after failure. JSONL declares that capability because each new materialization carries a host-private persistence operation id: the same operation retry is skipped under the cross-instance file lock, conflicting data for the same id fails closed, and a distinct operation still appends even when its generated value and second-rounded timestamp match history. The current generated event for one draft is chosen by physical JSONL append ordinal captured before timestamp sorting, so an operation appended after wall-clock rollback remains current after reopen; legacy id-less events retain exact-value retry compatibility. Persisted identities remain in the bounded cache so already-registered flight waiters can publish the same candidate. Durable persistence is outside the source transaction and deadline, cannot delay the response path or connection close, and can still be pending while list or approval uses the in-process candidate. A failed transport leaves the bounded materialized candidate available for a later successful completion without a second backend call. Timeout or connection close before enqueue leaves wire and durable cache untouched; an older identity can persist only its own delivered draft and cannot acquire a replacement token. A source change after inference or before enqueue returns `memory_summary_draft_stale`; a missing current prompt definition returns `runtime_prompt_skill_unavailable` before backend dispatch; malformed, oversized, backend-failed, cancelled, timed-out, or gracefully exhausted output without `done` returns no generated candidate and leaves the deterministic preview unchanged. Data after the first `done` is not consumed as summary output. `memory.summary.draft.approve` is the separate write command: the runtime recomputes the owner-scoped draft, overlays only a matching generated candidate whose historical prompt binding is still resolvable, validates optional expected metadata, and writes an idempotent runtime memory entry with id `memory-summary:<draft_id>`. An unavailable historical binding falls back to the deterministic preview. Approved summary entries carry durable `source` metadata that records `deterministic_preview` or `llm_summary_v1`, source session metadata, source range, and bounded visible-transcript source pointers. `memory.summary.draft.dismiss` persists an owner-scoped decision without creating or updating memory. Approved or dismissed drafts are hidden from later draft-list responses for that owner.

Generated-review persistence records the canonical requested model id, host-local `provider_qualified_model_id`, `prompt_skill_id`, and `prompt_skill_revision`, never the prompt body. Legacy generated events map to the separately pinned original-v1 memory-summary binding only when both prompt keys are genuinely absent; a missing legacy provider identity remains nil and cannot authorize provider-bound reuse. JSONL decoding rejects duplicate object names, including escaped-equivalent names, before Foundation decoding; explicit `null`, partial, malformed, wrong-skill, or nested provider-qualified provider-id metadata is invalid. These provenance fields are not protocol fields, are not returned in draft payloads, and cannot be selected by Android or another client.

Publication visibility is stricter than materialization. Only successful transport completion marks the exact in-process generated identity as review-visible. `memory.summary.drafts.list` may overlay that published identity while its durable write is pending, and explicit approval may use it only when `content` exactly matches the displayed generated text. A failed transport candidate remains available only for a later generation-publication retry; it is not listed or approvable. Explicit approval content must equal either the recomputed deterministic preview or a current-source, currently resolvable published/persisted generated candidate, otherwise the runtime returns `memory_summary_draft_stale`. Compatibility approval without `content` may use only a matching durable generated event and otherwise stores the deterministic preview. JSONL exact-operation idempotence scans every owner-scoped generated event under cross-instance file exclusion, so an A-to-B-to-A retry does not append historical A again or displace current B; physical append order selects a distinct later operation even if its timestamp moved backward.

Android correlates each memory-summary list, generate, approve, and dismiss exchange to the exact client-generated request id, channel identity, connection generation, and authenticated runtime-authority generation. Generated source metadata accepts canonical `llm_summary_v1`. Generated and approved source identity excludes only changing `inactive_seconds`; all other draft, session, source-range, and source-pointer identity fields remain exact. Approve and dismiss results must match the exact pending draft id and current visible source; approval also requires the canonical `memory-summary:<draft_id>` entry id, `enabled=true`, and content equal to the generated review text. Android sends the exact currently displayed `summary_preview` as approval `content`; it sends `expected_summary_method` only when the authenticated challenge negotiated `memory.summary.approval_method.v1`, and clears negotiated runtime capabilities on revocation or connection replacement. After exact correlation, a malformed, unknown-metadata, or otherwise noncanonical approve/dismiss result is terminal: it consumes correlation, clears the matching action UI, emits one retained authoritative refresh, and causes a later canonical result with the old request id to be ignored. A required or manual list refresh encountered behind pending actions is retained and emitted once after the last action terminates through success, stale or malformed result, protocol error, or send failure. Authentication revocation, successful reauthentication, channel replacement, disconnect, and receive failure clear all pending correlations, the retained refresh, and action UI state. Correlated `error` payloads apply the closed error shape before authentication handling, and unknown metadata is `invalid_payload`, not an authentication transition. Client-generated namespaced summary request ids allow arbitrarily delayed errors to be ignored after bounded list tombstones are evicted; this namespace is correlation metadata only and grants no protocol capability.

The current no-device contract covers 54 Swift memory-summary checks, sixteen generated-draft store checks, seven direct transport completion checks, one conditional challenge negotiation check, targeted Android protocol/ViewModel mixed-version checks, all 1,475 current-source Swift package tests, and the refreshed default aggregate. These checks prove the local protocol and enqueue/persistence state machine only; peer receipt, physical Android, optical QR, live-provider behavior, external networking, and production P2P/NAT behavior remain outside scope.
...
```json
{
  ...
  "payload": {
    "drafts": [
      {
        "id": "long-inactivity:v2:0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef",
        "session": {
...
Draft payloads always include `summary_method`, which is `deterministic_preview` or `llm_summary_v1`. Generated drafts also include optional `generated_at` and `generated_model_id`; deterministic drafts omit those fields. Host-private persistence operation ids are never included in draft payloads or Android models.

### `memory.summary.draft.generate`

Generates a review-required summary for one current owner-scoped draft. The request requires exact stale guards and an installed runtime-host local chat model. Cache identity additionally requires that exact requested model and the exact current host-local memory-summary prompt binding. The runtime does not trust the client-supplied session id as authority, never accepts client-selected prompt identity, and never stores the result as approved memory.

`memory.summary.draft.generate.payload` request data accepts exactly nonblank string `draft_id`, nonblank string `model`, nonblank string `expected_session_id`, and positive integer `expected_source_message_count`. The response accepts exactly `draft`, using the same closed draft shape as `memory.summary.drafts.list`. Unknown metadata or malformed values return `invalid_payload`. A missing or owner-inaccessible draft returns `memory_summary_draft_unavailable`; changed stale guards or a source mutation during inference return `memory_summary_draft_stale`; an unavailable exact current prompt definition returns `runtime_prompt_skill_unavailable`; malformed, blank, oversized, or failed model output returns `memory_summary_draft_generation_failed` without replacing the deterministic preview or writing memory. Android also requires generated `generated_model_id` to equal the pending request model; a mismatch is `invalid_payload`, preserves the deterministic draft, and triggers an authoritative draft refresh. Generated source identity excludes only changing `inactive_seconds`; every other draft, session, source-range, and source-pointer identity field remains exact.

The host's 60-second deadline begins before installed-model discovery and ends when the source-current response is handed to `RuntimeMessageSink` for transport enqueue/publication intent. JSONL instance/cross-instance locking or SQLite `BEGIN IMMEDIATE` keeps the source stable through that enqueue call. Production local and relay transports report Network.framework `contentProcessed` completion, and only success gates concurrent conditional durable cache persistence. Each exact materialized draft identity owns unique publication reservations. Publication and persistence pin that identity across replacement and bounded eviction; duplicate completion cannot release another reservation, cache-derived workers cannot reinsert a superseded snapshot, and insertion evicts only unpinned entries or fails closed if all 256 slots are pinned. The first successful callback starts persistence. One concurrent or later success requests one retry only when the store declares generated-draft caching idempotent; consuming it closes the identity at two total calls, and non-idempotent stores remain terminal after failure. Each newly materialized result owns a canonical host-private UUID persistence operation id. JSONL skips an exact retry of that operation under cross-instance exclusion, rejects conflicting data under the same id, and appends a distinct operation even when its complete generated value and second-rounded timestamp equal a historical result. Current generated-draft selection uses physical append ordinal rather than timestamp, so rollback cannot revive an older operation; legacy rows without an id retain exact-value retry compatibility. Persistence remains outside the source transaction and deadline and cannot delay the response path or connection close. A failed transport leaves the bounded in-memory candidate available for a later successful completion without another backend call. Enqueue or `contentProcessed` success does not claim peer receipt or end-to-end delivery.
...
```json
{
  ...
  "payload": {
    "draft_id": "long-inactivity:v2:0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef",
    "model": "ollama:llama3.1:8b",
...
Approves one currently available long-inactivity memory summary draft into runtime-owned memory for the authenticated trusted device. The runtime must look up the draft from its owner-scoped chat store by `draft_id`; it must not trust client-supplied session ids as authority. If `expected_session_id` or `expected_source_message_count` is supplied and no longer matches the recomputed draft, the runtime returns `memory_summary_draft_stale`. When `content` is supplied, it must exactly match the recomputed deterministic preview or a current-source published/persisted generated candidate. `expected_summary_method`, when present, must also match that exact candidate; this disambiguates byte-identical deterministic and generated review text. Arbitrary replacement text, failed-transport generated text, an unavailable requested method, or content that remains ambiguous without a method returns `memory_summary_draft_stale`. If the draft is unavailable or belongs to another owner, the runtime returns `memory_summary_draft_unavailable`.

`memory.summary.draft.approve.payload` accepts only string `draft_id`, optional string `content`, optional boolean `enabled`, optional string `expected_session_id`, optional integer `expected_source_message_count`, and optional enum `expected_summary_method` with value `deterministic_preview` or `llm_summary_v1`. `content` and `expected_summary_method` are exact reviewed-display bindings, not edit or generation-authority fields. Current clients send the displayed `summary_preview`. They send `expected_summary_method` only after the authenticated runtime returns `memory.summary.approval_method.v1` through the opt-in challenge capability array; an older runtime omits that capability, so the new client omits the field that the old closed payload would reject. Compatibility clients may omit both review bindings, in which case only a matching durable generated event can supply generated content and the runtime otherwise uses the deterministic preview. Malformed allowed fields, such as blank `draft_id`, non-string or blank `content`, non-boolean `enabled`, non-string or blank `expected_session_id`, string/fractional `expected_source_message_count`, or non-string/noncanonical `expected_summary_method` values, return `invalid_payload` instead of being coerced or treated as omitted. Clients must not supply `status` or `entry`; they are response-only approval data. Approval requests also must not carry backend URLs, provider URLs, backend credentials, route tokens, relay secrets, requested route tokens, workspace IDs, permission grants, source paths, source-control state, source metadata, persistence operation ids, or direct-store metadata.

`memory.summary.draft.approve` result payloads accept only `draft_id`, `status`, and `entry`. The approved-memory result `entry` uses the same closed memory-entry shape as `memory.list`: top-level `entry` accepts only `id`, `content`, `enabled`, `created_at`, `updated_at`, `source`, and `search`; `entry.source` accepts only `kind`, `draft_id`, `summary_method`, `session`, `source_message_count`, `source_range`, and `source_pointers`. Approved-memory result `entry.source.source_pointers` accepts only `session_id`, `message_index`, `role`, `created_at`, and `excerpt`. Approved source identity excludes only changing `entry.source.session.inactive_seconds`; all other identity fields remain exact. After exact request-id, channel, connection-generation, and authenticated-authority correlation, unsupported metadata or any malformed/noncanonical result is terminal: it consumes the correlation, clears approval UI, drains one deferred draft refresh, and makes a later canonical result carrying the old request id inert. Approval results must not carry backend URLs, provider URLs, backend credentials, route tokens, relay secrets, requested route tokens, workspace IDs, permission grants, source paths, source-control state, source approval handles, citations, trusted-source review data, audit handles, or direct-provider route material.
...
```json
{
  ...
  "payload": {
    ...
    "expected_session_id": "session-1",
    "expected_source_message_count": 6,
    "expected_summary_method": "deterministic_preview"
  }
}
```
...
```json
{
  ...
  "payload": {
    ...
    "status": "approved",
    "entry": {
      "id": "memory-summary:long-inactivity:v2:0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef",
      "content": "User: Summarize my preference.",
      "enabled": true,
...
`memory.summary.draft.dismiss` result payloads accept only `draft_id`, `status`, and `dismissed_at`. After exact request-id, channel, connection-generation, and authenticated-authority correlation, unsupported metadata or any malformed/noncanonical result is terminal: it consumes the correlation, clears dismissal UI, drains one deferred draft refresh, and makes a later canonical result carrying the old request id inert. Dismissal results must not carry backend URLs, provider URLs, backend credentials, route tokens, relay secrets, requested route tokens, workspace IDs, permission grants, source paths, source-control state, source approval handles, citations, trusted-source review data, audit handles, direct-store metadata, or direct-provider route material.
...
- Internal Python tools: reserve the `python.` namespace for future deterministic Python execution through the runtime host. The closed `runtime_python_sandbox_v1_recommended` review proposes `python.run` as the sole first message under a separately signed minimum-privilege App Sandbox/XPC worker, a pinned bundled interpreter, a restricted profile, exact per-run host approval, separate approval and execution deadlines, process-group cancellation, bounded closed results, and durable redacted audit. The review remains `proposed_not_selected`: `python.run` is not active, `python.exec` is not a second proposed message, and both remain rejected by current schema/router hygiene. Do not add active message names until the runtime Python sandbox, permission prompts, resource limits, and audit model are designed. The review adds no runner, action, interpreter, process, entitlement, file/network/process capability, or execution authorization.
