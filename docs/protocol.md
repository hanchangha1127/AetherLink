...
These active document messages are runtime-owned and read-only. They do not add semantic retrieval, embedding generation, source approval, citations, trusted-source review, permission or audit semantics, local client persistence, chat context injection, direct Android backend access, or production relay/session encryption.
...
The first runtime-side context compaction slice is also backend-call context only. When the runtime's heuristic character budget says the active `chat.send` history is too large, the runtime keeps the most recent client-visible messages verbatim, summarizes older active-session turns into a backend-only `system` message, and sends that compacted input to the selected backend. The backend-only summary may include deterministic source-span text, such as the client-visible conversation turn range that was compacted, so the model can reason about which active-session slice the summary covers. Runtime event stores may persist structural source-pointer metadata for the compacted range, including request/session identifiers and turn ranges, but not prompt excerpts or backend-only summary text. The client-visible transcript, `chat.messages.list`, `chat.sessions.list`, archive state, and delete state are not rewritten by compaction, and source-pointer metadata is not exposed through `chat.messages.list` or session search indexes. Archived sessions and deleted sessions must not be used as compaction inputs. The capability guard, runtime-owned memory prompt context, and compaction summary remain separate runtime-only system context so memory/capability guard separation is preserved.
...
`memory.list.payload` accepts only optional `query`. `query` must be a string when present; malformed allowed fields return `invalid_payload` instead of being coerced or treated as omitted. Nonblank lexical queries are bounded to 256 characters and 16 distinct normalized terms before runtime memory-store search dispatch. Overlong or excessive-term queries return `invalid_payload`. Clients must not supply `entries`; it is response-only memory list data. List requests also must not carry backend URLs, provider URLs, backend credentials, route tokens, relay secrets, requested route tokens, workspace IDs, permission grants, source paths, source-control state, or direct-store metadata.
...
- Session compaction: the first runtime-side heuristic slice can compact oversized active `chat.send` history before backend dispatch without changing client-visible history. It may persist runtime-owned structural source-pointer metadata for compacted active-turn ranges in the chat event store, while keeping backend-only summary text and source-pointer metadata out of `chat.messages.list` and session search indexes. Future messages may expose tokenizer-aware budgets, durable compacted session summaries, richer transcript source pointers, LLM-generated summaries, and longer-inactivity compact memory summaries. This is separate from model lifecycle messages such as unload-after-10-minutes-inactive.
- Embeddings/research: reserve the `embeddings.` namespace, keep `retrieval.query` as the only active deterministic lexical read-only document retrieval message, reserve unsupported `retrieval.*` messages beyond `retrieval.query`, keep `index.documents.list` as the only active read-only `index.` catalog message, reserve other `index.` messages, reserve the `research.` namespace, reserve the `citation.` namespace, keep `source_anchor.resolve` as the only active read-only `source_anchor.` resolver message, reserve unsupported `source_anchor.*` messages beyond `source_anchor.resolve`, reserve the `trusted_source.` namespace, and reserve the `source_control.` namespace for future runtime-side embedding generation, semantic retrieval, project/file indexing, research notebooks, source approval flows, trusted-source review, source citation, and source-control status or diff context. `retrieval.query` results may include a runtime-derived `source_anchor_id` computed from safe document/chunk metadata so future source approval or citation UI can point at a result without exposing paths; the field must match `source_anchor_[16 lowercase hex]`, and `source_anchor.resolve` accepts only `source_anchor_id` in requests and returns only `source_anchor_id`, `document`, and `chunk_summary`. Candidate future names such as `embeddings.create`, `index.build`, `research.brief.create`, `citation.sources.list`, `trusted_source.approve`, and `source_control.status` are examples only and are not active protocol messages. Embedding models must be listed and selected separately from chat/text-generation models, and future semantic retrieval/ranking/knowledge indexing must use the selected embedding model. These future messages are distinct from current `models.list` embedding model metadata, the current deterministic lexical `memory.list` query filter, the current deterministic lexical `retrieval.query` document-index filter, the current read-only `index.documents.list` catalog, the current read-only `source_anchor.resolve` resolver, and the current `chat.sessions.list` `embedding_model_id` search hint. Protocol schema hygiene rejects active message enum entries under `embeddings.`, unsupported `retrieval.`, unsupported `index.`, `research.`, `citation.`, unsupported `source_anchor.` beyond `source_anchor.resolve`, `trusted_source.`, and `source_control.` until the runtime semantic retrieval, indexing, research, source approval, citation, trusted-source, permission, and audit semantics are designed.
