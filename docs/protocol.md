...
Adaptive compaction inserts a fixed runtime-owned `system` provenance message and an adjacent `assistant` historical summary explicitly labeled as untrusted conversation data. User-controlled historical text never becomes generated system instructions. Runtime event stores may persist `adaptive_backend_only_summary_v2` metadata containing only structural source pointers and accounting fields such as estimator id, context window, reserve, hard input budget, and before/after estimates; they must not persist prompt excerpts or backend-only summary text in that metadata. The client-visible transcript, `chat.messages.list`, `chat.sessions.list`, archive state, and delete state are not rewritten, and compaction metadata is not exposed through transcript reads or session search indexes. Archived and deleted sessions are not compaction inputs.
...
- Embeddings/research: reserve the `embeddings.` namespace, keep `retrieval.query` as the only active deterministic lexical read-only document retrieval message, reserve unsupported `retrieval.*` messages beyond `retrieval.query`, keep `index.documents.list` as the only active read-only `index.` catalog message, reserve other `index.` messages, reserve the `research.` namespace, reserve the `citation.` namespace, keep `source_anchor.resolve` as the only active read-only `source_anchor.` resolver message, reserve unsupported `source_anchor.*` messages beyond `source_anchor.resolve`, reserve the `trusted_source.` namespace, and reserve the `source_control.` namespace for future project/file embedding generation, semantic retrieval, persistent indexing, research notebooks, source approval flows, trusted-source review, source citation, and source-control status or diff context. `retrieval.query` results may include a runtime-derived `source_anchor_id` computed from safe document/chunk metadata so future source approval or citation UI can point at a result without exposing paths; the field must match `source_anchor_[16 lowercase hex]`, and `source_anchor.resolve` accepts only `source_anchor_id` in requests and returns only `source_anchor_id`, `document`, and `chunk_summary`. Candidate future names such as `embeddings.create`, `index.build`, `research.brief.create`, `citation.sources.list`, `trusted_source.approve`, and `source_control.status` are examples only and are not active protocol messages. Embedding models are listed and selected separately from chat/text-generation models. The active `chat.sessions.list` and `memory.list` requests may use the selected embedding model for bounded runtime-host semantic ranking and owner-scoped persistent candidate-vector reuse without adding an `embeddings.*` message; semantic document retrieval and knowledge indexing remain future work. These future messages are distinct from current `models.list` embedding model metadata, current embedding-backed chat and approved-memory search, the deterministic lexical fallbacks, the deterministic lexical `retrieval.query` document-index filter, the current read-only `index.documents.list` catalog, and the current read-only `source_anchor.resolve` resolver. Protocol schema hygiene rejects active message enum entries under `embeddings.`, unsupported `retrieval.`, unsupported `index.`, `research.`, `citation.`, unsupported `source_anchor.*` beyond `source_anchor.resolve`, `trusted_source.`, and `source_control.` until the remaining runtime semantic retrieval, indexing, research, source approval, citation, trusted-source, permission, and audit semantics are designed.
