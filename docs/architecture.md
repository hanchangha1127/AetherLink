...
See [connection-overlay.md](connection-overlay.md) for the concrete phased design for paired identity, local direct routing, remote NAT traversal, privacy-preserving rendezvous/bootstrap/DHT options, and blind encrypted relay fallback.
...
Future peer discovery can use a DHT-like or bootstrap-peer layer, but only as a privacy-preserving rendezvous fabric for paired identities. Bootstrap peers may help a client and runtime host find each other's short-lived reachability records, but they must not become accounts, a directory of public runtime hosts, a backend URL registry, or an authority that grants trust. Trust comes only from QR pairing, pinned peer identity, challenge-response authentication, and the encrypted session.
...
| Local direct endpoint hints | Implemented as development/local fast path | USB reverse, emulator, Bonjour/local discovery, pairing QR host/port, and manual diagnostics can produce direct TCP route candidates. |
...
| Encrypted blind relay/TURN fallback | Interface scaffolded, transport not implemented | `Relay` route candidates and fallback connector injection are modeled so a future blind relay transport can be attempted after direct/P2P paths fail. No relay allocation, forwarding, or production encryption exists yet. |
