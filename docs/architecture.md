...
- SQLite chat append transactions validate each sanitized event and its targeted compaction binding, write the event, and add one event-level search projection atomically. A mutation-revision mismatch or projection-version migration takes the separate fail-closed repair path, which validates the complete durable log and rebuilds the projection once before ordinary incremental append resumes. Search first narrows owner-scoped substring and conservative multi-delta candidates, then replays only those sessions for exact JSONL-equivalent results.
...
- Host approval receipts are nonblocking one-use capabilities. A wrong, repeated, or concurrent consume attempt fails immediately; ambiguous durability clears the pending queue and leaves the coordinator recovery-only. Deadlines and caller cancellation do not wait for persistence adapters that ignore cancellation.
- Aggregate model residency owns a completion gate for every unload, including a self-started unload. A cancelled chat or embedding caller can stop waiting while the shared provider unload continues; no later caller can admit that model until the same completion gate resolves. Stream forwarding uses a fixed-capacity oldest-preserving buffer and fails closed if the consumer falls behind.
- Provider nonstreaming bodies and Android attachment reads are incremental bounded pipelines rather than whole-input reads. Document extraction first snapshots one descriptor-validated regular file into an owner-only bounded temporary artifact, so path replacement, symlinks, FIFOs, and mid-read growth cannot change the bytes seen by PDFKit or external conversion tools.
- Smoke-owned ephemeral relays bind their lifetime to the outer gate parent process. Bootstrap smoke additionally requires an allocation before peer admission. These lifecycle flags belong to local test orchestration and do not change the production transport topology or authority model.
...
- Embedding and research: embedding models are listed and selected separately
  from general text-generation/chat models. Embedding requests preserve
  per-input plain, retrieval-query, retrieval-document, and
  semantic-similarity roles through aggregate routing. Runtime chat, memory,
  and approved-document search use asymmetric retrieval roles; review-only
  duplicate suggestions use semantic-similarity roles. Ollama applies a
  recognized model profile's prompt format without changing the caller's
  stored text, rejects present-null or malformed profile metadata, and accounts
  for the effective profile's bounded prefix in per-input and batch budgets.
  The persistent embedding fingerprint includes the effective adapter profile,
  and an embedding result must report the same profile before cache reuse, so
  raw and role-aware vectors cannot share a cache identity. A selected model remains subject to the recorded
  multilingual quality gate; provider shape support alone is not a retrieval
  qualification.
