...
- The newest G7 data-integrity slice exercises the real
  `SQLiteRuntimeChatEventStore.append` transaction at an internal QA-only
  post-validation cache-flush checkpoint. Two isolated exact-child `SIGKILL`
  runs produce identical 2,674-byte results at SHA-256
  `a60207d2dcc2cca06d156a0295ee40850759c505b50bb3926e0210683a286e4a`.
  Both prove stable populated journal bytes and dirty state `(1, 1, 2)`, cold
  recovery to `(0, -1, 0)`, and exact retry to 48 event/FTS rows with state
  `(48, 48, 2)`. The 16 Python contract tests, two focused Swift regressions,
  independent readback, product CI contract, and CI mutation self-test pass.
  PR/main runs contract units; `main` additionally runs the repeated producer
  and readback. Current workflow raw/parsed SHA-256 values are
  ...
  This is same-host QA-forced cache-flush evidence, not natural COMMIT timing,
  power-loss, clean-machine, device/network, hosted CI, G7 exit, or V1 proof.
- The newest macOS G6/G7 slice stages
  `AetherLink.app`, `AetherLink.dSYM`, and `source-receipt.json` from one stable
  source snapshot, then atomically swaps that complete three-entry generation
  into `dist/unsealed-package-only`. A failed build or replacement leaves the
  preceding generation intact. The checker binds the canonical receipt to
  ledger `1.0.0+24` and the current source, reads receipt and source stably on
  both sides of the app/dSYM readback, reads the physical output bytes twice,
  and invokes `lipo`/`dwarfdump` only against its temporary copy. It reports
  each generation's sizes, closed-tree SHA-256 identities, thin arm64
  architecture, and shared UUID at execution time. This generic output gate
  does not freeze a future generation's identities; the producer-bound current-
  run lifecycle successor instead derives and validates one observed generation
  inside that same run. Neither contract claims reproducibility.
  `_CodeSignature`/`CodeResources` are absent from the outer app, while a
  linker-generated executable signature remains outside the no-seal claim.
  Producer/archive tests pass 17/17 and 78/78. `main` runs this producer and
  direct reader after focused Swift result verification, then the macOS
  Release diagnostics producer/readback, two isolated
  install/launch/SIGKILL/relaunch observations, and an independent held-source
  current-run readback. The clean-checkout lifecycle unit step
  passes 54/54. Treat this as local same-host unsealed lifecycle proof, not
  bit-for-bit build reproducibility, signing, notarization, channel-valid or
  clean-machine install/launch, update/rollback, device/network, hosted CI,
  canonical G6/G7 exit, RC/GA, or V1 evidence.
- The newest G6/G7 Android Release slice keeps one strict-lock Release graph
  plus direct readback on pull requests. On `main`, a source-bound producer
  validates the product CI contract before each of two separate offline
  `--rerun-tasks` Release graphs, snapshots A, and independently reads B. The
  source stays fixed across before/between/after snapshots at 276 files and
  SHA-256
  `18d0aa2083dec01ad63c1422d716c6465f2155c501dbb03299223b28f3ddd284`.
  The 9,576,378-byte APK SHA-256 is
  `0b64b0665859eb1cb68891bc67db0edb6b55a6fcece4f25929ba875950fa4fb2`;
  the 10,695,033-byte AAB SHA-256 is
  `fd79f18967af1a76c63fdc3f222df796100dccfd0e85d2c3898c4bcec0b94a7a`.
  Across the closed 53-file output graph, 48 files are raw-byte-identical,
  including the zero-issue 86-byte lint XML at SHA-256
  `224d8ee1273d5e13d2c5a6ad745b7a4acacdf9458356b0cf1e7762d7b6309b90`.
  The two DM ZIPs, `mapping.prt`, `resources.txt`, and `seeds.txt` retain distinct
  raw identities but equal bounded logical/canonical identities; both runs
  share comparison-graph SHA-256
  `54e8699cafe30f9aa034bad8efe01cca5554987f36079f912afb6208133b7e30`.
  Readback binds ledger `1.0.0+24`, two baseline-profile API ranges and their
  DM files, APK/AAB manifests and locale/config resources, six R8 files, raw
  mapping and logical PRT
  identities, five arm64 JNI members, 191 SDK dependencies with text/protobuf
  graph and lock closure, APK/AAB DEX identity, and the current
  upstream-prestripped native-symbol state. The mode-0600 create-only result
  is bound from current bytes by each candidate and intentionally has no
  cross-run documentation hash.
  Its independent checker, 23 regressions, the archive suite's 78 tests,
  product CI contract, and mutation self-test pass. The macOS PR job also checks all 36
  tracked document contracts and two exact mode regressions without reading
  ignored `dist/` evidence. Workflow raw/parsed SHA-256 values are
  ...
  These are same-host current-toolchain unsigned build/readback facts only:
  cross-host bit-for-bit reproducibility, signing, store, install, launch,
  device/network, release-to-release, hosted-run, G6 exit,
...
- The current-source disposable Android lifecycle V1 companion record is
  `build/qa/android-headless-api36-1-20260802T180031Z-0a8e7473/result.json`.
  It passes 13/13 scenarios in 192.995 seconds; the canonical 50,450-byte result
  binds source SHA-256
  `0ac2aec99995a024eaef2c8ad66420d9a8107b2fef17185002eae8214edbbaaa`
  and result SHA-256 is
  `93e5884ef64edef127c6dcfcb916c42f7ccf7f3ff88c1330b87db4eeb580ad20`.
  Independent readback verifies absolute nested UI/locale deadlines, twelve
  `stat-before -> cmdline -> stat-after` PID/start-tick observations, and four
  CAMERA `dumpsys package` states `false / false / true / false`. Follow-system
  acceptance also requires a fully visible enabled checked row inside every
  scroll viewport plus the fixed English pairing-screen anchor. The associated
  runner/checker suite passes 45 tests, and cleanup leaves no ADB device or
  owned emulator. Treat this as V1-lane unsigned Debug emulator evidence
  only: physical optical
  QR/camera, TalkBack/OEM/API matrix, background/Doze/reboot, production network,
  signing/store, G5/G6 exit, G7, and V1 release remain open.
- Android 13+ app-language startup treats a supported nonempty
  `LocaleManager.applicationLocales` snapshot as authoritative. When a legacy
  API 26-through-32 explicit language meets an initially empty platform list,
  a one-time two-phase migration durably records the pending tag, writes it to
  the platform, reads it back, and then records completion. Interruption retries
  the pending tag without another save; after completion, an external clear
  stays Follow system and cannot resurrect the old language. API 26 through 32
  do not start the marker or write the platform list. Explicit English remains
  distinct from empty/system English, and the first localized frame uses the
  reconciled persisted/platform snapshot. The storage, duplicate-save
  ViewModel, writer, and API 32/33/36 production-lifecycle regressions pass
  6/6; the API 33 path covers migration, recreation/cold launch, external
  clear, and external Korean override. A disposable API 36.1
  ...
  reachability without a FATAL or ANR. The migration is JVM/Robolectric proof,
  not a physical OS-upgrade observation. Physical QR, TalkBack, OEM, signing,
...
- The repository includes `.github/workflows/product-quality.yml` as a
  ...
  jobs with exact Swift and Android product allowlists, plus the 78-test
  release archive contract and strict-lock Android Release APK/AAB assembly,
  lint, and direct output readback. Pushes to `main` additionally add a macOS
  Release product build and the exact 19-class complete Android app unit lane
  with pre-run source marker and post-run binding/readback. Both triggers run
  54 clean-checkout-safe current-unsealed lifecycle contract tests: 35 runner
  tests, 11 portable evidence-checker snapshot/schema tests, and eight portable
  current-run checker tests, including dynamic running-code CDHash, post-reap
  log, held-source cross-binding, and acquisition-time same-byte replacement
  regressions without reading ignored local lifecycle evidence. On `main`, the
  unsealed package readback is followed by the macOS Release diagnostics
  producer/readback and then the two-observation lifecycle producer and
  independent readback in a private `.build` directory. Before the focused Swift run, the
  macOS lane lists the package tests and requires the same filter to select the
  exact 217-test identity manifest. `script/check_product_ci.py` pins that
  shape and rejects broad,
  device/network, signing, publication, deployment, offline, or
  `main-full` scope expansion.
  The same lane validates the checked-in offline compliance catalog and runs
  the exact 22-test deterministic SPDX/license render and independent
  reconstruction manifest with zero skips. The exact command body and test
  identities are pinned, and mutation tests reject omission, replacement,
  network-refresh substitution, or bypass with raw `unittest` discovery.
  ...
  then the tracked-only documentation contract and its two bounded mode tests,
  without entering ignored-evidence or paused mixed security paths. A Psych AST pre-pass
  ...
  `d7361e671c72620957f795e4975776c1612688c72b5109facf59308d82de1d2b`.
  Current local parity proves 2,166 discovered XCTest specifiers, the exact
  217-test focused identity manifest and 217/217 execution, the exact
  twelve-report 45/45 focused Android manifest, and the 19-report 1,226/1,226
  complete Android source-marker/result-binding readback with zero skips or
  failures. The Swift marker admits only the 27 explicit package target roots,
  binds 216 source files, and excludes sibling build outputs. Its runner parses
  the temporary serial console and rechecks source/marker freshness before
  publication; nonzero, invalid, oversized, drifted, and launch-error fixture
  processes preserve the existing canonical log. The contract and mutation
  self-test pass. The Android focused
  allowlist directly executes both the changed-session scroll-boundary
  regression and the three-result exact font-scale qualification rather than
  only compiling their test source.
  Hosted run
  ...
  development-server publication handling, Swift discovery contract, complete
  Android main lane, checker, and documentation remain unstaged and uncommitted
  and are not covered by a hosted run.
