...
shared UUID at execution time; those locally linked identities are not frozen
in tracked source and are not a bit-for-bit reproducibility claim. The producer
and reader suites pass 17/17 and 78/78; the product CI contract and mutation
self-test pass. This closes the current-source macOS unsealed direct-output
gap. The outer app has no `_CodeSignature` or `CodeResources`, but the
executable may retain a linker-generated ad-hoc signature; neither Developer
ID signing nor notarization is claimed. Installation, launch, clean-machine,
...
graphs each succeed across 175 actionable tasks and produce byte-identical
...
release archive suite passes 73/73, while the product CI contract and mutation
...
`dist/` evidence remains local full-checker scope. Workflow raw and parsed
...
The latest current-source G5/G6 Android headless lifecycle record is
...
it does not satisfy the physical-device, signed-artifact, production-network,
release-to-release, G5/G6 exit, G7, or V1-production gates.
...
compilation, the 68-test release archive contract, and strict-lock Android
...
same Release steps. The macOS job first requires the focused filter to resolve against the
...
Swift/Android product test allowlists, affected compilation, a 217-identity
Swift discovery preflight, the 68-test release archive contract, and Release
APK/AAB compilation, lint, and direct output readback on pull requests and
`main`. The macOS lane includes a tracked-only 36-document contract that
passes in a temporary tracked snapshot without `.git` or ignored `dist/`, plus
two exact mode regressions. The full local checker retains fail-closed raw
evidence readback. The `main`-only complete Android step is ordered as pre-run source
...
current local evidence passes 1,226/1,226 and 73/73. Hosted run
