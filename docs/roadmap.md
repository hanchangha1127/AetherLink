...
The newest G6/G7 release-quality slice keeps an ordered strict-lock
`assembleRelease -> bundleRelease -> lintRelease -> direct build-output
readback` on pull requests. On `main`, a source-bound producer validates the
CI contract and executes two separate offline `--rerun-tasks` Release graphs
before an independent A/B checker and direct output readback. The source stays
fixed at 276 files and SHA-256
...
Forty-eight of the closed 53 output files are raw-byte-identical, including
the zero-issue 86-byte lint XML at SHA-256
`224d8ee1273d5e13d2c5a6ad745b7a4acacdf9458356b0cf1e7762d7b6309b90`.
The two DM ZIPs, `mapping.prt`, `resources.txt`, and `seeds.txt` differ only below their
raw identities and match through the existing bounded logical/canonical
comparisons. Both runs share comparison-graph SHA-256
`54e8699cafe30f9aa034bad8efe01cca5554987f36079f912afb6208133b7e30`.
The mode-0600 create-only result is bound from current bytes by each candidate
and intentionally has no cross-run documentation hash. Its independent
producer/checker suite passes 23/23.
...
This closes the local same-host/current-toolchain unsigned A/B output-semantic
gap only. It does not claim all 53 raw outputs or cross-host builds are byte-
identical. It is not signing, store publication, installation,
physical/device/network or release-to-release
...
It passes 13/13 scenarios in 192.995 seconds; its canonical 50,450-byte result
binds source SHA-256
`0ac2aec99995a024eaef2c8ad66420d9a8107b2fef17185002eae8214edbbaaa`
and has result SHA-256
...
launch, regrant, and fixed revoke. Follow-system acceptance additionally
requires a fully visible enabled checked row inside every scroll viewport plus
the fixed English pairing-screen anchor. Its 45 runner/checker unit and
mutation tests pass. Cleanup leaves no ADB device or owned emulator. This
strengthens emulator evidence;
within the V1 lane alone it does not satisfy background/Doze/reboot,
...
current local evidence passes 1,226/1,226 and 78/78. The schedule-only Android
Nightly subset now builds one Debug APK, runs V1 13/13 and then V2 5/5 on an
arm64 API 36.1 emulator, independently validates each captured evidence graph,
and can seal both ordered lanes plus schema-2 provenance into one deterministic
USTAR for local and downloaded-byte readback. V2 includes future-version
saved-data preservation and versionless-data migration stability across two
distinct cold launches each. Its exact contract passes 101/101, and fresh
local evidence binds both lanes to the same
49,508,473-byte APK. The latest local pair has standalone readback but no
commit-bound provenance or USTAR claim. No
successful hosted scheduled run of the current workflow bytes is recorded, and
this Android emulator subset does not satisfy the complete Nightly product row.
Hosted run
...
relabeled, broad mixed suites are not invoked, and no complete or successfully
hosted current-source Nightly, physical-device, network, signing, publication,
or deployment evidence is claimed. SwiftPM and Gradle still compile their
complete package/app test-source graphs before applying the execution
selectors, so this is not a separately built non-security test target. The
Android hosted lanes also depend on their runner images' SDK 36/Build Tools
36.0.0 inventory and normal dependency downloads; dependency-byte hermeticity
remains open.
