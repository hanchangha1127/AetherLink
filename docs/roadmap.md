...
2. Screenshot-based client UI polish for a cleaner modern/classic chat surface, drawer, model selector, settings, and transcript spacing. Current no-device guardrail: a representative populated Android chat surface now renders at compact width and large font across the supported app languages with top bar, transcript attachment, reasoning, latest actions, jump control, and composer bounds checked together; real screenshots and physical-device review still remain.
...
6. Continue expanding smoke tests beyond the current pairing, authenticated model list, streaming chat, cancel, attachments, model-residency unload behavior, unload-failure health redaction, and untrusted-client rejection coverage.
