# Bounded dependency source acquisition Wave13 readback execution permit v1

This document explains the machine-checked, one-use Wave13 readback permit.
...
acquisition attempt `eb05816e0b897ea8c3ad8b7089668e91`; it does not authorize
...
`build/offline-source/pion-ice-v4.3.0/dependencies/.wave-13-v1-readback.claim`
...
The recorder independently holds an exact 27-file snapshot:

- 15 acquisition-authority files, including the Wave12 one-use claim retained
  as the V11 predecessor namespace anchor;
...
failure record and every portable name beginning `.wave-13-v1-staging-` must
...
- 411 `.mod` response bytes;
- 5,097,127 ZIP response bytes;
- 5,097,538 accepted response bytes;
- 1,647 ZIP entries;
- 20,065,482 ZIP-uncompressed bytes.

The aggregate ceilings are 4 MiB for `.mod` responses, 64 MiB for ZIP
responses, and 68 MiB across all accepted responses. Across all ZIPs, no more
than 80,000 entries or 512 MiB of uncompressed bytes are permitted.
...
V11 checker and tests are live-held predecessor authority; V10 tests are
...
`bounded-dependency-source-acquisition-wave13-readback-v1.json`, followed by
`bounded-dependency-source-acquisition-wave13-readback-manifest-v1.json`.
