# Wave7 bounded dependency-source acquisition permit v1

This document explains the machine-checked, one-use Wave7 acquisition permit.
The canonical authority record is the adjacent JSON file.

## Scope

- The permit covers exactly 30 public `GET` requests to
  `https://proxy.golang.org:443`: one `.mod` and one `.zip` resource for each
  of the 15 identity-complete Wave7 module/version vertices, in exact lexical
...
- The 30-resource portfolio retains conservative fixed aggregate response
...
- An immutable single-assignment phase ledger records dispatch boundaries and
  ...
  exactly 30 committed dispatch, response, validation, and persistence
  boundaries.
...
- Production fetches remain reachable only through the pinned
  `Wave7 -> Wave4` wrapper. Individual, aggregate `.mod`, aggregate ZIP,
