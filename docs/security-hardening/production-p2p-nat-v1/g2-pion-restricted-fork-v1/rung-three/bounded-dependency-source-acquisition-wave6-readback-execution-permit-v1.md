# Bounded dependency source acquisition Wave6 readback execution permit v1

This permit authorizes exactly one offline, independent readback of the
already-consumed successful Wave6 v1 source acquisition. It does not authorize
another acquisition attempt. The frozen acquisition attempt is
`5e0828c2e5dc1ce7ef2a06dd235d5076`.

The readback recorder must first create
`build/offline-source/pion-ice-v4.3.0/dependencies/.wave-6-v1-readback.claim`
...
directories, evidence, 36 accepted files, acquisition receipt, and acquisition
manifest. This is an exact 54-file frozen snapshot. Exact path, raw SHA-256,
byte count, mode, owner UID, link count, and directory inventory are required.
The acquisition failure record must remain lexically absent. Every Wave6 v1
...
bindings, recomputes the exact 36-resource order and aggregate byte counters,
...
The fixed totals are 18 mod files, 18 ZIP files, 2,220 mod-response bytes,
36,113,195 ZIP-response bytes, 36,115,415 accepted response bytes,
7,758 ZIP entries, and 138,523,078 ZIP-uncompressed bytes. The per-ZIP
...
interchanged. All 18 tuples retain
...
`bounded-dependency-source-acquisition-wave6-readback-v1.json`, followed by
`bounded-dependency-source-acquisition-wave6-readback-manifest-v1.json`; both
...
`.bounded-dependency-source-acquisition-wave6-readback-v1.json.tmp-` or
`.bounded-dependency-source-acquisition-wave6-readback-manifest-v1.json.tmp-`.
