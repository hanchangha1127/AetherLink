# Bounded dependency source acquisition Wave19 readback execution permit v1

This document explains the machine-checked, one-use Wave19 readback permit.
...
acquisition attempt `f10c20196d994afe3a8eba830eb42614`; it does not authorize
...
`build/offline-source/pion-ice-v4.3.0/dependencies/.wave-19-v1-readback.claim`
...
The recorder must independently hold an exact 23-file snapshot:

- 15 acquisition-authority files, including the Wave18 one-use claim retained
  as the Combined V17 predecessor namespace anchor;
- the Wave19 acquisition claim and evidence;
- 4 accepted resources: 2 `.mod` files and 2 ZIP files;
- the Wave19 acquisition receipt and manifest.

The final and accepted directory inventories are exact. The acquisition
failure record and every portable name beginning `.wave-19-v1-staging-` must
...
two tuples and four ordered resources, recomputes every `.mod` Hash1 H1 value,
and recomputes the module-ZIP HashZip/Hash1 H1 value. ZIP checks
...
- 2 `.mod` files and 2 ZIP files;
- 415 `.mod` response bytes;
- 11,453,540 ZIP response bytes;
- 11,453,955 accepted response bytes;
- 931 ZIP entries;
- 46,404,827 ZIP-uncompressed bytes.

The aggregate ceilings are 2 MiB for `.mod` responses, 32 MiB for ZIP
responses, and 34 MiB across all accepted responses. Across all ZIPs, no more
than 40,000 entries or 256 MiB of uncompressed bytes are permitted.

Both tuples and all four resources carry strict
`selectedByGraphAlgorithm: false`. This is a verified graph fact, not
readback, acquisition, product, or release authority. The Combined V17 checker
and tests are live-held predecessor authority; V16 tests are not live-held.
...
`bounded-dependency-source-acquisition-wave19-readback-v1.json`, followed by
`bounded-dependency-source-acquisition-wave19-readback-manifest-v1.json`.
