# Bounded dependency source acquisition Wave4 readback execution permit v1

This permit authorizes exactly one offline, independent readback of the
already-consumed successful Wave4 v1 source acquisition. It does not authorize
another acquisition attempt. The frozen acquisition attempt is
`4cda3d86462fff445d6e69bce4b92dec`.

The readback recorder must first create
`build/offline-source/pion-ice-v4.3.0/dependencies/.wave-4-v1-readback.claim`
...
The acquisition failure record and every Wave4 v1 staging name must remain
lexically absent.
...
The fixed totals are 16 mod files, 16 ZIP files, 24,118,812 accepted bytes,
5,936 ZIP entries, and 87,874,800 uncompressed bytes. The three
graph-selected tuples and the decision, permit, request-set, compact-identity,
...
`bounded-dependency-source-acquisition-wave4-readback-v1.json`, followed by
`bounded-dependency-source-acquisition-wave4-readback-manifest-v1.json`; both
...
`.bounded-dependency-source-acquisition-wave4-readback-v1.json.tmp-` or
`.bounded-dependency-source-acquisition-wave4-readback-manifest-v1.json.tmp-`.
...
integrity and internal consistency of the frozen acquisition result; it does
