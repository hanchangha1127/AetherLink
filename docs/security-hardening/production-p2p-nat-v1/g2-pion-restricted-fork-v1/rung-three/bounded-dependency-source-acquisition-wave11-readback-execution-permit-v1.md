# Bounded dependency source acquisition Wave11 readback execution permit v1

This permit authorizes exactly one offline, independent readback of the
already-consumed successful Wave11 v1 source acquisition. It does not authorize
another acquisition attempt. The frozen acquisition attempt is
`ac18b8fda0a80a132510efd5dd17d5b7`.

The readback recorder must first create
`build/offline-source/pion-ice-v4.3.0/dependencies/.wave-11-v1-readback.claim`
...
directories, evidence, 18 accepted files, acquisition receipt, and acquisition
manifest. This is an exact 36-file frozen snapshot. Exact path, raw SHA-256,
byte count, mode, owner UID, link count, and directory inventory are required.
The acquisition failure record must remain lexically absent. Every Wave11 v1
...
bindings, recomputes the exact 18-resource order and aggregate byte counters,
...
The fixed totals are 9 mod files, 9 ZIP files, 617 mod-response bytes,
16,363,277 ZIP-response bytes, 16,363,894 accepted response bytes,
3,329 ZIP entries, and 64,428,507 ZIP-uncompressed bytes. The per-ZIP
...
interchanged. All nine exact tuples and all 18 resources carry strict boolean
`selectedByGraphAlgorithm: false`. This selector is a verified graph fact, not
readback, acquisition, release, or product authority. The decision, permit, request-set,
compact-identity, full-witness, held-source-binding, exact request-resource,
and combined fixed-point V9 predecessor digests are also recomputed or exactly
bound. The 14-file authority uses the exact V9 checker and live-held V9 tests
referenced by the Wave11 acquisition permit. V8 tests remain historical-only
...
`bounded-dependency-source-acquisition-wave11-readback-v1.json`, followed by
`bounded-dependency-source-acquisition-wave11-readback-manifest-v1.json`; both
...
`.bounded-dependency-source-acquisition-wave11-readback-v1.json.tmp-` or
`.bounded-dependency-source-acquisition-wave11-readback-manifest-v1.json.tmp-`.
