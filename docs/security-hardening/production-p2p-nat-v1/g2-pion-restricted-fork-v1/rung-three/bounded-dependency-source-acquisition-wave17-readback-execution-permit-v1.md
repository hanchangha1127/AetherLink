# Bounded dependency source acquisition Wave17 readback execution permit v1

This document explains the machine-checked, one-use Wave17 readback permit.
...
acquisition attempt `117fb836380658986632911b9508e274`; it does not authorize
...
`build/offline-source/pion-ice-v4.3.0/dependencies/.wave-17-v1-readback.claim`
...
The recorder must independently hold an exact 21-file snapshot:

- 15 acquisition-authority files, including the Wave16 one-use claim retained
  as the Combined V15 predecessor namespace anchor;
- the Wave17 acquisition claim and evidence;
- 2 accepted `.mod` and ZIP resources;
- the Wave17 acquisition receipt and manifest.

The final and accepted directory inventories are exact. The acquisition
failure record and every portable name beginning `.wave-17-v1-staging-` must
...
one tuple and two ordered resources, recomputes the `.mod` Hash1 H1 value,
...
- 1 `.mod` file and 1 ZIP file;
- 301 `.mod` response bytes;
- 3,450,399 ZIP response bytes;
- 3,450,700 accepted response bytes;
- 1,550 ZIP entries;
- 9,108,004 ZIP-uncompressed bytes.

The aggregate ceilings are 1 MiB for `.mod` responses, 16 MiB for ZIP
responses, and 17 MiB across all accepted responses. Across all ZIPs, no more
than 20,000 entries or 128 MiB of uncompressed bytes are permitted.

The tuple and both resources carry strict
`selectedByGraphAlgorithm: false`. This is a verified graph fact, not
readback, acquisition, product, or release authority. The Combined V15 checker
and tests are live-held predecessor authority; V14 tests are not live-held.
...
`bounded-dependency-source-acquisition-wave17-readback-v1.json`, followed by
`bounded-dependency-source-acquisition-wave17-readback-manifest-v1.json`.
