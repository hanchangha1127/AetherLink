# Wave10 bounded dependency-source acquisition permit v1

This document explains the machine-checked, one-use Wave10 acquisition permit.
...
- The canonical direct command is
  `["/Applications/Xcode.app/Contents/Developer/usr/bin/python3", "-I", "-B", "-S", "script/acquire_p2p_nat_g2_pion_rung3_dependency_wave10_v1_once.py", "--execute"]`
...
- The execution gate also reads process arguments from macOS
  ...
  `["/Applications/Xcode.app/Contents/Developer/Library/Frameworks/Python3.framework/Versions/3.9/Resources/Python.app/Contents/MacOS/Python", "-I", "-B", "-S", "script/acquire_p2p_nat_g2_pion_rung3_dependency_wave10_v1_once.py", "--execute"]`.
...
- The permit covers exactly 22 public `GET` requests to
  `https://proxy.golang.org:443`: one `.mod` and one `.zip` resource for each
  of the 11 identity-complete Wave10 module/version vertices, in exact bound
...
- Module paths and versions use the decision's lowercase-ASCII direct-proxy
  ...
  or collapse them.
- The graph algorithm selected only tuple 11,
  `golang.org/x/xerrors@v0.0.0-20190717185122-a985d3407aa7`, so request
  ordinals 21 and 22 carry `selectedByGraphAlgorithm: true`; every other
  request carries `false`. That flag is a bound graph fact, not acquisition,
  publication, release, or product authority. The acquisition permit still
  covers all 22 exact resources.
...
- The 22-resource portfolio retains conservative fixed aggregate response
...
- An immutable single-assignment phase ledger records dispatch boundaries and
  ...
  exactly 22 committed dispatch, response, validation, and persistence
  boundaries.
...
- Production fetches remain reachable only through the pinned
  `Wave10 -> Wave4` wrapper. Individual, aggregate `.mod`, aggregate ZIP,
  aggregate total, and ZIP inventory limits fail closed before a resource can
  be counted as validated or persisted.
- The live predecessor binding is the exact V8 checker and V8 tests package.
  V7 tests appear only as historical metadata and are not a live-held input to
  this permit.
