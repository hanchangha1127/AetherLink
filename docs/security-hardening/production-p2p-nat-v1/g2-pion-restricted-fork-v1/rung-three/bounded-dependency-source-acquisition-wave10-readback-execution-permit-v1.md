# Bounded dependency source acquisition Wave10 readback execution permit v1

This permit authorizes exactly one offline, independent readback of the
already-consumed successful Wave10 v1 source acquisition. It does not authorize
another acquisition attempt. The frozen acquisition attempt is
`ffe70ee4562fcfc9e0fd6c9c4e136bd9`.

The readback recorder must first create
`build/offline-source/pion-ice-v4.3.0/dependencies/.wave-10-v1-readback.claim`
...
directories, evidence, 22 accepted files, acquisition receipt, and acquisition
manifest. This is an exact 40-file frozen snapshot. Exact path, raw SHA-256,
byte count, mode, owner UID, link count, and directory inventory are required.
The acquisition failure record must remain lexically absent. Every Wave10 v1
...
Each held file or directory records every initial relative directory component,
its descriptor and initial safe shape, and its held parent/name edge. Its
barrier verifies the held descriptor, that retained parent/name edge, and a
fresh no-follow reopen of the complete current component prefix. This includes
the already-open claim descriptor and the later receipt and manifest holds.
An ancestor replacement observed at a barrier therefore fails closed even if
the same terminal subtree and file inode have been moved back beneath the new
ancestor. A project-root replacement observed at a barrier also fails closed.
Every root-relative intermediate directory descriptor opened for the retained
snapshot remains owned through verification and cleanup.
...
bindings, recomputes the exact 22-resource order and aggregate byte counters,
...
The fixed totals are 11 mod files, 11 ZIP files, 1,400 mod-response bytes,
27,772,126 ZIP-response bytes, 27,773,526 accepted response bytes,
4,872 ZIP entries, and 108,563,141 ZIP-uncompressed bytes. The per-ZIP
...
interchanged. Exactly tuple 11,
`golang.org/x/xerrors@v0.0.0-20190717185122-a985d3407aa7`, is selected by the
graph algorithm, so only request ordinals 21 and 22 carry
`selectedByGraphAlgorithm: true`; every other resource carries strict boolean
`false`. This selector is a verified graph fact, not readback, acquisition,
release, or product authority. The decision, permit, request-set,
compact-identity, full-witness, held-source-binding, exact request-resource,
and combined fixed-point V8 predecessor digests are also recomputed or exactly
bound. The 14-file authority uses the exact V8 checker and live-held V8 tests
referenced by the Wave10 acquisition permit. V7 tests remain historical-only
metadata and are absent from live readback authority.
...
`bounded-dependency-source-acquisition-wave10-readback-v1.json`, followed by
`bounded-dependency-source-acquisition-wave10-readback-manifest-v1.json`; both
...
The command records successful return from the one-use execution before it
canonicalizes or writes the success report to stdout. If canonicalization or
the first stdout write then fails, the fallback state is exactly
`consumed_success_reporting_failed` with failure code
`E_POST_SUCCESS_REPORTING`, phase `reporting`, retry forbidden,
`readbackPublicationComplete: true`, and
`completionAppliesToRetainedSnapshot: true`. It never reports
`failed_closed_not_consumed` after successful execution return. A persistent
stdout failure may still propagate and does not change the already-complete
filesystem namespace. A preflight-only reporting failure remains
...
`.bounded-dependency-source-acquisition-wave10-readback-v1.json.tmp-` or
`.bounded-dependency-source-acquisition-wave10-readback-manifest-v1.json.tmp-`.
