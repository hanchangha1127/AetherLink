# Wave13 bounded dependency-source acquisition permit v1

This document explains the machine-checked, one-use Wave13 acquisition permit.
...
- The permit covers exactly 8 public `GET` requests to
  ...
  `golang.org/x/mod@v0.26.0`, `golang.org/x/net@v0.42.0`,
  `golang.org/x/sys@v0.34.0`, and
...
- The bound limits are 1 MiB per `.mod`, 16 MiB per ZIP, 4 MiB aggregate
  `.mod`, 64 MiB aggregate ZIP, and 68 MiB aggregate total. Across all four
  ZIPs, at most 80,000 entries and 512 MiB uncompressed bytes are allowed.
...
- The exact historical
  ...
  Wave13 claim and does not mark this permit consumed.
...
- The canonical command is
  `["/Applications/Xcode.app/Contents/Developer/usr/bin/python3", "-I", "-B", "-S", "script/acquire_p2p_nat_g2_pion_rung3_dependency_wave13_v1_once.py", "--execute"]`
