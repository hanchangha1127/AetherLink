# Wave9 bounded dependency-source acquisition permit v1

This document explains the machine-checked, one-use Wave9 acquisition permit.
...
- The canonical direct command is
  `["/Applications/Xcode.app/Contents/Developer/usr/bin/python3", "-I", "-B", "-S", "script/acquire_p2p_nat_g2_pion_rung3_dependency_wave9_v1_once.py", "--execute"]`
...
- The execution gate also reads process arguments from macOS
  ...
  `["/Applications/Xcode.app/Contents/Developer/Library/Frameworks/Python3.framework/Versions/3.9/Resources/Python.app/Contents/MacOS/Python", "-I", "-B", "-S", "script/acquire_p2p_nat_g2_pion_rung3_dependency_wave9_v1_once.py", "--execute"]`.
...
- The permit covers exactly 20 public `GET` requests to
  `https://proxy.golang.org:443`: one `.mod` and one `.zip` resource for each
  of the 10 identity-complete Wave9 module/version vertices, in exact bound
...
- The 20-resource portfolio retains conservative fixed aggregate response
...
- An immutable single-assignment phase ledger records dispatch boundaries and
  ...
  exactly 20 committed dispatch, response, validation, and persistence
  boundaries.
...
- Production fetches remain reachable only through the pinned
  `Wave9 -> Wave4` wrapper. Individual, aggregate `.mod`, aggregate ZIP,
