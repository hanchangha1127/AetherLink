# Bounded dependency source acquisition Wave18 readback execution permit v1

This document explains the machine-checked, one-use Wave18 readback permit.
...
acquisition attempt `4380f5bbcd3366154b05111381ccab18`; it does not authorize
...
`build/offline-source/pion-ice-v4.3.0/dependencies/.wave-18-v1-readback.claim`
...
- 15 acquisition-authority files, including the Wave17 one-use claim retained
  as the Combined V16 predecessor namespace anchor;
- the Wave18 acquisition claim and evidence;
- 6 accepted resources: 3 `.mod` files and 3 ZIP files;
- the Wave18 acquisition receipt and manifest.

The final and accepted directory inventories are exact. The acquisition
failure record and every portable name beginning `.wave-18-v1-staging-` must
...
three tuples and six ordered resources, recomputes every `.mod` Hash1 H1 value,
...
- 279 `.mod` response bytes;
- 2,108,821 ZIP response bytes;
- 2,109,100 accepted response bytes;
- 971 ZIP entries;
- 7,225,800 ZIP-uncompressed bytes.
...
readback, acquisition, product, or release authority. The Combined V16 checker
and tests are live-held predecessor authority; V15 tests are not live-held.
...
`bounded-dependency-source-acquisition-wave18-readback-v1.json`, followed by
`bounded-dependency-source-acquisition-wave18-readback-manifest-v1.json`.
