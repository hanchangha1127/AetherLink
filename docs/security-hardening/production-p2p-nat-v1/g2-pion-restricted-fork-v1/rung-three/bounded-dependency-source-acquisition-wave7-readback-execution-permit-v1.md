# Bounded dependency source acquisition Wave7 readback execution permit v1

This permit authorizes exactly one offline, independent readback of the
already-consumed successful Wave7 v1 source acquisition. It does not authorize
another acquisition attempt. The frozen acquisition attempt is
`c15f4504ae880326144eca93dc91e37b`.

The readback recorder must first create
`build/offline-source/pion-ice-v4.3.0/dependencies/.wave-7-v1-readback.claim`
...
manifest. This is an exact 48-file frozen snapshot. Exact path, raw SHA-256,
byte count, mode, owner UID, link count, and directory inventory are required.
The acquisition failure record must remain lexically absent. Every Wave7 v1
...
The fixed totals are 15 mod files, 15 ZIP files, 1,908 mod-response bytes,
32,350,343 ZIP-response bytes, 32,352,251 accepted response bytes,
6,469 ZIP entries, and 128,224,223 ZIP-uncompressed bytes. The per-ZIP
...
interchanged. All 15 tuples retain
...
fixed-point V5 predecessor digests are also recomputed or exactly bound. The
14-file authority uses the exact V5 checker and test bindings referenced by
the Wave7 acquisition permit instead of an unrelated candidate pair.
...
`bounded-dependency-source-acquisition-wave7-readback-v1.json`, followed by
`bounded-dependency-source-acquisition-wave7-readback-manifest-v1.json`; both
...
`.bounded-dependency-source-acquisition-wave7-readback-v1.json.tmp-` or
`.bounded-dependency-source-acquisition-wave7-readback-manifest-v1.json.tmp-`.
