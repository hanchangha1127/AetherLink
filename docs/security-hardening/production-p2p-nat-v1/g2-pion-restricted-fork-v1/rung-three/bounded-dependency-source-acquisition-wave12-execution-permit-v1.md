# Wave12 bounded dependency-source acquisition permit v1

This document explains the machine-checked, one-use Wave12 acquisition permit.
...
- The permit covers exactly 8 public `GET` requests to
  `https://proxy.golang.org:443`: `.mod` then `.zip` for
  `golang.org/x/crypto@v0.41.0`, `golang.org/x/term@v0.34.0`,
  `golang.org/x/text@v0.28.0`, and `golang.org/x/tools@v0.35.0`.
...
- The bound limits are 1 MiB per `.mod`, 16 MiB per ZIP, 8 MiB aggregate
  `.mod`, 128 MiB aggregate ZIP, and 128 MiB aggregate total, plus the
  canonical ZIP-inventory and deadline limits. Accepted bytes require the
  bound `h1:` verification. ZIPs additionally require an exact
  module/version prefix, safe shape, CRC validation, expansion bounds, and
  `.mod` parity. No extraction is authorized.
...
- Claim and namespace uncertainty fail closed as consumed uncertainty. Failure
  ...
  and exactly 8 committed dispatch, response, validation, and persistence
  boundaries.
...
- The canonical command is
  `["/Applications/Xcode.app/Contents/Developer/usr/bin/python3", "-I", "-B", "-S", "script/acquire_p2p_nat_g2_pion_rung3_dependency_wave12_v1_once.py", "--execute"]`
