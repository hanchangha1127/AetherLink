# Bounded dependency source acquisition Wave8 readback execution permit v1

This permit authorizes exactly one offline, independent readback of the
already-consumed successful Wave8 v1 source acquisition. It does not authorize
another acquisition attempt. The frozen acquisition attempt is
`6d8ea4473126c853b439c56a895f9c28`.

The readback recorder must first create
`build/offline-source/pion-ice-v4.3.0/dependencies/.wave-8-v1-readback.claim`
...
directories, evidence, 28 accepted files, acquisition receipt, and acquisition
...
The acquisition failure record must remain lexically absent. Every Wave8 v1
...
bindings, recomputes the exact 28-resource order and aggregate byte counters,
...
The fixed totals are 14 mod files, 14 ZIP files, 1,730 mod-response bytes,
35,195,229 ZIP-response bytes, 35,196,959 accepted response bytes,
4,907 ZIP entries, and 144,867,307 ZIP-uncompressed bytes. The per-ZIP
...
interchanged. All 14 tuples retain
...
fixed-point V6 predecessor digests are also recomputed or exactly bound. The
14-file authority uses the exact V6 checker and test bindings referenced by
the Wave8 acquisition permit instead of an unrelated candidate pair.
...
`bounded-dependency-source-acquisition-wave8-readback-v1.json`, followed by
`bounded-dependency-source-acquisition-wave8-readback-manifest-v1.json`; both
...
`.bounded-dependency-source-acquisition-wave8-readback-v1.json.tmp-` or
`.bounded-dependency-source-acquisition-wave8-readback-manifest-v1.json.tmp-`.
