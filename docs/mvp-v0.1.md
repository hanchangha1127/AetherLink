...
AetherLink v0.1 proves the smallest useful local loop: a client pairs with AetherLink Runtime, the runtime host talks to local model backends, and the client receives streamed chat output. The current implementation has mobile-client and desktop-runtime targets, but there is no cloud backend and no direct client-to-Ollama or client-to-LM-Studio access.

## Exact Scope

- AetherLink Runtime app runs the local runtime boundary.
...
- The normal chat picker shows installed runtime-host-local chat models. Ollama cloud/source metadata is compatibility metadata, not a default, recommendation, or normal v0.1 chat selection path.
...
- Additional client targets, Windows runtime targets, DGX OS-class runtime targets, or other runtime-host targets.
...
- AetherLink Runtime starts a local runtime transport.
- AetherLink Runtime displays remote-route QR pairing state and only generates a production pairing QR when eligible remote route material is available.
- Client scans the remote-route pairing QR, receives accepted pairing, stores the trusted runtime, and restores it after app restart.
...
- Runtime host stores chat processing events and exposes authenticated `chat.sessions.list` and `chat.messages.list` for runtime-owned session summaries/transcripts. The client requests those history messages after authentication, includes archived runtime sessions when syncing summaries, and merges them into its local UI cache while keeping local-only/manual-title state and deletion suppression.
...
- `apps/android/core/transport`: current client transport for the runtime host.
...
2. Harden runtime-host-side Ollama adapter tests for health, model list, streaming, cancellation, and structured errors.
3. Stabilize the runtime-host development transport and runtime message router.
4. Connect the AetherLink Runtime app lifecycle to start/stop the local runtime transport cleanly.
5. Implement client runtime transport connection lifecycle and error mapping.
6. Wire client `runtime.health` and connection status UI.
7. Wire client `models.list`, model selection, and empty/error states.
8. Wire client `chat.send`, `chat.delta`, and `chat.done` into the chat UI.
9. Wire client `chat.cancel` and cancellation UI state.
...
2. Launch AetherLink Runtime and confirm local backend status is visible.
3. Configure an eligible remote route on the runtime host, generate the pairing QR, and scan it from the client.
...
The authenticated mock smoke proves the local protocol loop after pairing/auth. It does not automate the physical remote-route QR camera scan.
