...
An unmatched room now receives one monotonic deadline at first matcher insertion. The default is `60` seconds, capped by allocation lease expiry, and same-role replacement inherits the original deadline. Timeout, disconnect, match, generation invalidation, replacement cleanup, and close release exact matcher-owned source and identity waiting counts before the accepted socket permit. Active bridge activation cancels the waiting timer, and established encrypted forwarding remains unthrottled.
