...
- RAG/research action namespaces stay closed to the implemented contracts: `retrieval.query`, `index.documents.list`, `research.brief.create`, `research.notebooks.list`, `citation.resolve`, `source_anchor.resolve`, `chat.source_attribution.resolve`, and the four `trusted_source.*` review operations are active. `embeddings.*`, unsupported retrieval/index/research messages including `research.web.query`, unsupported citation/source-anchor/trusted-source messages, and `source_control.*` remain reserved. Research notebook traffic is authenticated and capability-gated, reuses revision-bound device grants and the existing chat stream, and must not expose source text, paths, URLs, backend settings, tokens, vectors, or prompt internals in notebook summaries.
