#!/usr/bin/env python3
"""Tests for the Wave6 acquisition readback permit package."""
...
SPEC = importlib.util.spec_from_file_location("wave6_readback_permit_tests", PATH)
C = importlib.util.module_from_spec(SPEC)
assert SPEC.loader is not None
SPEC.loader.exec_module(C)


class Wave6ReadbackPermitTests(unittest.TestCase):
    def test_01_frozen_snapshot_constants_are_exact(self):
        self.assertEqual(C.ATTEMPT_ID, "5e0828c2e5dc1ce7ef2a06dd235d5076")
        self.assertEqual(len(C.ACQUISITION_AUTHORITY), 14)
        self.assertEqual(len(C.ACCEPTED_FILES), 36)
        self.assertEqual(
        ...
        self.assertEqual(payload["frozenFileCount"], 54)
        self.assertEqual(payload["selectedTupleCount"], 0)
        self.assertEqual(payload["aggregateZipEntryCount"], 7_758)
        self.assertEqual(
    ...
    def test_03_exact_package_and_package_only_preflight(self):
        ...
        self.assertEqual(summary["acceptedResourceCount"], 36)
