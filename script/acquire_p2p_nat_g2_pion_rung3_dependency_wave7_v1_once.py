#!/usr/bin/env python3
"""Consume the Wave7 v1 permit once and acquire 30 verified resources."""
...
if not (
    ...
    raise RuntimeError("Wave7 acquisition requires `python3 -I -B -S`")
...
EXPECTED_CHECKER_RAW = "b07526b2ff74085c72bf967b4f26360bef94360007d29b813d42c33dbd614725"
...
class ImmutablePhaseLedger:
    ...
    def source_acquisition_state(self) -> str:
        ...
        if self.response_committed_count == 30:
...
def _validate_resource_contract(
    ...
    for index, source in enumerate(rows):
        ...
        require(
            ...
            and row["tupleId"] == f"wave7-{tuple_order:03d}-{digest[:12]}"
            and row["kind"] == kind
            and row["method"] == "GET"
...
def _attempt(
    ...
    *,
    ...
    try:
        ...
        failure = {
            "documentType": "aetherlink.wave7-source-acquisition-failure",
            "schemaVersion": "1.0",
...
def validate_execution_context() -> dict[str, Any]:
    ...
    return {
        "documentType":
            "aetherlink.wave7-source-acquisition-execution-context-check",
        "schemaVersion": "1.0",
...
def error_document(error: AcquisitionError) -> dict[str, Any]:
    ...
    return {
        "documentType": "aetherlink.wave7-source-acquisition-error",
        "schemaVersion": "1.0",
        "status": status,
