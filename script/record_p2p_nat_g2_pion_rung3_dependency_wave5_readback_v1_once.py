#!/usr/bin/env python3
"""Record one independent offline readback of the Wave5 v1 acquisition."""
...
if not (
    ...
    raise RuntimeError("Wave5 readback recorder requires `python3 -I -B -S`")
...
if O_NOFOLLOW is None:
    raise RuntimeError("Wave5 readback recorder requires O_NOFOLLOW")
...
EXPECTED_READBACK_CHECKER_RAW = "23de57b5b29d4f86f944a604fd1194e9c763627d7ebc25ce87e689e599292180"
...
def load_readback_checker() -> types.ModuleType:
    ...
    try:
        ...
        os.close(fd)
    module = types.ModuleType("wave5_acquisition_readback_permit_v1")
...
def verify_snapshot(snapshot: FrozenSnapshot) -> dict[str, Any]:
    ...
    permit_path = (
        PERMIT.BASE
        + "/bounded-dependency-source-acquisition-wave5-execution-permit-v1.json"
    )
    ...
    require(
        len(mod_by_tuple) == 15
        and aggregate_mod == 1_697
        and aggregate_zip == 26_122_192
        and aggregate == 26_123_889
        and aggregate_zip_entries == 6_038
        and aggregate_zip_uncompressed == 101_774_573
...
def create_readback_claim(
    ...
    claim = content_bound(
        {
            "documentType": "aetherlink.wave5-acquisition-readback-one-use-claim",
            "schemaVersion": "1.0",
            "status": "consumed_active",
    ...
    try:
        try:
            ...
            receipt = content_bound(
                {
                    "documentType": "aetherlink.wave5-source-acquisition-readback",
                    "schemaVersion": "1.0",
                    "status": (
                        "wave5_acquisition_retained_snapshot_"
            ...
            manifest = content_bound(
                {
                    "documentType": (
                        "aetherlink.wave5-source-acquisition-readback-manifest"
                    ),
                    "schemaVersion": "1.0",
                    "status": (
                        "wave5_acquisition_retained_snapshot_"
...
def main(argv: Sequence[str] | None = None) -> int:
    try:
        ...
        if args.preflight:
            result = {
                "documentType": "aetherlink.wave5-acquisition-readback-preflight",
                "schemaVersion": "1.0",
        ...
        sys.stdout.buffer.write(
            canonical_bytes(
                {
                    "documentType": "aetherlink.wave5-acquisition-readback-error",
                    "schemaVersion": "1.0",
                    "status": status,
