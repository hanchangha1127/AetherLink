#!/usr/bin/env python3
"""Tests for the externally pinned, read-only Wave5 candidate checker."""
...
def require_isolated_interpreter() -> None:
    ...
    if not (
        ...
        raise RuntimeError(
            "Wave5 candidate tests require unoptimized "
            "`python3 -I -B -S`"
        )
...
CHECKER_PATH = (
    ROOT
    / "script"
    / "check_p2p_nat_g2_pion_rung3_dependency_wave5_candidate_v1.py"
)
...
def preflight_checker_source(raw: bytes) -> str:
    ...
    compile_call = compile_calls[0]
        ...
        enclosing_function(compile_call) == "load_v3_checker",
        "compile scope",
    )
        ...
        and compile_call.args[1].id == "V3_CHECKER_PATH",
        "compile filename",
    )
    ensure(
    ...
    exec_call = exec_calls[0]
        ...
        enclosing_function(exec_call) == "load_v3_checker",
        "exec scope",
    )
...
def load_checker(preflighted_raw: bytes):
    ...
    module = types.ModuleType("wave5_candidate_tests_target")
...
class Wave5CandidateTests(unittest.TestCase):
    maxDiff = None
    ...
    def test_01_exact_external_package_pins(self):
        self.assertEqual(
            hashlib.sha256((ROOT / CHECKER.V3_CHECKER_PATH).read_bytes())
            .hexdigest(),
            CHECKER.V3_CHECKER_RAW_SHA256,
        )
        self.assertEqual(
            hashlib.sha256((ROOT / CHECKER.V3_TESTS_PATH).read_bytes())
            .hexdigest(),
            CHECKER.V3_TESTS_RAW_SHA256,
        )
        self.assertEqual(
            CHECKER.V3_CHECKER_NORMALIZED_SHA256,
    ...
    def test_03_tuple_projection_is_deterministic(self):
        first = CHECKER.wave5_rows(CHECKER.expected_frontier_rows())
        second = CHECKER.wave5_rows(CHECKER.expected_frontier_rows())
        self.assertEqual(first, second)
        self.assertEqual(
            [row["tupleOrder"] for row in first],
            list(range(1, 16)),
        )
        self.assertEqual(len({row["tupleId"] for row in first}), 15)
        self.assertEqual(
            first[0]["tupleId"],
            (
                "wave5-001-"
                + hashlib.sha256(
                    b"github.com/stretchr/testify\nv1.8.0\n"
                ).hexdigest()[:12]
            ),
        )
        self.assertTrue(
    ...
    def test_04_semantic_mutations_fail_even_when_content_is_rebound(self):
        base_candidate = {
            ...
            "inputSet": {
                "heldSourceInputCount": 133,
                "resourceCount": 132,
                "modCount": 66,
                "zipCount": 66,
                "uniqueModuleVersionTupleCount": 66,
                "aggregateRawByteSize": 97_140_866,
                "combinedInputSetSha256": CHECKER.V3_INPUT_SET_SHA256,
                "sourceBindings": [],
            },
            "graphDiscovery": {
                "newTupleCount": 15,
                ...
                "moduleNodeCount": 82,
                "moduleEdgeCount": 215,
                "nodeSetSha256": CHECKER.V3_NODE_SET_SHA256,
                "edgeSetSha256": CHECKER.V3_EDGE_SET_SHA256,
                "moduleNodeSetSha256":
                    CHECKER.V3_MODULE_NODE_SET_SHA256,
                "moduleEdgeSetSha256":
                    CHECKER.V3_MODULE_EDGE_SET_SHA256,
                "moduleGraphAndFrontierSha256":
                    CHECKER.V3_MODULE_GRAPH_AND_FRONTIER_SHA256,
                "reconstructionProjectionSha256":
                    CHECKER.V3_RECONSTRUCTION_PROJECTION_SHA256,
                "graphSha256": CHECKER.V3_GRAPH_SHA256,
                "exactFrontier": CHECKER.expected_frontier_rows(),
            },
            "coverage": {
                "archiveCount": 67,
                "aggregateEntryCount": 20_772,
                "aggregateUncompressedByteCount": 356_904_520,
                "goSourceFileCount": 16_479,
                "semanticParsedGoSourceCount": 14_850,
                "testdataSemanticExclusionCount": 1_629,
            },
            "checkerVerification": {
                "fullInputReconstructionCount": 2,
                "predecessorFullInputReconstructionCount": 2,
                "totalFullInputReconstructionCount": 4,
                "underlyingIndependentGraphAlgorithmCount": 8,
                "pinnedV2PredecessorExecuted": True,
                "canonicalGraphEqualityVerified": True,
                "calculatedFixedPointCandidate": False,
            },
            "operationCounters": {
                "heldSourceInputCount": 133,
                "heldTerminalEvidenceCount": 24,
                "heldToolInputCount": 4,
                "stableReadPassesPerHeldInput": 2,
                "fullSourceReconstructionCount": 4,
                "archiveOpenCount": 236,
        ...
        original = CHECKER.V3_CANDIDATE_CONTENT_SHA256
        original_source_bindings = CHECKER.V3_SOURCE_BINDINGS_SHA256
        CHECKER.V3_CANDIDATE_CONTENT_SHA256 = (
            rebound_base["contentBinding"]["sha256"]
        )
        CHECKER.V3_SOURCE_BINDINGS_SHA256 = CHECKER.sha256_bytes(
            CHECKER.compact_json_bytes([])
        )
        try:
            self.assertEqual(
                CHECKER.validate_v3_candidate(rebound_base),
                CHECKER.expected_frontier_rows(),
            )
        finally:
            CHECKER.V3_CANDIDATE_CONTENT_SHA256 = original
            CHECKER.V3_SOURCE_BINDINGS_SHA256 = original_source_bindings
        ...
        for section, key, value, expected_code in mutations:
            with self.subTest(section=section, key=key):
                ...
                original_source_bindings = (
                    CHECKER.V3_SOURCE_BINDINGS_SHA256
                )
                CHECKER.V3_CANDIDATE_CONTENT_SHA256 = (
                ...
                try:
                    with self.assertRaisesRegex(
                        CHECKER.Wave5CandidateFailure,
                        f"^{expected_code}$",
                    ):
                        CHECKER.validate_v3_candidate(mutated)
                finally:
                    CHECKER.V3_CANDIDATE_CONTENT_SHA256 = original
                    CHECKER.V3_SOURCE_BINDINGS_SHA256 = (
                        original_source_bindings
    ...
    def test_05_bootstrap_pin_rejects_hash_hardlink_and_symlink(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            target.chmod(0o644)
            with self.assertRaises(CHECKER.Wave5CandidateFailure):
    ...
    def test_07_live_candidate_is_exact_canonical_and_non_authorizing(self):
        ...
        self.assertEqual(
            candidate["sourceCandidateBinding"]["sourceBindingsSha256"],
            CHECKER.V3_SOURCE_BINDINGS_SHA256,
        )
        wave = candidate["wave"]
        self.assertEqual(wave["tupleCount"], 15)
        self.assertEqual(wave["graphSelectedTupleCount"], 0)
        self.assertEqual(
        ...
        self.assertEqual(
            wave["tuples"],
            CHECKER.wave5_rows(CHECKER.expected_frontier_rows()),
        )
        ...
        self.assertEqual(
            counters["directV3FullSourceReconstructionCount"],
            2,
        )
        self.assertEqual(counters["totalFullSourceReconstructionCount"], 4)
        self.assertEqual(counters["predecessorArchiveOpenCount"], 102)
        self.assertEqual(counters["directV3ArchiveOpenCount"], 134)
        self.assertEqual(counters["totalArchiveOpenCount"], 236)
        self.assertEqual(
            counters["inheritedFullSourceReconstructionCount"],
            4,
        )
        self.assertEqual(counters["inheritedArchiveOpenCount"], 236)
