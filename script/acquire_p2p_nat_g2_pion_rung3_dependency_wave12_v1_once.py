#!/usr/bin/env python3
"""Consume the Wave12 v1 permit once and acquire 8 verified resources."""
...
if not (
    ...
    raise RuntimeError("Wave12 acquisition requires `python3 -I -B -S`")
...
CHECKER_PATH = Path(__file__).with_name(
    "check_p2p_nat_g2_pion_rung3_dependency_wave12_acquisition_v1.py"
)
# Finalized reverse pin to the exact Wave12 acquisition checker bytes. The
...
EXPECTED_CHECKER_RAW = "b00ea74bf16e02d429ecf9130ac15ffba9b594a0ae105aa620cd2439cda9bcc1"
...
def _validate_resource_contract(
    ...
    for index, source in enumerate(rows):
        ...
        require(
            ...
            and row["tupleId"] == f"wave12-{tuple_order:03d}-{digest[:12]}"
            and row["kind"] == kind
            and row["method"] == "GET"
...
def _attempt(
    ...
    *,
    ...
    try:
        ...
        failure = {
            "documentType": "aetherlink.wave12-source-acquisition-failure",
            "schemaVersion": "1.0",
...
def validate_execution_context() -> dict[str, Any]:
    ...
    return {
        "documentType":
            "aetherlink.wave12-source-acquisition-execution-context-check",
        "schemaVersion": "1.0",
...
def error_document(error: AcquisitionError) -> dict[str, Any]:
    ...
    return {
        "documentType": "aetherlink.wave12-source-acquisition-error",
        "schemaVersion": "1.0",
        "status": status,
