#!/usr/bin/env python3
"""Adversarial offline tests for the one-use Wave10 acquisition runner."""
...
if not (
    ...
    raise RuntimeError("Wave10 runner tests require `python3 -I -B -S`")
...
RUNNER_PATH = Path(__file__).with_name(
    "acquire_p2p_nat_g2_pion_rung3_dependency_wave10_v1_once.py"
)
SPEC = importlib.util.spec_from_file_location(
    "wave10_source_acquirer_v1",
    RUNNER_PATH,
)
...
def fixture() -> tuple[list[dict[str, object]], dict[int, bytes]]:
    ...
    ordinal = 0
    for tuple_order in range(1, 11 + 1):
        if tuple_order == 11:
        ...
        tuple_id = f"wave10-{tuple_order:03d}-{digest[:12]}"
        selected = tuple_order == 11
        mod = f"module {module}\n\ngo 1.22\n".encode()
        archive = make_zip(
            module,
            version,
...
class Wave10RunnerTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_success_claims_before_22_fetches_and_manifest_is_last(self) -> None:
        rows, bodies = fixture()
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(calls, list(range(1, 23)))
            self.assertEqual(
                receipt["status"],
            ...
            self.assertEqual(len(list(accepted.iterdir())), 22)
            self.assertTrue(
                terminal(root, runner.CHECK.CLAIM_PATH).exists()
            )
            self.assertTrue(
    ...
    def test_canonical_preflight_context_composes_without_writes(self) -> None:
        ...
        self.assertEqual(result["requestCount"], 22)
    ...
    def test_semantic_or_cardinality_mutation_fails_before_claim(self) -> None:
        ...
        mutations = (
            ...
            lambda rows: rows.__setitem__(21, dict(rows[20])),
            lambda rows: rows[21].__setitem__(
                "requestOrdinal",
                23,
            ),
    ...
    def test_lower_ascii_direct_proxy_identity_is_enforced(self) -> None:
        def rewritten_pair(module: str, version: str):
            ...
            for index, kind in enumerate(("mod", "zip")):
                ...
                rows[index]["tupleId"] = f"wave10-001-{digest[:12]}"
    ...
    def test_wave10_direct_fetch_reaches_wave4_and_translates_errors(
        self,
    ) -> None:
        resource = fixture()[0][0]
