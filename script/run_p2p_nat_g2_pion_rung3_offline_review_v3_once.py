#!/usr/bin/env python3
"""Run the separate single-use Pion ICE v4.3.0 v3 static inventory permit.

The default mode validates the tracked v3 permit only.  Explicit execution
creates a durable v3 claim, loads only checker-validated in-memory review
...
def require_isolated_interpreter() -> None:
    ...
    if not (
        ...
        raise RuntimeError(
            "offline review v3 runner requires unoptimized `python3 -I -B -S`"
        )
...
CHECKER_SOURCE_PATH = "script/check_p2p_nat_g2_pion_rung3_execution_permit_v3.py"
EXPECTED_CHECKER_RAW_SHA256 = (
    "dbb0e05fc63e39d0947eeb0bc9b66182b8e560d621f32fbff92504a6f8fb29ce"
)
MAXIMUM_TRUST_ROOT_SOURCE_BYTES = 2_097_152

OUTPUT_PARENT = PurePosixPath("build/offline-source/pion-ice-v4.3.0/review-v3")
...
CANDIDATE_TOP_LEVEL_KEYS = frozenset(
    {
        "schemaVersion",
        "meaning",
        "sourceEntryCount",
        ...
        "representativeLimitPerRule",
        "completeObservationEncodingVersion",
        "representativeRankEncodingVersion",
        "totals",
        "patchUnits",
    }
)
CANDIDATE_TOTAL_KEYS = frozenset(
    {"hitCount", "recordedRepresentativeCount", "omittedHitCount", "truncated"}
)
CANDIDATE_PATCH_UNIT_KEYS = frozenset(
    {
        "patchUnit",
        "meaning",
        "totalHitCount",
        "recordedRepresentativeCount",
        "omittedHitCount",
        "truncated",
        "completeObservationSha256",
        "rules",
    }
)
...
CANDIDATE_REPRESENTATIVE_KEYS = frozenset(
    {"path", "line", "ruleId", "rankSha256"}
)


class ReviewError(RuntimeError):
    """Fail-closed v3 review error."""
...
def read_stable_checker_source() -> bytes:
    ...
    require(
        parts == ("script", "check_p2p_nat_g2_pion_rung3_execution_permit_v3.py"),
        "v3 checker trust-root path drifted",
    )
    ...
    try:
        ...
        try:
            ...
            raise ReviewError(f"unable to open v3 checker trust root safely: {error}") from error
        before = os.fstat(source_fd)
        require(stat.S_ISREG(before.st_mode), "v3 checker trust root is not a regular file")
        require(before.st_uid == os.getuid(), "v3 checker trust root owner drifted")
        require(before.st_nlink == 1, "v3 checker trust-root link count drifted")
        require(not before.st_mode & 0o022, "v3 checker trust root is group- or world-writable")
        require(0 < before.st_size <= MAXIMUM_TRUST_ROOT_SOURCE_BYTES,
                "v3 checker trust-root byte size is invalid")
        chunks: list[bytes] = []
        ...
        require(
            ...
            "v3 checker trust root changed during its single read pass",
        )
        ...
        require(
            ...
            "v3 checker trust-root path no longer names the reviewed descriptor",
        )
        require(b"\x00" not in raw, "v3 checker trust root contains NUL")
        raw.decode("utf-8", errors="strict")
        return raw
    except UnicodeDecodeError as error:
        raise ReviewError("v3 checker trust root is not strict UTF-8") from error
    except OSError as error:
        raise ReviewError(f"unable to read v3 checker trust root safely: {error}") from error
    finally:
        if source_fd >= 0:
            os.close(source_fd)
        os.close(current_fd)
...
def load_checker_trust_root() -> ModuleType:
    ...
    require(
        ...
        "v3 checker trust-root raw digest mismatch",
    )
    try:
        code = compile(
            raw, CHECKER_SOURCE_PATH, "exec", flags=0, dont_inherit=True, optimize=0
        )
        module = ModuleType("g2_pion_rung3_execution_permit_v3_local_trust_root")
        ...
        raise ReviewError(f"unable to execute v3 checker trust-root bytes: {error}") from error
    require(callable(getattr(module, "validate_repository", None)),
            "v3 checker omitted validate_repository")
    require(callable(getattr(module, "load_validated_review_modules", None)),
            "v3 checker omitted load_validated_review_modules")
    return module
...
def write_all(fd: int, payload: bytes) -> None:
    ...
    while offset < len(payload):
        ...
        if written <= 0:
            raise ReviewError("short write while publishing v3 review evidence")
        offset += written
    ...
    try:
        for index, component in enumerate(OUTPUT_PARENT.parts):
            ...
            validate_directory_descriptor(
                next_fd,
                f"v3 review output ancestor {component!r}",
        ...
        raise ReviewError(f"unable to open secure v3 review output directory: {error}") from error
    finally:
        if current_fd >= 0:
            os.close(current_fd)
...
def name_exists(directory_fd: int, name: str) -> bool:
    try:
        ...
        raise ReviewError(f"unable to inspect v3 output name {name!r}: {error}") from error
    return True
...
def create_claim(directory_fd: int, permit_sha256: str) -> tuple[bytes, str]:
    for name in (CLAIM_NAME, RESULT_NAME, MANIFEST_NAME, TEMP_RESULT_NAME, TEMP_MANIFEST_NAME):
        if name_exists(directory_fd, name):
            raise ReviewError(f"single-use v3 review cannot start because {name!r} already exists")
    claim = {
    ...
    try:
        ...
        require(
            ...
            "v3 one-use claim failed ownership or mode validation",
        )
        os.fsync(fd)
        os.fsync(directory_fd)
        return payload, sha256_bytes(payload)
    except FileExistsError as error:
        raise ReviewError("one-use v3 review permit was already consumed") from error
    finally:
        if fd >= 0:
            os.close(fd)
...
def create_temporary_report(directory_fd: int, name: str, payload: bytes) -> int:
    require(len(payload) <= MAXIMUM_JSON_REPORT_BYTES,
            f"v3 report {name!r} exceeds its byte bound")
    try:
        fd = os.open(name, create_report_file_flags(), mode=0o600, dir_fd=directory_fd)
    except OSError as error:
        raise ReviewError(f"unable to create temporary v3 report {name!r}: {error}") from error
    try:
        os.fchmod(fd, 0o600)
        write_all(fd, payload)
        os.fsync(fd)
        validate_report_descriptor(fd, payload, f"temporary v3 report {name!r}", 1)
        require_name_matches_descriptor(directory_fd, name, fd, "temporary v3 report")
        return fd
    except BaseException:
        os.close(fd)
        raise
...
def validate_published_report(
    ...
    validate_report_descriptor(fd, payload, f"published v3 report {final_name!r}", 2)
    require_name_matches_descriptor(directory_fd, final_name, fd, "published v3 report")
...
def publish_no_replace(
    ...
    validate_report_descriptor(
        held_fd,
        expected_payload,
        f"temporary v3 report {temporary_name!r}",
        1,
    )
    require_name_matches_descriptor(directory_fd, temporary_name, held_fd, "temporary v3 report")
    try:
        os.link(
            temporary_name,
            final_name,
        ...
        validate_report_descriptor(
            held_fd,
            expected_payload,
            f"linked v3 report {final_name!r}",
            2,
        )
        require_name_matches_descriptor(directory_fd, temporary_name, held_fd, "linked temporary v3 report")
        require_name_matches_descriptor(directory_fd, final_name, held_fd, "linked final v3 report")
        os.fsync(directory_fd)
        ...
        try:
            ...
            if isinstance(error, OSError):
                raise ReviewError(
                    f"atomic no-replace v3 publication failed for {final_name!r}: {error}"
                ) from error
            raise ReviewError(
                f"v3 publication stopped before final name {final_name!r} appeared: "
                f"{type(error).__name__}: {error}"
            ) from error
        except OSError as stat_error:
            raise PublishedReportStateError(
                f"v3 review report {final_name!r} publication state cannot be inspected: "
                f"{stat_error}; original={type(error).__name__}: {error}"
            ) from error
        raise PublishedReportStateError(
            f"v3 review report {final_name!r} exists but publication completion is "
...
def read_pinned_receipt_archive_path(permit: Mapping[str, Any]) -> str:
    ...
    require(isinstance(binding, Mapping), "validated v3 permit omitted archiveIdentityBinding")
    receipt_path = binding.get("receiptPath")
    receipt_sha256 = binding.get("receiptRawSha256")
    require(receipt_path == RECEIPT_PATH, "validated v3 permit receipt path drifted")
    require(isinstance(receipt_sha256, str) and len(receipt_sha256) == 64,
            "v3 permit receipt hash is invalid")
...
def load_validated_authority() -> tuple[ModuleType, dict[str, Any]]:
    ...
    try:
        ...
        raise ReviewError(f"rung-three v3 execution permit validation failed: {error}") from error
    require(isinstance(result, dict), "v3 permit checker returned an invalid result")
    require(
        set(result)
        == {
            "permit",
        ...
        "v3 permit checker result schema drifted",
    )
    require(isinstance(result["permit"], dict), "v3 checker did not return the validated permit")
    require(
        ...
        "v3 checker returned an invalid permit digest",
    )
    require(
        result["archiveOpenCount"] == 0
        and result["archiveReadPassCount"] == 0
        and result["buildPathReadCount"] == 0
        and result["outputPathReadCount"] == 0
        and result["fileWriteCount"] == 0
        and result["permitConsumptionState"] == "not_inspected"
        and isinstance(result["authorityReadPaths"], tuple)
        and all(
            isinstance(path, str)
            and not path.startswith("build/")
            and not path.lower().endswith(".zip")
            for path in result["authorityReadPaths"]
        ),
        "v3 checker crossed its read-only pre-consumption boundary",
    )
...
def _u16_field(raw: bytes) -> bytes:
    require(len(raw) <= 0xFFFF, "candidate representative field exceeds u16")
    return struct.pack(">H", len(raw)) + raw


def representative_rank_sha256(path: str, line_number: int, rule_id: str) -> str:
    path_bytes = path.encode("utf-8", errors="strict")
    rule_bytes = rule_id.encode("utf-8", errors="strict")
    identity = (
    ...
    return sha256_bytes(REPRESENTATIVE_RANK_DOMAIN + identity)


def candidate_hit_identity(path_bytes: bytes, line_number: int, rule_id: str) -> bytes:
    rule_bytes = rule_id.encode("utf-8", errors="strict")
    return (
        struct.pack(">I", len(path_bytes))
        + path_bytes
...
def candidate_representative_sort_key(
...
def retain_candidate_representative(
    ...
    *,
    ...
    representative = {
        ...
        "line": line_number,
        "ruleId": rule_id,
        "rankSha256": representative_rank_sha256(path, line_number, rule_id),
    }
    representatives.append(representative)
    representatives.sort(key=candidate_representative_sort_key)
    if len(representatives) > REPRESENTATIVE_LIMIT_PER_RULE:
        representatives.pop()


def independently_aggregate_candidate_inventory(
    source_entries: Sequence[tuple[str, bytes]],
) -> dict[str, Any]:
    """Independently recompute the complete bounded v3 lexical observation.

    Only counts, one streaming digest per patch unit, and at most eight
    representatives per rule are retained.  No complete hit list is stored.
    """

    require(
        isinstance(source_entries, Sequence)
        and not isinstance(source_entries, (str, bytes))
        and len(source_entries) <= MAXIMUM_ENTRIES,
        "independent candidate source entry sequence exceeds its bound",
    )
    ordered_entries: list[tuple[bytes, str, bytes]] = []
    ...
    for entry in source_entries:
        require(
            ...
            "independent candidate source entry shape drifted",
        )
        path, raw = entry
        path_bytes = path.encode("utf-8", errors="strict")
        require(
            path_bytes
            and len(path_bytes) <= MAXIMUM_PATH_BYTES
            and path_bytes not in seen_paths
            and len(raw) <= MAXIMUM_TEXT_FILE_BYTES
            and b"\x00" not in raw,
            "independent candidate source path or byte bounds drifted",
        )
        seen_paths.add(path_bytes)
        total_source_bytes += len(raw)
        require(
            total_source_bytes <= MAXIMUM_TOTAL_UNCOMPRESSED_BYTES,
            "independent candidate source total bytes exceed their bound",
        )
        ordered_entries.append((path_bytes, path, raw))
    ...
    for patch_unit, rules in REVIEW_RULES.items():
        ...
        unit_states[patch_unit] = {
            ...
            "rules": {
                ...
                for rule_id, _pattern in rules
            },
        }

    total_logical_lines = 0
    for path_bytes, path, raw in ordered_entries:
        try:
            logical_lines = raw.decode("utf-8", errors="strict").splitlines()
        except UnicodeDecodeError as error:
            raise ReviewError(
                f"independent candidate source {path!r} is not strict UTF-8"
            ) from error
        require(
            len(logical_lines) <= MAXIMUM_LOGICAL_LINES_PER_SOURCE,
            f"independent candidate source {path!r} exceeds its line bound",
        )
        total_logical_lines += len(logical_lines)
        require(
            total_logical_lines <= MAXIMUM_TOTAL_LOGICAL_LINES,
            "independent candidate source lines exceed their total bound",
        )
            ...
            for patch_unit, rules in REVIEW_RULES.items():
                ...
                for rule_id, pattern in rules:
                    if re.search(pattern, line) is None:
                        continue
                    unit_state["hasher"].update(
                        candidate_hit_identity(path_bytes, line_number, rule_id)
                    )
                    unit_state["hasher"].update(line_sha256)
                    unit_state["total"] += 1
                    rule_state = unit_state["rules"][rule_id]
                    rule_state["total"] += 1
                    retain_candidate_representative(
        ...
        patch_units.append(
            {
                ...
                "meaning": LEXICAL_CANDIDATE_MEANING,
                "totalHitCount": unit_total,
                "recordedRepresentativeCount": unit_recorded,
                "omittedHitCount": unit_total - unit_recorded,
                "truncated": unit_total > unit_recorded,
                "completeObservationSha256": unit_state["hasher"].hexdigest(),
                "rules": rule_rows,
            }
        )
        overall_hits += unit_total
        overall_recorded += unit_recorded

    return {
        "schemaVersion": "3.0",
        "meaning": LEXICAL_CANDIDATE_MEANING,
        "sourceEntryCount": len(ordered_entries),
...
def validated_candidate_path(
    path: Any,
    *,
    source_lines_by_path: Mapping[str, int],
) -> str:
    require(isinstance(path, str) and path in source_lines_by_path,
            "candidate representative path is not an observed Go source")
    pure = PurePosixPath(path)
    require(
        path
        and "\\" not in path
        ...
        and len(path.encode("utf-8", errors="strict")) <= MAXIMUM_PATH_BYTES,
        "candidate representative path is unsafe or noncanonical",
    )
    return path
...
def validate_candidate_inventory(
    ...
    *,
    ...
    """Revalidate the pinned v3 aggregator's compact lexical output.

    This intentionally validates the complete arithmetic, exact schemas,
    patch/rule order, rank calculation, representative ordering, and
    source-path/line crosswalk without retaining or recording source bodies.
    """

    require(isinstance(candidate, dict), "v3 candidate aggregator returned a non-object")
    require(set(candidate) == CANDIDATE_TOP_LEVEL_KEYS,
            "v3 candidate aggregator top-level schema drifted")
    require(
        candidate.get("schemaVersion") == "3.0"
        and candidate.get("meaning") == LEXICAL_CANDIDATE_MEANING
        and type(candidate.get("representativeLimitPerRule")) is int
        and candidate.get("representativeLimitPerRule") == REPRESENTATIVE_LIMIT_PER_RULE
        and type(candidate.get("completeObservationEncodingVersion")) is int
        and candidate.get("completeObservationEncodingVersion") == 1
        and type(candidate.get("representativeRankEncodingVersion")) is int
        and candidate.get("representativeRankEncodingVersion") == 1,
        "v3 candidate aggregator identity or encoding drifted",
    )

    source_lines_by_path: dict[str, int] = {}
    total_source_bytes = 0
    ...
    for entry in source_entries:
        require(
            ...
            and isinstance(entry[0], str)
            and isinstance(entry[1], bytes),
            "runner supplied an invalid Go source entry to the aggregator",
        )
        path, raw = entry
        require(path not in source_lines_by_path, "runner supplied duplicate Go source path")
        require(len(raw) <= MAXIMUM_TEXT_FILE_BYTES and b"\x00" not in raw,
                f"Go source {path!r} exceeds text bounds or contains NUL")
        try:
            logical_lines = len(raw.decode("utf-8", errors="strict").splitlines())
        except UnicodeDecodeError as error:
            raise ReviewError(f"Go source {path!r} is not strict UTF-8") from error
        source_lines_by_path[path] = logical_lines
        total_source_bytes += len(raw)
        total_source_lines += logical_lines
    require(
        type(candidate.get("sourceEntryCount")) is int
        and candidate.get("sourceEntryCount") == len(source_entries)
        and type(candidate.get("sourceTotalBytes")) is int
        and candidate.get("sourceTotalBytes") == total_source_bytes
        and type(candidate.get("sourceLogicalLineCount")) is int
        and candidate.get("sourceLogicalLineCount") == total_source_lines,
        "v3 candidate source totals do not match the observed Go sources",
    )

    patch_rows = candidate.get("patchUnits")
    require(isinstance(patch_rows, list) and len(patch_rows) == len(PATCH_UNITS),
            "v3 candidate patch-unit rows are missing or duplicated")
    aggregate_hit_count = 0
    aggregate_recorded_count = 0
    for unit_index, (expected_unit, expected_rules) in enumerate(REVIEW_RULES.items()):
        unit = patch_rows[unit_index]
        require(isinstance(unit, dict) and set(unit) == CANDIDATE_PATCH_UNIT_KEYS,
                "v3 candidate patch-unit schema drifted")
        require(
            unit.get("patchUnit") == expected_unit
            and unit.get("meaning") == LEXICAL_CANDIDATE_MEANING
            and isinstance(unit.get("completeObservationSha256"), str)
            and re.fullmatch(r"[0-9a-f]{64}", unit["completeObservationSha256"]) is not None,
            f"v3 candidate patch-unit identity or digest drifted for {expected_unit!r}",
        )
        unit_total = exact_nonnegative_int(
            unit.get("totalHitCount"), f"{expected_unit} totalHitCount"
        )
        unit_recorded = exact_nonnegative_int(
            unit.get("recordedRepresentativeCount"),
            f"{expected_unit} recordedRepresentativeCount",
        )
        unit_omitted = exact_nonnegative_int(
            unit.get("omittedHitCount"), f"{expected_unit} omittedHitCount"
        )
        require(unit_total > 0, f"patch unit {expected_unit!r} has no lexical candidates")
        rules = unit.get("rules")
        require(isinstance(rules, list) and len(rules) == len(expected_rules),
                f"v3 candidate rule rows drifted for {expected_unit!r}")
        summed_total = 0
        summed_recorded = 0
        for rule_index, (expected_rule_id, expected_regex) in enumerate(expected_rules):
            rule = rules[rule_index]
            require(isinstance(rule, dict) and set(rule) == CANDIDATE_RULE_KEYS,
                    "v3 candidate rule schema drifted")
            require(
                rule.get("ruleId") == expected_rule_id
                and rule.get("regex") == expected_regex,
                f"v3 candidate rule identity drifted for {expected_rule_id!r}",
            )
            rule_total = exact_nonnegative_int(
                rule.get("totalHitCount"), f"{expected_rule_id} totalHitCount"
            )
            rule_recorded = exact_nonnegative_int(
                rule.get("recordedRepresentativeCount"),
                f"{expected_rule_id} recordedRepresentativeCount",
            )
            rule_omitted = exact_nonnegative_int(
                rule.get("omittedHitCount"), f"{expected_rule_id} omittedHitCount"
            )
            expected_recorded = min(rule_total, REPRESENTATIVE_LIMIT_PER_RULE)
            require(
                rule_recorded == expected_recorded
                and rule_omitted == rule_total - rule_recorded
                and rule.get("truncated") is (rule_total > rule_recorded),
                f"v3 candidate rule arithmetic drifted for {expected_rule_id!r}",
            )
            representatives = rule.get("representatives")
            require(
                isinstance(representatives, list)
                and len(representatives) == rule_recorded
                and len(representatives) <= REPRESENTATIVE_LIMIT_PER_RULE,
                f"v3 candidate representatives drifted for {expected_rule_id!r}",
            )
            expected_order: list[tuple[str, bytes, int, bytes]] = []
            identities: set[tuple[str, int, str]] = set()
            for representative in representatives:
                require(
                    isinstance(representative, dict)
                    and set(representative) == CANDIDATE_REPRESENTATIVE_KEYS,
                    "v3 candidate representative schema drifted",
                )
                path = validated_candidate_path(
                ...
                require(
                    type(line) is int
                    and 1 <= line <= source_lines_by_path[path]
                    and representative.get("ruleId") == expected_rule_id,
                    f"v3 candidate representative line/rule drifted for {expected_rule_id!r}",
                )
                identity = (path, line, expected_rule_id)
                require(identity not in identities,
                        f"v3 candidate representative is duplicated for {expected_rule_id!r}")
                identities.add(identity)
                rank = representative.get("rankSha256")
                require(
                    rank == representative_rank_sha256(path, line, expected_rule_id),
                    f"v3 candidate representative rank drifted for {expected_rule_id!r}",
                )
                expected_order.append(
                    (rank, path.encode("utf-8"), line, expected_rule_id.encode("utf-8"))
                )
            require(expected_order == sorted(expected_order),
                    f"v3 candidate representative order drifted for {expected_rule_id!r}")
            summed_total += rule_total
            summed_recorded += rule_recorded
        require(
            unit_total == summed_total
            and unit_recorded == summed_recorded
            and unit_omitted == unit_total - unit_recorded
            and unit.get("truncated") is (unit_total > unit_recorded),
            f"v3 candidate patch-unit arithmetic drifted for {expected_unit!r}",
        )
        aggregate_hit_count += unit_total
        aggregate_recorded_count += unit_recorded

    totals = candidate.get("totals")
    require(isinstance(totals, dict) and set(totals) == CANDIDATE_TOTAL_KEYS,
            "v3 candidate overall totals schema drifted")
    total_hits = exact_nonnegative_int(totals.get("hitCount"), "candidate total hitCount")
    total_recorded = exact_nonnegative_int(
        totals.get("recordedRepresentativeCount"),
        "candidate total recordedRepresentativeCount",
    )
    total_omitted = exact_nonnegative_int(
        totals.get("omittedHitCount"), "candidate total omittedHitCount"
    )
    require(
        total_hits == aggregate_hit_count
        and total_recorded == aggregate_recorded_count
        and total_omitted == total_hits - total_recorded
        and totals.get("truncated") is (total_hits > total_recorded),
        "v3 candidate overall arithmetic drifted",
    )
    canonical = canonical_json_bytes(candidate)
    require(len(canonical) <= MAXIMUM_JSON_REPORT_BYTES,
            "v3 candidate aggregator output exceeds its JSON byte bound")
    independently_recomputed = independently_aggregate_candidate_inventory(
        source_entries
    )
    require(
        candidate == independently_recomputed,
        "v3 candidate aggregator output does not exactly match the runner's "
...
def creator_provenance_by_relative_path(
    ...
    require(isinstance(policy, Mapping), "v3 ZIP inspection omitted creatorMetadataPolicy")
    ...
    require(isinstance(rows, Sequence) and not isinstance(rows, (str, bytes)),
            "v3 ZIP inspection omitted creator entry metadata")
...
def build_review_documents(
    *,
    ...
    aggregate_candidate_inventory: Any,
    ...
    require(
        ...
        "v3 ZIP inspection omitted entries",
    )
    ...
    for entry in entries:
        require(isinstance(entry, Mapping), "v3 ZIP inspection returned a non-object entry")
        path = entry.get("relativePath")
        raw = entry.get("bytes")
        size = entry.get("size")
        digest = entry.get("sha256")
        require(isinstance(path, str) and isinstance(raw, bytes), "v3 ZIP entry shape drifted")
        require(
            type(size) is int
            and size == len(raw)
            and isinstance(digest, str)
            and digest == sha256_bytes(raw),
            f"v3 ZIP entry metadata mismatch for {path!r}",
        )
        ...
        if category == "go_source":
            ...
            source_entries.append((path, raw))
    ...
    source_entries.sort(key=lambda entry: entry[0].encode("utf-8"))
    require(callable(aggregate_candidate_inventory),
            "validated v3 review adapter omitted aggregate_candidate_inventory")
    try:
        candidate_value = aggregate_candidate_inventory(tuple(source_entries))
    except Exception as error:
        raise ReviewError(f"offline v3 candidate aggregation failed: {error}") from error
    candidate_inventory = validate_candidate_inventory(
        candidate_value,
    ...
    result_document: dict[str, Any] = {
        ...
        "status": (
            "rung3_v3_exact_lexical_candidate_aggregation_recorded_"
            "awaiting_completion_manifest"
        ),
        "result": (
            "bounded_exact_candidate_totals_digests_and_ranked_representatives_"
            "recorded_semantic_review_not_performed"
        ),
    ...
    require(len(result_payload) <= MAXIMUM_JSON_REPORT_BYTES,
            "v3 review result exceeds its JSON byte bound")
    ...
    manifest_document: dict[str, Any] = {
        ...
        "result": (
            "bounded_v3_exact_candidate_totals_digests_and_ranked_"
            "representatives_publication_committed_semantic_review_not_performed"
        ),
    ...
    require(len(manifest_payload) <= MAXIMUM_JSON_REPORT_BYTES,
            "v3 review manifest exceeds its JSON byte bound")
...
def check_permit() -> dict[str, Any]:
    ...
    return {
        "documentType": "aetherlink.g2-pion-rung3-offline-review-v3-run-result",
        "schemaVersion": "3.0",
        "status": "v3_permit_validated_consumption_state_not_inspected",
...
def execute_permit() -> dict[str, Any]:
    ...
    if output_fd is None:
        raise ReviewError("secure v3 review output directory was not created")
    archive_fd = -1
    archive_parent_fd = -1
    result_temp_fd = -1
    manifest_temp_fd = -1
    ...
    try:
        ...
        try:
            ...
            raise ReviewError(
                f"validated v3 review module load failed after permit consumption: {error}"
            ) from error
        inspect_module_zip = getattr(adapter, "inspect_module_zip", None)
        require(callable(inspect_module_zip), "validated v3 review adapter omitted inspect_module_zip")
        aggregate_candidate_inventory = getattr(
            adapter, "aggregate_candidate_inventory", None
        )
        require(
            callable(aggregate_candidate_inventory),
            "validated v3 review adapter omitted aggregate_candidate_inventory",
        )
        ...
        try:
            ...
            raise ReviewError(f"offline ZIP v3 validation failed: {error}") from error
        require(isinstance(inspection, Mapping), "v3 review adapter returned a non-object")
        result_payload, manifest_payload, _result_document, manifest_document = (
            build_review_documents(
                ...
                aggregate_candidate_inventory=aggregate_candidate_inventory,
            )
        )
        ...
        require(len(published) == 2, "v3 review reports were not both published")
        validate_published_report(
            output_fd,
            RESULT_NAME,
            result_temp_fd,
        ...
        if observed_final_names or final_state_inspection_error is not None:
            raise PublishedReportStateError(
                "offline v3 review partially published evidence and is fail-closed; "
        ...
        if isinstance(error, ReviewError):
            ...
            raise
        raise ReviewError(
            f"offline v3 review was consumed and failed closed with "
