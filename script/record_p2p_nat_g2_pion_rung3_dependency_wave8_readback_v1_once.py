#!/usr/bin/env python3
"""Record one independent offline readback of the Wave8 v1 acquisition."""
...
if not (
    ...
    raise RuntimeError("Wave8 readback recorder requires `python3 -I -B -S`")
...
if O_NOFOLLOW is None:
    raise RuntimeError("Wave8 readback recorder requires O_NOFOLLOW")
READBACK_CHECKER_PATH = Path(__file__).with_name(
    "check_p2p_nat_g2_pion_rung3_dependency_wave8_"
    "readback_execution_permit_v1.py"
)
EXPECTED_READBACK_CHECKER_RAW = "f1073471aecc3d4003e6e85fdbdc5add92064ee9576d9b285e97fde66b59f4a9"
...
def load_readback_checker() -> types.ModuleType:
    ...
    module = types.ModuleType("wave8_acquisition_readback_permit_v1")
...
def verify_snapshot(snapshot: FrozenSnapshot) -> dict[str, Any]:
    ...
    permit_path = (
        PERMIT.BASE
        + "/bounded-dependency-source-acquisition-wave8-execution-permit-v1.json"
    )
    ...
    v6 = acquisition_permit["predecessorBindings"]["combinedFixedPointV6"]
    predecessor_authority_rows = [
        {
            "path": v6["checkerPath"],
            "rawSha256": v6["checkerRawSha256"],
        },
        {
            "path": v6["testsPath"],
            "rawSha256": v6["testsRawSha256"],
        },
    ]
    ...
    require(
        ...
        == {
            ...
            "checkerNormalizedSha256": (
                PERMIT.EXPECTED_V6_CHECKER_NORMALIZED
            ),
            "testsPath": (
            ...
            "contentSha256": PERMIT.EXPECTED_V6_CONTENT,
            "combinedInputSetSha256": PERMIT.EXPECTED_V6_INPUT_SET,
            "graphSha256": PERMIT.EXPECTED_V6_GRAPH,
            "frontierSha256": PERMIT.EXPECTED_V6_FRONTIER,
    ...
    require(
        len(resources) == 28
        and [row["requestOrdinal"] for row in resources] == list(range(1, 29))
        and [row["kind"] for row in resources] == ["mod", "zip"] * 14
    ...
    require(
        len(mod_by_tuple) == 14
        and aggregate_mod == 1_730
        and aggregate_zip == 35_195_229
        and aggregate == 35_196_959
        and aggregate_zip_entries == 4_907
        and aggregate_zip_uncompressed == 144_867_307
    ...
    evidence_expected = {
        "documentType": "aetherlink.wave8-source-acquisition-evidence",
        "schemaVersion": "1.0",
        "attemptId": PERMIT.ATTEMPT_ID,
        "requestCount": 28,
    ...
    receipt_expected = {
        "documentType": "aetherlink.wave8-source-acquisition-receipt",
        "schemaVersion": "1.0",
        ...
        "aggregateZipEntryCount": aggregate_zip_entries,
        "aggregateZipUncompressedBytes": aggregate_zip_uncompressed,
        "dispatchBoundaryCount": 28,
        "responseCommittedCount": 28,
        "responseCommittedBytes": aggregate,
        "validationCommittedCount": 28,
        "persistenceCommittedCount": 28,
    ...
    return {
        ...
        "combinedFixedPointV6ContentSha256": PERMIT.EXPECTED_V6_CONTENT,
        "compactIdentitySha256": PERMIT.EXPECTED_COMPACT_IDENTITY,
        "fullWitnessSha256": PERMIT.EXPECTED_FULL_WITNESS,
...
def create_readback_claim(
    ...
    claim = content_bound(
        {
            "documentType": "aetherlink.wave8-acquisition-readback-one-use-claim",
            "schemaVersion": "1.0",
            "status": "consumed_active",
    ...
    try:
        ...
        try:
            ...
            receipt = content_bound(
                {
                    "documentType": "aetherlink.wave8-source-acquisition-readback",
                    "schemaVersion": "1.0",
                    "status": (
                        "wave8_acquisition_retained_snapshot_"
            ...
            manifest = content_bound(
                {
                    "documentType": (
                        "aetherlink.wave8-source-acquisition-readback-manifest"
                    ),
                    "schemaVersion": "1.0",
                    "status": (
                        "wave8_acquisition_retained_snapshot_"
...
def main(argv: Sequence[str] | None = None) -> int:
    try:
        ...
        if args.preflight:
            result = {
                "documentType": "aetherlink.wave8-acquisition-readback-preflight",
                "schemaVersion": "1.0",
        ...
        sys.stdout.buffer.write(
            canonical_bytes(
                {
                    "documentType": "aetherlink.wave8-acquisition-readback-error",
                    "schemaVersion": "1.0",
                    "status": status,
