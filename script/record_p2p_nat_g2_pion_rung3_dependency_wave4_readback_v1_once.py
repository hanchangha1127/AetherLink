#!/usr/bin/env python3
"""Record one independent offline readback of the Wave4 v1 acquisition."""
...
if not (
    ...
    raise RuntimeError("Wave4 readback recorder requires `python3 -I -B -S`")
...
if O_NOFOLLOW is None:
    raise RuntimeError("Wave4 readback recorder requires O_NOFOLLOW")
READBACK_CHECKER_PATH = Path(__file__).with_name(
    "check_p2p_nat_g2_pion_rung3_dependency_wave4_"
    "readback_execution_permit_v1.py"
)
EXPECTED_READBACK_CHECKER_RAW = "bfdc28ba61ccca37e626571d137718077eb20075122fb55db67ab000365087ca"
...
def load_readback_checker() -> types.ModuleType:
    ...
    try:
        ...
        os.close(fd)
    module = types.ModuleType("wave4_acquisition_readback_permit_v1")
...
def verify_snapshot(snapshot: FrozenSnapshot) -> dict[str, Any]:
    ...
    permit_path = (
        PERMIT.BASE
        + "/bounded-dependency-source-acquisition-wave4-execution-permit-v1.json"
    )
    ...
    require(
        len(authority_hashes) == 12
        and len(bound_authority_rows) == 11
    ...
    require(len(selected_tuple_ids) == 3, "E_RESOURCES", phase)
    accepted_specs = {
    ...
    require(
        len(mod_by_tuple) == 16
        and aggregate_mod == 1_901
        and aggregate_zip == 24_116_911
        and aggregate == 24_118_812
    ...
    return {
        "status": "wave4_acquisition_independently_verified",
...
def create_readback_claim(
    ...
    claim = content_bound(
        {
            "documentType": "aetherlink.wave4-acquisition-readback-one-use-claim",
            "schemaVersion": "1.0",
            "status": "consumed_active",
    ...
    try:
        try:
            ...
            receipt = content_bound(
                {
                    "documentType": "aetherlink.wave4-source-acquisition-readback",
                    "schemaVersion": "1.0",
                    "status": "wave4_acquisition_independently_read_back",
                    "readbackAttemptId": readback_attempt_id,
                    "acquisitionAttemptId": PERMIT.ATTEMPT_ID,
            ...
            manifest = content_bound(
                {
                    "documentType": (
                        "aetherlink.wave4-source-acquisition-readback-manifest"
                    ),
                    "schemaVersion": "1.0",
                    "status": "wave4_acquisition_readback_publication_complete",
                    "readbackAttemptId": readback_attempt_id,
                    "acquisitionAttemptId": PERMIT.ATTEMPT_ID,
                    "authorityBinding": package["authorityBinding"],
...
def main(argv: Sequence[str] | None = None) -> int:
    try:
        ...
        if args.preflight:
            result = {
                "documentType": "aetherlink.wave4-acquisition-readback-preflight",
                "schemaVersion": "1.0",
        ...
        sys.stdout.buffer.write(
            canonical_bytes(
                {
                    "documentType": "aetherlink.wave4-acquisition-readback-error",
                    "schemaVersion": "1.0",
                    "status": status,
