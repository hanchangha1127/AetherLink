#!/usr/bin/env python3
"""Tests for the Wave15 acquisition readback permit package."""
...
def _deny_test_network(*_args, **_kwargs):
    ...
    raise AssertionError(
        "offline Wave15 readback tests must never create network connections"
    )
...
SPEC = importlib.util.spec_from_file_location("wave15_readback_permit_tests", PATH)
...
def _synthetic_package_raw() -> tuple[dict[str, bytes], str, str]:
    ...
    reader_raw = b"synthetic Wave15 readback reader\\n"
...
class Wave15ReadbackPermitTests(unittest.TestCase):
    def tearDown(self) -> None:
        self.assertEqual(NETWORK_ATTEMPTS, [])

    def test_01_frozen_snapshot_constants_are_exact(self):
        self.assertEqual(C.ATTEMPT_ID, "c5db51cfd9a295b448927cca36d1ea07")
        self.assertEqual(len(C.ACQUISITION_AUTHORITY), 15)
        self.assertEqual(len(C.ACCEPTED_FILES), 10)
        self.assertEqual(
        ...
        acquisition_permit_path = (
            f"{C.BASE}/bounded-dependency-source-acquisition-wave15-"
        ...
        self.assertEqual(payload["frozenFileCount"], 29)
        self.assertEqual(
        ...
        self.assertEqual(payload["aggregateZipEntryCount"], 1_645)
        self.assertEqual(
        ...
        v13 = acquisition_permit["predecessorBindings"][
        ...
        self.assertEqual(
            payload["predecessorBindings"]["combinedFixedPointV13"][
                "contentSha256"
            ],
            C.EXPECTED_V13_CONTENT,
        )
        self.assertFalse(
            any(
    ...
    def test_08_wave15_cardinality_hash_aggregate_and_limit_drift_rejects(self):
        original, _ = _synthetic_permit()
        snapshot = original["frozenAcquisitionSnapshot"]

        def frozen_28(value):
            value["frozenAcquisitionSnapshot"]["frozenFileCount"] = 28
        ...
        def authority_14(value):
            ...
            value["frozenAcquisitionSnapshot"]["acquisitionAuthority"] = [
                ...
                if not row["path"].endswith("/.wave-14-v1.claim")
            ]
        ...
        def bound_rows_14(value):
            ...
            predecessor.pop("wave14NamespaceAnchor")

        def predecessor_v12(value):
        ...
        def wave14_hashes(value):
            value["frozenAcquisitionSnapshot"].update(
                {
                    "frozenFilesCanonicalSha256": (
                        "905f7a4e90abbe1fb311385e001fac94a"
                        "1dee32235b408a794e663eb049458ec"
                    ),
                    "acceptedResourceHashSetCanonicalSha256": (
                        "23a5e8e4efaa6d0cf63549eaa686e5b9e"
                        "365d38b832be5f5f14e0e8722a327ec"
                    ),
                }
            )
        ...
        for package_path in package_paths:
            ...
            self.assertNotIn("golang.org/x/" + "crypto", source)
            self.assertNotIn("golang.org/x/" + "term", source)
            self.assertNotIn("golang.org/x/" + "text", source)
            self.assertNotIn("golang.org/x/" + "tools", source)
        for production_path in (
            ...
            if not path.exists():
                continue
            source = path.read_text(encoding="utf-8")
            self.assertNotIn("combinedFixedPointV" + "12", source)
            self.assertNotIn(".wave-" + "13-v1.claim", source)
