#!/usr/bin/env python3
"""Tests for the one-use offline Wave5 acquisition readback recorder."""
...
PATH = Path(__file__).with_name(
    "record_p2p_nat_g2_pion_rung3_dependency_wave5_readback_v1_once.py"
)
SPEC = importlib.util.spec_from_file_location("wave5_readback_recorder_tests", PATH)
...
class Wave5ReadbackRecorderTests(unittest.TestCase):
    ...
    def test_05_live_retained_snapshot_verifies_twice_read_only(self):
        ...
        try:
            ...
            self.assertEqual(first["authorityFileCount"], 12)
            self.assertEqual(first["selectedTupleCount"], 0)
            self.assertEqual(first["aggregateAcceptedBytes"], 26_123_889)
            self.assertEqual(first["aggregateZipEntryCount"], 6_038)
            self.assertEqual(
