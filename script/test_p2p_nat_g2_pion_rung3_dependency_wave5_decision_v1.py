#!/usr/bin/env python3
"""Adversarial tests for the offline Wave5 identity decision checker."""
...
if not (
    ...
    raise RuntimeError("Wave5 decision tests require `python3 -I -B -S`")
...
READER_PATH = (
    ...
    "bounded-dependency-source-identity-and-acquisition-decision-wave5-v1.md"
)
DECISION_PATH = (
    "docs/security-hardening/production-p2p-nat-v1/"
    "g2-pion-restricted-fork-v1/rung-three/"
    "bounded-dependency-source-identity-and-acquisition-decision-wave5-v1.json"
)
...
def load_checker(raw: bytes) -> types.ModuleType:
    module = types.ModuleType("wave5_decision_checker_test_subject")
    module.__dict__.update(
        {
            "__cached__": None,
            "__file__": str(ROOT / CHECKER_PATH),
            "__loader__": None,
            "__name__": "wave5_decision_checker_test_subject",
            "__package__": None,
        }
    )
    code = compile(
...
def load_wave5_candidate_checker() -> types.ModuleType:
    path = CHECKER.WAVE5_CHECKER_PATH
    raw = (ROOT / path).read_bytes()
    if sha256(raw) != CHECKER.WAVE5_CHECKER_RAW_SHA256:
        raise RuntimeError("Wave5 candidate checker preflight failed")
    module = types.ModuleType("wave5_candidate_checker_test_subject")
    module.__dict__.update(
        {
            "__cached__": None,
            "__file__": str(ROOT / path),
            "__loader__": None,
            "__name__": "wave5_candidate_checker_test_subject",
            "__package__": None,
        }
    )
...
class UnitBoundaryTests(unittest.TestCase):
    ...
    def test_go_sum_rejects_bad_shape_and_utf8(self) -> None:
        ...
        common = {
            ...
            "entry_path": "parent@v3.0.0/go.sum",
        }
    ...
    def test_held_namespace_rejects_late_collision_and_root_rebind(
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with CHECKER.HeldNamespace(root) as held:
                claim = dependency / Path(CHECKER.WAVE5_CLAIM_PATH).name
                claim.write_bytes(b"late")
                with self.assertRaises(CHECKER.DecisionFailure):
                    held.final_barrier()
                claim.unlink()
                staging = dependency / (
                    CHECKER.WAVE5_STAGING_PREFIX + "late"
                )
                staging.mkdir()
                with self.assertRaises(CHECKER.DecisionFailure):
                    held.final_barrier()
                staging.rmdir()
                final = dependency / CHECKER.WAVE5_FINAL_NAME
                final.mkdir()
                with self.assertRaises(CHECKER.DecisionFailure):
                    held.final_barrier()
                final.rmdir()
    ...
    def test_wave5_candidate_content_is_reconstructed_not_copied(
        self,
    ) -> None:
        wave5 = load_wave5_candidate_checker()
        rows = wave5.wave5_rows(wave5.expected_frontier_rows())
        combined = {
            "contentBinding": {
                "sha256": CHECKER.COMBINED_V3_CONTENT_SHA256,
            }
        }
        projected = CHECKER.reconstruct_wave5_candidate(
            wave5=wave5,
        ...
        self.assertEqual(
            ...
            CHECKER.WAVE5_CANDIDATE_CONTENT_SHA256,
        )
        ...
        self.assertEqual(
            (
                ...
                counters["directV3ArchiveOpenCount"],
        ...
        wave5.V3_TOTAL_RECONSTRUCTION_COUNT = 2
        wave5.V3_TOTAL_ARCHIVE_OPEN_COUNT = 134
        try:
            with self.assertRaises(CHECKER.DecisionFailure):
                CHECKER.reconstruct_wave5_candidate(
                    wave5=wave5,
                    combined_candidate=combined,
                    wave_rows=rows,
                )
        ...
        original = CHECKER.WAVE5_CANDIDATE_CONTENT_SHA256
        CHECKER.WAVE5_CANDIDATE_CONTENT_SHA256 = "0" * 64
        try:
            with self.assertRaises(CHECKER.DecisionFailure):
                CHECKER.reconstruct_wave5_candidate(
                    wave5=wave5,
            ...
            CHECKER.WAVE5_CANDIDATE_CONTENT_SHA256 = original
...
class LiveRepositoryTests(unittest.TestCase):
    ...
    def test_expected_bytes_and_materialized_decision(self) -> None:
        ...
        self.assertEqual(
            (
                ...
                parsed["operationCounters"]["directV3ArchiveOpenCount"],
                parsed["operationCounters"]["totalArchiveOpenCount"],
                parsed["operationCounters"]["inheritedArchiveOpenCount"],
                parsed["operationCounters"]["identityWitnessScanCount"],
                parsed["operationCounters"][
                    "identityWitnessArchiveOpenCount"
                ],
            ),
            (2, 2, 4, 4, 102, 134, 236, 236, 2, 134),
        )
        self.assertNotEqual(
            (
                parsed["operationCounters"][
        ...
        self.assertEqual(len({row["url"] for row in requests}), 30)
        self.assertEqual(
        ...
        self.assertEqual(
            ...
            [
                ("v0.17.0", x_sys[0]["goModH1"]),
                ("v0.20.0", x_sys[0]["goModH1"]),
            ],
        )
        x_sys_mod_requests = [
            row
    ...
    def test_normal_checker_summary(self) -> None:
        ...
        self.assertEqual(summary["completeIdentityPairCount"], 15)
