#!/usr/bin/env python3
"""Tests for the externally pinned, read-only Wave6 candidate checker."""
...
def require_isolated_interpreter() -> None:
    ...
    if not (
        ...
        raise RuntimeError(
            "Wave6 candidate tests require unoptimized "
            "`python3 -I -B -S`"
        )
...
CHECKER_PATH = (
    ROOT
    / "script"
    / "check_p2p_nat_g2_pion_rung3_dependency_wave6_candidate_v1.py"
)
...
def preflight_checker_source(raw: bytes) -> str:
    ...
    compile_call = compile_calls[0]
        ...
        enclosing_function(compile_call) == "load_v4_checker",
        "compile scope",
    )
        ...
        and compile_call.args[1].id == "V4_CHECKER_PATH",
        "compile filename",
    )
    ensure(
    ...
    exec_call = exec_calls[0]
        ...
        enclosing_function(exec_call) == "load_v4_checker",
        "exec scope",
    )
...
def load_checker(preflighted_raw: bytes):
    ...
    module = types.ModuleType("wave6_candidate_tests_target")
...
class Wave6CandidateTests(unittest.TestCase):
    maxDiff = None
    ...
    def test_01_exact_external_package_pins(self):
        self.assertEqual(
            hashlib.sha256((ROOT / CHECKER.V4_CHECKER_PATH).read_bytes())
            .hexdigest(),
            CHECKER.V4_CHECKER_RAW_SHA256,
        )
        self.assertEqual(
            hashlib.sha256((ROOT / CHECKER.V4_TESTS_PATH).read_bytes())
            .hexdigest(),
            CHECKER.V4_TESTS_RAW_SHA256,
        )
        self.assertEqual(
            CHECKER.V4_CHECKER_NORMALIZED_SHA256,
    ...
    def test_03_tuple_projection_is_deterministic(self):
        first = CHECKER.wave6_rows(CHECKER.expected_frontier_rows())
        second = CHECKER.wave6_rows(CHECKER.expected_frontier_rows())
        self.assertEqual(first, second)
        self.assertEqual(
            [row["tupleOrder"] for row in first],
            list(range(1, 19)),
        )
        self.assertEqual(len({row["tupleId"] for row in first}), 18)
        self.assertEqual(
            first[0]["tupleId"],
            (
                "wave6-001-"
                + hashlib.sha256(
                    b"github.com/stretchr/objx\nv0.4.0\n"
                ).hexdigest()[:12]
            ),
        )
        self.assertTrue(
    ...
    def test_04_semantic_mutations_fail_even_when_content_is_rebound(self):
        base_candidate = {
            ...
            "graphDiscovery": {
                ...
                "moduleNodeCount": 100,
                "moduleEdgeCount": 247,
                "nodeSetSha256": CHECKER.V4_NODE_SET_SHA256,
                "edgeSetSha256": CHECKER.V4_EDGE_SET_SHA256,
                "moduleNodeSetSha256":
                    CHECKER.V4_MODULE_NODE_SET_SHA256,
                "moduleEdgeSetSha256":
                    CHECKER.V4_MODULE_EDGE_SET_SHA256,
                "moduleGraphAndFrontierSha256":
                    CHECKER.V4_MODULE_GRAPH_AND_FRONTIER_SHA256,
                "reconstructionProjectionSha256":
                    CHECKER.V4_RECONSTRUCTION_PROJECTION_SHA256,
            ...
            "predecessorVerification": {
                "wave5ReadbackCompletionAppliesToRetainedSnapshot": True,
            ...
            "authority": {
                key: False
                for key in CHECKER.EXPECTED_V4_AUTHORITY_KEYS
            },
        }
        ...
        self.assertEqual(
            ...
            CHECKER.EXPECTED_V4_AUTHORITY_KEYS,
        )
        self.assertTrue(
            all(
                value is False
        ...
        original = CHECKER.V4_CANDIDATE_CONTENT_SHA256
        original_source_bindings = CHECKER.V4_SOURCE_BINDINGS_SHA256
        CHECKER.V4_CANDIDATE_CONTENT_SHA256 = (
            rebound_base["contentBinding"]["sha256"]
        )
        CHECKER.V4_SOURCE_BINDINGS_SHA256 = CHECKER.sha256_bytes(
            CHECKER.compact_json_bytes([])
        )
        try:
            self.assertEqual(
                CHECKER.validate_v4_candidate(rebound_base),
                CHECKER.expected_frontier_rows(),
            )
        finally:
            CHECKER.V4_CANDIDATE_CONTENT_SHA256 = original
            CHECKER.V4_SOURCE_BINDINGS_SHA256 = original_source_bindings
        ...
        for section, key, value, expected_code in mutations:
            with self.subTest(section=section, key=key):
                ...
                original_source_bindings = (
                    CHECKER.V4_SOURCE_BINDINGS_SHA256
                )
                CHECKER.V4_CANDIDATE_CONTENT_SHA256 = (
                ...
                try:
                    with self.assertRaisesRegex(
                        CHECKER.Wave6CandidateFailure,
                        f"^{expected_code}$",
                    ):
                        CHECKER.validate_v4_candidate(mutated)
                finally:
                    CHECKER.V4_CANDIDATE_CONTENT_SHA256 = original
                    CHECKER.V4_SOURCE_BINDINGS_SHA256 = (
                        original_source_bindings
        ...
        for mutation_name, mutate in (
            (
                "selector",
                lambda value: value["graphDiscovery"]["exactFrontier"][0]
            ...
            with self.subTest(mutation=mutation_name):
                ...
                try:
                    with self.assertRaises(CHECKER.Wave6CandidateFailure):
                        CHECKER.validate_v4_candidate(mutated)
                finally:
                    CHECKER.V4_CANDIDATE_CONTENT_SHA256 = original
                    CHECKER.V4_SOURCE_BINDINGS_SHA256 = (
    ...
    def test_07_live_candidate_is_exact_canonical_and_non_authorizing(self):
        ...
        wave = candidate["wave"]
        self.assertEqual(wave["tupleCount"], 18)
        ...
        self.assertEqual(
            wave["tuples"],
            CHECKER.wave6_rows(CHECKER.expected_frontier_rows()),
        )
        ...
        self.assertEqual(counters["totalFullSourceReconstructionCount"], 6)
        self.assertEqual(counters["predecessorArchiveOpenCount"], 236)
        self.assertEqual(counters["directV4ArchiveOpenCount"], 164)
        self.assertEqual(counters["totalArchiveOpenCount"], 400)
        self.assertEqual(
            counters["inheritedFullSourceReconstructionCount"],
            6,
        )
        self.assertEqual(counters["inheritedArchiveOpenCount"], 400)
    ...
    def test_08_live_run_is_canonical_byte_stable(self):
        first = self.live_runs[0]
        self.assertEqual(first.returncode, 0)
        self.assertEqual(first.stderr, b"")
        self.assertEqual(
            first.stdout,
            CHECKER.canonical_json_bytes(json.loads(first.stdout)),
        )
