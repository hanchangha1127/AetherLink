#!/usr/bin/env python3
"""Adversarial offline tests for the one-use Wave16 acquisition runner."""
...
if not (
    ...
    raise RuntimeError("Wave16 runner tests require `python3 -I -B -S`")
...
EXPECTED_WAVE16_RESOURCE_CONTRACT_SHA256 = (
    "6244c9f9c0b8d7413f5da8fb65145a67900c664448201baf798da398d2557e1f"
)


RUNNER_PATH = Path(__file__).with_name(
    "acquire_p2p_nat_g2_pion_rung3_dependency_wave16_v1_once.py"
)
# The runner carries the finalized Wave16 acquisition-checker reverse pin.
...
def exact_resource_oracle() -> list[dict[str, object]]:
    ...
    for tuple_order, (identity, resource_names) in enumerate(
        zip(EXPECTED_WAVE16_IDENTITY, EXPECTED_WAVE16_RESOURCE_NAMES),
        1,
    ):
...
def fixture() -> tuple[list[dict[str, object]], dict[int, bytes]]:
    ...
    ordinal = 0
    for tuple_order in range(1, 3 + 1):
        module = f"example.test/dependency{tuple_order}"
        version = f"v1.0.{tuple_order}"
        digest = runner.sha256(f"{module}\n{version}\n".encode())
        tuple_id = f"wave16-{tuple_order:03d}-{digest[:12]}"
        selected = False
        mod = f"module {module}\n\ngo 1.22\n".encode()
        archive = make_zip(
            module,
...
class Wave16RunnerTests(unittest.TestCase):
    def setUp(self) -> None:
        self.wave4_network_patcher = mock.patch.object(
            runner.WAVE4,
            "direct_fetch",
            side_effect=AssertionError(
                "offline Wave16 tests must never reach the network"
            ),
        )
        ...
        self.https_network_patcher = mock.patch(
            ...
            side_effect=AssertionError(
                "offline Wave16 tests must never create HTTPS connections"
            ),
        )
    ...
    def test_exact_three_tuple_six_resource_contract_is_closed(
        self,
    ) -> None:
        self.assertEqual(
            runner.EXPECTED_WAVE16_IDENTITY,
            EXPECTED_WAVE16_IDENTITY,
        )
        ...
        self.assertEqual(
            runner.EXPECTED_WAVE16_RESOURCE_CONTRACT_SHA256,
            EXPECTED_WAVE16_RESOURCE_CONTRACT_SHA256,
        )
        self.assertEqual(
        ...
        self.assertEqual(
            ...
            list(EXPECTED_WAVE16_RESOURCE_NAMES),
        )
        self.assertTrue(
            all(
    ...
    def test_success_claims_before_6_fetches_and_manifest_is_last(self) -> None:
        rows, bodies = fixture()
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(len(list(accepted.iterdir())), 6)
            self.assertTrue(
                terminal(root, runner.CHECK.CLAIM_PATH).exists()
            )
            self.assertTrue(
    ...
    def test_every_resource_field_is_required_on_all_6_rows(self) -> None:
        original, _ = fixture()
    ...
    def test_lower_ascii_direct_proxy_identity_is_enforced(self) -> None:
        def rewritten_pair(module: str, version: str):
            ...
            for index, kind in enumerate(("mod", "zip")):
                ...
                rows[index]["tupleId"] = f"wave16-001-{digest[:12]}"
    ...
    def test_wave16_direct_fetch_reaches_wave4_and_translates_errors(
        self,
    ) -> None:
        resource = fixture()[0][0]
