#!/usr/bin/env python3
"""Focused offline tests for the one-use Wave18 acquisition runner."""
...
if not (
    ...
    raise RuntimeError("Wave18 runner tests require `python3 -I -B -S`")
...
def _deny_network(*_args: object, **_kwargs: object) -> None:
    raise AssertionError("offline Wave18 tests must never create a connection")
...
RUNNER_CALL_SURFACE_SHA256 = (
    "3e8136fd3d434bc41f08bf591e78fbb7dd9f2d809015a3d631be093d5f21265e"
)
TUPLES = (
    (
        "golang.org/x/mod",
...
def preload_runner_gate(
    ...
    if type(permit) is not dict:
        raise RuntimeError("Wave18 runner test permit schema")
    tool_bindings = permit.get("toolBindings")
    if type(tool_bindings) is not list:
        raise RuntimeError("Wave18 runner test tool bindings")
    runner_bindings = [
        row
    ...
    if len(runner_bindings) != 1:
        raise RuntimeError(
            "Wave18 runner test runner binding cardinality"
        )
    binding = runner_bindings[0]
    if (
        set(binding)
        != {"normalizedSha256", "path", "rawSha256", "role"}
        or binding["role"] != "wave18_one_use_runner"
        ...
        raise RuntimeError("Wave18 runner test preload gate")
    return binding
...
SPEC = importlib.util.spec_from_file_location(
    "wave18_source_acquirer_v1_test_subject",
    RUNNER_PATH,
)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError("Wave18 runner test module spec")
...
def values(rows: list[dict[str, object]]) -> dict[str, object]:
    ...
    permit["requestContract"]["requestCount"] = 6
...
class Wave18RunnerTests(unittest.TestCase):
    maxDiff = None

    def test_01_static_surface_bindings_and_exact_six_resources(self) -> None:
        ...
        self.assertEqual(
            runner.EXPECTED_WAVE18_RESOURCE_CONTRACT_SHA256,
            RESOURCE_CONTRACT_SHA256,
        )
        self.assertEqual(
    ...
    def test_06_resource_schema_and_bool_int_aliases_fail_closed(self) -> None:
        ...
        candidate = copy.deepcopy(rows)
        candidate[2]["expectedH1"] = candidate[4]["expectedH1"]
        rejected(candidate)
        candidate = copy.deepcopy(rows)
    ...
    def test_07_immutable_ledger_exact_six_and_uncertainty_states(self) -> None:
        ...
        success = ledger.success_fields(6)
        self.assertEqual(
            (
    ...
    def test_08_mod_zip_h1_and_archive_safety_are_in_memory_only(self) -> None:
        ...
        for tuple_order in range(1, 4):
            ...
            for bad_root_go_mod in (
                ...
                root_values: dict[int, object] = {
                    current: True for current in range(1, 4)
                }
        ...
        metadata_cases = (
            (
                "entryCount",
                "zip_entry_counts",
                {current: 2 for current in range(1, 4)},
    ...
    def test_11_mock_attempt_success_is_claim_first_manifest_last(self) -> None:
        ...
        self.assertEqual(fetches, list(range(1, 7)))
        self.assertEqual(
            (
        ...
        self.assertEqual(receipt["modCount"], 3)
        self.assertEqual(receipt["zipCount"], 3)
        ...
        response_limits = (
            (
                ...
                sum(len(bodies[ordinal]) for ordinal in (2, 4, 6)),
    ...
    def test_12_mock_attempt_failure_ledgers_and_uncertainty_are_exact(
        ...
        rows, bodies = fixture()
        for failed_ordinal in range(1, 7):
            namespace = PureNamespace()
            with (
                self.subTest(
                    failure_kind="h1",
    ...
    def test_15_held_entry_rejects_hardlink_and_name_rebind(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with runner.ExecutionNamespace(root) as namespace:
                ...
                self.assertEqual(len(namespace.resources), 6)
                with self.assertRaises(
                    runner.AcquisitionError
                ) as replay:
                    runner.create_claim(
    ...
    def test_18_error_documents_never_request_authentication_or_retry(
        ...
        predecessor_leaf = ".wave-" + "17-v1.claim"
        predecessor_path = (
        ...
        self.assertEqual(wave17_values, [predecessor_path])
        self.assertEqual(
        ...
        package_paths = (
            PERMIT_PATH,
            ROOT / BASE
            / "bounded-dependency-source-acquisition-wave18-execution-permit-v1.md",
            ROOT / CHECKER_PATH,
            RUNNER_PATH,
