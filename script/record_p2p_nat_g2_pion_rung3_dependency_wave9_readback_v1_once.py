#!/usr/bin/env python3
"""Record one independent offline readback of the Wave9 v1 acquisition."""
...
if not (
    ...
    raise RuntimeError("Wave9 readback recorder requires `python3 -I -B -S`")
...
if O_NOFOLLOW is None:
    raise RuntimeError("Wave9 readback recorder requires O_NOFOLLOW")
READBACK_CHECKER_PATH = Path(__file__).with_name(
    "check_p2p_nat_g2_pion_rung3_dependency_wave9_"
    "readback_execution_permit_v1.py"
)
EXPECTED_READBACK_CHECKER_RAW = "012128231fbb7dd5ac0de4139af5fd98b5da21301ae298c61109eb9eeed0bd65"
...
MAX_ZIP_UNCOMPRESSED_BYTES_ACROSS_ALL = 1 * 1024 * 1024 * 1024
MAX_ZIP_FILES_ACROSS_ALL = 300_000
...
def load_readback_checker() -> types.ModuleType:
    ...
    module = types.ModuleType("wave9_acquisition_readback_permit_v1")
...
def verify_snapshot(snapshot: FrozenSnapshot) -> dict[str, Any]:
    ...
    permit_path = (
        PERMIT.BASE
        + "/bounded-dependency-source-acquisition-wave9-execution-permit-v1.json"
    )
    ...
    v7 = acquisition_permit["predecessorBindings"]["combinedFixedPointV7"]
    predecessor_authority_rows = [
        {
            "path": v7["checkerPath"],
            "rawSha256": v7["checkerRawSha256"],
        },
        {
            "path": v7["testsPath"],
            "rawSha256": v7["testsRawSha256"],
        },
    ]
    ...
    require(
        len(authority_hashes) == 14
        and len(bound_authority_rows) == 13
    ...
    require(
        ...
        == {
            "checkerPath": (
                "script/check_p2p_nat_g2_pion_combined_fixed_point_v7.py"
            ),
            "checkerRawSha256": authority_hashes[
                "script/check_p2p_nat_g2_pion_combined_fixed_point_v7.py"
            ],
            "checkerNormalizedSha256": (
                PERMIT.EXPECTED_V7_CHECKER_NORMALIZED
            ),
            "testsPath": (
                "script/test_p2p_nat_g2_pion_combined_fixed_point_v7.py"
            ),
            "testsRawSha256": authority_hashes[
                "script/test_p2p_nat_g2_pion_combined_fixed_point_v7.py"
            ],
            "contentSha256": PERMIT.EXPECTED_V7_CONTENT,
            "combinedInputSetSha256": PERMIT.EXPECTED_V7_INPUT_SET,
            "graphSha256": PERMIT.EXPECTED_V7_GRAPH,
            "frontierSha256": PERMIT.EXPECTED_V7_FRONTIER,
    ...
    require(
        len(resources) == 20
        and [row["requestOrdinal"] for row in resources] == list(range(1, 21))
        and [row["kind"] for row in resources] == ["mod", "zip"] * 10
    ...
    require(len(selected_tuple_ids) == 0, "E_RESOURCES", phase)
    accepted_specs = {
    ...
    require(
        len(mod_by_tuple) == 10
        and aggregate_mod == 1_181
        and aggregate_zip == 16_284_759
        and aggregate == 16_285_940
        and aggregate_zip_entries == 5_349
        and aggregate_zip_uncompressed == 54_936_288
    ...
    evidence_expected = {
        "documentType": "aetherlink.wave9-source-acquisition-evidence",
        "schemaVersion": "1.0",
        "attemptId": PERMIT.ATTEMPT_ID,
        "requestCount": 20,
    ...
    receipt_expected = {
        "documentType": "aetherlink.wave9-source-acquisition-receipt",
        "schemaVersion": "1.0",
        ...
        "modCount": 10,
        "zipCount": 10,
        "acceptedResourceCount": 20,
        "aggregateResponseBytes": aggregate,
        "aggregateModResponseBytes": aggregate_mod,
        "aggregateZipResponseBytes": aggregate_zip,
        "aggregateZipEntryCount": aggregate_zip_entries,
        "aggregateZipUncompressedBytes": aggregate_zip_uncompressed,
        "dispatchBoundaryCount": 20,
        "responseCommittedCount": 20,
        "responseCommittedBytes": aggregate,
        "validationCommittedCount": 20,
        "persistenceCommittedCount": 20,
    ...
    return {
        ...
        "combinedFixedPointV7ContentSha256": PERMIT.EXPECTED_V7_CONTENT,
        "compactIdentitySha256": PERMIT.EXPECTED_COMPACT_IDENTITY,
        "fullWitnessSha256": PERMIT.EXPECTED_FULL_WITNESS,
...
def create_readback_claim(
    ...
    claim = content_bound(
        {
            "documentType": "aetherlink.wave9-acquisition-readback-one-use-claim",
            "schemaVersion": "1.0",
            "status": "consumed_active",
    ...
    try:
        ...
        try:
            ...
            receipt = content_bound(
                {
                    "documentType": "aetherlink.wave9-source-acquisition-readback",
                    "schemaVersion": "1.0",
                    "status": (
            ...
            manifest = content_bound(
                {
                    "documentType": (
                        "aetherlink.wave9-source-acquisition-readback-manifest"
                    ),
                    "schemaVersion": "1.0",
                    "status": (
                        "wave9_acquisition_retained_snapshot_"
...
def main(argv: Sequence[str] | None = None) -> int:
    try:
        ...
        if args.preflight:
            result = {
                "documentType": "aetherlink.wave9-acquisition-readback-preflight",
                "schemaVersion": "1.0",
        ...
        sys.stdout.buffer.write(
            canonical_bytes(
                {
                    "documentType": "aetherlink.wave9-acquisition-readback-error",
                    "schemaVersion": "1.0",
                    "status": status,
