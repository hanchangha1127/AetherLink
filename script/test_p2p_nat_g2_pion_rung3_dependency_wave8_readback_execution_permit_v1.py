#!/usr/bin/env python3
"""Tests for the Wave8 acquisition readback permit package."""
...
SPEC = importlib.util.spec_from_file_location("wave8_readback_permit_tests", PATH)
C = importlib.util.module_from_spec(SPEC)
assert SPEC.loader is not None
SPEC.loader.exec_module(C)


class Wave8ReadbackPermitTests(unittest.TestCase):
    def test_01_frozen_snapshot_constants_are_exact(self):
        self.assertEqual(C.ATTEMPT_ID, "6d8ea4473126c853b439c56a895f9c28")
        self.assertEqual(len(C.ACQUISITION_AUTHORITY), 14)
        self.assertEqual(len(C.ACCEPTED_FILES), 28)
        self.assertEqual(
        ...
        self.assertEqual(payload["aggregateZipEntryCount"], 4_907)
        self.assertEqual(
        ...
        self.assertEqual(
            payload["predecessorBindings"]["combinedFixedPointV6"][
                "contentSha256"
            ],
            C.EXPECTED_V6_CONTENT,
        )
    ...
    def test_03_exact_package_and_package_only_preflight(self):
        ...
        self.assertEqual(summary["acceptedResourceCount"], 28)
