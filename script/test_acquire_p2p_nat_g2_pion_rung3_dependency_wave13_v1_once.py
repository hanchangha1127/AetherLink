#!/usr/bin/env python3
"""Adversarial offline tests for the one-use Wave13 acquisition runner."""
...
if not (
    ...
    raise RuntimeError("Wave13 runner tests require `python3 -I -B -S`")
...
RUNNER_PATH = Path(__file__).with_name(
    "acquire_p2p_nat_g2_pion_rung3_dependency_wave13_v1_once.py"
)
# The runner carries the finalized Wave13 acquisition-checker reverse pin.
...
def exact_resource_oracle() -> list[dict[str, object]]:
    ...
    for tuple_order, identity in enumerate(
        ...
        tuple_id = f"wave13-{tuple_order:03d}-{digest[:12]}"
...
class Wave13RunnerTests(unittest.TestCase):
    def setUp(self) -> None:
        self.wave4_network_patcher = mock.patch.object(
            runner.WAVE4,
            "direct_fetch",
            side_effect=AssertionError(
                "offline Wave13 tests must never reach the network"
            ),
        )
        ...
        self.https_network_patcher = mock.patch(
            ...
            side_effect=AssertionError(
                "offline Wave13 tests must never create HTTPS connections"
            ),
        )
    ...
    def test_exact_four_tuple_eight_resource_contract_is_closed(
        ...
        self.assertEqual(
            runner.EXPECTED_WAVE13_IDENTITY,
            EXPECTED_WAVE13_IDENTITY,
        )
        ...
        self.assertEqual(
            runner.EXPECTED_WAVE13_RESOURCE_CONTRACT_SHA256,
            EXPECTED_WAVE13_RESOURCE_CONTRACT_SHA256,
        )
        self.assertEqual(
    ...
    def test_lower_ascii_direct_proxy_identity_is_enforced(self) -> None:
        def rewritten_pair(module: str, version: str):
            ...
            for index, kind in enumerate(("mod", "zip")):
                ...
                rows[index]["tupleId"] = f"wave13-001-{digest[:12]}"
    ...
    def test_wave13_direct_fetch_reaches_wave4_and_translates_errors(
        self,
    ) -> None:
        resource = fixture()[0][0]
