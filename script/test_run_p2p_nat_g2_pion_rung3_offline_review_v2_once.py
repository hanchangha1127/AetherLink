#!/usr/bin/env python3
"""Synthetic-only tests for the separate one-use G2 rung-three v2 runner."""
...
RUNNER_PATH = ROOT / "script/run_p2p_nat_g2_pion_rung3_offline_review_v2_once.py"
...
RUNNER = load_source_without_importlib(
    RUNNER_PATH,
    name="g2_pion_rung3_review_v2_runner_test_subject",
)

SOURCE_BODY_SENTINEL = "V2_SYNTHETIC_SOURCE_BODY_MUST_NOT_BE_RECORDED_7f40db"
...
def authority_fixture() -> dict[str, object]:
    return {
        "permit": {
            "permitId": "g2-pion-rung3-offline-review-execution-permit-v2",
...
def synthetic_execution_environment(
    ...
    *,
    ...
    adapter = SimpleNamespace(inspect_module_zip=inspect_mock)
    checker = ModuleType("synthetic_v2_execution_permit_checker")
    ...
    with ExitStack() as stack:
        ...
        stack.enter_context(
            mock.patch.object(
                RUNNER,
                "OUTPUT_PARENT",
                RUNNER.PurePosixPath("build/synthetic-review-v2"),
            )
        )
        stack.enter_context(
        ...
        yield {
            ...
            "output": root / "build/synthetic-review-v2",
            "raw": archive_raw,
            "checker": checker,
            "adapter": adapter,
...
class G2PionRung3OfflineReviewV2RunnerTests(unittest.TestCase):
    ...
    def test_00_nonisolated_source_stops_before_non_builtin_imports(self) -> None:
        ...
        namespace = {"__file__": RUNNER_PATH.name, "__name__": "nonisolated_v2_probe"}
        with (
...
"""
        ...
        with tempfile.TemporaryDirectory() as temporary_root:
            ...
            target.chmod(0o600)
            (script / "check_p2p_nat_g2_pion_rung3_execution_permit_v2.py").symlink_to(target)
            with (
                mock.patch.object(RUNNER, "ROOT", root),
    ...
    def test_02_runner_is_separate_from_v1_and_has_no_forbidden_api_imports(self) -> None:
        ...
        self.assertEqual(RUNNER.OUTPUT_PARENT.name, "review-v2")
        self.assertEqual(RUNNER.CLAIM_NAME, ".g2-pion-ice-v4.3.0-rung3-offline-review-v2.claim")
        self.assertEqual(RUNNER.RESULT_NAME, "offline-source-review-result-v2.json")
        self.assertEqual(RUNNER.MANIFEST_NAME, "offline-source-review-manifest-v2.json")

    def test_03_success_calls_adapter_once_and_publishes_bound_v2_schema(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_root:
            with synthetic_execution_environment(temporary_root) as environment:
                ...
                self.assertEqual(outcome["schemaVersion"], "2.0")
                self.assertEqual(outcome["reviewAdapterInvocationCount"], 1)
                self.assertTrue(outcome["postRunReadbackRequired"])
                self.assertEqual(result["schemaVersion"], "2.0")
                self.assertEqual(
                    result["reviewId"],
                    "g2-pion-ice-v4.3.0-rung3-offline-source-review-v2",
                )
                self.assertFalse(result["publicationCompletion"]["complete"])
                self.assertEqual(
                    manifest["manifestId"],
                    "g2-pion-ice-v4.3.0-rung3-offline-source-review-runtime-manifest-v2",
                )
                ...
                self.assertEqual(claim["schemaVersion"], "2.0")
    ...
    def test_07_adapter_failure_is_one_call_and_retains_claim(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_root:
            with synthetic_execution_environment(
                ...
                with self.assertRaisesRegex(RUNNER.ReviewError, "v2 validation failed"):
                    RUNNER.execute_permit()
                environment["inspectMock"].assert_called_once()
                output = environment["output"]
                self.assertTrue((output / RUNNER.CLAIM_NAME).exists())
    ...
    def test_09_check_mode_does_not_inspect_consumption_or_archive(self) -> None:
        checker = ModuleType("synthetic_v2_checker")
        checker.validate_repository = mock.Mock(return_value=authority_fixture())
        ...
        with tempfile.TemporaryDirectory() as temporary_root:
            with synthetic_execution_environment(temporary_root) as environment:
                ...
                with self.assertRaisesRegex(
                    RUNNER.ReviewError,
                    "single-use v2 review cannot start",
                ):
                    RUNNER.execute_permit()
                self.assertEqual(
    ...
    def test_12_postclaim_foreign_temp_is_not_deleted_without_owned_fd(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_root:
            with synthetic_execution_environment(temporary_root) as environment:
    ...
    def test_13_temp_name_swap_cannot_publish_false_success(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_root:
            with synthetic_execution_environment(temporary_root) as environment:
    ...
    def test_14_base_exception_after_link_is_partial_publication(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_root:
            with synthetic_execution_environment(temporary_root) as environment:
                original_link = RUNNER.os.link
                injected = False
    ...
    def test_15_exception_after_publish_return_is_reobserved_as_partial(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_root:
            with synthetic_execution_environment(temporary_root) as environment:
                original_publish = RUNNER.publish_no_replace
                interrupted = False
