#!/usr/bin/env python3
"""Record one independent offline readback of the Wave11 v1 acquisition."""
...
if not (
    ...
    raise RuntimeError("Wave11 readback recorder requires `python3 -I -B -S`")
...
if O_NOFOLLOW is None:
    raise RuntimeError("Wave11 readback recorder requires O_NOFOLLOW")
READBACK_CHECKER_PATH = Path(__file__).with_name(
    "check_p2p_nat_g2_pion_rung3_dependency_wave11_"
    "readback_execution_permit_v1.py"
)
EXPECTED_READBACK_CHECKER_RAW = "895fad76d8563ebd57d7f8ca31d8cb44d15724414550d1eb1bc22c4f3d4cc124"
...
def load_readback_checker() -> types.ModuleType:
    ...
    module = types.ModuleType("wave11_acquisition_readback_permit_v1")
...
def verify_snapshot(snapshot: FrozenSnapshot) -> dict[str, Any]:
    ...
    v9 = acquisition_permit["predecessorBindings"]["combinedFixedPointV9"]
    predecessor_authority_rows = [
        {
            "path": v9["checkerPath"],
            "rawSha256": v9["checkerRawSha256"],
        },
        {
            "path": v9["testsPath"],
            "rawSha256": v9["testsRawSha256"],
        },
    ]
    ...
    require(
        ...
        and acquisition_permit["requestContract"]["tupleCount"] == 9
        and acquisition_permit["requestContract"]["requestCount"] == 18
        ...
        == {
            ...
            "checkerNormalizedSha256": (
                PERMIT.EXPECTED_V9_CHECKER_NORMALIZED
            ),
            "testsPath": (
            ...
            "contentSha256": PERMIT.EXPECTED_V9_CONTENT,
            "combinedInputSetSha256": PERMIT.EXPECTED_V9_INPUT_SET,
            "graphSha256": PERMIT.EXPECTED_V9_GRAPH,
            "frontierSha256": PERMIT.EXPECTED_V9_FRONTIER,
    ...
    require(
        len(resources) == 18
        and [row["requestOrdinal"] for row in resources] == list(range(1, 19))
        and [row["kind"] for row in resources] == ["mod", "zip"] * 9
    ...
    require(
        len(mod_by_tuple) == 9
        and aggregate_mod == 617
        and aggregate_zip == 16_363_277
        and aggregate == 16_363_894
        and aggregate_zip_entries == 3_329
        and aggregate_zip_uncompressed == 64_428_507
    ...
    evidence_expected = {
        "documentType": "aetherlink.wave11-source-acquisition-evidence",
        "schemaVersion": "1.0",
        "attemptId": PERMIT.ATTEMPT_ID,
        "requestCount": 18,
    ...
    receipt_expected = {
        "documentType": "aetherlink.wave11-source-acquisition-receipt",
        "schemaVersion": "1.0",
        ...
        "aggregateZipEntryCount": aggregate_zip_entries,
        "aggregateZipUncompressedBytes": aggregate_zip_uncompressed,
        "dispatchBoundaryCount": 18,
        "responseCommittedCount": 18,
        "responseCommittedBytes": aggregate,
        "validationCommittedCount": 18,
        "persistenceCommittedCount": 18,
    ...
    return {
        ...
        "combinedFixedPointV9ContentSha256": PERMIT.EXPECTED_V9_CONTENT,
        "compactIdentitySha256": PERMIT.EXPECTED_COMPACT_IDENTITY,
        "fullWitnessSha256": PERMIT.EXPECTED_FULL_WITNESS,
...
def create_readback_claim(
    ...
    claim = content_bound(
        {
            "documentType": "aetherlink.wave11-acquisition-readback-one-use-claim",
            "schemaVersion": "1.0",
            "status": "consumed_active",
    ...
    try:
        ...
        try:
            ...
            receipt = content_bound(
                {
                    "documentType": "aetherlink.wave11-source-acquisition-readback",
                    "schemaVersion": "1.0",
                    "status": (
                        "wave11_acquisition_retained_snapshot_"
            ...
            manifest = content_bound(
                {
                    "documentType": (
                        "aetherlink.wave11-source-acquisition-readback-manifest"
                    ),
                    "schemaVersion": "1.0",
                    "status": (
                        "wave11_acquisition_retained_snapshot_"
...
def _post_success_reporting_failure() -> dict[str, Any]:
    return {
        "documentType": "aetherlink.wave11-acquisition-readback-error",
        "schemaVersion": "1.0",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if args.preflight:
            preflight()
            result = {
                "documentType": "aetherlink.wave11-acquisition-readback-preflight",
                "schemaVersion": "1.0",
