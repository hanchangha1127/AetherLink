#!/usr/bin/env python3
"""Adversarial offline tests for the one-use Wave15 acquisition runner."""
...
if not (
    ...
    raise RuntimeError("Wave15 runner tests require `python3 -I -B -S`")
...
EXPECTED_WAVE15_RESOURCE_CONTRACT_SHA256 = (
    "2898a6d51c5c660f0c714524b862c5f715cb1d6971b57cfc737c666953068603"
)


RUNNER_PATH = Path(__file__).with_name(
    "acquire_p2p_nat_g2_pion_rung3_dependency_wave15_v1_once.py"
)
# The runner carries the finalized Wave15 acquisition-checker reverse pin.
# This suite exercises only fake fetches and local temporary directories; it
# never dispatches the production entry point or creates a network connection.
SPEC = importlib.util.spec_from_file_location(
    "wave15_source_acquirer_v1",
    RUNNER_PATH,
)
...
def exact_resource_oracle() -> list[dict[str, object]]:
    ...
    for tuple_order, (identity, resource_names) in enumerate(
        zip(EXPECTED_WAVE15_IDENTITY, EXPECTED_WAVE15_RESOURCE_NAMES),
        1,
    ):
        module, version, mod_h1, zip_h1 = identity
        tuple_id, mod_name, zip_name = resource_names
        digest = runner.sha256(f"{module}\n{version}\n".encode())
        accepted_names = {"mod": mod_name, "zip": zip_name}
            ...
            rows.append(
                {
                    ...
                    "acceptedFileName": accepted_names[kind],
                }
            )
    return rows
...
def fixture() -> tuple[list[dict[str, object]], dict[int, bytes]]:
    ...
    ordinal = 0
    for tuple_order in range(1, 5 + 1):
        module = f"example.test/dependency{tuple_order}"
        version = f"v1.0.{tuple_order}"
        digest = runner.sha256(f"{module}\n{version}\n".encode())
        tuple_id = f"wave15-{tuple_order:03d}-{digest[:12]}"
        selected = False
        mod = f"module {module}\n\ngo 1.22\n".encode()
        archive = make_zip(
            module,
...
class Wave15RunnerTests(unittest.TestCase):
    def setUp(self) -> None:
        self.wave4_network_patcher = mock.patch.object(
            runner.WAVE4,
            "direct_fetch",
            side_effect=AssertionError(
                "offline Wave15 tests must never reach the network"
            ),
        )
        ...
        self.https_network_patcher = mock.patch(
            ...
            side_effect=AssertionError(
                "offline Wave15 tests must never create HTTPS connections"
            ),
        )
    ...
    def test_exact_five_tuple_ten_resource_contract_is_closed(
        self,
    ) -> None:
        self.assertEqual(
            runner.EXPECTED_WAVE15_IDENTITY,
            EXPECTED_WAVE15_IDENTITY,
        )
        ...
        self.assertEqual(
            runner.EXPECTED_WAVE15_RESOURCE_CONTRACT_SHA256,
            EXPECTED_WAVE15_RESOURCE_CONTRACT_SHA256,
        )
        self.assertEqual(
        ...
        self.assertEqual(
            [
                (
                    ...
                    rows[index + 1]["acceptedFileName"],
                )
                for index in range(0, 10, 2)
            ],
            list(EXPECTED_WAVE15_RESOURCE_NAMES),
        )
        self.assertTrue(
            all(
                row["selectedByGraphAlgorithm"] is False
                and row["host"] == "proxy.golang.org"
                and row["port"] == 443
                and row["maximumResponseBodyBytes"]
                == (
                    1_048_576
                    if row["kind"] == "mod"
                    else 16_777_216
        ...
        self.assertIs(permit["structurePreparationOnly"], False)
        self.assertIs(permit["executionReady"], True)
        self.assertEqual(
        ...
        self.assertTrue(
            authority["dnsTcpTlsHttpsToExactProxyAuthorized"]
        )
        for key in (
    ...
    def test_success_claims_before_10_fetches_and_manifest_is_last(self) -> None:
        rows, bodies = fixture()
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(len(list(accepted.iterdir())), 10)
            self.assertTrue(
                terminal(root, runner.CHECK.CLAIM_PATH).exists()
            )
            self.assertTrue(
    ...
    def test_canonical_preflight_context_composes_without_writes(self) -> None:
        ...
        self.assertEqual(
            permit["requestContract"]["resources"],
            exact_resource_oracle(),
        )
    ...
    def test_semantic_or_cardinality_mutation_fails_before_claim(self) -> None:
        ...
        mutations = (
            ...
            lambda rows: rows.__setitem__(9, dict(rows[8])),
    ...
    def test_every_resource_field_is_required_on_all_10_rows(self) -> None:
        original, _ = fixture()
    ...
    def test_every_authority_and_filesystem_value_rejects_mutation_before_claim(
        self,
    ) -> None:
        rows, _ = fixture()
        ...
        baseline = values(copy.deepcopy(rows))["permit"]
        self.assertEqual(
            baseline["authority"],
            runner.EXPECTED_AUTHORITY,
        )
        self.assertEqual(
            baseline["filesystemAuthority"],
            runner.EXPECTED_FILESYSTEM_AUTHORITY,
        )
        self.assertEqual(
            set(false_authority_keys),
            {
                key
                for key, value in runner.EXPECTED_AUTHORITY.items()
                if value is False
            },
        )
        self.assertEqual(
            set(true_authority_keys),
            {
                key
                for key, value in runner.EXPECTED_AUTHORITY.items()
                if value is True
        ...
        def assert_rejected(
            ...
            mutated["permit"][section][key] = rejected_value
            namespace = mock.Mock()
            fetch = mock.Mock(
                side_effect=AssertionError(
        ...
        for key in false_authority_keys:
            for unauthorized in (True, 0):
                with self.subTest(key=key, unauthorized=unauthorized):
                    assert_rejected("authority", key, unauthorized)
        for key in true_authority_keys:
            for unauthorized in (False, 1):
                with self.subTest(key=key, unauthorized=unauthorized):
                    assert_rejected("authority", key, unauthorized)
        for key, expected in runner.EXPECTED_FILESYSTEM_AUTHORITY.items():
            rejected_values = (
                (False, 1)
                if expected is True
                else (True, 0)
    ...
    def test_execution_readiness_flags_are_exact_before_claim(self) -> None:
        rows, _ = fixture()
        mutations = (
            ("structurePreparationOnly", True),
            ("structurePreparationOnly", 0),
            ("executionReady", False),
            ("executionReady", 1),
        )
        for key, rejected_value in mutations:
            with self.subTest(key=key, rejected_value=rejected_value):
                mutated = values(copy.deepcopy(rows))
                mutated["permit"][key] = rejected_value
                namespace = mock.Mock()
                fetch = mock.Mock(
                    side_effect=AssertionError(
                        "readiness mutation reached fetch"
                    )
                )
                with self.assertRaises(runner.AcquisitionError) as caught:
                    runner._attempt(fetch, mutated, namespace)
    ...
    def test_lower_ascii_direct_proxy_identity_is_enforced(self) -> None:
        def rewritten_pair(module: str, version: str):
            ...
            for index, kind in enumerate(("mod", "zip")):
                ...
                rows[index]["tupleId"] = f"wave15-001-{digest[:12]}"
    ...
    def test_wave15_direct_fetch_reaches_wave4_and_translates_errors(
        self,
    ) -> None:
        resource = fixture()[0][0]
