...
MODULE_PATH = ROOT / "script/check_g7_reviewed_nonsecurity_swift_addon_v3.py"
if str(ROOT) not in sys.path:
    sys.path.insert(0, str(ROOT))
SPEC = importlib.util.spec_from_file_location(
    "script.check_g7_reviewed_nonsecurity_swift_addon_v3",
    MODULE_PATH,
)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError("cannot load reviewed Swift V3 add-on checker")
MODULE = importlib.util.module_from_spec(SPEC)
sys.modules[SPEC.name] = MODULE
SPEC.loader.exec_module(MODULE)


class ReviewedNonsecuritySwiftAddonV3Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.partition, failures = MODULE.contract_inputs()
        if failures or cls.partition is None:
            raise AssertionError(f"current V3 contract inputs failed: {failures}")

    def test_current_partition_is_complete_and_exact(self) -> None:
        partition = self.partition
        self.assertEqual(len(partition.discovered), 2_173)
        self.assertEqual(len(partition.antecedent_distinct), 1_023)
        self.assertEqual(len(partition.reviewed_input), 1_150)
        self.assertEqual(len(partition.selected), 97)
        self.assertEqual(len(partition.excluded_by_scope), 966)
        self.assertEqual(len(partition.excluded_external), 87)
        self.assertEqual(len(partition.remaining), 1_053)
        self.assertEqual(len(partition.distinct_after_addon), 1_120)
        self.assertEqual(
    ...
    def test_manifest_raw_bytes_sorting_and_identity_hash_are_exact(self) -> None:
        ...
        self.assertEqual(len(set(identities)), 97)
        self.assertEqual(
    ...
    def test_manifest_wrong_mode_and_symlink_are_rejected(self) -> None:
        ...
        with tempfile.TemporaryDirectory(
            prefix="aetherlink-v3-manifest-test-",
            dir=build_root,
        ) as temporary:
            root = Path(temporary)
    ...
    def test_selected_module_counts_are_exact(self) -> None:
        ...
        self.assertEqual(sum(MODULE.SELECTED_MODULE_COUNTS.values()), 97)

    def test_external_and_scope_classification_hashes_are_exact(self) -> None:
        external, failures = MODULE.external_or_socket_tests(
            self.partition.reviewed_input,
            self.partition.selected,
        )
        self.assertEqual(failures, [])
    ...
    def test_exact_regex_selects_only_the_97_identities(self) -> None:
    ...
    def test_output_directory_symlink_and_open_mode_are_rejected(self) -> None:
        build_root = MODULE.ROOT / ".build"
        with tempfile.TemporaryDirectory(
            prefix="aetherlink-v3-directory-test-",
            dir=build_root,
        ) as temporary:
            root = Path(temporary)
            target = root / "target"
    ...
    def test_same_count_selected_identity_substitution_fails(self) -> None:
        discovered, failures = MODULE.addon_v2.load_discovered_tests()
        self.assertEqual(failures, [])
        assert discovered is not None
        antecedent, failures = MODULE.addon_v2.partition_shape_failures(discovered)
        self.assertEqual(failures, [])
    ...
    def test_run_wrapper_forwards_exact_source_and_no_exclusions(self) -> None:
        ...
        self.assertEqual(marker.call_args.kwargs["expected_count"], 97)
    ...
    def test_execution_contract_round_trip_and_mutations(self) -> None:
        ...
        build_root = MODULE.ROOT / ".build"
        with tempfile.TemporaryDirectory(
            prefix="aetherlink-v3-execution-test-",
            dir=build_root,
        ) as temporary:
            ...
            with mock.patch.object(MODULE, "EXECUTION_CONTRACT_PATH", contract_path):
                ...
                self.assertEqual(document["runtimeExpected"]["tests"], 97)
    ...
    def test_result_payload_pins_v3_partition_and_limitations(self) -> None:
        ...
        with (
            mock.patch.object(MODULE, "stable_record", side_effect=fixture_record),
            mock.patch.object(
                MODULE,
                "physical_output_directory_failures",
                return_value=[],
        ...
        expected = {
            "antecedentDistinct": (1_023, MODULE.ANTECEDENT_DISTINCT_TEST_MANIFEST_SHA256),
            "discovered": (2_173, MODULE.DISCOVERED_TEST_MANIFEST_SHA256),
            "distinctAfterAddon": (1_120, MODULE.DISTINCT_AFTER_ADDON_TEST_MANIFEST_SHA256),
            "excludedByScope": (966, MODULE.EXCLUDED_BY_SCOPE_TEST_MANIFEST_SHA256),
            "excludedExternal": (87, MODULE.EXCLUDED_EXTERNAL_TEST_MANIFEST_SHA256),
            "newExecuted": (97, MODULE.NEW_TEST_MANIFEST_SHA256),
            "remaining": (1_053, MODULE.REMAINING_TEST_MANIFEST_SHA256),
            "reviewedInput": (1_150, MODULE.REVIEWED_INPUT_TEST_MANIFEST_SHA256),
        }
    ...
    def test_result_readback_rejects_stale_count_hash_and_boolean(self) -> None:
        ...
        mutations = (
            ("count", lambda value: value["partition"]["newExecuted"].__setitem__("tests", 96)),
        ...
        build_root = MODULE.ROOT / ".build"
        with tempfile.TemporaryDirectory(
            prefix="aetherlink-v3-result-test-",
            dir=build_root,
        ) as temporary:
