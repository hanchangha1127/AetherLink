#!/usr/bin/env python3
"""Focused adversarial tests for the read-only Wave14 decision checker."""
...
if not (
    ...
    raise RuntimeError("Wave14 decision tests require `python3 -I -B -S`")
...
DECISION_PATH = (
    ...
    "bounded-dependency-source-identity-and-acquisition-decision-wave14-v1.json"
)
READER_PATH = (
    "docs/security-hardening/production-p2p-nat-v1/"
    "g2-pion-restricted-fork-v1/rung-three/"
    "bounded-dependency-source-identity-and-acquisition-decision-wave14-v1.md"
)
...
EXPECTED_V12_PIN_ORACLE = {
    ...
    "totalGraphArchiveOpenCount": 2_644,
}

EXPECTED_WAVE13_NAMESPACE_ANCHOR = {
    "path":
...
EXPECTED_OPERATION_COUNTER_ORACLE = {
    ...
    "totalFullSourceReconstructionCount": 22,
    "predecessorGraphArchiveOpenCount": 2_310,
    "currentV12GraphArchiveOpenCount": 334,
    "totalV12GraphArchiveOpenCount": 2_644,
    "identityWitnessScanCount": 2,
    "identityWitnessArchiveOpenCount": 334,
    "overallDecisionExecutionArchiveOpenCount": 2_978,
...
EXPECTED_SUMMARY_ORACLE = {
    ...
    "tupleCount": 4,
    "parentDeclarationCount": 4,
    "moduleZipH1WitnessCount": 4,
    "goModH1WitnessCount": 4,
...
EXPECTED_HELD_SOURCE_INPUT_COUNT = 333
EXPECTED_ARCHIVE_COUNT = 167
...
WAVE13_GO_SUM_DELTA_ORACLE = {
    ...
    "predecessorGoSumEntryCount": 113,
    "acceptedDirectory": (
    ...
    "archives": (
        (
            ...
            "559ae04b112e304086df0300cf0bee7ce21497c36c088c461d68891b55905605",
            ("golang.org/x/mod@v0.26.0/go.sum",),
        ),
        (
            "002-ca3882149832dac56a85.zip",
            "cc9f79d1c8a796c756b636cee007b4ac5eba7b1f3276cd42658f4ac4a28cfbc1",
            ("golang.org/x/net@v0.42.0/go.sum",),
        ),
        (
            "003-affb7f9946408283a16e.zip",
            "27d5489227865ffffc3fbbbaf6100952ca2925eba19cfdff01e6e76b986b4bb4",
            (),
        ),
        (
            "004-afa9b13f01de51bd6d80.zip",
            "05ec70dbe6ef46c881f7d7ebe70ff61d9e998d2b4bbdbe6ff968870ef86c67b9",
            ("golang.org/x/telemetry@v0.0.0-20250710130107-8d8967aff50b/go.sum",),
        ),
    ),
}
...
EXPECTED_ACCEPTED_FILENAMES = (
    "001-6ade739bf483ce7bbe3a.mod",
    "001-6ade739bf483ce7bbe3a.zip",
    "002-47100744b1c211a90d13.mod",
    "002-47100744b1c211a90d13.zip",
    "003-1bdf857599830a828b79.mod",
    "003-1bdf857599830a828b79.zip",
    "004-b142b05fc3ea3268cdd3.mod",
    "004-b142b05fc3ea3268cdd3.zip",
)
...
def load_checker(raw: bytes) -> types.ModuleType:
    ...
    module = types.ModuleType("wave14_decision_checker_test_subject")
    module.__dict__.update(
        {
            "__cached__": None,
            "__file__": str(path),
            "__loader__": None,
            "__name__": "wave14_decision_checker_test_subject",
            "__package__": None,
        }
    )
    exec(
...
# Independently recomputed final Wave14 checker AST seals.
EXPECTED_CHECKER_CALL_COUNT = 745
EXPECTED_CHECKER_CALL_SURFACE_SHA256 = (
    "30d7640eaa1ff89eb602a7966d60033fdb7fe6a9de905d0df9b7c9e50f7aae88"
)
EXPECTED_CHECKER_FULL_AST_SHA256 = (
    "1f0fbcb91ee0c89ef288e1f158674f143cce5ed1437f4ad32076bd87265a1825"
)
        ...
        if isinstance(call.func, ast.Name) and call.func.id in {
            ...
            if call.func.id == "compile":
                ...
                valid = (
                    ...
                    and call.args[1].id == "V12_CHECKER_PATH"
...
class UnitBoundaryTests(unittest.TestCase):
    ...
    def test_07_frontier_and_request_order_are_exact(self) -> None:
        ...
        self.assertEqual(
            ...
            EXPECTED_V12_PIN_ORACLE["frontierSha256"],
        )
        self.assertNotEqual(
    ...
    def test_07c_v12_predecessor_pins_are_independent(self) -> None:
        actual = {
            "checkerPath": CHECKER.V12_CHECKER_PATH,
            "checkerRawSha256": CHECKER.V12_CHECKER_RAW_SHA256,
            "checkerNormalizedSha256":
                CHECKER.V12_CHECKER_NORMALIZED_SHA256,
            "testsPath": CHECKER.V12_TESTS_PATH,
            "testsRawSha256": CHECKER.V12_TESTS_RAW_SHA256,
            "contentSha256": CHECKER.V12_CANDIDATE_CONTENT_SHA256,
            "combinedInputSetSha256": CHECKER.V12_INPUT_SET_SHA256,
            "sourceBindingsSha256":
                CHECKER.V12_SOURCE_BINDINGS_SHA256,
            "graphSha256": CHECKER.V12_GRAPH_SHA256,
            "frontierSha256": CHECKER.V12_FRONTIER_SHA256,
            "fixedPointReached": False,
            "frontierTupleCount": CHECKER.EXPECTED_FRONTIER_COUNT,
            "totalFullSourceReconstructionCount": 22,
            "totalGraphArchiveOpenCount": 2_644,
        }
        self.assertEqual(actual, EXPECTED_V12_PIN_ORACLE)
        self.assertEqual(
            CHECKER.EXPECTED_FRONTIER_COUNT,
        ...
        checker_raw = (
            ROOT / EXPECTED_V12_PIN_ORACLE["checkerPath"]
        ).read_bytes()
        tests_raw = (
            ROOT / EXPECTED_V12_PIN_ORACLE["testsPath"]
        ).read_bytes()
        self.assertEqual(
            sha256(checker_raw),
            EXPECTED_V12_PIN_ORACLE["checkerRawSha256"],
        )
        self.assertEqual(
            sha256(independent_normalized_checker_bytes(checker_raw)),
            EXPECTED_V12_PIN_ORACLE["checkerNormalizedSha256"],
        )
        self.assertEqual(
            sha256(tests_raw),
            EXPECTED_V12_PIN_ORACLE["testsRawSha256"],
        )

    def test_07d_go_sum_count_includes_exact_wave13_delta(self) -> None:
        decision_raw = (
            ROOT / WAVE13_GO_SUM_DELTA_ORACLE["decisionPath"]
        ).read_bytes()
        self.assertEqual(
            sha256(decision_raw),
            WAVE13_GO_SUM_DELTA_ORACLE["decisionRawSha256"],
        )
        ...
        delta_count = 0
        accepted = (
            ROOT / WAVE13_GO_SUM_DELTA_ORACLE["acceptedDirectory"]
        )
        for name, expected_sha256, expected_go_sums in (
            WAVE13_GO_SUM_DELTA_ORACLE["archives"]
        ):
...
class HardeningBoundaryTests(unittest.TestCase):
    ...
    def test_wave14_namespace_rejects_stale_and_portable_aliases(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            try:
                CHECKER.validate_wave14_namespace_absent(*fds)
                cases = (
                    (dependency, ".WAVE-14-v1.claim"),
                    (
                        docs,
                        "bounded-dependency-source-acquisition-"
                        "wave14-execution-permit-v1.json",
                    ),
                    (
                        scripts,
                        "check_p2p_nat_g2_pion_rung3_dependency_"
                        "wave14_acquisition_v1.py",
                    ),
                )
        ...
        with mock.patch.object(
            ...
            with self.assertRaises(
                ...
                CHECKER.validate_wave14_namespace_absent(1, 2, 3)
        self.assertEqual(caught.exception.code, "E_WAVE14_NAMESPACE")
...
class LiveRepositoryTests(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        cls.package_raw = {
            ...
            CHECKER.V12_CHECKER_PATH:
                (ROOT / CHECKER.V12_CHECKER_PATH).read_bytes(),
            CHECKER.V12_TESTS_PATH:
                (ROOT / CHECKER.V12_TESTS_PATH).read_bytes(),
        }
    ...
    def test_08_self_reader_tests_and_v12_pins(self) -> None:
        self.assertEqual(
            sha256(
                CHECKER.normalized_self_bytes(
        ...
        self.assertEqual(
            sha256(self.package_raw[CHECKER.V12_CHECKER_PATH]),
            EXPECTED_V12_PIN_ORACLE["checkerRawSha256"],
        )
        self.assertEqual(
            sha256(self.package_raw[CHECKER.V12_TESTS_PATH]),
            EXPECTED_V12_PIN_ORACLE["testsRawSha256"],
        )
    ...
    def test_09_materialized_decision_is_exact_canonical_output(
        ...
        self.assertEqual(
            ...
            EXPECTED_V12_PIN_ORACLE["sourceBindingsSha256"],
        )
        self.assertEqual(
            (
                identity["tupleCount"],
    ...
    def test_10_authority_scope_and_counters_are_closed(self) -> None:
        ...
        self.assertEqual(
            ...
            (
                ...
                EXPECTED_V12_PIN_ORACLE["checkerNormalizedSha256"],
                EXPECTED_V12_PIN_ORACLE["testsRawSha256"],
                EXPECTED_V12_PIN_ORACLE["contentSha256"],
                EXPECTED_V12_PIN_ORACLE["combinedInputSetSha256"],
                EXPECTED_V12_PIN_ORACLE["sourceBindingsSha256"],
                EXPECTED_V12_PIN_ORACLE["graphSha256"],
                EXPECTED_V12_PIN_ORACLE["frontierSha256"],
                EXPECTED_V12_PIN_ORACLE["frontierTupleCount"],
            ),
        )
        self.assertEqual(
            (
        ...
        cases = (
            ...
            (
                lambda value:
                    value["predecessorBindings"][
                        "combinedFixedPointV12"
                    ]["wave13NamespaceAnchor"].__setitem__(
                        "rawSha256",
                        "0" * 64,
                    ),
                "E_DECISION_LINEAGE",
            ...
            (
                lambda value:
                    value["predecessorBindings"][
                        "combinedFixedPointV12"
                    ].__setitem__("v11TestsLiveHeld", True),
                "E_DECISION_LINEAGE",
            ),
            (
                lambda value:
