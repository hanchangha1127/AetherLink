#!/usr/bin/env python3
"""Tests for the Wave18 acquisition readback permit package."""
...
SPEC = importlib.util.spec_from_file_location("wave18_readback_permit_tests", PATH)
...
def _synthetic_package_raw() -> tuple[dict[str, bytes], str, str]:
    ...
    reader_raw = b"synthetic Wave18 readback reader\\n"
    raw = {
...
class Wave18ReadbackPermitTests(unittest.TestCase):
    def tearDown(self) -> None:
        self.assertEqual(NETWORK_ATTEMPTS, [])

    def test_01_frozen_snapshot_constants_are_exact(self):
        self.assertEqual(C.ATTEMPT_ID, "4380f5bbcd3366154b05111381ccab18")
        self.assertEqual(
        ...
        self.assertEqual(C.ACCEPTED_DIRECTORY["linkCount"], 8)
        self.assertEqual(C.ACCEPTED_DIRECTORY["exactFileCount"], 6)
        self.assertEqual(len(C.ACQUISITION_AUTHORITY), 15)
        self.assertEqual(len(C.ACCEPTED_FILES), 6)
        self.assertEqual(
        ...
        acquisition_permit_path = (
            f"{C.BASE}/bounded-dependency-source-acquisition-wave18-"
        ...
        self.assertEqual(payload["frozenFileCount"], 25)
        self.assertEqual(
        ...
        self.assertEqual(payload["aggregateZipEntryCount"], 971)
        self.assertEqual(
        ...
        v16 = acquisition_permit["predecessorBindings"][
        ...
        self.assertEqual(
            payload["predecessorBindings"]["combinedFixedPointV16"][
                "contentSha256"
            ],
            C.EXPECTED_V16_CONTENT,
        )
        self.assertFalse(
            any(
    ...
    def test_06_full_ast_call_surfaces_reject_same_count_bypasses(self):
        preload_mutations = (
            (
                "constant",
                b'ATTEMPT_ID = "4380f5bbcd3366154b05111381ccab18"',
                b'ATTEMPT_ID = "5380f5bbcd3366154b05111381ccab18"',
            ),
            (
                "branch",
    ...
    def test_08_wave18_cardinality_hash_aggregate_and_limit_drift_rejects(self):
        original, _ = _synthetic_permit()
        snapshot = original["frozenAcquisitionSnapshot"]

        def stale_wave17_frozen_21(value):
            value["frozenAcquisitionSnapshot"]["frozenFileCount"] = 21
        ...
        def bound_rows_14(value):
            ...
            predecessor.pop("wave17NamespaceAnchor")

        def predecessor_v15(value):
        ...
        def stale_wave17_aggregates(value):
        ...
        def stale_wave17_hashes(value):
            value["frozenAcquisitionSnapshot"].update(
                {
                    "frozenFilesCanonicalSha256": (
                        "bea9d0c6a260407e34524b5aced01cf9"
                        "a334c36a6f882350e57f02107b1008c8"
                    ),
                    "acceptedResourceHashSetCanonicalSha256": (
                        "7bee498b9c53d5d834fad61a286216279"
                        "1ad46f45471199389046fb466c16cfa"
                    ),
                }
            )

        def stale_wave17_limits(value):
            value["resourceLimits"].update(
                {
                    "maximumAcceptedResourceCount": 2,
                    "maximumAggregateModBytes": 1 * 1024 * 1024,
                    "maximumAggregateZipBytes": 16 * 1024 * 1024,
                    "maximumAggregateAcceptedBytes": 17 * 1024 * 1024,
        ...
        for mutation in (
            stale_wave17_frozen_21,
            authority_14,
            bound_rows_14,
            predecessor_v15,
            stale_wave17_aggregates,
            stale_wave17_hashes,
            stale_wave17_limits,
        ):
            changed = copy.deepcopy(original)
