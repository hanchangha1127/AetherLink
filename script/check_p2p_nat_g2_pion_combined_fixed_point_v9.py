#!/usr/bin/env python3
"""Recompute the exact 299-input Wave1..Wave10 graph without publishing.

Run only with ``python3 -I -B -S``. The checker pins the immutable v8
combined checker, all Wave10 decision/acquisition/readback controls, and the
root archive plus 149 mod and 149 zip inputs. Every source input is opened
...
def require_isolated_interpreter() -> None:
    ...
    if not (
        ...
        raise RuntimeError(
            "combined fixed-point v9 checker requires unoptimized "
            "`python3 -I -B -S`"
        )
...
SELF_PATH = "script/check_p2p_nat_g2_pion_combined_fixed_point_v9.py"
...
CHECKER_ID = (
    ...
    "wave7-wave8-wave9-wave10-check-v9"
)
...
WAVE10_MANIFEST_PATH = (
    f"{BASE}/bounded-dependency-source-acquisition-wave10-manifest-v1.json"
)
WAVE10_READBACK_PERMIT_PATH = (
    f"{BASE}/bounded-dependency-source-acquisition-wave10-"
    "readback-execution-permit-v1.json"
)
WAVE10_READBACK_PATH = (
    f"{BASE}/bounded-dependency-source-acquisition-wave10-readback-v1.json"
)
WAVE10_READBACK_MANIFEST_PATH = (
    f"{BASE}/bounded-dependency-source-acquisition-wave10-"
    "readback-manifest-v1.json"
)
WAVE10_ACCEPTED_DIRECTORY = (
    "build/offline-source/pion-ice-v4.3.0/dependencies/"
    "wave-10-v1/accepted"
)
WAVE10_ACQUISITION_CLAIM_PATH = (
    "build/offline-source/pion-ice-v4.3.0/dependencies/.wave-10-v1.claim"
)
WAVE10_EVIDENCE_PATH = (
    "build/offline-source/pion-ice-v4.3.0/dependencies/"
    "wave-10-v1/evidence.json"
)
WAVE10_READBACK_CLAIM_PATH = (
    "build/offline-source/pion-ice-v4.3.0/dependencies/"
    ".wave-10-v1-readback.claim"
)
WAVE10_FAILURE_PATH = (
    f"{BASE}/bounded-dependency-source-acquisition-wave10-failure-v1.json"
)
WAVE10_STAGING_PREFIX = ".wave-10-v1-staging-"
WAVE10_READBACK_TEMP_PREFIXES = (
...
WAVE10_CONTROL_METADATA = {
    WAVE10_DECISION_PATH: (24_723, 0o644),
    WAVE10_PERMIT_PATH: (27_327, 0o644),
    WAVE10_RECEIPT_PATH: (1_673, 0o600),
    WAVE10_MANIFEST_PATH: (465, 0o600),
    WAVE10_READBACK_PERMIT_PATH: (21_681, 0o644),
    WAVE10_READBACK_PATH: (12_917, 0o600),
    WAVE10_READBACK_MANIFEST_PATH: (2_302, 0o600),
}
WAVE10_CONTENT_SHA256 = {
    WAVE10_DECISION_PATH:
...
WAVE10_READBACK_AUTHORITY_BINDING = {
    "checker": {
        "path": (
            "script/check_p2p_nat_g2_pion_rung3_dependency_wave10_"
            "readback_execution_permit_v1.py"
        ),
        "rawSha256": WAVE10_READBACK_CHECKER_RAW_SHA256,
    },
    "permit": {
        "contentSha256": WAVE10_CONTENT_SHA256[WAVE10_READBACK_PERMIT_PATH],
        "path": WAVE10_READBACK_PERMIT_PATH,
        "rawSha256": WAVE10_CONTROL_SHA256[WAVE10_READBACK_PERMIT_PATH],
    },
    "recorder": {
        "path": (
        ...
        "rawSha256": WAVE10_READBACK_RECORDER_RAW_SHA256,
    },
}
WAVE10_DECISION_TOOL_BINDINGS = [
    {
        "normalizedSha256":
        ...
        "role": "current_wave10_decision_checker",
    },
    {
        "path":
        ...
        "role": "current_wave10_decision_tests",
    },
    {
        "normalizedSha256": V8_CHECKER_NORMALIZED_SHA256,
        "path": V8_CHECKER_PATH,
        "rawSha256": V8_CHECKER_RAW_SHA256,
        "role": "immutable_combined_v8_checker",
    },
    {
        "path": V8_TESTS_PATH,
        "rawSha256": V8_TESTS_RAW_SHA256,
        "role": "immutable_combined_v8_tests",
    },
]
WAVE10_ACQUISITION_TOOL_BINDINGS = [
    {
        "path":
            "script/check_p2p_nat_g2_pion_rung3_dependency_wave10_acquisition_v1.py",
        "rawSha256": WAVE10_ACQUISITION_CHECKER_RAW_SHA256,
    },
    {
        "path":
    ...
    },
    {
        ...
        "rawSha256": WAVE10_ACQUISITION_RUNNER_RAW_SHA256,
    },
    {
        "path":
...
WAVE10_READBACK_TOOL_BINDINGS = [
    WAVE10_READBACK_AUTHORITY_BINDING["checker"],
    {
        "path": (
    ...
    WAVE10_READBACK_AUTHORITY_BINDING["recorder"],
    {
        "path": (
...
def wave10_digest_bytes(value: Any) -> bytes:
    return json.dumps(
        value,
        ensure_ascii=False,
        sort_keys=True,
...
def harden_checker_module(module: types.ModuleType) -> types.ModuleType:
    ...
    if module.__dict__.get("_v9_safe_hardened") is True:
        return module
    module.__dict__["_v9_safe_hardened"] = True
...
def wave10_control_bindings() -> list[dict[str, Any]]:
    return [
        {
            "path": path,
            "rawSha256": digest,
            "maximumBytes": JSON_MAXIMUM_BYTES,
            "ownerOnly": WAVE10_CONTROL_METADATA[path][1] == 0o600,
            "kind": "terminal_evidence",
        }
        for path, digest in WAVE10_CONTROL_SHA256.items()
    ]


def parse_wave10_documents(
    runner: types.ModuleType,
    held: Any,
) -> dict[str, dict[str, Any]]:
    result: dict[str, dict[str, Any]] = {}
    for path in WAVE10_CONTROL_SHA256:
        value = runner.strict_json(held.raw[path], path)
        check(type(value) is dict, "E_WAVE10_JSON")
        result[path] = value
    return result


def wave10_auxiliary_evidence_bindings() -> list[dict[str, Any]]:
    return [
        {
            "path": WAVE10_ACQUISITION_CLAIM_PATH,
            "rawSha256": WAVE10_ACQUISITION_CLAIM_RAW_SHA256,
            "maximumBytes": 417,
            "ownerOnly": True,
            "kind": "consumed_acquisition_claim",
        },
        {
            "path": WAVE10_EVIDENCE_PATH,
            "rawSha256": WAVE10_ACQUISITION_EVIDENCE_RAW_SHA256,
            "maximumBytes": 9_031,
            "ownerOnly": True,
            "kind": "frozen_acquisition_evidence",
        },
        {
            "path": WAVE10_READBACK_CLAIM_PATH,
            "rawSha256": WAVE10_READBACK_CLAIM_RAW_SHA256,
            "maximumBytes": 1_255,
            "ownerOnly": True,
...
def validate_wave10_completed_namespace(
    control_held: Any,
    documents: Mapping[str, Mapping[str, Any]],
) -> None:
    acquisition_claim = control_held.files[WAVE10_ACQUISITION_CLAIM_PATH]
    readback_claim = control_held.files[WAVE10_READBACK_CLAIM_PATH]
    evidence = control_held.files[WAVE10_EVIDENCE_PATH]
    readback = control_held.files[WAVE10_READBACK_PATH]
    readback_manifest = control_held.files[WAVE10_READBACK_MANIFEST_PATH]
    for path, (expected_bytes, expected_mode) in WAVE10_CONTROL_METADATA.items():
    ...
    exact_dependency_names = {
        Path(WAVE10_ACQUISITION_CLAIM_PATH).name,
        Path(WAVE10_READBACK_CLAIM_PATH).name,
    }
    exact_base_names = {
        Path(path).name for path in WAVE10_CONTROL_SHA256
    }
    ...
    check(
        ...
        and not any(
            name.startswith(portable_name(WAVE10_STAGING_PREFIX))
            for name in normalized_dependency_names
        )
        and portable_name(Path(WAVE10_FAILURE_PATH).name)
        not in normalized_base_names
        and not any(
            name.startswith(portable_name(prefix))
            for name in normalized_base_names
            for prefix in WAVE10_READBACK_TEMP_PREFIXES
        )
    ...
    readback_permit = documents[WAVE10_READBACK_PERMIT_PATH]
    ...
    check(
        type(accepted_files) is list and len(accepted_files) == 22,
        "E_WAVE10_NAMESPACE",
    )
    ...
    check(
        len(expected_accepted_names) == 22
        and all(
            Path(row["path"]).parent.as_posix() == WAVE10_ACCEPTED_DIRECTORY
            for row in accepted_files
        ),
        "E_WAVE10_NAMESPACE",
    )
    ...
    try:
        ...
        check(
            ...
            and accepted_info.st_nlink == 24
            and stat.S_IMODE(accepted_info.st_mode) == 0o700
            and len(accepted_names) == 22
            and set(accepted_names) == expected_accepted_names
            and len(set(normalized_accepted_names)) == 22,
            "E_WAVE10_NAMESPACE",
        )
    finally:
        if accepted_fd >= 0:
...
def validate_wave10_consumed_claims(
    runner: types.ModuleType,
    ...
    check(
        ...
        == {
            "attemptId": WAVE10_ATTEMPT_ID,
            "checkerRawSha256": WAVE10_ACQUISITION_CHECKER_RAW_SHA256,
            "documentType": "aetherlink.wave10-source-acquisition-claim",
            "externalAuthenticationRequired": False,
            "permitContentSha256": WAVE10_CONTENT_SHA256[WAVE10_PERMIT_PATH],
            "requestCount": 22,
            "schemaVersion": "1.0",
            "status": "consumed_active",
            "userActionRequired": False,
        },
        "E_WAVE10_ACQUISITION_CLAIM",
    )
    ...
    check(
        ...
        == {
            ...
            "sha256": WAVE10_READBACK_CLAIM_CONTENT_SHA256,
        }
        and sha256_bytes(runner.canonical_json_bytes(without))
        == WAVE10_READBACK_CLAIM_CONTENT_SHA256
        and readback_claim.get("acquisitionAttemptId") == WAVE10_ATTEMPT_ID
        and readback_claim.get("readbackAttemptId")
        == WAVE10_READBACK_ATTEMPT_ID
        and readback_claim.get("authorityBinding")
        == WAVE10_READBACK_AUTHORITY_BINDING
        and readback_claim.get("documentType")
        == "aetherlink.wave10-acquisition-readback-one-use-claim"
        ...
        "E_WAVE10_READBACK_CLAIM",
    )


def validate_wave10_evidence(
    runner: types.ModuleType,
    raw: bytes,
    evidence_document: Mapping[str, Any],
    documents: Mapping[str, Mapping[str, Any]],
) -> None:
    check(type(evidence_document) is dict, "E_WAVE10_EVIDENCE")
    readback = documents[WAVE10_READBACK_PATH]
    ...
    check(
        ...
        and sha256_bytes(raw) == WAVE10_ACQUISITION_EVIDENCE_RAW_SHA256
        and evidence_document.get("documentType")
        == "aetherlink.wave10-source-acquisition-evidence"
        and evidence_document.get("schemaVersion") == "1.0"
        and evidence_document.get("attemptId") == WAVE10_ATTEMPT_ID
        and evidence_document.get("requestCount") == 22
        and evidence_document.get("aggregateResponseBytes") == 27_773_526
        and evidence_document.get("aggregateModResponseBytes") == 1_400
        and evidence_document.get("aggregateZipResponseBytes") == 27_772_126
        and evidence_document.get("aggregateZipEntryCount") == 4_872
        and evidence_document.get("aggregateZipUncompressedBytes")
        == 108_563_141
        and type(evidence_resources) is list
        and len(evidence_resources) == 22
...
def verify_wave10_content_bindings(
    v4: types.ModuleType,
    runner: types.ModuleType,
    documents: Mapping[str, Mapping[str, Any]],
) -> None:
    v4.verify_modern_content_binding(
        runner,
        documents[WAVE10_DECISION_PATH],
        WAVE10_CONTENT_SHA256[WAVE10_DECISION_PATH],
        "decision_without_contentBinding",
    )
    v4.verify_modern_content_binding(
        runner,
        documents[WAVE10_PERMIT_PATH],
        WAVE10_CONTENT_SHA256[WAVE10_PERMIT_PATH],
    ...
    for path in (
        ...
        v4.verify_content_binding(
            runner,
            documents[path],
            WAVE10_CONTENT_SHA256[path],
        )


def validate_v8_predecessor_candidate(
    runner: types.ModuleType,
    ...
    check(
        ...
        == WAVE10_HELD_SOURCE_BINDINGS_SHA256
        and predecessor.get("graphSha256") == V8_GRAPH_SHA256
        and predecessor.get("frontierSha256") == V8_FRONTIER_SHA256
        and predecessor.get("frontierTupleCount") == 11
        and predecessor.get("fixedPointReached") is False
        and predecessor.get("checkerPath") == V8_CHECKER_PATH
        and predecessor.get("checkerRawSha256") == V8_CHECKER_RAW_SHA256
        and predecessor.get("checkerNormalizedSha256")
        == V8_CHECKER_NORMALIZED_SHA256
        and predecessor.get("testsPath") == V8_TESTS_PATH
        and predecessor.get("testsRawSha256") == V8_TESTS_RAW_SHA256
        and predecessor.get("totalFullSourceReconstructionCount") == 14
        and predecessor.get("totalGraphArchiveOpenCount") == 1_366
        ...
        and predecessor.get("v7TestsBindingScope")
        == "historical_metadata_only_not_live_held"
        and predecessor.get("v7TestsLiveHeld") is False
        ...
        == {
            ...
            "finalNamespaceReverifiedAfterCombinedV8Reconstruction": True,
            "historicalExact38FrozenSnapshotDescriptorSetBound": True,
            "liveFinalAndAcceptedInventoriesVerifiedAtCombinedV8Barrier": True,
            "liveTerminalControlMetadataVerifiedAtCombinedV8Barrier": True,
        ...
        == {
            ...
            "sha256": V8_CANDIDATE_CONTENT_SHA256,
        }
        and sha256_bytes(runner.canonical_json_bytes(without))
        == V8_CANDIDATE_CONTENT_SHA256
        and candidate.get("schemaVersion") == "8.0"
        and candidate.get("documentType")
        == (
            "aetherlink.g2-pion-combined-wave1-wave2-wave3-wave4-"
            "wave5-wave6-wave7-wave8-wave9-fixed-point-candidate"
        )
        ...
        == (
            ...
            "wave1_through_wave9_source_bytes"
        )
        ...
        == {
            ...
            "frontierTupleCount": 11,
            "frontierSha256": V8_FRONTIER_SHA256,
        }
        and type(inputs) is dict
        and inputs.get("heldSourceInputCount") == 277
        and inputs.get("combinedInputSetSha256") == V8_INPUT_SET_SHA256
        and sha256_bytes(runner.canonical_json_bytes(source_bindings))
        == V8_INPUT_SET_SHA256
        and sha256_bytes(wave10_digest_bytes(source_bindings))
        == WAVE10_HELD_SOURCE_BINDINGS_SHA256
        and len({row["path"] for row in source_bindings}) == 277
        and sum(row["kind"] == "root_zip" for row in source_bindings) == 1
        and sum(row["kind"] == "mod" for row in source_bindings) == 138
        and sum(row["kind"] == "zip" for row in source_bindings) == 138
        ...
        and graph.get("newTupleCount") == 11
        and graph.get("graphSha256") == V8_GRAPH_SHA256
        and type(frontier) is list
        and len(frontier) == 11
        and sha256_bytes(runner.canonical_json_bytes(frontier))
        == V8_FRONTIER_SHA256
        and candidate.get("authority") == V8_AUTHORITY,
        "E_V8_PREDECESSOR",
    )
    return {
        ...
        "wave9CompletionAppliesToRetainedSnapshot": True,
        "wave9CurrentPathIdentityGuaranteedThroughManifestPublication": False,
        "wave9SameUidConcurrentRenameOrReplacementAfterLastBarrierPrevented":
            False,
        "v8TestsBindingScope": "historical_metadata_only_not_live_held",
        "v8TestsLiveHeld": False,
    }


def wave10_request_resources(
    v4: types.ModuleType,
    runner: types.ModuleType,
    documents: Mapping[str, Mapping[str, Any]],
) -> list[dict[str, Any]]:
    decision = documents[WAVE10_DECISION_PATH]
    permit = documents[WAVE10_PERMIT_PATH]
    receipt = documents[WAVE10_RECEIPT_PATH]
    manifest = documents[WAVE10_MANIFEST_PATH]
    readback_permit = documents[WAVE10_READBACK_PERMIT_PATH]
    readback = documents[WAVE10_READBACK_PATH]
    readback_manifest = documents[WAVE10_READBACK_MANIFEST_PATH]
    verify_wave10_content_bindings(v4, runner, documents)
    ...
    check(
        ...
        == {
            ...
            "sha256": WAVE10_CONTENT_SHA256[WAVE10_DECISION_PATH],
        }
        and decision.get("status")
        == (
            "wave10_exact_11_frontier_identity_classified_11_complete_"
            "0_blocked_acquisition_ready_not_authorized"
        )
        and decision.get("authority") == WAVE10_DECISION_AUTHORITY
        and decision.get("toolBindings") == WAVE10_DECISION_TOOL_BINDINGS
        and type(resolution) is dict
        and resolution.get("tupleCount") == 11
        and resolution.get("completeIdentityPairCount") == 11
        and resolution.get("blockedTupleCount") == 0
        and resolution.get("graphSelectedTupleCount") == 1
        and resolution.get("versionSpecificNonSelectedTupleCount") == 10
        and resolution.get("compactIdentitySha256")
        == WAVE10_COMPACT_IDENTITY_SHA256
        and resolution.get("fullWitnessSha256")
        == WAVE10_FULL_WITNESS_SHA256
        and type(held_set) is dict
        and held_set.get("sourceBindingCount") == 277
        ...
        and preparation.get("requestCount") == 22
        ...
        and len(identity_tuples) == 11
        and type(source_requests) is list
        and len(source_requests) == 22
        and sha256_bytes(wave10_digest_bytes(source_requests))
        == WAVE10_REQUEST_SET_SHA256,
        "E_WAVE10_DECISION",
    )
    ...
    check(
        ...
        == {
            ...
            "sha256": WAVE10_CONTENT_SHA256[WAVE10_PERMIT_PATH],
        }
        and permit.get("status") == "authorized_not_consumed"
        and permit.get("authority") == WAVE10_ACQUISITION_AUTHORITY
        and permit.get("toolBindings") == WAVE10_ACQUISITION_TOOL_BINDINGS
        and permit.get("decisionBinding", {}).get("rawSha256")
        == WAVE10_CONTROL_SHA256[WAVE10_DECISION_PATH]
        ...
        == {
            "checkerNormalizedSha256": V8_CHECKER_NORMALIZED_SHA256,
            "checkerPath": V8_CHECKER_PATH,
            "checkerRawSha256": V8_CHECKER_RAW_SHA256,
            "combinedInputSetSha256": V8_INPUT_SET_SHA256,
            "contentSha256": V8_CANDIDATE_CONTENT_SHA256,
            "frontierSha256": V8_FRONTIER_SHA256,
            "graphSha256": V8_GRAPH_SHA256,
            "sourceBindingsSha256": WAVE10_HELD_SOURCE_BINDINGS_SHA256,
            "testsPath": V8_TESTS_PATH,
            "testsRawSha256": V8_TESTS_RAW_SHA256,
            "v7TestsBindingScope":
                "historical_metadata_only_not_live_held",
            "v7TestsLiveHeld": False,
        }
        and type(contract) is dict
        and contract.get("requestCount") == 22
        and contract.get("tupleCount") == 11
    ...
    check(
        ...
        and receipt.get("attemptId") == WAVE10_ATTEMPT_ID
        and receipt.get("acceptedPath") == WAVE10_ACCEPTED_DIRECTORY
        and receipt.get("acceptedResourceCount") == 22
        and receipt.get("modCount") == 11
        and receipt.get("zipCount") == 11
        and receipt.get("aggregateResponseBytes") == 27_773_526
        and receipt.get("aggregateModResponseBytes") == 1_400
        and receipt.get("aggregateZipResponseBytes") == 27_772_126
        and receipt.get("aggregateZipEntryCount") == 4_872
        and receipt.get("aggregateZipUncompressedBytes") == 108_563_141
    ...
    check(
        ...
        == {
            "attemptId": WAVE10_ATTEMPT_ID,
            "documentType": "aetherlink.wave10-source-acquisition-manifest",
            "manifestWrittenLast": True,
            "receiptPath": WAVE10_RECEIPT_PATH,
            "receiptRawSha256": WAVE10_CONTROL_SHA256[WAVE10_RECEIPT_PATH],
            "schemaVersion": "1.0",
    ...
    snapshot_predecessor = (
        snapshot.get("predecessorBindings", {}).get("combinedFixedPointV8")
        if type(snapshot) is dict
        else None
    )
    final_directory = (
    ...
    check(
        ...
        and exact_keys(
            ...
            (
                ...
                "combinedFixedPointV8PredecessorBindingRecomputed",
                ...
                "exact22ResourceOrderAndAggregateRecomputed",
                "exact40FrozenFileSnapshotRequired",
                ...
                "v7TestsLiveHeld", "v8TestsLiveHeld",
    ...
    check(
        ...
        == {
            ...
            "sha256": WAVE10_CONTENT_SHA256[WAVE10_READBACK_PERMIT_PATH],
        }
        and readback_permit.get("status") == "authorized_not_consumed"
        and readback_permit.get("authority") == WAVE10_READBACK_AUTHORITY
        ...
        is True
        and verification_contract.get("exact40FrozenFileSnapshotRequired")
        is True
        ...
        is False
        and verification_contract.get("v7TestsLiveHeld") is False
        and verification_contract.get("v8TestsLiveHeld") is True
        and verification_contract.get(
            "requestResourcesCanonicalSha256Recomputed"
        ) is True
        ...
        and snapshot.get("attemptId") == WAVE10_ATTEMPT_ID
        and identity_bindings
        == {
            "compactIdentitySha256": WAVE10_COMPACT_IDENTITY_SHA256,
            "fullWitnessSha256": WAVE10_FULL_WITNESS_SHA256,
            "heldSourceBindingsSha256":
                WAVE10_HELD_SOURCE_BINDINGS_SHA256,
        ...
        == {
            ...
            "stagingPrefix": ".wave-10-v1-staging-",
        }
        and snapshot.get("frozenFileCount") == 40
        and snapshot.get("frozenFilesCanonicalSha256")
        == WAVE10_FROZEN_FILE_SET_SHA256
        and snapshot.get("acceptedResourceCount") == 22
        and snapshot.get("selectedTupleCount") == 1
        and snapshot.get("selectedRequestOrdinals") == [21, 22]
        and snapshot.get("modCount") == 11
        and snapshot.get("zipCount") == 11
        and snapshot.get("aggregateAcceptedBytes") == 27_773_526
        and snapshot.get("aggregateModBytes") == 1_400
        and snapshot.get("aggregateZipBytes") == 27_772_126
        and snapshot.get("aggregateZipEntryCount") == 4_872
        and snapshot.get("aggregateZipUncompressedBytes") == 108_563_141
        ...
        and acquisition_authority_by_path.get(
            (
                "script/acquire_p2p_nat_g2_pion_rung3_dependency_wave10_"
                "v1_once.py"
            ),
            {},
        ).get("rawSha256")
        ...
        == {
            ...
            "mode": "0600",
            "ownerUid": os.geteuid(),
            "path": WAVE10_RECEIPT_PATH,
            "rawSha256": WAVE10_CONTROL_SHA256[WAVE10_RECEIPT_PATH],
        }
        ...
        == {
            ...
            "mode": "0600",
            "ownerUid": os.geteuid(),
            "path": WAVE10_MANIFEST_PATH,
            "rawSha256": WAVE10_CONTROL_SHA256[WAVE10_MANIFEST_PATH],
        }
        ...
        and accepted.get("path") == WAVE10_ACCEPTED_DIRECTORY
        and accepted.get("mode") == "0700"
        and accepted.get("ownerUid") == os.geteuid()
        and accepted.get("linkCount") == 24
        and accepted.get("exactFileCount") == 22
        and type(accepted_files) is list
        and len(accepted_files) == 22
        and sha256_bytes(
    ...
    check(
        ...
        == {
            ...
            "authorityFileCount",
            "combinedFixedPointV8ContentSha256",
    ...
    check(
        ...
        == {
            "algorithm":
                "sha256(canonical-json-without-contentBinding)",
            "sha256": WAVE10_CONTENT_SHA256[WAVE10_READBACK_PATH],
        }
        and readback.get("status")
        == "wave10_acquisition_retained_snapshot_independently_read_back"
        and readback.get("acquisitionAttemptId") == WAVE10_ATTEMPT_ID
        and readback.get("readbackAttemptId") == WAVE10_READBACK_ATTEMPT_ID
        ...
        == {
            "bytes": 1_255,
            "contentSha256": WAVE10_READBACK_CLAIM_CONTENT_SHA256,
            "linkCount": 1,
            "mode": "0600",
            "ownerUid": os.geteuid(),
            "path": (
        ...
        == "wave10_acquisition_retained_snapshot_independently_verified"
        ...
        and verified.get("acceptedResourceCount") == 22
        ...
        and verified.get("selectedTupleCount") == 1
        == snapshot.get("selectedTupleCount")
        and verified.get("selectedRequestOrdinals") == [21, 22]
        ...
        == 108_563_141
        and verified.get("combinedFixedPointV8ContentSha256")
        == V8_CANDIDATE_CONTENT_SHA256
    ...
    check(
        ...
        == {
            "algorithm":
                "sha256(canonical-json-without-contentBinding)",
            "sha256": WAVE10_CONTENT_SHA256[
                WAVE10_READBACK_MANIFEST_PATH
            ],
        }
        and readback_manifest.get("status")
        == "wave10_acquisition_retained_snapshot_readback_publication_complete"
        and readback_manifest.get("acquisitionAttemptId") == WAVE10_ATTEMPT_ID
        ...
        == {
            "bytes": 12_917,
            "contentSha256": WAVE10_CONTENT_SHA256[WAVE10_READBACK_PATH],
            "linkCount": 1,
            "mode": "0600",
            "ownerUid": os.geteuid(),
            "path": WAVE10_READBACK_PATH,
            "rawSha256": WAVE10_CONTROL_SHA256[WAVE10_READBACK_PATH],
        }
    ...
    check(
        len(accepted_by_name) == 22
        and len(verified_by_name) == 22
        and len(identity_by_order) == 11,
        "E_WAVE10_RESOURCE",
    )
    ...
    for index, value in enumerate(resources, start=1):
        ...
        check(
            ...
            == f"wave10-{tuple_order:03}-{digest[:12]}"
        ...
        check(
            ...
            == f"{WAVE10_ACCEPTED_DIRECTORY}/{name}"
...
def combined_source_bindings(
    ...
    bindings = v8.combined_source_bindings(
        v7,
        v6,
        v5,
        v4,
    ...
    bindings.extend(wave10_request_resources(v4, runner, wave10_documents))
    check(
        len(bindings) == 299
        and sum(row["kind"] == "root_zip" for row in bindings) == 1
        and sum(row["kind"] == "mod" for row in bindings) == 149
        and sum(row["kind"] == "zip" for row in bindings) == 149
    ...
    check(
        len({row["path"] for row in bindings}) == 299
        and len(
            {
    ...
    check(
        ...
        and sorted(order for _, _, order in pair_kinds) == list(range(1, 150)),
        "E_COMBINED_INPUT",
    )
    return bindings
...
def derive_and_validate_tool_paths(
    ...
    declared_v8_paths = v8.TRANSITIVE_CHECKER_PATHS
    ...
    check(
        ...
        and type(declared_v8_paths) is set
        and declared_v8_paths
        == TRANSITIVE_CHECKER_PATHS - {V8_CHECKER_PATH}
        ...
        == len(
            declared_v8_paths
            | {V8_CHECKER_PATH, v8.V1_PROVIDER_PATH}
        )
        and V8_TESTS_PATH not in direct_binding_paths
        and V8_TESTS_PATH not in predecessor_binding_paths,
        "E_TOOL_BINDINGS",
    )
    transitive_paths = direct_binding_paths | declared_v8_paths
    check(
        transitive_paths
        == TRANSITIVE_CHECKER_PATHS | {SELF_PATH, V1_PROVIDER_PATH}
        and V8_TESTS_PATH not in transitive_paths,
        "E_TOOL_BINDINGS",
    )
...
def execute_reconstruction_protocol_prefix(
    ...
    validate_wave10_completed_namespace(control_held, wave10_documents)
...
def generate_candidate(root: Path = ROOT) -> dict[str, Any]:
    ...
    with (
        ...
        v8 = load_v8_checker(v8_held)
        predecessor_candidate = v8.generate_candidate(root)
        v8 = harden_checker_module(v8)
        ...
        with v4.PinnedCodeFile(
            ...
            with v1.PinnedRunnerFile(root) as provider_held:
                ...
                check(
                    v8.PinnedCodeFile is PinnedCodeFile
                ...
                auxiliary_evidence = wave10_auxiliary_evidence_bindings()
                check(
                    len(controls) == 66
                ...
                with runner.HeldInputSet(
                    ...
                    wave10_documents = parse_wave10_documents(
                        runner,
                        control_held,
                    )
                    predecessor_verification = (
                        validate_v8_predecessor_candidate(
                            runner,
                            predecessor_candidate,
                            wave10_documents[WAVE10_DECISION_PATH],
                        )
                    )
                    ...
                    wave10_evidence_raw = control_held.raw[WAVE10_EVIDENCE_PATH]
                    wave10_evidence_document = runner.strict_json(
                        wave10_evidence_raw,
                        WAVE10_EVIDENCE_PATH,
                    )
                    validate_wave10_evidence(
                        runner,
                        wave10_evidence_raw,
                        wave10_evidence_document,
                        wave10_documents,
                    )
                    validate_wave10_consumed_claims(
                        runner,
                        control_held.raw[WAVE10_ACQUISITION_CLAIM_PATH],
                        control_held.raw[WAVE10_READBACK_CLAIM_PATH],
                    )
                    with runner.HeldInputSet(root, bindings) as source_held:
                        ...
                        check(
                            ...
                            and sha256_bytes(wave10_digest_bytes(projection))
                            == V9_SOURCE_BINDINGS_SHA256,
                            "E_COMBINED_INPUT",
                        )
                        ...
                        body = {
                            ...
                            "checkerVerification": {
                                ...
                                "underlyingIndependentGraphAlgorithmCount":
                                    ...
                                    + 4,
                                "pinnedV8PredecessorExecuted": True,
                                "v8TestsBindingScope":
                                    "historical_metadata_only_not_live_held",
                                "v8TestsLiveHeld": False,
                                ...
                                "wave10HistoricalExact40FrozenSnapshotDescriptorSetBound":
                                    True,
                                "wave10LiveTerminalControlMetadataVerified":
                                    True,
                                "wave10LiveFinalAndAcceptedInventoriesVerified":
                                    True,
                                "wave10FinalNamespaceReverifiedAfterReconstruction":
                                    True,
                                "wave10RetainedFdPreManifestBarrierCount": 3,
                                "wave10CompletionAppliesToRetainedSnapshot":
                                    True,
...
def error_document_bytes() -> bytes:
    return (
        json.dumps(
            {
                "documentType": (
                    "aetherlink.g2-pion-combined-wave1-through-wave10-"
