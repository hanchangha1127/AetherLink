#!/usr/bin/env python3
"""Record one independent offline readback of the Wave10 v1 acquisition."""
...
if not (
    ...
    raise RuntimeError("Wave10 readback recorder requires `python3 -I -B -S`")
...
if O_NOFOLLOW is None:
    raise RuntimeError("Wave10 readback recorder requires O_NOFOLLOW")
...
EXPECTED_READBACK_CHECKER_RAW = "b118273738520e78c669a7f07a4639a94147134af6c9ffaffa4ac441e9e7cfd0"
...
def load_readback_checker() -> types.ModuleType:
    ...
    module = types.ModuleType("wave10_acquisition_readback_permit_v1")
...
def validate_resource_selection(
    ...
    require(
        selected_request_ordinals == [21, 22]
        and selected_tuple_ids == {"wave10-011-4638677582e2"}
        and all(
            row["module"] == "golang.org/x/xerrors"
            and row["version"]
...
def verify_snapshot(snapshot: FrozenSnapshot) -> dict[str, Any]:
    ...
    v8 = acquisition_permit["predecessorBindings"]["combinedFixedPointV8"]
    predecessor_authority_rows = [
        {
            "path": v8["checkerPath"],
            "rawSha256": v8["checkerRawSha256"],
        },
        {
            "path": v8["testsPath"],
            "rawSha256": v8["testsRawSha256"],
        },
    ]
    ...
    require(
        ...
        and acquisition_permit["requestContract"]["tupleCount"] == 11
        and acquisition_permit["requestContract"]["requestCount"] == 22
        ...
        == {
            ...
            "checkerNormalizedSha256": (
                PERMIT.EXPECTED_V8_CHECKER_NORMALIZED
            ),
            "testsPath": (
            ...
            "contentSha256": PERMIT.EXPECTED_V8_CONTENT,
            "combinedInputSetSha256": PERMIT.EXPECTED_V8_INPUT_SET,
            "graphSha256": PERMIT.EXPECTED_V8_GRAPH,
            "frontierSha256": PERMIT.EXPECTED_V8_FRONTIER,
    ...
    require(
        len(resources) == 22
        and [row["requestOrdinal"] for row in resources] == list(range(1, 23))
        and [row["kind"] for row in resources] == ["mod", "zip"] * 11
    ...
    require(
        len(mod_by_tuple) == 11
        and aggregate_mod == 1_400
        and aggregate_zip == 27_772_126
        and aggregate == 27_773_526
        and aggregate_zip_entries == 4_872
        and aggregate_zip_uncompressed == 108_563_141
    ...
    evidence_expected = {
        "documentType": "aetherlink.wave10-source-acquisition-evidence",
        "schemaVersion": "1.0",
        "attemptId": PERMIT.ATTEMPT_ID,
        "requestCount": 22,
    ...
    receipt_expected = {
        "documentType": "aetherlink.wave10-source-acquisition-receipt",
        "schemaVersion": "1.0",
        ...
        "aggregateZipEntryCount": aggregate_zip_entries,
        "aggregateZipUncompressedBytes": aggregate_zip_uncompressed,
        "dispatchBoundaryCount": 22,
        "responseCommittedCount": 22,
        "responseCommittedBytes": aggregate,
        "validationCommittedCount": 22,
        "persistenceCommittedCount": 22,
    ...
    return {
        ...
        "combinedFixedPointV8ContentSha256": PERMIT.EXPECTED_V8_CONTENT,
        "compactIdentitySha256": PERMIT.EXPECTED_COMPACT_IDENTITY,
        "fullWitnessSha256": PERMIT.EXPECTED_FULL_WITNESS,
...
def create_readback_claim(
    ...
    claim = content_bound(
        {
            "documentType": "aetherlink.wave10-acquisition-readback-one-use-claim",
            "schemaVersion": "1.0",
            "status": "consumed_active",
    ...
    try:
        ...
        try:
            ...
            receipt = content_bound(
                {
                    "documentType": "aetherlink.wave10-source-acquisition-readback",
                    "schemaVersion": "1.0",
                    "status": (
                        "wave10_acquisition_retained_snapshot_"
            ...
            manifest = content_bound(
                {
                    "documentType": (
                        "aetherlink.wave10-source-acquisition-readback-manifest"
                    ),
                    "schemaVersion": "1.0",
                    "status": (
                        "wave10_acquisition_retained_snapshot_"
...
def _post_success_reporting_failure() -> dict[str, Any]:
    return {
        "documentType": "aetherlink.wave10-acquisition-readback-error",
        "schemaVersion": "1.0",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if args.preflight:
            preflight()
            result = {
                "documentType": "aetherlink.wave10-acquisition-readback-preflight",
                "schemaVersion": "1.0",
