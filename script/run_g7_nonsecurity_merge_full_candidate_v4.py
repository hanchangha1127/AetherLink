#!/usr/bin/env python3
"""Compose the V3 Merge-full evidence with the current V4 Swift add-on.

The V3 candidate remains immutable evidence for 75 composed commands.  This
successor runs only the four V4 add-on commands against an exact evidence-only
...
if __package__:
    ...
    from script import run_g7_nonsecurity_merge_full_candidate_v3 as base
else:
    import check_g7_reviewed_nonsecurity_swift_addon_v4 as addon_v4
    import run_g7_nonsecurity_merge_full_candidate_v3 as base
...
IMPLEMENTATION_PATHS = tuple(
    sorted(
        set(base.IMPLEMENTATION_PATHS) | set(V4_IMPLEMENTATION_PATHS),
...
ARTIFACT_PATHS = tuple(
    sorted(
        set(base.ARTIFACT_PATHS) | set(V4_ADDON_ARTIFACT_PATHS),
        key=lambda path: path.as_posix().encode("ascii"),
    )
)
...
def validate_static_contract() -> None:
    ...
    if len(EXPECTED_COMMAND_IDS) != 4 or len(set(EXPECTED_COMMAND_IDS)) != 4:
        raise CandidateError("V4 successor must execute four unique commands")
    ...
    if len(ARTIFACT_PATHS) != 46 or len(set(ARTIFACT_PATHS)) != 46:
    ...
    if len(IMPLEMENTATION_PATHS) != 23 or len(set(IMPLEMENTATION_PATHS)) != 23:
    ...
    if RESULT_RELATIVE_PATH in ARTIFACT_PATHS:
        raise CandidateError("V4 candidate must not recursively record itself")
    if RESULT_RELATIVE_PATH in IMPLEMENTATION_PATHS:
        raise CandidateError("V4 candidate result must not be an implementation input")
...
def current_v4_addon_failures() -> list[str]:
    partition, failures = addon_v4.contract_inputs()
    if partition is None:
        return failures or ["V4 add-on contract inputs are unavailable"]
...
def candidate_payload(
    *,
    ...
    v3_artifact_preservation: dict[str, object],
    ...
    return {
        ...
        "source": source,
        "v3ArtifactPreservation": v3_artifact_preservation,
    }
...
def produce_candidate(
    *,
    ...
    if result_path != canonical_result_path:
        raise CandidateError(
            "candidate result must use the exact canonical V4 result path"
        )

    ensure_private_output_directory(result_path.parent, root=root)
    source_before = runtime.source_snapshot(root=root)
    v3_candidate_before = runtime.stable_file_record(
        V3_CANDIDATE_RELATIVE_PATH,
        root=root,
        maximum_bytes=RESULT_MAX_BYTES,
    )
    if v3_candidate_before != EXPECTED_V3_CANDIDATE_RECORD:
        raise CandidateError("V3 antecedent candidate identity differs before execution")
    antecedent_failures = addon_v4.candidate_antecedent_failures()
    if antecedent_failures:
        raise CandidateError(
            "V4 source projection failed before execution: "
            + "; ".join(antecedent_failures)
        )
    antecedent_artifacts_before = tuple(
        runtime.stable_file_record(path, root=root)
        for path in base.ARTIFACT_PATHS
    )
    antecedent_implementation_before = tuple(
    ...
    for gate in ALL_GATES:
        if gate.identifier == "g7-reviewed-nonsecurity-swift-addon-v4-prepare":
    ...
    v3_candidate_after = runtime.stable_file_record(
        V3_CANDIDATE_RELATIVE_PATH,
        root=root,
        maximum_bytes=RESULT_MAX_BYTES,
    )
    if v3_candidate_after != v3_candidate_before:
        raise CandidateError("V3 antecedent candidate changed during execution")
    antecedent_artifacts_after = tuple(
        runtime.stable_file_record(path, root=root)
        for path in base.ARTIFACT_PATHS
    )
    if antecedent_artifacts_after != antecedent_artifacts_before:
        raise CandidateError("V3 antecedent artifact set changed during execution")
    antecedent_implementation_after = tuple(
        runtime.stable_file_record(
            path,
            root=root,
            maximum_bytes=4 * 1024 * 1024,
    ...
    if antecedent_implementation_after != antecedent_implementation_before:
        raise CandidateError("V3 antecedent implementation changed during execution")
    v3_artifact_preservation = {
        "after": v3_candidate_after,
        "before": v3_candidate_before,
        "preservedDuringRun": True,
    }
    postflight_failures = current_v4_addon_failures()
    if postflight_failures:
        raise CandidateError(
            "V3/V4 antecedent evidence changed during execution: "
    ...
    if artifacts != artifacts_before_postflight:
        raise CandidateError("V3/V4 artifact bytes changed during postflight")
    v4_addon_result = next(
        record
        for record in artifacts
        if record["path"] == V4_ADDON_RESULT_RELATIVE_PATH.as_posix()
    )
    ...
    payload = candidate_payload(
        ...
        v3_artifact_preservation=v3_artifact_preservation,
        composition=evidence_composition(
            v3_candidate=v3_candidate_after,
            v4_addon_result=v4_addon_result,
        ),
    )
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="exact canonical V4 result path (alternate paths are rejected)",
    )
    parser.add_argument(
        "--preserve-pid",
        type=int,
    ...
    try:
        ...
        print(
            f"G7 non-security Merge-full V4 candidate failed: {error}",
            file=os.sys.stderr,
        )
        return 1
    print(
        "G7 non-security Merge-full V4 composed candidate published: "
