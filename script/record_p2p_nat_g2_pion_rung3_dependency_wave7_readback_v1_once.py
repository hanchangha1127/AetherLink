#!/usr/bin/env python3
"""Record one independent offline readback of the Wave7 v1 acquisition."""
...
if not (
    ...
    raise RuntimeError("Wave7 readback recorder requires `python3 -I -B -S`")
...
if O_NOFOLLOW is None:
    raise RuntimeError("Wave7 readback recorder requires O_NOFOLLOW")
READBACK_CHECKER_PATH = Path(__file__).with_name(
    "check_p2p_nat_g2_pion_rung3_dependency_wave7_"
    "readback_execution_permit_v1.py"
)
EXPECTED_READBACK_CHECKER_RAW = "b2c77d3f59ff342ae6ef48f4e2bf13ba2e51772c00ad217ffcd19c4706cb0ad7"
...
def load_readback_checker() -> types.ModuleType:
    ...
    module = types.ModuleType("wave7_acquisition_readback_permit_v1")
...
def verify_snapshot(snapshot: FrozenSnapshot) -> dict[str, Any]:
    ...
    permit_path = (
        PERMIT.BASE
        + "/bounded-dependency-source-acquisition-wave7-execution-permit-v1.json"
    )
    ...
    v5 = acquisition_permit["predecessorBindings"]["combinedFixedPointV5"]
    predecessor_authority_rows = [
        {
            "path": v5["checkerPath"],
            "rawSha256": v5["checkerRawSha256"],
        },
        {
            "path": v5["testsPath"],
            "rawSha256": v5["testsRawSha256"],
        },
    ]
    ...
    require(
        ...
        == {
            ...
            "checkerNormalizedSha256": (
                PERMIT.EXPECTED_V5_CHECKER_NORMALIZED
            ),
            "testsPath": (
            ...
            "contentSha256": PERMIT.EXPECTED_V5_CONTENT,
            "combinedInputSetSha256": PERMIT.EXPECTED_V5_INPUT_SET,
            "graphSha256": PERMIT.EXPECTED_V5_GRAPH,
            "frontierSha256": PERMIT.EXPECTED_V5_FRONTIER,
    ...
    claim_expected = {
        "documentType": "aetherlink.wave7-source-acquisition-claim",
        "schemaVersion": "1.0",
    ...
    require(
        len(resources) == 30
        and [row["requestOrdinal"] for row in resources] == list(range(1, 31))
        and [row["kind"] for row in resources] == ["mod", "zip"] * 15
    ...
    require(
        len(mod_by_tuple) == 15
        and aggregate_mod == 1_908
        and aggregate_zip == 32_350_343
        and aggregate == 32_352_251
        and aggregate_zip_entries == 6_469
        and aggregate_zip_uncompressed == 128_224_223
    ...
    evidence_expected = {
        "documentType": "aetherlink.wave7-source-acquisition-evidence",
        "schemaVersion": "1.0",
        "attemptId": PERMIT.ATTEMPT_ID,
        "requestCount": 30,
    ...
    receipt_expected = {
        "documentType": "aetherlink.wave7-source-acquisition-receipt",
        "schemaVersion": "1.0",
        ...
        "aggregateZipEntryCount": aggregate_zip_entries,
        "aggregateZipUncompressedBytes": aggregate_zip_uncompressed,
        "dispatchBoundaryCount": 30,
        "responseCommittedCount": 30,
        "responseCommittedBytes": aggregate,
        "validationCommittedCount": 30,
        "persistenceCommittedCount": 30,
    ...
    return {
        ...
        "combinedFixedPointV5ContentSha256": PERMIT.EXPECTED_V5_CONTENT,
        "compactIdentitySha256": PERMIT.EXPECTED_COMPACT_IDENTITY,
        "fullWitnessSha256": PERMIT.EXPECTED_FULL_WITNESS,
...
def create_readback_claim(
    ...
    claim = content_bound(
        {
            "documentType": "aetherlink.wave7-acquisition-readback-one-use-claim",
            "schemaVersion": "1.0",
            "status": "consumed_active",
    ...
    try:
        ...
        try:
            ...
            receipt = content_bound(
                {
                    "documentType": "aetherlink.wave7-source-acquisition-readback",
                    "schemaVersion": "1.0",
                    "status": (
                        "wave7_acquisition_retained_snapshot_"
            ...
            manifest = content_bound(
                {
                    "documentType": (
                        "aetherlink.wave7-source-acquisition-readback-manifest"
                    ),
                    "schemaVersion": "1.0",
                    "status": (
                        "wave7_acquisition_retained_snapshot_"
...
def main(argv: Sequence[str] | None = None) -> int:
    try:
        ...
        if args.preflight:
            result = {
                "documentType": "aetherlink.wave7-acquisition-readback-preflight",
                "schemaVersion": "1.0",
        ...
        sys.stdout.buffer.write(
            canonical_bytes(
                {
                    "documentType": "aetherlink.wave7-acquisition-readback-error",
                    "schemaVersion": "1.0",
                    "status": status,
