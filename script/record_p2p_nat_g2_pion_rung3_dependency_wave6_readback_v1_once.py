#!/usr/bin/env python3
"""Record one independent offline readback of the Wave6 v1 acquisition."""
...
if not (
    ...
    raise RuntimeError("Wave6 readback recorder requires `python3 -I -B -S`")
...
if O_NOFOLLOW is None:
    raise RuntimeError("Wave6 readback recorder requires O_NOFOLLOW")
...
def load_readback_checker() -> types.ModuleType:
    ...
    module = types.ModuleType("wave6_acquisition_readback_permit_v1")
...
def verify_snapshot(snapshot: FrozenSnapshot) -> dict[str, Any]:
    ...
    permit_path = (
        PERMIT.BASE
        + "/bounded-dependency-source-acquisition-wave6-execution-permit-v1.json"
    )
    ...
    claim_expected = {
        "documentType": "aetherlink.wave6-source-acquisition-claim",
        "schemaVersion": "1.0",
    ...
    require(
        len(resources) == 36
        and [row["requestOrdinal"] for row in resources] == list(range(1, 37))
        and [row["kind"] for row in resources] == ["mod", "zip"] * 18
    ...
    require(
        len(mod_by_tuple) == 18
        and aggregate_mod == 2_220
        and aggregate_zip == 36_113_195
        and aggregate == 36_115_415
        and aggregate_zip_entries == 7_758
        and aggregate_zip_uncompressed == 138_523_078
    ...
    evidence_expected = {
        "documentType": "aetherlink.wave6-source-acquisition-evidence",
        "schemaVersion": "1.0",
        "attemptId": PERMIT.ATTEMPT_ID,
        "requestCount": 36,
    ...
    receipt_expected = {
        "documentType": "aetherlink.wave6-source-acquisition-receipt",
        "schemaVersion": "1.0",
        ...
        "aggregateZipEntryCount": aggregate_zip_entries,
        "aggregateZipUncompressedBytes": aggregate_zip_uncompressed,
        "dispatchBoundaryCount": 36,
        "responseCommittedCount": 36,
        "responseCommittedBytes": aggregate,
        "validationCommittedCount": 36,
        "persistenceCommittedCount": 36,
    ...
    try:
        ...
        try:
            ...
            receipt = content_bound(
                {
                    "documentType": "aetherlink.wave6-source-acquisition-readback",
                    "schemaVersion": "1.0",
                    "status": (
                        "wave6_acquisition_retained_snapshot_"
            ...
            manifest = content_bound(
                {
                    "documentType": (
                        "aetherlink.wave6-source-acquisition-readback-manifest"
                    ),
                    "schemaVersion": "1.0",
                    "status": (
                        "wave6_acquisition_retained_snapshot_"
...
def main(argv: Sequence[str] | None = None) -> int:
    try:
        ...
        if args.preflight:
            result = {
                "documentType": "aetherlink.wave6-acquisition-readback-preflight",
                "schemaVersion": "1.0",
        ...
        sys.stdout.buffer.write(
            canonical_bytes(
                {
                    "documentType": "aetherlink.wave6-acquisition-readback-error",
                    "schemaVersion": "1.0",
                    "status": status,
