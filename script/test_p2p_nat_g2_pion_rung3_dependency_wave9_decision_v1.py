#!/usr/bin/env python3
"""Focused adversarial tests for the read-only Wave9 decision checker."""
...
if not (
    ...
    raise RuntimeError("Wave9 decision tests require `python3 -I -B -S`")
...
DECISION_PATH = (
    ...
    "bounded-dependency-source-identity-and-acquisition-decision-wave9-v1.json"
)
READER_PATH = (
    "docs/security-hardening/production-p2p-nat-v1/"
    "g2-pion-restricted-fork-v1/rung-three/"
    "bounded-dependency-source-identity-and-acquisition-decision-wave9-v1.md"
)
...
def load_checker() -> types.ModuleType:
    ...
    raw = path.read_bytes()
    module = types.ModuleType("wave9_decision_checker_test_subject")
    module.__dict__.update(
        {
            "__cached__": None,
            "__file__": str(path),
            "__loader__": None,
            "__name__": "wave9_decision_checker_test_subject",
            "__package__": None,
        }
    )
    exec(
...
class UnitBoundaryTests(unittest.TestCase):
    ...
    def test_07_frontier_and_request_order_are_exact(self) -> None:
        ...
        self.assertEqual(
            ...
            CHECKER.V7_FRONTIER_SHA256,
        )
        self.assertNotEqual(
            CHECKER.V7_INPUT_SET_SHA256,
            CHECKER.V7_SOURCE_BINDINGS_SHA256,
        )
        self.assertEqual(
            CHECKER.EXPECTED_IDENTITY[1][-3:],
            (2, 2, 2),
        )
        self.assertEqual(
            CHECKER.EXPECTED_IDENTITY[2][0:2],
            ("golang.org/x/net", "v0.44.0"),
        )
        self.assertEqual(
            CHECKER.EXPECTED_IDENTITY[3][0:2],
            ("golang.org/x/net", "v0.45.0"),
        )
        self.assertEqual(
            CHECKER.EXPECTED_IDENTITY[2][2],
            CHECKER.EXPECTED_IDENTITY[3][2],
        )
        self.assertNotEqual(
            CHECKER.EXPECTED_IDENTITY[2][3],
            CHECKER.EXPECTED_IDENTITY[3][3],
        )
        rows = [
            {
                "tupleOrder": order,
...
class HardeningBoundaryTests(unittest.TestCase):
    ...
    def test_wave9_namespace_rejects_stale_and_portable_aliases(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            try:
                CHECKER.validate_wave9_namespace_absent(*fds)
                cases = (
                    (dependency, ".WAVE-9-v1.claim"),
                    (
                        docs,
                        "bounded-dependency-source-acquisition-"
                        "wave9-execution-permit-v1.json",
                    ),
                    (
                        scripts,
                        "check_p2p_nat_g2_pion_rung3_dependency_"
                        "wave9_acquisition_v1.py",
                    ),
                )
        ...
        with mock.patch.object(
            ...
            with self.assertRaises(
                ...
                CHECKER.validate_wave9_namespace_absent(1, 2, 3)
        self.assertEqual(caught.exception.code, "E_WAVE9_NAMESPACE")
...
class LiveRepositoryTests(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        cls.package_raw = {
            ...
            CHECKER.V7_TESTS_PATH:
                (ROOT / CHECKER.V7_TESTS_PATH).read_bytes(),
        }
    ...
    def test_08_self_reader_tests_and_v7_pins(self) -> None:
        self.assertEqual(
            sha256(
        ...
        self.assertEqual(
            sha256(self.package_raw[CHECKER.V7_CHECKER_PATH]),
            CHECKER.V7_CHECKER_RAW_SHA256,
        )
        self.assertEqual(
            sha256(self.package_raw[CHECKER.V7_TESTS_PATH]),
            CHECKER.V7_TESTS_RAW_SHA256,
        )
    ...
    def test_10_authority_scope_and_counters_are_closed(self) -> None:
        ...
        self.assertEqual(
            lineage,
            {
                ...
                "contentSha256":
                    CHECKER.V7_CANDIDATE_CONTENT_SHA256,
