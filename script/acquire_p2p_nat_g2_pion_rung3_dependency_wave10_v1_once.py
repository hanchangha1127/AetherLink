#!/usr/bin/env python3
"""Consume the Wave10 v1 permit once and acquire 22 verified resources."""
...
if not (
    ...
    raise RuntimeError("Wave10 acquisition requires `python3 -I -B -S`")
...
EXPECTED_CHECKER_RAW = "fe39b88609bbca78461bf7db416cb311143371d68ebab176703bbc2c7a81eaec"
...
class ImmutablePhaseLedger:
    ...
    def source_acquisition_state(self) -> str:
        ...
        if self.response_committed_count == 22:
...
class ExecutionNamespace:
    ...
    def barrier(self, state: ExecutionState) -> None:
        ...
        if state in {
            ...
            require(
                len(self.resources) == 22 and self.evidence is not None,
                "E_CARDINALITY",
                phase,
            )
...
def _validate_resource_contract(
    ...
    require(
        ...
        and permit["requestContract"]["requestCount"] == 22
        and len(rows) == 22
    ...
    for index, source in enumerate(rows):
        ...
        require(
            ...
            and row["tupleId"] == f"wave10-{tuple_order:03d}-{digest[:12]}"
            and row["kind"] == kind
            and row["method"] == "GET"
    ...
    require(
        len({row["acceptedFileName"] for row in result}) == 22
        and len({row["url"] for row in result}) == 22
        and [
            row["requestOrdinal"]
            for row in result
...
def _attempt(
    ...
    *,
    ...
    try:
        ...
        failure = {
            "documentType": "aetherlink.wave10-source-acquisition-failure",
            "schemaVersion": "1.0",
...
def validate_execution_context() -> dict[str, Any]:
    ...
    return {
        "documentType":
            "aetherlink.wave10-source-acquisition-execution-context-check",
        "schemaVersion": "1.0",
...
def error_document(error: AcquisitionError) -> dict[str, Any]:
    ...
    return {
        "documentType": "aetherlink.wave10-source-acquisition-error",
        "schemaVersion": "1.0",
        "status": status,
