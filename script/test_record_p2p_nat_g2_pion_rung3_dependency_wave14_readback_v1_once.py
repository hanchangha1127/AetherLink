#!/usr/bin/env python3
"""Tests for the one-use offline Wave14 acquisition readback recorder."""
...
PATH = Path(__file__).with_name(
    "record_p2p_nat_g2_pion_rung3_dependency_wave14_readback_v1_once.py"
)
# The recorder deliberately retains an all-zero reverse pin until the separate
# Wave14 readback checker/permit bytes are frozen. This suite remains sealed
# until that pin is replaced with the finalized checker raw SHA-256.
SPEC = importlib.util.spec_from_file_location("wave14_readback_recorder_tests", PATH)
...
class Wave14ReadbackRecorderTests(unittest.TestCase):
    ...
    def test_05_live_retained_snapshot_verifies_twice_read_only(self):
        ...
        try:
            ...
            self.assertEqual(
                ...
                "7fef20e6c3931b698f32b2a71f8a596a",
            )
            ...
            self.assertEqual(first["aggregateModBytes"], 753)
            self.assertEqual(first["aggregateZipBytes"], 15_050_695)
            self.assertEqual(first["aggregateAcceptedBytes"], 15_051_448)
            self.assertEqual(first["aggregateZipEntryCount"], 2_571)
            self.assertEqual(
    ...
    def test_05_selector_counts_and_v12_binding_fail_closed(self):
        snapshot = R.FrozenSnapshot()
        permit_path = (
            R.PERMIT.BASE
    ...
    def test_21_wave14_uncompressed_limits_are_per_zip_and_across_all(self):
        self.assertEqual(
