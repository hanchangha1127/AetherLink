#!/usr/bin/env python3
"""Record one independent offline readback of the Wave17 v1 acquisition."""
...
if not (
    ...
    raise RuntimeError("Wave17 readback recorder requires `python3 -I -B -S`")
...
if O_NOFOLLOW is None:
    raise RuntimeError("Wave17 readback recorder requires O_NOFOLLOW")
...
EXPECTED_READBACK_CHECKER_RAW = "f9ceb4756e2ef7cf3c13cd5c3b21399d1f5f44aef3f0c8781ba6dfa67d11bb3f"
...
MAX_ZIP_UNCOMPRESSED_BYTES_ACROSS_ALL = 128 * 1024 * 1024
MAX_ZIP_FILES_ACROSS_ALL = 20_000
...
def load_readback_checker() -> types.ModuleType:
    ...
    module = types.ModuleType("wave17_acquisition_readback_permit_v1")
...
def verify_snapshot(snapshot: FrozenSnapshot) -> dict[str, Any]:
    ...
    v15 = acquisition_permit["predecessorBindings"]["combinedFixedPointV15"]
    ...
    require(
        ...
        and acquisition_permit["requestContract"]["tupleCount"] == 1
        and acquisition_permit["requestContract"]["requestCount"] == 2
        ...
        == {
            ...
            "checkerNormalizedSha256": (
                PERMIT.EXPECTED_V15_CHECKER_NORMALIZED
            ),
            "testsPath": (
            ...
            "contentSha256": PERMIT.EXPECTED_V15_CONTENT,
            "combinedInputSetSha256": PERMIT.EXPECTED_V15_INPUT_SET,
            "graphSha256": PERMIT.EXPECTED_V15_GRAPH,
            "frontierSha256": PERMIT.EXPECTED_V15_FRONTIER,
    ...
    require(
        len(resources) == 2
        and [row["requestOrdinal"] for row in resources] == [1, 2]
        and [row["kind"] for row in resources] == ["mod", "zip"]
    ...
    require(
        len(mod_by_tuple) == 1
        and aggregate_mod == 301
        and aggregate_zip == 3_450_399
        and aggregate == 3_450_700
        and aggregate_zip_entries == 1_550
        and aggregate_zip_uncompressed == 9_108_004
    ...
    evidence_expected = {
        "documentType": "aetherlink.wave17-source-acquisition-evidence",
        "schemaVersion": "1.0",
        "attemptId": PERMIT.ATTEMPT_ID,
        "requestCount": 2,
    ...
    receipt_expected = {
        "documentType": "aetherlink.wave17-source-acquisition-receipt",
        "schemaVersion": "1.0",
        ...
        "aggregateZipEntryCount": aggregate_zip_entries,
        "aggregateZipUncompressedBytes": aggregate_zip_uncompressed,
        "dispatchBoundaryCount": 2,
        "responseCommittedCount": 2,
        "responseCommittedBytes": aggregate,
        "validationCommittedCount": 2,
        "persistenceCommittedCount": 2,
    ...
    return {
        ...
        "combinedFixedPointV15ContentSha256": PERMIT.EXPECTED_V15_CONTENT,
        "compactIdentitySha256": PERMIT.EXPECTED_COMPACT_IDENTITY,
        "fullWitnessSha256": PERMIT.EXPECTED_FULL_WITNESS,
...
def create_readback_claim(
    ...
    claim = content_bound(
        {
            "documentType": "aetherlink.wave17-acquisition-readback-one-use-claim",
            "schemaVersion": "1.0",
            "status": "consumed_active",
    ...
    try:
        ...
        try:
            ...
            receipt = content_bound(
                {
                    "documentType": "aetherlink.wave17-source-acquisition-readback",
                    "schemaVersion": "1.0",
                    "status": (
                        "wave17_acquisition_retained_snapshot_"
            ...
            manifest = content_bound(
                {
                    "documentType": (
                        "aetherlink.wave17-source-acquisition-readback-manifest"
                    ),
                    "schemaVersion": "1.0",
                    "status": (
                        "wave17_acquisition_retained_snapshot_"
...
def _post_success_reporting_failure() -> dict[str, Any]:
    return {
        "documentType": "aetherlink.wave17-acquisition-readback-error",
        "schemaVersion": "1.0",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if args.preflight:
            preflight()
            result = {
                "documentType": "aetherlink.wave17-acquisition-readback-preflight",
                "schemaVersion": "1.0",
