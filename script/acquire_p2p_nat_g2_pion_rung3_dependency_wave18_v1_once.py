#!/usr/bin/env python3
"""Consume the Wave18 v1 permit once and acquire 6 verified resources."""
...
if not (
    ...
    raise RuntimeError("Wave18 acquisition requires `python3 -I -B -S`")
...
CHECKER_PATH = Path(__file__).with_name(
    "check_p2p_nat_g2_pion_rung3_dependency_wave18_acquisition_v1.py"
)
# Finalized reverse pin to the exact Wave18 acquisition checker bytes. The
...
EXPECTED_CHECKER_RAW = "c454de5191c7a9de4bccaa4c8aa00f914e74e2c9bb4c4bf59164b827a0c6bfc5"
EXPECTED_WAVE18_IDENTITY = (
    (
        "golang.org/x/mod",
        "v0.24.0",
...
EXPECTED_WAVE18_RESOURCE_CONTRACT_SHA256 = (
...
def _exact_int_binding(actual: object, expected: object) -> bool:
    return (
        type(actual) is int
...
def _int_floor_binding(actual: object, floor: object) -> bool:
    return (
        type(actual) is int
...
def _assert_primitive_contract() -> None:
    require(
        ...
        and _exact_int_binding(
            WAVE4.CHECK.MAX_MOD_BYTES,
            CHECK.MAX_MOD_BYTES,
        )
        and _exact_int_binding(
            VALIDATION.CHECK.MAX_MOD_BYTES,
            CHECK.MAX_MOD_BYTES,
        )
        and _exact_int_binding(
            WAVE4.CHECK.MAX_ZIP_BYTES,
            CHECK.MAX_ZIP_BYTES,
        )
        and _exact_int_binding(
            VALIDATION.CHECK.MAX_ZIP_BYTES,
            CHECK.MAX_ZIP_BYTES,
        )
        and _exact_int_binding(
            WAVE4.CHECK.MAX_ZIP_NAME_BYTES,
            CHECK.MAX_ZIP_NAME_BYTES,
        )
        and _exact_int_binding(
            VALIDATION.CHECK.MAX_ZIP_NAME_BYTES,
            CHECK.MAX_ZIP_NAME_BYTES,
        )
        and _exact_int_binding(
            WAVE4.CHECK.MAX_ZIP_FILES,
            CHECK.MAX_ZIP_FILES,
        )
        and _exact_int_binding(
            VALIDATION.CHECK.MAX_ZIP_FILES,
            CHECK.MAX_ZIP_FILES,
        )
        and _exact_int_binding(
            WAVE4.CHECK.MAX_ZIP_UNCOMPRESSED_BYTES,
            CHECK.MAX_ZIP_UNCOMPRESSED_BYTES,
        )
        and _exact_int_binding(
            VALIDATION.CHECK.MAX_ZIP_UNCOMPRESSED_BYTES,
            CHECK.MAX_ZIP_UNCOMPRESSED_BYTES,
        )
        and _exact_int_binding(
            WAVE4.CHECK.MAX_ZIP_FILE_BYTES,
            CHECK.MAX_ZIP_FILE_BYTES,
        )
        and _exact_int_binding(
            VALIDATION.CHECK.MAX_ZIP_FILE_BYTES,
            CHECK.MAX_ZIP_FILE_BYTES,
        )
        and _exact_int_binding(
            WAVE4.CHECK.MAX_AGGREGATE_BYTES,
            VALIDATION.CHECK.MAX_AGGREGATE_BYTES,
        )
        and _int_floor_binding(
            WAVE4.CHECK.MAX_AGGREGATE_BYTES,
            CHECK.MAX_AGGREGATE_BYTES,
        )
        and _exact_int_binding(
            WAVE4.CHECK.MAX_HEADER_BYTES,
            CHECK.MAX_HEADER_BYTES,
        )
        and _exact_int_binding(
            VALIDATION.CHECK.MAX_HEADER_BYTES,
            CHECK.MAX_HEADER_BYTES,
        )
        and _exact_int_binding(
            WAVE4.CHECK.PER_REQUEST_DEADLINE_MS,
            CHECK.PER_REQUEST_DEADLINE_MS,
        )
        and _exact_int_binding(
            VALIDATION.CHECK.PER_REQUEST_DEADLINE_MS,
            CHECK.PER_REQUEST_DEADLINE_MS,
        )
        and _exact_int_binding(
            WAVE4.CHECK.WHOLE_ATTEMPT_DEADLINE_MS,
            CHECK.WHOLE_ATTEMPT_DEADLINE_MS,
        )
        and _exact_int_binding(
            VALIDATION.CHECK.WHOLE_ATTEMPT_DEADLINE_MS,
            CHECK.WHOLE_ATTEMPT_DEADLINE_MS,
        )
...
class ImmutablePhaseLedger:
    ...
    def source_acquisition_state(self) -> str:
        ...
        if self.response_committed_count == 6:
...
class ExecutionNamespace:
    ...
    def barrier(self, state: ExecutionState) -> None:
        ...
        readback_claim = _portable(
            Path(CHECK.READBACK_CLAIM_PATH).name
        )
        readback_temp = _portable(CHECK.READBACK_TEMP_PREFIX)
        staging_names = [
            name for name in dependency_names if name.startswith(staging)
        ]
        require(
            readback_claim not in dependency_names
            and not any(
                name.startswith(readback_temp)
        ...
        if state in {
            ...
            require(
                len(self.resources) == 6 and self.evidence is not None,
                "E_CARDINALITY",
                phase,
            )
...
def preflight() -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    require(
        ...
        and type(summary["requestCount"]) is int
        and summary["requestCount"] == 6
...
def _validate_resource_contract(
    ...
    require(
        ...
        and permit["requestContract"]["requestCount"] == 6
        and type(permit["requestContract"]["tupleCount"]) is int
        and permit["requestContract"]["tupleCount"] == 3
        and len(rows) == 6
        and sha256(canonical_bytes(rows))
        == permit["requestContract"]["resourcesCanonicalSha256"]
        == EXPECTED_WAVE18_RESOURCE_CONTRACT_SHA256
        and permit["requestContract"][
            "decisionToPermitTypedProjectionCanonicalSha256"
        ]
        == CHECK.DECISION_TO_PERMIT_PROJECTION_SHA256,
        "E_RESOURCES",
        "preflight",
    )
    ...
    for index, source in enumerate(rows):
        ...
        require(
            ...
            and row["tupleId"] == f"wave18-{tuple_order:03d}-{digest[:12]}"
            and row["kind"] == kind
            and row["method"] == "GET"
    ...
    require(
        len({row["acceptedFileName"] for row in result}) == 6
        and len({row["url"] for row in result}) == 6
        and all(
        ...
        and tuple(
            (
                ...
                result[index]["expectedH1"],
                result[index + 1]["expectedH1"],
            )
            for index in range(0, 6, 2)
        )
        == EXPECTED_WAVE18_IDENTITY,
    ...
    try:
        ...
        for ordinal, resource in enumerate(rows, 1):
            ...
            if resource["kind"] == "mod":
                ...
                root_go_mod_present = verified.get("rootGoModPresent")
                require(
                    type(root_go_mod_present) is bool
                    and root_go_mod_present is True,
                    "E_ZIP_ROOT_GO_MOD",
                    "zip",
                )
                entry_count = verified.get("entryCount")
                uncompressed_bytes = verified.get("uncompressedBytes")
                require(
                    type(entry_count) is int
                    and 1 <= entry_count <= CHECK.MAX_ZIP_FILES
                ...
                all_zip_entries += entry_count
                all_zip_uncompressed += uncompressed_bytes
                require(
        ...
        evidence_payload = {
            "documentType": "aetherlink.wave18-source-acquisition-evidence",
            "schemaVersion": "1.0",
            "attemptId": attempt_id,
            "requestCount": expected_count,
        ...
        receipt = {
            "documentType": "aetherlink.wave18-source-acquisition-receipt",
            "schemaVersion": "1.0",
        ...
        failure = {
            "documentType": "aetherlink.wave18-source-acquisition-failure",
            "schemaVersion": "1.0",
...
def validate_execution_context() -> dict[str, Any]:
    ...
    return {
        "documentType":
            "aetherlink.wave18-source-acquisition-execution-context-check",
        "schemaVersion": "1.0",
...
def error_document(error: AcquisitionError) -> dict[str, Any]:
    ...
    return {
        "documentType": "aetherlink.wave18-source-acquisition-error",
        "schemaVersion": "1.0",
        "status": status,
