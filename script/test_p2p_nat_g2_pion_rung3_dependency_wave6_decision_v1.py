#!/usr/bin/env python3
"""Adversarial tests for the offline Wave6 identity decision checker."""
...
if not (
    ...
    raise RuntimeError("Wave6 decision tests require `python3 -I -B -S`")
...
READER_PATH = (
    ...
    "bounded-dependency-source-identity-and-acquisition-decision-wave6-v1.md"
)
DECISION_PATH = (
    "docs/security-hardening/production-p2p-nat-v1/"
    "g2-pion-restricted-fork-v1/rung-three/"
    "bounded-dependency-source-identity-and-acquisition-decision-wave6-v1.json"
)
...
def load_checker(raw: bytes) -> types.ModuleType:
    module = types.ModuleType("wave6_decision_checker_test_subject")
    module.__dict__.update(
        {
            "__cached__": None,
            "__file__": str(ROOT / CHECKER_PATH),
            "__loader__": None,
            "__name__": "wave6_decision_checker_test_subject",
            "__package__": None,
        }
    )
    code = compile(
...
def load_wave6_candidate_checker() -> types.ModuleType:
    path = CHECKER.WAVE6_CHECKER_PATH
    raw = (ROOT / path).read_bytes()
    if sha256(raw) != CHECKER.WAVE6_CHECKER_RAW_SHA256:
        raise RuntimeError("Wave6 candidate checker preflight failed")
    module = types.ModuleType("wave6_candidate_checker_test_subject")
    module.__dict__.update(
        {
            "__cached__": None,
            "__file__": str(ROOT / path),
            "__loader__": None,
            "__name__": "wave6_candidate_checker_test_subject",
            "__package__": None,
        }
    )
...
SEMANTIC_PACKAGE_RAW = {
    CHECKER.THIS_CHECKER_PATH: CHECKER_RAW,
    CHECKER.THIS_TESTS_PATH: (ROOT / CHECKER.THIS_TESTS_PATH).read_bytes(),
}
...
class UnitBoundaryTests(unittest.TestCase):
    ...
    def test_held_namespace_rejects_late_collision_and_root_rebind(
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with CHECKER.HeldNamespace(root) as held:
                claim = dependency / Path(CHECKER.WAVE6_CLAIM_PATH).name
                claim.write_bytes(b"late")
                with self.assertRaises(CHECKER.DecisionFailure):
                    held.final_barrier()
                claim.unlink()
                staging = dependency / (
                    CHECKER.WAVE6_STAGING_PREFIX + "late"
                )
                staging.mkdir()
                with self.assertRaises(CHECKER.DecisionFailure):
                    held.final_barrier()
                staging.rmdir()
                final = dependency / CHECKER.WAVE6_FINAL_NAME
                final.mkdir()
                with self.assertRaises(CHECKER.DecisionFailure):
                    held.final_barrier()
                final.rmdir()
    ...
    def test_wave6_candidate_content_is_reconstructed_not_copied(
        self,
    ) -> None:
        wave6 = load_wave6_candidate_checker()
        rows = wave6.wave6_rows(wave6.expected_frontier_rows())
        combined = {
            "contentBinding": {
                "sha256": CHECKER.COMBINED_V4_CONTENT_SHA256,
            }
        }
        projected = CHECKER.reconstruct_wave6_candidate(
            wave6=wave6,
        ...
        self.assertEqual(
            ...
            CHECKER.WAVE6_CANDIDATE_CONTENT_SHA256,
        )
        counters = projected["operationCounters"]
        self.assertEqual(
            (
                counters["predecessorFullSourceReconstructionCount"],
                counters["directV4FullSourceReconstructionCount"],
        ...
        self.assertEqual(
            (
                counters["predecessorArchiveOpenCount"],
                counters["directV4ArchiveOpenCount"],
        ...
        wave6.V4_TOTAL_RECONSTRUCTION_COUNT = 2
        wave6.V4_TOTAL_ARCHIVE_OPEN_COUNT = 164
        try:
            with self.assertRaises(CHECKER.DecisionFailure):
                CHECKER.reconstruct_wave6_candidate(
                    wave6=wave6,
                    combined_candidate=combined,
                    wave_rows=rows,
                )
        ...
        original = CHECKER.WAVE6_CANDIDATE_CONTENT_SHA256
        CHECKER.WAVE6_CANDIDATE_CONTENT_SHA256 = "0" * 64
        try:
            with self.assertRaises(CHECKER.DecisionFailure):
                CHECKER.reconstruct_wave6_candidate(
                    wave6=wave6,
            ...
            CHECKER.WAVE6_CANDIDATE_CONTENT_SHA256 = original
...
class LiveRepositoryTests(unittest.TestCase):
    ...
    def test_expected_bytes_and_materialized_decision(self) -> None:
        ...
        self.assertEqual(
            parsed["heldSourceInputSet"]["sourceBindingCount"],
            163,
        )
        self.assertEqual(
            parsed["heldSourceInputSet"]["sourceBindingsSha256"],
            CHECKER.COMBINED_SOURCE_BINDINGS_SHA256,
        )
        self.assertEqual(
            (
                parsed["heldSourceInputSet"]["archiveCount"],
                parsed["heldSourceInputSet"]["externalModCount"],
                parsed["heldSourceInputSet"]["embeddedRootGoModCount"],
            ),
            (82, 81, 1),
        )
        self.assertEqual(
            (
                parsed["identityResolution"]["graphSelectedTupleCount"],
                parsed["identityResolution"][
                    "versionSpecificNonSelectedTupleCount"
                ],
                parsed["identityResolution"]["parentDeclarationCount"],
                parsed["identityResolution"][
                    "moduleZipH1WitnessCount"
                ],
                parsed["identityResolution"]["goModH1WitnessCount"],
            ),
            (0, 18, 18, 18, 25),
        )
        self.assertEqual(
            (
                parsed["operationCounters"][
                ...
                parsed["operationCounters"][
                    "predecessorV3GraphArchiveOpenCount"
                ],
                parsed["operationCounters"]["currentV4GraphArchiveOpenCount"],
                parsed["operationCounters"]["totalV4GraphArchiveOpenCount"],
                parsed["operationCounters"]["identityWitnessScanCount"],
                parsed["operationCounters"][
                    "identityWitnessArchiveOpenCount"
                ],
                parsed["operationCounters"][
                    "overallDecisionExecutionArchiveOpenCount"
                ],
            ),
            (4, 2, 6, 236, 164, 400, 2, 164, 564),
        )
        self.assertTrue(
        ...
        self.assertTrue(
            parsed["authority"]
            and all(value is False for value in parsed["authority"].values())
        )
        self.assertEqual(
            parsed["authority"],
            CHECKER.EXPECTED_DECISION_AUTHORITY,
        )
        self.assertEqual(len(parsed["authority"]), 22)
        identity_by_order = {
            row["tupleOrder"]: row
            for row in parsed["identityResolution"]["tuples"]
        }
        identities = list(identity_by_order.values())
        ...
        self.assertEqual(len({row["url"] for row in requests}), 36)
        self.assertEqual(
        ...
        self.assertEqual(len(x_sys), 2)
        self.assertEqual(
            [(row["version"], row["goModH1"]) for row in x_sys],
            [
                ("v0.12.0", x_sys[0]["goModH1"]),
                ("v0.5.0", x_sys[1]["goModH1"]),
            ],
        )
        x_sys_mod_requests = [
            row
            ...
            if row["module"] == "golang.org/x/sys"
            and row["version"] in {"v0.12.0", "v0.5.0"}
            and row["resourceKind"] == "mod"
        ]
        self.assertEqual(len(x_sys_mod_requests), 2)
        self.assertEqual(
            len({row["url"] for row in x_sys_mod_requests}),
            2,
        )
        self.assertEqual(
            len(
                {
                    ...
                    for row in x_sys_mod_requests
                }
            ),
            2,
        )

    def test_semantic_unknown_authority_order_selector_frontier_h1_counter_content_and_lineage_mutations_fail_closed(
        self,
    ) -> None:
        raw = (ROOT / DECISION_PATH).read_bytes()
        ...
        def expect_rejected(mutate, code: str) -> None:
        ...
        mutations = (
            ...
            (
                lambda value: value["nonClaims"].append(
                    "unexpected non-claim"
                ),
                "E_DECISION_BINDINGS",
            ),
            (
            ...
            (
                lambda value: value["identityResolution"]["tuples"][0]
                .__setitem__("selectedByGraphAlgorithm", True),
                "E_DECISION_IDENTITY",
            ),
            (
            ...
            (
                lambda value: value["identityResolution"]["tuples"][0]
                .__setitem__("moduleZipH1", VALID_H1_B),
                "E_DECISION_IDENTITY",
            ),
            (
            ...
            (
                lambda value: value["predecessorBindings"][
                    "wave6Candidate"
                ].__setitem__("contentSha256", "0" * 64),
                "E_DECISION_LINEAGE",
            ),
        )
        for mutate, code in mutations:
            expect_rejected(mutate, code)

        bad_content = copy.deepcopy(expected)
        bad_content["contentBinding"]["sha256"] = "0" * 64
        with self.assertRaisesRegex(
            CHECKER.DecisionFailure,
            "^E_DECISION_CONTENT$",
        ):
    ...
    def test_normal_checker_summary(self) -> None:
        ...
        self.assertEqual(
            summary["status"],
            "validated_18_of_18_acquisition_ready_not_authorized",
        )
        self.assertTrue(summary["validationPassed"])
        self.assertEqual(summary["completeIdentityPairCount"], 18)
