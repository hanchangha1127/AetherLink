#!/usr/bin/env python3
"""Shared fail-closed primitives for the G2 Pion dependency wave-two v2 tools.
...
CLAIM_NAME = ".wave-2-v2.claim"
STAGING_PREFIX = ".wave-2-v2-staging-"
WAVE_PARENT_NAME = "wave-2-v2"
FINAL_DIRECTORY_NAME = "accepted"
FINAL_DIRECTORY_PATH = (
    "build/offline-source/pion-ice-v4.3.0/dependencies/wave-2-v2/accepted"
)
DECISION_FINAL_DIRECTORY_PATH = (
    "build/offline-source/pion-ice-v4.3.0/dependencies/wave-2-v1/accepted"
)
EXPECTED_V2_ORDERED_RESOURCE_SET_SHA256 = (
...
READBACK_MANIFEST_PATH = (
    f"{BASE}/bounded-dependency-source-acquisition-wave2-readback-manifest-v2.json"
)
...
def projected_v2_resources(
    decision: Mapping[str, Any],
) -> list[dict[str, Any]]:
    """Project only the decision's v1 output paths into the v2 namespace."""
...
def v2_ordered_resource_set_sha256(
    decision: Mapping[str, Any],
) -> str:
    digest = sha256_bytes(
        canonical_json_bytes(projected_v2_resources(decision))
    )
    require(
        digest == EXPECTED_V2_ORDERED_RESOURCE_SET_SHA256,
        "E_WAVE_RESOURCES",
    )
    return digest
