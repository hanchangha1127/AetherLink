#!/usr/bin/env python3
"""Recompute the bounded Wave1+Wave2+Wave3+Wave4 graph without publishing.
...
133 source inputs by descriptor.  It writes only one canonical candidate to
...
def require_isolated_interpreter() -> None:
    ...
    if not (
        ...
        raise RuntimeError(
            "combined fixed-point v3 checker requires unoptimized "
            "`python3 -I -B -S`"
        )
...
SELF_PATH = "script/check_p2p_nat_g2_pion_combined_fixed_point_v3.py"
...
CHECKER_ID = (
    "g2-pion-ice-v4.3.0-combined-wave1-wave2-wave3-wave4-check-v3"
)
...
def load_v2_checker(held: PinnedCodeFile) -> types.ModuleType:
    ...
    try:
        ...
        raise CombinedCheckFailure("E_V2_CHECKER_LOAD") from error
    check(
...
def validate_v2_predecessor_candidate(
    ...
    check(
        ...
        == {
            ...
            "sha256": WAVE4_PREDECESSOR_CONTENT_SHA256,
        }
        ...
        and graph.get("graphSha256") == WAVE4_PREDECESSOR_GRAPH_SHA256
    ...
    return {
        ...
        "candidateContentSha256": WAVE4_PREDECESSOR_CONTENT_SHA256,
        "combinedInputSetSha256": WAVE4_PREDECESSOR_INPUT_SET_SHA256,
        "graphSha256": WAVE4_PREDECESSOR_GRAPH_SHA256,
        "frontierSha256": WAVE4_PREDECESSOR_FRONTIER_SHA256,
...
def combined_source_bindings(
    ...
    check(
        len({row["path"] for row in bindings}) == 133
        and len(
            {
    ...
    check(
        ...
        and sorted(order for _, _, order in pair_kinds) == list(range(1, 67)),
        "E_COMBINED_INPUT",
    )
    return bindings
...
def generate_candidate(root: Path = ROOT) -> dict[str, Any]:
    ...
    with (
        ...
        v2 = load_v2_checker(v2_held)
        v2_candidate = v2.generate_candidate(root)
        with v1.PinnedRunnerFile(root) as provider_held:
            runner = v1.load_pinned_runner(provider_held)
            predecessor_verification = validate_v2_predecessor_candidate(
                runner,
                v2_candidate,
            )
            controls = (
                ...
                with runner.HeldInputSet(root, bindings) as source_held:
                    ...
                    body = {
                        ...
                        "result": (
                            ...
                            "wave1_wave2_wave3_and_wave4_source_bytes"
                        ),
                        ...
                        "toolBindings": [
                            ...
                            {
                                "role": "immutable_v2_combined_checker_predecessor",
