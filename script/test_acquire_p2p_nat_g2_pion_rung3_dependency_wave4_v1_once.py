#!/usr/bin/env python3
"""Adversarial offline tests for the one-use Wave4 acquisition runner."""
...
if not (
    ...
    raise RuntimeError("Wave4 runner tests require `python3 -I -B -S`")
...
def fixture() -> tuple[list[dict[str, object]], dict[int, bytes]]:
    ...
    ordinal = 0
    for tuple_order in range(1, 17):
        module = f"example.test/dependency{tuple_order}"
        version = f"v1.0.{tuple_order}"
        digest = runner.sha256(f"{module}\n{version}\n".encode())
        tuple_id = f"wave4-{tuple_order:03d}-{digest[:12]}"
        selected = tuple_order in {1, 3, 11}
        mod = f"module {module}\n\ngo 1.22\n".encode()
        archive = make_zip(
            module,
            version,
        ...
        for kind, body, expected_h1, maximum in (
            (
                "mod",
                mod,
                runner.WAVE3.go_mod_h1(mod),
                runner.CHECK.MAX_MOD_BYTES,
            ),
            (
                "zip",
                archive,
                runner.WAVE3.module_zip_h1(
                    archive,
                    module,
                    version,
                ),
...
class Wave4RunnerTests(unittest.TestCase):
    def run_attempt(
        self,
        root: Path,
        fetch,
    ...
    def test_success_claims_before_32_fetches_and_manifest_is_last(self) -> None:
        rows, bodies = fixture()
    ...
    def test_canonical_preflight_context_composes_without_writes(self) -> None:
        ...
        self.assertEqual(result["requestCount"], 32)
    ...
    def test_semantic_or_cardinality_mutation_fails_before_claim(self) -> None:
        original, _ = fixture()
        mutations = (
            lambda rows: rows.__setitem__(31, dict(rows[30])),
            lambda rows: rows[31].__setitem__(
                "requestOrdinal",
                31,
            ),
