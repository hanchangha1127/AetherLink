#!/usr/bin/env python3
"""Tests for the one-use offline Wave11 acquisition readback recorder."""
...
PATH = Path(__file__).with_name(
    "record_p2p_nat_g2_pion_rung3_dependency_wave11_readback_v1_once.py"
)
SPEC = importlib.util.spec_from_file_location("wave11_readback_recorder_tests", PATH)
...
class Wave11ReadbackRecorderTests(unittest.TestCase):
    ...
    def test_05_live_retained_snapshot_verifies_twice_read_only(self):
        ...
        try:
            ...
            self.assertEqual(first["acceptedResourceCount"], 18)
            ...
            self.assertEqual(first["aggregateAcceptedBytes"], 16_363_894)
            self.assertEqual(first["aggregateZipEntryCount"], 3_329)
            self.assertEqual(
            ...
            self.assertEqual(len(first["resources"]), 18)
        finally:
            snapshot.close()
        self.assertFalse(os.path.lexists(claim))
    ...
    def test_05_selector_counts_and_v9_binding_fail_closed(self):
        snapshot = R.FrozenSnapshot()
        permit_path = (
            R.PERMIT.BASE
    ...
    def test_10_synthetic_execute_orders_claim_two_passes_receipt_manifest(self):
        ...
        def verify(snapshot):
            ...
            return {"acceptedResourceCount": 18}
    ...
    def test_12_receipt_only_gap_is_terminal_uncertainty(self):
        ...
        with mock.patch.object(
            ...
            R,
            "verify_snapshot",
            return_value={"acceptedResourceCount": 18},
        ), mock.patch.object(R, "atomic_publish", side_effect=publish):
            with self.assertRaises(R.ReadbackError) as caught:
                R.execute(
                    Path("/unused"),
    ...
    def test_21_wave11_uncompressed_limits_are_per_zip_and_across_all(self):
        self.assertEqual(
