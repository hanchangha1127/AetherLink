#!/usr/bin/env python3
"""Tests for the one-use offline Wave15 acquisition readback recorder."""
...
PATH = Path(__file__).with_name(
    "record_p2p_nat_g2_pion_rung3_dependency_wave15_readback_v1_once.py"
)
# The recorder deliberately retains an all-zero reverse pin until the separate
# Wave15 readback checker/permit bytes are frozen. This suite remains sealed
# until that pin is replaced with the finalized checker raw SHA-256.
SPEC = importlib.util.spec_from_file_location("wave15_readback_recorder_tests", PATH)
...
class Wave15ReadbackRecorderTests(unittest.TestCase):
    ...
    def test_05_live_retained_snapshot_verifies_twice_read_only(self):
        ...
        try:
            ...
            self.assertEqual(
                ...
                "c5db51cfd9a295b448927cca36d1ea07",
            )
            self.assertEqual(first["acceptedResourceCount"], 10)
            ...
            self.assertEqual(first["aggregateModBytes"], 452)
            self.assertEqual(first["aggregateZipBytes"], 5_064_794)
            self.assertEqual(first["aggregateAcceptedBytes"], 5_065_246)
            self.assertEqual(first["aggregateZipEntryCount"], 1_645)
            self.assertEqual(
    ...
    def test_05_selector_counts_and_v13_binding_fail_closed(self):
        snapshot = R.FrozenSnapshot()
        permit_path = (
            R.PERMIT.BASE
    ...
    def test_10_synthetic_execute_orders_claim_two_passes_receipt_manifest(self):
        ...
        def verify(snapshot):
            ...
            return {"acceptedResourceCount": 10}
    ...
    def test_12_receipt_only_gap_is_terminal_uncertainty(self):
        ...
        with mock.patch.object(
            ...
            R,
            "verify_snapshot",
            return_value={"acceptedResourceCount": 10},
        ), mock.patch.object(R, "atomic_publish", side_effect=publish):
            with self.assertRaises(R.ReadbackError) as caught:
                R.execute(
                    Path("/unused"),
    ...
    def test_21_wave15_uncompressed_limits_are_per_zip_and_across_all(self):
        self.assertEqual(
        ...
        self.assertEqual(R.MAX_AGGREGATE_MOD_BYTES, 5 * 1024 * 1024)
        self.assertEqual(R.MAX_AGGREGATE_ZIP_BYTES, 80 * 1024 * 1024)
        self.assertEqual(R.MAX_AGGREGATE_BYTES, 85 * 1024 * 1024)
        self.assertEqual(R.MAX_ZIP_FILES_ACROSS_ALL, 100_000)
