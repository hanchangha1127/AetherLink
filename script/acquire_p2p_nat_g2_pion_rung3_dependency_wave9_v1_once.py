#!/usr/bin/env python3
"""Consume the Wave9 v1 permit once and acquire 20 verified resources."""
...
if not (
    ...
    raise RuntimeError("Wave9 acquisition requires `python3 -I -B -S`")
...
EXPECTED_CHECKER_RAW = "cffa859723fb1688bea5cf7188885fe43443dd63c59d01c8805dfc61870b69a6"
...
class ImmutablePhaseLedger:
    ...
    def source_acquisition_state(self) -> str:
        ...
        if self.response_committed_count == 20:
...
class ExecutionNamespace:
    ...
    def barrier(self, state: ExecutionState) -> None:
        ...
        if state in {
            ...
            require(
                len(self.resources) == 20 and self.evidence is not None,
                "E_CARDINALITY",
                phase,
            )
...
def _validate_resource_contract(
    ...
    require(
        ...
        and permit["requestContract"]["requestCount"] == 20
        and len(rows) == 20
    ...
    for index, source in enumerate(rows):
        ...
        require(
            ...
            and row["tupleId"] == f"wave9-{tuple_order:03d}-{digest[:12]}"
            and row["kind"] == kind
            and row["method"] == "GET"
    ...
    require(
        len({row["acceptedFileName"] for row in result}) == 20
        and len({row["url"] for row in result}) == 20
        and sum(
...
def _attempt(
    ...
    *,
    ...
    try:
        ...
        failure = {
            "documentType": "aetherlink.wave9-source-acquisition-failure",
            "schemaVersion": "1.0",
...
def validate_execution_context() -> dict[str, Any]:
    ...
    return {
        "documentType":
            "aetherlink.wave9-source-acquisition-execution-context-check",
        "schemaVersion": "1.0",
...
def error_document(error: AcquisitionError) -> dict[str, Any]:
    ...
    return {
        "documentType": "aetherlink.wave9-source-acquisition-error",
        "schemaVersion": "1.0",
        "status": status,
