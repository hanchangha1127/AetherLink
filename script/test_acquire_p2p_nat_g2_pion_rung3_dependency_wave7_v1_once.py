#!/usr/bin/env python3
"""Adversarial offline tests for the one-use Wave7 acquisition runner."""
...
if not (
    ...
    raise RuntimeError("Wave7 runner tests require `python3 -I -B -S`")
...
RUNNER_PATH = Path(__file__).with_name(
    "acquire_p2p_nat_g2_pion_rung3_dependency_wave7_v1_once.py"
)
SPEC = importlib.util.spec_from_file_location(
    "wave7_source_acquirer_v1",
    RUNNER_PATH,
)
...
def fixture() -> tuple[list[dict[str, object]], dict[int, bytes]]:
    ...
    for tuple_order in range(1, 16):
        ...
        tuple_id = f"wave7-{tuple_order:03d}-{digest[:12]}"
        selected = False
        mod = f"module {module}\n\ngo 1.22\n".encode()
        archive = make_zip(
            module,
...
class Wave7RunnerTests(unittest.TestCase):
    def run_attempt(
        self,
        root: Path,
        fetch,
    ...
    def test_canonical_preflight_context_composes_without_writes(self) -> None:
        ...
        x_sys = [
            ...
            and row["version"] in {"v0.37.0", "v0.8.0"}
        ]
        self.assertEqual(
            {
                row["expectedH1"]
    ...
    def test_wave7_direct_fetch_reaches_wave4_and_translates_errors(
        self,
    ) -> None:
        resource = fixture()[0][0]
