#!/usr/bin/env python3
"""Independently read back and record wave-two v3 acquisition success."""
...
PERMIT_CHECKER_BOOTSTRAP = execute_module(
    "g2_wave2_readback_permit_checker_bootstrap",
    PERMIT_CHECKER_PATH,
    bootstrap_read(
        PERMIT_CHECKER_PATH,
        EXPECTED_PERMIT_CHECKER_RAW_SHA256,
...
def path_present(root: Path, relative: str) -> bool:
    try:
...
def require_success_namespace_exclusions(root: Path) -> None:
    if path_present(root, COMMON.FAILURE_RECEIPT_PATH):
        raise COMMON.Wave2Failure(
            "E_READBACK_NAMESPACE",
            "readback",
        )
...
def require_readback_publication_namespace(root: Path) -> None:
    require_success_namespace_exclusions(root)
    if any(
        path_present(root, relative)
        for relative in (
...
def load_authority() -> dict[str, Any]:
    ...
    try:
        ...
        if (
            ...
            != COMMON.v3_ordered_resource_set_sha256(decision)
            or receipt.get("compressionRatioPolicy")
            != "non_gating_bounded_telemetry"
            or type(
                receipt.get("archiveCountExceedingHistoricalV2Ratio")
            )
            is not int
            or not (
                0
                <= receipt["archiveCountExceedingHistoricalV2Ratio"]
                <= 15
            )
            ...
            or manifest.get("orderedResourceSetSha256")
            != COMMON.v3_ordered_resource_set_sha256(decision)
        ...
        if (
            ...
            or manifest.get("compressionRatioPolicy")
            != receipt.get("compressionRatioPolicy")
            or manifest.get("archiveCountExceedingHistoricalV2Ratio")
            != receipt.get("archiveCountExceedingHistoricalV2Ratio")
        ):
            raise COMMON.Wave2Failure(
...
def exact_source_record(
    ...
    if (
        ...
        or COMMON.validate_compression_telemetry(
...
def retained_directory_identity(info: os.stat_result) -> tuple[int, ...]:
    return (
        info.st_dev,
        info.st_ino,
        info.st_mode,
...
def revalidate_retained_resource_pass(guard: Mapping[str, Any]) -> None:
    ...
    for identity, fd in zip(identities, guard["resourceFds"]):
        ...
        if (
            ...
            raise COMMON.Wave2Failure("E_TOCTOU", "readback")
    root_initial = guard["rootIdentity"]
    dependency_initial = guard["dependencyIdentity"]
    ...
    dependency_named = os.stat(
        COMMON.DEPENDENCY_PARENT.as_posix(),
        dir_fd=root_fd,
    ...
    if (
        retained_directory_identity(os.fstat(root_fd)) != root_initial
        or retained_directory_identity(dependency_named)
        != dependency_initial
        or retained_directory_identity(os.fstat(dependency_fd))
        != dependency_initial
        or retained_directory_identity(wave_named) != wave_initial
        or retained_directory_identity(os.fstat(wave_fd))
        != wave_initial
        or retained_directory_identity(final_named) != final_initial
        or retained_directory_identity(os.fstat(final_fd))
        != final_initial
...
def validate_resource_pass(
    ...
    *,
    ...
    try:
        ...
        root_initial = retained_directory_identity(os.fstat(root_fd))
        dependency_initial = retained_directory_identity(
            os.fstat(dependency_fd)
        )
        wave_initial = retained_directory_identity(os.fstat(wave_fd))
        final_initial = retained_directory_identity(os.fstat(final_fd))
        ...
        for item, record in zip(items, records):
            ...
            if (
                ...
                or archive["compressionTelemetry"]
        ...
        guard = {
            ...
            "waveFd": wave_fd,
            "finalFd": final_fd,
            "rootIdentity": root_initial,
            "dependencyIdentity": dependency_initial,
        ...
        result = {
            ...
            "archiveCountExceedingHistoricalV2Ratio": sum(
                1
                for row in rows
...
def check_loaded(authority: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if (
        ...
        or first["compressionRatioPolicy"]
        != receipt.get("compressionRatioPolicy")
        or first["archiveCountExceedingHistoricalV2Ratio"]
        != receipt.get("archiveCountExceedingHistoricalV2Ratio")
    ):
        raise COMMON.Wave2Failure(
            "E_ACQUISITION_RECEIPT",
    ...
    return {
        "status": "wave2_v3_independent_readback_passed_not_recorded",
        ...
        **first,
        ...
        "nextAction": "record_wave2_v3_readback_receipt_then_manifest",
    }
...
def check() -> dict[str, Any]:
    ...
    try:
        ...
        raise COMMON.map_readback_failure(
            error,
            core=(authority or {}).get("core"),
            legacy=(authority or {}).get("legacy"),
        ) from None
    finally:
...
def record() -> dict[str, Any]:
    ...
    try:
        ...
        require_success_namespace_exclusions(ROOT)
        if path_present(ROOT, COMMON.READBACK_MANIFEST_PATH):
            raise COMMON.Wave2Failure(
                "E_READBACK_NAMESPACE",
                "readback",
            )
        manifest = {
            ...
            "resourceCount": 30,
            "tupleCount": 15,
            "compressionRatioPolicy": checked["compressionRatioPolicy"],
            "archiveCountExceedingHistoricalV2Ratio": checked[
                "archiveCountExceedingHistoricalV2Ratio"
            ],
...
def error_document(failure: Any) -> dict[str, Any]:
    return {
        "documentType": (
            "aetherlink.g2-pion-dependency-wave2-v3-readback-error"
        ),
        "schemaVersion": "1.0",
        "status": "failed",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        failure = COMMON.Wave2Failure("E_INTERNAL", "readback")
        print(json.dumps(error_document(failure), indent=2, sort_keys=True))
        return 1
