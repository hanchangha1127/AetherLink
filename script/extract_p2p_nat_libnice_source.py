#!/usr/bin/env python3
"""Safely extract the pinned libnice 0.1.23 source archive as data."""
...
def extract() -> tuple[int, int]:
    if sha256_path(ARCHIVE_PATH) != EXPECTED_ARCHIVE_SHA256:
        raise ExtractionError("libnice archive SHA-256 does not match the intake lock")
...
def main() -> int:
    ...
    print(f"extracted pinned libnice source files={file_count} bytes={total_bytes}")
    return 0
