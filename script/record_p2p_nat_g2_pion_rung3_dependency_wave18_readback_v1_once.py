#!/usr/bin/env python3
"""Record one independent offline readback of the Wave18 v1 acquisition."""
...
if not (
    ...
    raise RuntimeError("Wave18 readback recorder requires `python3 -I -B -S`")
...
if O_NOFOLLOW is None:
    raise RuntimeError("Wave18 readback recorder requires O_NOFOLLOW")
READBACK_CHECKER_PATH = Path(__file__).with_name(
    "check_p2p_nat_g2_pion_rung3_dependency_wave18_"
    "readback_execution_permit_v1.py"
)
EXPECTED_READBACK_CHECKER_RAW = "c20c83420c1c48b37fd76b3757f6b9951ebda9992a98192bc839082097380df5"
...
MAX_ZIP_UNCOMPRESSED_BYTES_ACROSS_ALL = 384 * 1024 * 1024
MAX_ZIP_FILES_ACROSS_ALL = 60_000
...
def load_readback_checker() -> types.ModuleType:
    ...
    module = types.ModuleType("wave18_acquisition_readback_permit_v1")
...
def validate_zip(
    ...
    try:
        ...
        if archive is not None:
            archive.close()
    require(
        root_mod is not None
        and mod_raw is not None
        and root_mod == mod_raw,
        "E_MOD_PARITY",
        "zip",
    )
    return {
...
def validate_resource_selection(
    ...
    resource_types = {
        ...
        "port": int,
        "requestOrdinal": int,
        "selectedByGraphAlgorithm": bool,
        "tupleDigestSha256": str,
        "tupleId": str,
        "tupleOrder": int,
        "url": str,
        "version": str,
    ...
    require(
        ...
        and all(
            type(row) is dict
            and set(row) == set(resource_types)
            and all(
                type(row[key]) is expected_type
                for key, expected_type in resource_types.items()
            )
...
def verify_snapshot(snapshot: FrozenSnapshot) -> dict[str, Any]:
    ...
    permit_path = (
        PERMIT.BASE
        + "/bounded-dependency-source-acquisition-wave18-execution-permit-v1.json"
    )
    ...
    v16 = acquisition_permit["predecessorBindings"]["combinedFixedPointV16"]
    predecessor_authority_rows = [
        {
            "path": v16["checkerPath"],
            "rawSha256": v16["checkerRawSha256"],
        },
        {
            "path": v16["testsPath"],
            "rawSha256": v16["testsRawSha256"],
        },
    ]
    bound_authority_rows = [
        *acquisition_permit["decisionBinding"]["files"],
        *predecessor_authority_rows,
        v16["wave17NamespaceAnchor"],
    ...
    require(
        ...
        and acquisition_permit["requestContract"]["tupleCount"] == 3
        and acquisition_permit["requestContract"]["requestCount"] == 6
        ...
        is False
        and acquisition_permit["requestContract"]["cookieAllowed"] is False
        and acquisition_permit["requestContract"][
        ...
        and exact_json_equal(
            ...
            "checkerNormalizedSha256": (
                PERMIT.EXPECTED_V16_CHECKER_NORMALIZED
            ),
            "testsPath": (
            ...
            "contentSha256": PERMIT.EXPECTED_V16_CONTENT,
            "combinedInputSetSha256": PERMIT.EXPECTED_V16_INPUT_SET,
            "graphSha256": PERMIT.EXPECTED_V16_GRAPH,
            "frontierSha256": PERMIT.EXPECTED_V16_FRONTIER,
        ...
        and exact_json_equal(
            acquisition_permit["identityBinding"],
            {
            "blockedTupleCount": 0,
        ...
        is False
        and acquisition_permit["authority"]["ownerProofRequired"] is False
    ...
    require(
        exact_json_equal(strict_json(claim_raw, "claim"), claim_expected)
    ...
    require(
        len(resources) == 6
        and [row["kind"] for row in resources]
    ...
    require(
        [row["requestOrdinal"] for row in resources]
        == [1, 2, 3, 4, 5, 6],
        "E_ORDER",
        phase,
    )
    ...
    for ordinal, resource in enumerate(resources, 1):
        ...
        require(
            len(raw) == accepted_specs[name]["bytes"]
            and sha256(raw) == accepted_specs[name]["rawSha256"],
            "E_ACCEPTED",
            phase,
        )
        require(
        ...
        if resource["kind"] == "mod":
            ...
            require(
                type(verified.get("rootGoModPresent")) is bool
                and verified["rootGoModPresent"] is True,
                "E_MOD_PARITY",
                phase,
            )
    ...
    require(
        len(mod_by_tuple) == 3
        and aggregate_mod == 279
        and aggregate_zip == 2_108_821
        and aggregate == 2_109_100
        and aggregate_zip_entries == 971
        and aggregate_zip_uncompressed == 7_225_800
    ...
    require(
        exact_json_equal(
            strict_json(evidence_raw, "evidence"),
            evidence_expected,
        )
    ...
    receipt_expected = {
        ...
        "aggregateZipEntryCount": aggregate_zip_entries,
        "aggregateZipUncompressedBytes": aggregate_zip_uncompressed,
        "dispatchBoundaryCount": 6,
        "responseCommittedCount": 6,
        "responseCommittedBytes": aggregate,
        "validationCommittedCount": 6,
        "persistenceCommittedCount": 6,
    ...
    require(
        exact_json_equal(
            strict_json(receipt_raw, "receipt"),
            receipt_expected,
        )
    ...
    require(
        exact_json_equal(
            strict_json(manifest_raw, "manifest"),
            manifest_expected,
        )
    ...
    return {
        ...
        "combinedFixedPointV16ContentSha256": PERMIT.EXPECTED_V16_CONTENT,
        "compactIdentitySha256": PERMIT.EXPECTED_COMPACT_IDENTITY,
        "fullWitnessSha256": PERMIT.EXPECTED_FULL_WITNESS,
...
def create_readback_claim(
    ...
    claim = content_bound(
        {
            "documentType": "aetherlink.wave18-acquisition-readback-one-use-claim",
            "schemaVersion": "1.0",
            "status": "consumed_active",
    ...
    try:
        ...
        try:
            ...
            receipt = content_bound(
                {
                    "documentType": "aetherlink.wave18-source-acquisition-readback",
                    "schemaVersion": "1.0",
                    "status": (
            ...
            manifest = content_bound(
                {
                    "documentType": (
                        "aetherlink.wave18-source-acquisition-readback-manifest"
                    ),
                    "schemaVersion": "1.0",
                    "status": (
                        "wave18_acquisition_retained_snapshot_"
...
def validate_argument_vector(argv: Sequence[str]) -> None:
    require(
        list(argv) in (["--preflight"], ["--execute"]),
        "E_ARGUMENT",
        "cli",
    )
...
def _post_success_reporting_failure() -> dict[str, Any]:
    return {
        "documentType": "aetherlink.wave18-acquisition-readback-error",
        "schemaVersion": "1.0",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if args.preflight:
            preflight()
            result = {
                "documentType": "aetherlink.wave18-acquisition-readback-preflight",
                "schemaVersion": "1.0",
