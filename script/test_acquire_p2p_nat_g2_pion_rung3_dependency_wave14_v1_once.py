#!/usr/bin/env python3
"""Adversarial offline tests for the one-use Wave14 acquisition runner."""
...
if not (
    ...
    raise RuntimeError("Wave14 runner tests require `python3 -I -B -S`")
...
EXPECTED_WAVE14_RESOURCE_CONTRACT_SHA256 = (
    "ba1741c181b834c42c2501232fc8d99e5dfae3e02fb1d927fdb56f3248d20b6e"
)


RUNNER_PATH = Path(__file__).with_name(
    "acquire_p2p_nat_g2_pion_rung3_dependency_wave14_v1_once.py"
)
# The runner carries the finalized Wave14 acquisition-checker reverse pin.
...
def exact_resource_oracle() -> list[dict[str, object]]:
    ...
    for tuple_order, identity in enumerate(
        ...
        tuple_id = f"wave14-{tuple_order:03d}-{digest[:12]}"
...
def fixture() -> tuple[list[dict[str, object]], dict[int, bytes]]:
    ...
    ordinal = 0
    for tuple_order in range(1, 4 + 1):
...
class Wave14RunnerTests(unittest.TestCase):
    def setUp(self) -> None:
        self.wave4_network_patcher = mock.patch.object(
            runner.WAVE4,
            "direct_fetch",
            side_effect=AssertionError(
                "offline Wave14 tests must never reach the network"
            ),
        )
        ...
        self.https_network_patcher = mock.patch(
            ...
            side_effect=AssertionError(
                "offline Wave14 tests must never create HTTPS connections"
            ),
        )
    ...
    def test_exact_four_tuple_eight_resource_contract_is_closed(
        self,
    ) -> None:
        self.assertEqual(
            runner.EXPECTED_WAVE14_IDENTITY,
            EXPECTED_WAVE14_IDENTITY,
        )
        ...
        self.assertEqual(
            runner.EXPECTED_WAVE14_RESOURCE_CONTRACT_SHA256,
            EXPECTED_WAVE14_RESOURCE_CONTRACT_SHA256,
        )
        self.assertEqual(
    ...
    def test_success_claims_before_8_fetches_and_manifest_is_last(self) -> None:
        rows, bodies = fixture()
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(len(list(accepted.iterdir())), 8)
            self.assertTrue(
                terminal(root, runner.CHECK.CLAIM_PATH).exists()
            )
            self.assertTrue(
    ...
    def test_canonical_preflight_context_composes_without_writes(self) -> None:
        ...
        self.assertEqual(result["requestCount"], 8)
    ...
    def test_semantic_or_cardinality_mutation_fails_before_claim(self) -> None:
        ...
        mutations = (
            ...
            lambda rows: rows.__setitem__(7, dict(rows[6])),
    ...
    def test_every_resource_field_is_required_on_all_8_rows(self) -> None:
        original, _ = fixture()
    ...
    def test_lower_ascii_direct_proxy_identity_is_enforced(self) -> None:
        def rewritten_pair(module: str, version: str):
            ...
            for index, kind in enumerate(("mod", "zip")):
                ...
                rows[index]["tupleId"] = f"wave14-001-{digest[:12]}"
    ...
    def test_wave14_direct_fetch_reaches_wave4_and_translates_errors(
        self,
    ) -> None:
        resource = fixture()[0][0]
