#!/usr/bin/env python3
"""Tests for the one-use offline Wave13 acquisition readback recorder."""
...
PATH = Path(__file__).with_name(
    "record_p2p_nat_g2_pion_rung3_dependency_wave13_readback_v1_once.py"
)
# The recorder deliberately retains an all-zero reverse pin until the separate
# Wave13 readback checker/permit bytes are frozen. This suite remains sealed
# until that pin is replaced with the finalized checker raw SHA-256.
SPEC = importlib.util.spec_from_file_location("wave13_readback_recorder_tests", PATH)
...
class Wave13ReadbackRecorderTests(unittest.TestCase):
    ...
    def test_05_live_retained_snapshot_verifies_twice_read_only(self):
        ...
        try:
            ...
            self.assertEqual(
                ...
                "eb05816e0b897ea8c3ad8b7089668e91",
            )
            ...
            self.assertEqual(first["aggregateModBytes"], 411)
            self.assertEqual(first["aggregateZipBytes"], 5_097_127)
            self.assertEqual(first["aggregateAcceptedBytes"], 5_097_538)
            self.assertEqual(first["aggregateZipEntryCount"], 1_647)
            self.assertEqual(
    ...
    def test_05_selector_counts_and_v11_binding_fail_closed(self):
        snapshot = R.FrozenSnapshot()
        permit_path = (
            R.PERMIT.BASE
    ...
    def test_21_wave13_uncompressed_limits_are_per_zip_and_across_all(self):
        self.assertEqual(
        ...
        self.assertEqual(R.MAX_AGGREGATE_MOD_BYTES, 4 * 1024 * 1024)
        self.assertEqual(R.MAX_AGGREGATE_ZIP_BYTES, 64 * 1024 * 1024)
        self.assertEqual(R.MAX_AGGREGATE_BYTES, 68 * 1024 * 1024)
        self.assertEqual(R.MAX_ZIP_FILES_ACROSS_ALL, 80_000)
