#!/usr/bin/env python3
"""Consume the Wave8 v1 permit once and acquire 28 verified resources."""
...
if not (
    ...
    raise RuntimeError("Wave8 acquisition requires `python3 -I -B -S`")
...
EXPECTED_CHECKER_RAW = "78132ce62e3ba4b74fb404983d55b003119106eba34c3cba6b6fbc7622a0ca20"
...
class ImmutablePhaseLedger:
    ...
    def source_acquisition_state(self) -> str:
        ...
        if self.response_committed_count == 28:
...
class ExecutionNamespace:
    ...
    def barrier(self, state: ExecutionState) -> None:
        ...
        if state in {
            ...
            require(
                len(self.resources) == 28 and self.evidence is not None,
                "E_CARDINALITY",
                phase,
            )
...
def _validate_resource_contract(
    ...
    require(
        ...
        and permit["requestContract"]["requestCount"] == 28
        and len(rows) == 28
    ...
    for index, source in enumerate(rows):
        ...
        require(
            ...
            and row["tupleId"] == f"wave8-{tuple_order:03d}-{digest[:12]}"
            and row["kind"] == kind
            and row["method"] == "GET"
    ...
    require(
        len({row["acceptedFileName"] for row in result}) == 28
        and len({row["url"] for row in result}) == 28
        and sum(
...
def _attempt(
    ...
    *,
    ...
    try:
        ...
        failure = {
            "documentType": "aetherlink.wave8-source-acquisition-failure",
            "schemaVersion": "1.0",
...
def validate_execution_context() -> dict[str, Any]:
    ...
    return {
        "documentType":
            "aetherlink.wave8-source-acquisition-execution-context-check",
        "schemaVersion": "1.0",
...
def error_document(error: AcquisitionError) -> dict[str, Any]:
    ...
    return {
        "documentType": "aetherlink.wave8-source-acquisition-error",
        "schemaVersion": "1.0",
        "status": status,
