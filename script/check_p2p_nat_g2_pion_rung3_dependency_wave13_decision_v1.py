#!/usr/bin/env python3
"""Validate the read-only Wave13 identity and acquisition decision.

Run only with ``python3 -I -B -S``.  The checker executes the exact pinned
combined-v11 candidate, then scans the retained 325 inputs twice using only the
...
def require_isolated_interpreter() -> None:
    ...
    if not (
        ...
        raise RuntimeError(
            "Wave13 decision checker requires unoptimized "
            "`python3 -I -B -S`"
        )
...
CHECKER_ID = "g2-pion-ice-v4.3.0-wave13-identity-acquisition-decision-check-v1"
DECISION_ID = (
    "g2-pion-ice-v4.3.0-rung3-bounded-dependency-source-identity-and-"
    "acquisition-decision-wave13-v1"
)
DEPENDENCY_ROOT = "build/offline-source/pion-ice-v4.3.0/dependencies"
NAMESPACE_ANCHOR_PATH = f"{DEPENDENCY_ROOT}/.wave-12-v1.claim"
NAMESPACE_ANCHOR_RAW_SHA256 = (
    "58145cf6660a9a6c3ed5ab36ec4f38df388e88d10c5a1e6820ca9416f06b8280"
)
WAVE13_CLAIM_PATH = f"{DEPENDENCY_ROOT}/.wave-13-v1.claim"
WAVE13_STAGING_PREFIX = ".wave-13-v1-staging-"
WAVE13_ACCEPTED_PATH = f"{DEPENDENCY_ROOT}/wave-13-v1/accepted"
...
EXPECTED_NON_CLAIMS = (
    ...
    "wave13_dependency_source_not_acquired",
...
def portable_name(value: str) -> str:
    require(type(value) is str, "E_WAVE13_NAMESPACE")
    return unicodedata.normalize("NFC", value).casefold()


def validate_wave13_namespace_absent(
    ...
    try:
        ...
        raise DecisionFailure("E_WAVE13_NAMESPACE") from error
    require(
    ...
    for name in dependency_names:
        ...
        require(
            not normalized.startswith(".wave-13-v1")
            and not normalized.startswith("wave-13-v1"),
            "E_WAVE13_NAMESPACE",
        )
    ...
    for name in docs_names:
        ...
        require(
            not normalized.startswith(
                "bounded-dependency-source-acquisition-wave13"
            ),
            "E_WAVE13_NAMESPACE",
        )
    ...
    for name in script_names:
        normalized = portable_name(name)
        if "p2p_nat_g2_pion" in normalized and "wave13" in normalized:
            require(
...
class BootstrapPinnedCodeFile:
    """Immediate-ownership bootstrap pin matching combined-v11 semantics."""
...
def load_v11_checker(
    held: BootstrapPinnedCodeFile,
) -> types.ModuleType:
    module = types.ModuleType("aetherlink_combined_v11_pinned")
    module.__dict__.update(
        {
            "__cached__": None,
            "__file__": str(ROOT / V11_CHECKER_PATH),
            "__loader__": None,
            "__name__": "aetherlink_combined_v11_pinned",
            "__package__": None,
        }
    )
    try:
        ...
        raise DecisionFailure("E_V11_LOAD") from error
    for name in (
        "PinnedCodeFile",
        "SafeHeldInputSet",
        "generate_candidate",
...
def require_closed_identity(scan: Mapping[str, Any]) -> None:
    ...
    require(
        ...
        and scan["archiveCount"] == 163
        and scan["externalModCount"] == 162
...
def validate_v11_candidate(
    candidate: Mapping[str, Any],
) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    require(type(candidate) is dict, "E_V11_CANDIDATE")
    ...
    require(
        ...
        and inputs.get("resourceCount") == 324
        and inputs.get("modCount") == 162
        and inputs.get("zipCount") == 162
        ...
        and inputs.get("uniqueModuleVersionTupleCount") == 162
        and inputs.get("aggregateRawByteSize") == 302_389_009
    ...
    require(
        ...
        == 40
        and verification.get("pinnedV10PredecessorExecuted") is True
        and verification.get("v10TestsBindingScope")
        == "historical_metadata_only_not_live_held"
        and verification.get("v10TestsLiveHeld") is False
        and type(predecessor_verification) is dict
        and predecessor_verification.get("v10TestsBindingScope")
        == "historical_metadata_only_not_live_held"
        and predecessor_verification.get("v10TestsLiveHeld") is False
        ...
        and verification.get("wave12RetainedFdPreManifestBarrierCount")
        == 3
        and verification.get("wave12CompletionAppliesToRetainedSnapshot")
        is True
        and verification.get(
        ...
        and verification.get("hardenedCheckerModuleCount") == 10
        and verification.get("providerFacadeLoadCount") == 10
        ...
        and counters.get("directArchiveOpenCount") == 326
        and counters.get("inheritedArchiveOpenCount") == 1_984
        and counters.get("totalArchiveOpenCount") == 2_310
        and counters.get("archiveOpenCount") == 2_310
        and counters.get("heldSourceInputCount") == 325
        and counters.get("heldTerminalEvidenceCount") == 80
        and counters.get("heldAuxiliaryEvidenceCount") == 3
        and counters.get("heldToolInputCount") == 10
        and counters.get("transitiveDistinctToolPathCount") == 12
        and counters.get("stableReadPassesPerHeldInput") == 2
        and counters.get("directFullSourceReconstructionCount") == 2
        and counters.get("inheritedFullSourceReconstructionCount") == 18
        and counters.get("totalFullSourceReconstructionCount") == 20
        ...
        and len(tool_bindings) == 10
        and type(terminal_bindings) is list
        and len(terminal_bindings) == 80
...
def expected_tool_bindings(
    ...
    require(
        ...
        == TESTS_RAW_SHA256
        and sha256_bytes(package_raw[V11_CHECKER_PATH])
        == V11_CHECKER_RAW_SHA256
        and sha256_bytes(
            package_raw[V11_TESTS_PATH]
        )
        == V11_TESTS_RAW_SHA256,
        "E_TOOL_IDENTITY",
    )
    ...
    *,
    ...
    return {
        "documentType": (
            ...
            "identity-and-acquisition-decision-wave13"
        ),
        "schemaVersion": "1.0",
        ...
        "date": "2026-07-27",
        "status": (
            "wave13_exact_4_frontier_identity_classified_"
        ...
        "predecessorBindings": {
            "combinedFixedPointV11": {
                ...
                "totalGraphArchiveOpenCount": 2310,
        ...
        "sourceAcquisitionPreparation": {
            ...
            "modulePathEncoding":
                "current_wave13_lowercase_ascii_direct_proxy_path",
            "claimPath": WAVE13_CLAIM_PATH,
            "stagingDirectoryPrefix": WAVE13_STAGING_PREFIX,
            "acceptedDirectoryPath": WAVE13_ACCEPTED_PATH,
        ...
        "operationCounters": {
            ...
            "totalFullSourceReconstructionCount": 20,
            "predecessorGraphArchiveOpenCount": 1984,
            "currentV11GraphArchiveOpenCount": 326,
            "totalV11GraphArchiveOpenCount": 2310,
            "identityWitnessScanCount": 2,
            "identityWitnessArchiveOpenCount": 326,
            "overallDecisionExecutionArchiveOpenCount": 2636,
        ...
        "closure": {
            "wave13IdentityResolved": True,
            "wave13AcquisitionReady": True,
            "wave13AcquisitionComplete": False,
        ...
        "nextAction": (
            "prepare_separate_one_use_8_resource_wave13_source_"
...
def validate_semantic_decision(
    ...
    combined = (
        predecessors.get("combinedFixedPointV11")
    ...
    require(
        exact_keys(predecessors, {"combinedFixedPointV11"})
        ...
        and combined["v10TestsLiveHeld"] is False
        and all(
    ...
    require(
        ...
        and held["sourceBindingCount"] == 325
        and held["sourceBindingsSha256"]
        == V11_SOURCE_BINDINGS_SHA256
        and held["archiveCount"] == 163
        and held["externalModCount"] == 162
    ...
    require(
        ...
        == "current_wave13_lowercase_ascii_direct_proxy_path"
        and preparation["claimPath"] == WAVE13_CLAIM_PATH
        and preparation["stagingDirectoryPrefix"]
        == WAVE13_STAGING_PREFIX
        and preparation["acceptedDirectoryPath"]
        == WAVE13_ACCEPTED_PATH
...
def evaluate(
    ...
    *,
    ...
    with ExitStack() as stack:
        ...
        v11_tests_held = stack.enter_context(
            v11.PinnedCodeFile(
                root,
                V11_TESTS_PATH,
                V11_TESTS_RAW_SHA256,
        ...
        package_raw = {
            ...
            V11_CHECKER_PATH: v11_held.raw,
            V11_TESTS_PATH: v11_tests_held.raw,
        }
        held: list[Any] = [
            self_held,
            v11_held,
        ...
        validate_wave13_namespace_absent(
            namespace_anchor_held.parent_fd,
            reader_held.parent_fd,
            self_held.parent_fd,
        )
        identity_barrier(root, held)
        candidate = v11.generate_candidate(root)
        identity_barrier(root, held)
        wave_rows, source_bindings = validate_v11_candidate(candidate)
        v11 = v11.harden_checker_module(v11)
        ...
        require(
            type(runner) is v11.ReadOnlyProviderFacade
            and v10.PinnedCodeFile is v11.PinnedCodeFile
            and v9.PinnedCodeFile is v11.PinnedCodeFile
            and v8.PinnedCodeFile is v11.PinnedCodeFile
            and v7.PinnedCodeFile is v11.PinnedCodeFile
            and v6.PinnedCodeFile is v11.PinnedCodeFile
            and v5.PinnedCodeFile is v11.PinnedCodeFile
            and v4.PinnedCodeFile is v11.PinnedCodeFile
            and v1.PinnedRunnerFile is v11.SafePinnedRunnerFile,
            "E_SAFE_RUNTIME",
        )
        ...
        require(
            ...
            == source_bindings
            and len(full_source_bindings) == 325
            and all(
                type(row) is dict
