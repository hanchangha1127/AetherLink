#!/usr/bin/env python3
"""Adversarial offline tests for the one-use Wave9 acquisition runner."""
...
if not (
    ...
    raise RuntimeError("Wave9 runner tests require `python3 -I -B -S`")
...
RUNNER_PATH = Path(__file__).with_name(
    "acquire_p2p_nat_g2_pion_rung3_dependency_wave9_v1_once.py"
)
SPEC = importlib.util.spec_from_file_location(
    "wave9_source_acquirer_v1",
    RUNNER_PATH,
)
...
class Wave9RunnerTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_runpy_shape_mismatch_is_rejected_as_misconfiguration(
        ...
        try:
            ...
            ), mock.patch.object(runner.sys, "stdout", sink):
    ...
    def test_success_claims_before_20_fetches_and_manifest_is_last(self) -> None:
        rows, bodies = fixture()
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(len(list(accepted.iterdir())), 20)
            self.assertTrue(
                terminal(root, runner.CHECK.CLAIM_PATH).exists()
            )
            self.assertTrue(
    ...
    def test_canonical_preflight_context_composes_without_writes(self) -> None:
        ...
        x_net = [
            ...
            if row["module"] == "golang.org/x/net"
            and row["version"] in {"v0.44.0", "v0.45.0"}
        ]
        self.assertEqual(
            {
                row["expectedH1"]
        ...
        self.assertEqual(
            len({row["acceptedFileName"] for row in x_net}),
            4,
        )
        result = runner.validate_execution_context()
        self.assertTrue(result["validationPassed"])
        self.assertEqual(result["requestCount"], 20)
    ...
    def test_semantic_or_cardinality_mutation_fails_before_claim(self) -> None:
        ...
        mutations = (
            ...
            lambda rows: rows.__setitem__(19, dict(rows[18])),
            lambda rows: rows[19].__setitem__(
                "requestOrdinal",
                21,
            ),
    ...
    def test_lower_ascii_direct_proxy_identity_is_enforced(self) -> None:
        def rewritten_pair(module: str, version: str):
            ...
            for index, kind in enumerate(("mod", "zip")):
                ...
                rows[index]["tupleId"] = f"wave9-001-{digest[:12]}"
    ...
    def test_wave9_direct_fetch_reaches_wave4_and_translates_errors(
        self,
    ) -> None:
        resource = fixture()[0][0]
