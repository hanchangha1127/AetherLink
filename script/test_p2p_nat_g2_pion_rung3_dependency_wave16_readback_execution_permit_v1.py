#!/usr/bin/env python3
"""Tests for the Wave16 acquisition readback permit package."""
...
def _deny_test_network(*_args, **_kwargs):
    ...
    raise AssertionError(
        "offline Wave16 readback tests must never create network connections"
    )
...
SPEC = importlib.util.spec_from_file_location("wave16_readback_permit_tests", PATH)
...
def _synthetic_package_raw() -> tuple[dict[str, bytes], str, str]:
    ...
    reader_raw = b"synthetic Wave16 readback reader\\n"
...
class Wave16ReadbackPermitTests(unittest.TestCase):
    def tearDown(self) -> None:
        self.assertEqual(NETWORK_ATTEMPTS, [])

    def test_01_frozen_snapshot_constants_are_exact(self):
        self.assertEqual(C.ATTEMPT_ID, "fff8d6073748eab6fd1a05c79c57a84f")
        self.assertEqual(
        ...
        acquisition_permit_path = (
            f"{C.BASE}/bounded-dependency-source-acquisition-wave16-"
        ...
        self.assertEqual(payload["aggregateZipEntryCount"], 948)
        self.assertEqual(
        ...
        v14 = acquisition_permit["predecessorBindings"][
        ...
        self.assertEqual(
            payload["predecessorBindings"]["combinedFixedPointV14"][
                "contentSha256"
            ],
            C.EXPECTED_V14_CONTENT,
        )
        self.assertFalse(
            any(
    ...
    def test_08_wave16_cardinality_hash_aggregate_and_limit_drift_rejects(self):
        original, _ = _synthetic_permit()
        snapshot = original["frozenAcquisitionSnapshot"]

        def frozen_29(value):
            value["frozenAcquisitionSnapshot"]["frozenFileCount"] = 29
        ...
        def authority_14(value):
            ...
            value["frozenAcquisitionSnapshot"]["acquisitionAuthority"] = [
                ...
                if not row["path"].endswith("/.wave-15-v1.claim")
            ]
        ...
        def bound_rows_14(value):
            ...
            predecessor.pop("wave15NamespaceAnchor")

        def predecessor_v13(value):
        ...
        def wave15_hashes(value):
            value["frozenAcquisitionSnapshot"].update(
                {
                    "frozenFilesCanonicalSha256": (
                        "b7df25872029064c136ab99be564cd201"
                        "3fef7047ecb9588175909f3b31951d8"
                    ),
                    "acceptedResourceHashSetCanonicalSha256": (
                        "9255922769ca442cdc555467158bd4a1c1"
                        "399398d4ee1f5ced42a677b35f140d"
                    ),
                }
            )
        ...
        for production_path in (
            ...
            if not path.exists():
                continue
            source = path.read_text(encoding="utf-8")
            self.assertNotIn("combinedFixedPointV" + "13", source)
            self.assertNotIn(".wave-" + "14-v1.claim", source)
