#!/usr/bin/env python3
"""Tests for the one-use offline Wave19 acquisition readback recorder."""
...
PATH = Path(__file__).with_name(
    "record_p2p_nat_g2_pion_rung3_dependency_wave19_readback_v1_once.py"
)
...
# Wave19 readback checker/permit bytes are frozen. This suite remains sealed
# until that pin is replaced with the finalized checker raw SHA-256.
SPEC = importlib.util.spec_from_file_location("wave19_readback_recorder_tests", PATH)
...
class Wave19ReadbackRecorderTests(unittest.TestCase):
    ...
    def test_05_live_retained_snapshot_verifies_twice_read_only(self):
        ...
        try:
            ...
            self.assertEqual(
                ...
                "f10c20196d994afe3a8eba830eb42614",
            )
            self.assertEqual(first["acceptedResourceCount"], 4)
            ...
            self.assertEqual(first["aggregateModBytes"], 415)
            self.assertEqual(first["aggregateZipBytes"], 11_453_540)
            self.assertEqual(first["aggregateAcceptedBytes"], 11_453_955)
            self.assertEqual(first["aggregateZipEntryCount"], 931)
            self.assertEqual(
    ...
    def test_05_selector_counts_and_v17_binding_fail_closed(self):
        snapshot = R.FrozenSnapshot()
        permit_path = (
            R.PERMIT.BASE
            + "/bounded-dependency-source-acquisition-wave19-"
        ...
        try:
            ...
            for index in range(4):
                resources = copy.deepcopy(baseline)
                resources[index]["requestOrdinal"] = index + 5
            ...
            authority_mutations = [
                ...
                (
                    "stale-v16-predecessor",
                    lambda value: value.__setitem__(
                        "predecessorBindings",
                        {
                            "combinedFixedPointV" + "16": value[
                                "predecessorBindings"
    ...
    def test_05_verify_snapshot_fixed_record_and_validator_mutations_fail(self):
        ...
        try:
            ...
            for field in ("entryCount", "uncompressedBytes"):
                for target_zip_index in range(2):
                    with self.subTest(
    ...
    def test_10_synthetic_execute_orders_claim_two_passes_receipt_manifest(self):
        ...
        def verify(snapshot):
            ...
            return {"acceptedResourceCount": 4}
    ...
    def test_12_receipt_only_gap_is_terminal_uncertainty(self):
        ...
        with mock.patch.object(
            ...
            R,
            "verify_snapshot",
            return_value={"acceptedResourceCount": 4},
        ), mock.patch.object(R, "atomic_publish", side_effect=publish):
            with self.assertRaises(R.ReadbackError) as caught:
                R.execute(
                    Path("/unused"),
    ...
    def test_21_wave19_uncompressed_limits_are_per_zip_and_across_all(self):
        self.assertEqual(
        ...
        self.assertEqual(R.MAX_AGGREGATE_MOD_BYTES, 2 * 1024 * 1024)
        self.assertEqual(R.MAX_AGGREGATE_ZIP_BYTES, 32 * 1024 * 1024)
        self.assertEqual(R.MAX_AGGREGATE_BYTES, 34 * 1024 * 1024)
        self.assertEqual(R.MAX_ZIP_FILES_ACROSS_ALL, 40_000)
    ...
    def test_40_imported_main_enforces_exact_argument_vectors(self):
        ...
        synthetic_result = {
            "documentType": "synthetic-wave19-readback-result",
            "status": "complete",
        }
        with mock.patch.object(
            sys,
