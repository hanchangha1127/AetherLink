#!/usr/bin/env python3
"""Record one independent offline readback of the Wave19 v1 acquisition."""
...
if not (
    ...
    raise RuntimeError("Wave19 readback recorder requires `python3 -I -B -S`")
...
if O_NOFOLLOW is None:
    raise RuntimeError("Wave19 readback recorder requires O_NOFOLLOW")
...
EXPECTED_READBACK_CHECKER_RAW = "03b6c0ab22c59b325a42ffdeff42883923b6bcb5a70a66ca8d6a9a27fce4bc0e"
...
MAX_ZIP_UNCOMPRESSED_BYTES_ACROSS_ALL = 256 * 1024 * 1024
MAX_ZIP_FILES_ACROSS_ALL = 40_000
...
def load_readback_checker() -> types.ModuleType:
    ...
    module = types.ModuleType("wave19_acquisition_readback_permit_v1")
...
def verify_snapshot(snapshot: FrozenSnapshot) -> dict[str, Any]:
    ...
    v17 = acquisition_permit["predecessorBindings"]["combinedFixedPointV17"]
    ...
    require(
        ...
        and acquisition_permit["requestContract"]["tupleCount"] == 2
        and acquisition_permit["requestContract"]["requestCount"] == 4
        ...
        and exact_json_equal(
            ...
            "checkerNormalizedSha256": (
                PERMIT.EXPECTED_V17_CHECKER_NORMALIZED
            ),
            "testsPath": (
            ...
            "contentSha256": PERMIT.EXPECTED_V17_CONTENT,
            "combinedInputSetSha256": PERMIT.EXPECTED_V17_INPUT_SET,
            "graphSha256": PERMIT.EXPECTED_V17_GRAPH,
            "frontierSha256": PERMIT.EXPECTED_V17_FRONTIER,
    ...
    require(
        len(mod_by_tuple) == 2
        and aggregate_mod == 415
        and aggregate_zip == 11_453_540
        and aggregate == 11_453_955
        and aggregate_zip_entries == 931
        and aggregate_zip_uncompressed == 46_404_827
    ...
    receipt_expected = {
        ...
        "aggregateZipEntryCount": aggregate_zip_entries,
        "aggregateZipUncompressedBytes": aggregate_zip_uncompressed,
        "dispatchBoundaryCount": 4,
        "responseCommittedCount": 4,
        "responseCommittedBytes": aggregate,
        "validationCommittedCount": 4,
        "persistenceCommittedCount": 4,
    ...
    return {
        ...
        "combinedFixedPointV17ContentSha256": PERMIT.EXPECTED_V17_CONTENT,
        "compactIdentitySha256": PERMIT.EXPECTED_COMPACT_IDENTITY,
        "fullWitnessSha256": PERMIT.EXPECTED_FULL_WITNESS,
...
def create_readback_claim(
    ...
    claim = content_bound(
        {
            "documentType": "aetherlink.wave19-acquisition-readback-one-use-claim",
            "schemaVersion": "1.0",
            "status": "consumed_active",
    ...
    try:
        ...
        try:
            ...
            receipt = content_bound(
                {
                    "documentType": "aetherlink.wave19-source-acquisition-readback",
                    "schemaVersion": "1.0",
                    "status": (
                        "wave19_acquisition_retained_snapshot_"
            ...
            manifest = content_bound(
                {
                    "documentType": (
                        "aetherlink.wave19-source-acquisition-readback-manifest"
                    ),
                    "schemaVersion": "1.0",
                    "status": (
                        "wave19_acquisition_retained_snapshot_"
...
def _post_success_reporting_failure() -> dict[str, Any]:
    return {
        "documentType": "aetherlink.wave19-acquisition-readback-error",
        "schemaVersion": "1.0",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if args.preflight:
            preflight()
            result = {
                "documentType": "aetherlink.wave19-acquisition-readback-preflight",
                "schemaVersion": "1.0",
