#!/usr/bin/env python3
"""Tests for the one-use offline Wave17 acquisition readback recorder."""
...
PATH = Path(__file__).with_name(
    "record_p2p_nat_g2_pion_rung3_dependency_wave17_readback_v1_once.py"
)
# The recorder deliberately retains an all-zero reverse pin until the separate
# Wave17 readback checker/permit bytes are frozen. This suite remains sealed
# until that pin is replaced with the finalized checker raw SHA-256.
SPEC = importlib.util.spec_from_file_location("wave17_readback_recorder_tests", PATH)
...
class Wave17ReadbackRecorderTests(unittest.TestCase):
    ...
    def test_05_live_retained_snapshot_verifies_twice_read_only(self):
        ...
        try:
            ...
            self.assertEqual(
                ...
                "117fb836380658986632911b9508e274",
            )
            self.assertEqual(first["acceptedResourceCount"], 2)
            ...
            self.assertEqual(first["aggregateModBytes"], 301)
            self.assertEqual(first["aggregateZipBytes"], 3_450_399)
            self.assertEqual(first["aggregateAcceptedBytes"], 3_450_700)
            self.assertEqual(first["aggregateZipEntryCount"], 1_550)
            self.assertEqual(
    ...
    def test_05_selector_counts_and_v15_binding_fail_closed(self):
        snapshot = R.FrozenSnapshot()
        permit_path = (
            R.PERMIT.BASE
        ...
        try:
            ...
            authority_mutations = [
                ...
                (
                    "stale-v14-predecessor",
                    lambda value: value.__setitem__(
                        "predecessorBindings",
                        {
                            "combinedFixedPointV" + "14": value[
                                "predecessorBindings"
    ...
    def test_10_synthetic_execute_orders_claim_two_passes_receipt_manifest(self):
        ...
        def verify(snapshot):
            ...
            return {"acceptedResourceCount": 2}
    ...
    def test_12_receipt_only_gap_is_terminal_uncertainty(self):
        ...
        with mock.patch.object(
            ...
            R,
            "verify_snapshot",
            return_value={"acceptedResourceCount": 2},
        ), mock.patch.object(R, "atomic_publish", side_effect=publish):
            with self.assertRaises(R.ReadbackError) as caught:
                R.execute(
                    Path("/unused"),
    ...
    def test_21_wave17_uncompressed_limits_are_per_zip_and_across_all(self):
        self.assertEqual(
        ...
        self.assertEqual(R.MAX_AGGREGATE_MOD_BYTES, 1 * 1024 * 1024)
        self.assertEqual(R.MAX_AGGREGATE_ZIP_BYTES, 16 * 1024 * 1024)
        self.assertEqual(R.MAX_AGGREGATE_BYTES, 17 * 1024 * 1024)
        self.assertEqual(R.MAX_ZIP_FILES_ACROSS_ALL, 20_000)
    ...
    def test_40_imported_main_enforces_exact_argument_vectors(self):
        ...
        synthetic_result = {
            "documentType": "synthetic-wave17-readback-result",
            "status": "complete",
        }
        with mock.patch.object(
            sys,
