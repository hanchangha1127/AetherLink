#!/usr/bin/env python3
"""Validate the read-only Wave11 identity and acquisition decision.

Run only with ``python3 -I -B -S``.  The checker executes the exact pinned
combined-v9 candidate, then scans the retained 299 inputs twice using only the
...
def require_isolated_interpreter() -> None:
    ...
    if not (
        ...
        raise RuntimeError(
            "Wave11 decision checker requires unoptimized "
            "`python3 -I -B -S`"
        )
...
CHECKER_ID = "g2-pion-ice-v4.3.0-wave11-identity-acquisition-decision-check-v1"
DECISION_ID = (
    "g2-pion-ice-v4.3.0-rung3-bounded-dependency-source-identity-and-"
    "acquisition-decision-wave11-v1"
)
DEPENDENCY_ROOT = "build/offline-source/pion-ice-v4.3.0/dependencies"
NAMESPACE_ANCHOR_PATH = f"{DEPENDENCY_ROOT}/.wave-10-v1.claim"
NAMESPACE_ANCHOR_RAW_SHA256 = (
    "5260f5d7e7473013871573717848a3e8eae868a47ab2bfe538340d681ec4a6de"
)
WAVE11_CLAIM_PATH = f"{DEPENDENCY_ROOT}/.wave-11-v1.claim"
WAVE11_STAGING_PREFIX = ".wave-11-v1-staging-"
WAVE11_ACCEPTED_PATH = f"{DEPENDENCY_ROOT}/wave-11-v1/accepted"
...
EXPECTED_FRONTIER_COUNT = 9
EXPECTED_GRAPH_SELECTED_TUPLE_COUNT = 0
EXPECTED_GO_SUM_ENTRY_COUNT = 105
EXPECTED_PARENT_DECLARATION_COUNT = 12
EXPECTED_GO_MOD_H1_WITNESS_COUNT = 68
EXPECTED_MODULE_ZIP_H1_WITNESS_COUNT = 13
...
EXPECTED_NON_CLAIMS = (
    ...
    "wave11_dependency_source_not_acquired",
...
def portable_name(value: str) -> str:
    require(type(value) is str, "E_WAVE11_NAMESPACE")
    return unicodedata.normalize("NFC", value).casefold()


def validate_wave11_namespace_absent(
    ...
    try:
        ...
        raise DecisionFailure("E_WAVE11_NAMESPACE") from error
    require(
    ...
    for name in dependency_names:
        ...
        require(
            not normalized.startswith(".wave-11-v1")
            and not normalized.startswith("wave-11-v1"),
            "E_WAVE11_NAMESPACE",
        )
    ...
    for name in docs_names:
        ...
        require(
            not normalized.startswith(
                "bounded-dependency-source-acquisition-wave11"
            ),
            "E_WAVE11_NAMESPACE",
        )
    ...
    for name in script_names:
        normalized = portable_name(name)
        if "p2p_nat_g2_pion" in normalized and "wave11" in normalized:
            require(
...
class BootstrapPinnedCodeFile:
    """Immediate-ownership bootstrap pin matching combined-v9 semantics."""
...
def load_v9_checker(
    ...
    module = types.ModuleType("aetherlink_combined_v9_pinned")
    module.__dict__.update(
        {
            "__cached__": None,
            "__file__": str(ROOT / V9_CHECKER_PATH),
            "__loader__": None,
            "__name__": "aetherlink_combined_v9_pinned",
            "__package__": None,
        }
    )
    try:
        ...
        raise DecisionFailure("E_V9_LOAD") from error
    for name in (
        "PinnedCodeFile",
        "SafeHeldInputSet",
        "generate_candidate",
...
def require_closed_identity(scan: Mapping[str, Any]) -> None:
    ...
    require(
        ...
        and scan["archiveCount"] == 150
        and scan["externalModCount"] == 149
...
def validate_v9_candidate(
    candidate: Mapping[str, Any],
) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    require(type(candidate) is dict, "E_V9_CANDIDATE")
    ...
    require(
        ...
        == (
            ...
            "wave4-wave5-wave6-wave7-wave8-wave9-wave10-"
    ...
    require(
        ...
        and inputs.get("resourceCount") == 298
        and inputs.get("modCount") == 149
        and inputs.get("zipCount") == 149
        ...
        and inputs.get("uniqueModuleVersionTupleCount") == 149
        and inputs.get("aggregateRawByteSize") == 270_988_846
    ...
    require(
        ...
        == 32
        and verification.get("pinnedV8PredecessorExecuted") is True
        and verification.get("v8TestsBindingScope")
        == "historical_metadata_only_not_live_held"
        and verification.get("v8TestsLiveHeld") is False
        and type(predecessor_verification) is dict
        and predecessor_verification.get("v8TestsBindingScope")
        == "historical_metadata_only_not_live_held"
        and predecessor_verification.get("v8TestsLiveHeld") is False
        ...
        and verification.get("wave10RetainedFdPreManifestBarrierCount")
        == 3
        and verification.get("wave10CompletionAppliesToRetainedSnapshot")
        is True
        and verification.get(
        ...
        and verification.get("hardenedCheckerModuleCount") == 8
        and verification.get("providerFacadeLoadCount") == 8
        ...
        and counters.get("directArchiveOpenCount") == 300
        and counters.get("inheritedArchiveOpenCount") == 1_366
        and counters.get("totalArchiveOpenCount") == 1_666
        and counters.get("archiveOpenCount") == 1_666
        and counters.get("heldSourceInputCount") == 299
        and counters.get("heldTerminalEvidenceCount") == 66
        and counters.get("heldAuxiliaryEvidenceCount") == 3
        and counters.get("heldToolInputCount") == 8
        and counters.get("transitiveDistinctToolPathCount") == 10
        and counters.get("stableReadPassesPerHeldInput") == 2
        and counters.get("directFullSourceReconstructionCount") == 2
        and counters.get("inheritedFullSourceReconstructionCount") == 14
        and counters.get("totalFullSourceReconstructionCount") == 16
        ...
        and len(tool_bindings) == 8
        and type(terminal_bindings) is list
        and len(terminal_bindings) == 66
...
def expected_tool_bindings(
    ...
    require(
        ...
        == TESTS_RAW_SHA256
        and sha256_bytes(package_raw[V9_CHECKER_PATH])
        == V9_CHECKER_RAW_SHA256
        and sha256_bytes(
            package_raw[V9_TESTS_PATH]
        )
        == V9_TESTS_RAW_SHA256,
        "E_TOOL_IDENTITY",
    )
    ...
    *,
    ...
    return {
        "documentType": (
            ...
            "identity-and-acquisition-decision-wave11"
        ),
        "schemaVersion": "1.0",
        ...
        "date": "2026-07-26",
        "status": (
            "wave11_exact_9_frontier_identity_classified_"
            "9_complete_0_blocked_acquisition_ready_not_authorized"
        ),
        "result": (
            f"exact_9_version_vertices_"
            f"{EXPECTED_GRAPH_SELECTED_TUPLE_COUNT}_selected_"
            f"{EXPECTED_FRONTIER_COUNT - EXPECTED_GRAPH_SELECTED_TUPLE_COUNT}"
            "_nonselected_9_complete_h1_pairs_"
        ...
        "sourceAcquisitionPreparation": {
            ...
            "modulePathEncoding":
                "current_wave11_lowercase_ascii_direct_proxy_path",
            "claimPath": WAVE11_CLAIM_PATH,
            "stagingDirectoryPrefix": WAVE11_STAGING_PREFIX,
            "acceptedDirectoryPath": WAVE11_ACCEPTED_PATH,
        ...
        "operationCounters": {
            "combinedV9CandidateInvocationCount": 1,
            "predecessorFullSourceReconstructionCount": 14,
            "directV9FullSourceReconstructionCount": 2,
            "totalFullSourceReconstructionCount": 16,
            "predecessorGraphArchiveOpenCount": 1366,
            "currentV9GraphArchiveOpenCount": 300,
            "totalV9GraphArchiveOpenCount": 1666,
            "identityWitnessScanCount": 2,
            "identityWitnessArchiveOpenCount": 300,
            "overallDecisionExecutionArchiveOpenCount": 1966,
        ...
        "closure": {
            "wave11IdentityResolved": True,
            "wave11AcquisitionReady": True,
            "wave11AcquisitionComplete": False,
        ...
        "nextAction": (
            "prepare_separate_one_use_18_resource_wave11_source_"
...
def validate_semantic_decision(
    ...
    combined = (
        predecessors.get("combinedFixedPointV9")
    ...
    require(
        exact_keys(predecessors, {"combinedFixedPointV9"})
        ...
        and combined["v8TestsLiveHeld"] is False
        and all(
    ...
    require(
        ...
        and held["sourceBindingCount"] == 299
        and held["sourceBindingsSha256"]
        == V9_SOURCE_BINDINGS_SHA256
        and held["archiveCount"] == 150
        and held["externalModCount"] == 149
    ...
    require(
        ...
        == "current_wave11_lowercase_ascii_direct_proxy_path"
        and preparation["claimPath"] == WAVE11_CLAIM_PATH
        and preparation["stagingDirectoryPrefix"]
        == WAVE11_STAGING_PREFIX
        and preparation["acceptedDirectoryPath"]
        == WAVE11_ACCEPTED_PATH
...
def evaluate(
    ...
    *,
    ...
    with ExitStack() as stack:
        ...
        v9_tests_held = stack.enter_context(
            v9.PinnedCodeFile(
                root,
                V9_TESTS_PATH,
                V9_TESTS_RAW_SHA256,
        ...
        package_raw = {
            ...
            V9_CHECKER_PATH: v9_held.raw,
            V9_TESTS_PATH: v9_tests_held.raw,
        }
        held: list[Any] = [
            self_held,
            v9_held,
        ...
        validate_wave11_namespace_absent(
            namespace_anchor_held.parent_fd,
            reader_held.parent_fd,
            self_held.parent_fd,
        )
        identity_barrier(root, held)
        candidate = v9.generate_candidate(root)
        identity_barrier(root, held)
        wave_rows, source_bindings = validate_v9_candidate(candidate)
        v9 = v9.harden_checker_module(v9)
        ...
        require(
            type(runner) is v9.ReadOnlyProviderFacade
            and v8.PinnedCodeFile is v9.PinnedCodeFile
            and v7.PinnedCodeFile is v9.PinnedCodeFile
            and v6.PinnedCodeFile is v9.PinnedCodeFile
            and v5.PinnedCodeFile is v9.PinnedCodeFile
            and v4.PinnedCodeFile is v9.PinnedCodeFile
            and v1.PinnedRunnerFile is v9.SafePinnedRunnerFile,
            "E_SAFE_RUNTIME",
        )
        ...
        require(
            ...
            == source_bindings
            and len(full_source_bindings) == 299
            and all(
                type(row) is dict
    ...
    return expected, {
        ...
        "schemaVersion": "1.0",
        "status":
            "validated_9_of_9_acquisition_ready_not_authorized",
...
def error_document(code: str) -> dict[str, Any]:
    return {
        "documentType":
            "aetherlink.wave11-identity-acquisition-decision-error",
        "schemaVersion": "1.0",
