...
CHECKER_PATH = (
    ROOT / "script/check_p2p_nat_g2_pion_dependency_wave2_success_v2.py"
)
...
class WaveTwoReadbackTests(unittest.TestCase):
    ...
    def record_for(self, item) -> dict:
        return {
            ...
            "modLinkCount": 1,
            "zipLinkCount": 1,
            "compressionRatioLimitPassed": True,
        }
    ...
    def test_13_ratio_gate_drift_is_rejected(self) -> None:
        item = self.items[0]
        record = self.record_for(item)
        record["compressionRatioLimitPassed"] = False
        with self.assertRaises(COMMON.Wave2Failure):
            CHECKER.exact_source_record(item, record)
