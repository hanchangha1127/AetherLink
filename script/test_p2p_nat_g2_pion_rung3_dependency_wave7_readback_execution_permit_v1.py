#!/usr/bin/env python3
"""Tests for the Wave7 acquisition readback permit package."""
...
SPEC = importlib.util.spec_from_file_location("wave7_readback_permit_tests", PATH)
C = importlib.util.module_from_spec(SPEC)
assert SPEC.loader is not None
SPEC.loader.exec_module(C)


class Wave7ReadbackPermitTests(unittest.TestCase):
    def test_01_frozen_snapshot_constants_are_exact(self):
        self.assertEqual(C.ATTEMPT_ID, "c15f4504ae880326144eca93dc91e37b")
        ...
        self.assertEqual(payload["frozenFileCount"], 48)
        self.assertEqual(payload["selectedTupleCount"], 0)
        self.assertEqual(payload["aggregateZipEntryCount"], 6_469)
        self.assertEqual(
        ...
        self.assertEqual(
            payload["predecessorBindings"]["combinedFixedPointV5"][
                "contentSha256"
            ],
            C.EXPECTED_V5_CONTENT,
        )
