#!/usr/bin/env python3
"""Tests for the Wave11 acquisition readback permit package."""
...
def _deny_test_network(*_args, **_kwargs):
    ...
    raise AssertionError(
        "offline Wave11 readback tests must never create network connections"
    )
...
SPEC = importlib.util.spec_from_file_location("wave11_readback_permit_tests", PATH)
C = importlib.util.module_from_spec(SPEC)
assert SPEC.loader is not None
SPEC.loader.exec_module(C)


class Wave11ReadbackPermitTests(unittest.TestCase):
    def tearDown(self) -> None:
        self.assertEqual(NETWORK_ATTEMPTS, [])

    def test_01_frozen_snapshot_constants_are_exact(self):
        self.assertEqual(C.ATTEMPT_ID, "ac18b8fda0a80a132510efd5dd17d5b7")
        self.assertEqual(len(C.ACQUISITION_AUTHORITY), 14)
        self.assertEqual(len(C.ACCEPTED_FILES), 18)
        self.assertEqual(
        ...
        self.assertEqual(payload["frozenFileCount"], 36)
        self.assertEqual(
        ...
        self.assertEqual(payload["aggregateZipEntryCount"], 3_329)
        self.assertEqual(
        ...
        self.assertEqual(
            payload["predecessorBindings"]["combinedFixedPointV9"][
                "contentSha256"
            ],
            C.EXPECTED_V9_CONTENT,
        )
        self.assertFalse(
            any(
                "combined_fixed_point_v" + "8.py" in path
    ...
    def test_03_exact_package_and_package_only_preflight(self):
        ...
        self.assertEqual(summary["acceptedResourceCount"], 18)
        self.assertEqual(summary["selectedTupleCount"], 0)
        self.assertEqual(summary["selectedRequestOrdinals"], [])
    ...
    def test_08_stale_cardinality_selector_and_v8_mutations_reject(self):
        raw = C.package_raw(False)
        original = C.content_bound(C.expected_payload_from_package(raw))
        for mutation in (
            ...
            lambda value: value["frozenAcquisitionSnapshot"].__setitem__(
                "predecessorBindings",
                {
                    "combinedFixedPointV" + "8": value[
                        "frozenAcquisitionSnapshot"
                    ]["predecessorBindings"]["combinedFixedPointV9"]
                },
            ),
        ):
            changed = copy.deepcopy(original)
        ...
        for package_path in package_paths:
            source = (C.ROOT / package_path).read_text(encoding="utf-8")
        ...
        for production_path in (
            ...
            source = (C.ROOT / production_path).read_text(encoding="utf-8")
            self.assertNotIn('"requestCount": ' + "22", source)
            self.assertNotIn('"acceptedResourceCount": ' + "22", source)
            self.assertNotIn("len(resources) == " + "22", source)
            self.assertNotIn("exact" + "22", source)
            self.assertNotIn("exact" + "40", source)
            self.assertNotIn("combinedFixedPointV" + "8", source)
