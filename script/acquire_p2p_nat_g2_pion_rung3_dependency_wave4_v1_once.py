#!/usr/bin/env python3
"""Consume the Wave4 v1 permit once and acquire 32 verified resources."""
...
if not (
    ...
    raise RuntimeError("Wave4 acquisition requires `python3 -I -B -S`")
...
EXPECTED_CHECKER_RAW = "37a0266f3b4310f1980c70d26cfd10b98bb32ebf4e81f96193e40d4ebb9c0dbd"
WAVE3_RUNNER_PATH = Path(__file__).with_name(
    "acquire_p2p_nat_g2_pion_rung3_dependency_wave3_v1_once.py"
)
EXPECTED_WAVE3_RUNNER_RAW = (
...
WAVE3, WAVE3_RUNNER_RAW = _load_exact(
    WAVE3_RUNNER_PATH,
    EXPECTED_WAVE3_RUNNER_RAW,
    "wave3_acquisition_primitives_v1",
)
...
def _assert_primitive_contract() -> None:
    require(
        sha256(WAVE3_RUNNER_RAW) == CHECK.EXPECTED_WAVE3_RUNNER_RAW
        and WAVE3.CHECK.PROXY_HOST == CHECK.PROXY_HOST
        and WAVE3.CHECK.MAX_MOD_BYTES == CHECK.MAX_MOD_BYTES
        and WAVE3.CHECK.MAX_ZIP_BYTES == CHECK.MAX_ZIP_BYTES
        and WAVE3.CHECK.MAX_AGGREGATE_BYTES == CHECK.MAX_AGGREGATE_BYTES
        and WAVE3.CHECK.MAX_HEADER_BYTES == CHECK.MAX_HEADER_BYTES
        and WAVE3.CHECK.PER_REQUEST_DEADLINE_MS
        == CHECK.PER_REQUEST_DEADLINE_MS
        and WAVE3.CHECK.WHOLE_ATTEMPT_DEADLINE_MS
        == CHECK.WHOLE_ATTEMPT_DEADLINE_MS
        and callable(WAVE3.direct_fetch)
        and callable(WAVE3.validate_mod)
        and callable(WAVE3.validate_zip)
        and callable(WAVE3.decode_h1),
        "E_PRIMITIVE_BINDING",
        "bootstrap",
    )
...
def _primitive_call(function: Callable[..., Any], *args: Any) -> Any:
    ...
    except WAVE3.AcquisitionError as error:
...
def direct_fetch(resource: Mapping[str, Any], deadline: float) -> bytes:
    ...
    return _primitive_call(WAVE3.direct_fetch, resource, deadline)
...
class ExecutionNamespace:
    ...
    def barrier(self, state: ExecutionState) -> None:
        ...
        if state in {
            ...
            require(
                len(self.resources) == 32 and self.evidence is not None,
                "E_CARDINALITY",
                phase,
            )
...
def preflight() -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    require(
        ...
        and permit["authority"]["wave4SourceAcquisitionAuthorizedOnce"]
        is True
...
def _validate_resource_contract(
    ...
    require(
        type(rows) is list
        and permit["requestContract"]["requestCount"] == 32
        and len(rows) == 32
    ...
    for index, source in enumerate(rows):
        ...
        require(
            ...
            and row["tupleId"] == f"wave4-{tuple_order:03d}-{digest[:12]}"
            and row["kind"] == kind
            and row["method"] == "GET"
        ...
        _primitive_call(WAVE3.decode_h1, row["expectedH1"], "preflight")
        result.append(row)
    ...
    require(
        len({row["acceptedFileName"] for row in result}) == 32
        and len({row["url"] for row in result}) == 32
        and sum(
...
def _attempt(
    ...
    *,
    ...
    claim = {
        "documentType": "aetherlink.wave4-source-acquisition-claim",
        "schemaVersion": "1.0",
    ...
    try:
        ...
        evidence_payload = {
            "documentType": "aetherlink.wave4-source-acquisition-evidence",
            "schemaVersion": "1.0",
            "attemptId": attempt_id,
            "requestCount": expected_count,
        ...
        receipt = {
            "documentType": "aetherlink.wave4-source-acquisition-receipt",
            "schemaVersion": "1.0",
        ...
        manifest = {
            "documentType": "aetherlink.wave4-source-acquisition-manifest",
            "schemaVersion": "1.0",
            "status": "consumed_success_pending_independent_readback",
            "attemptId": attempt_id,
        ...
        failure = {
            "documentType": "aetherlink.wave4-source-acquisition-failure",
            "schemaVersion": "1.0",
...
def validate_execution_context() -> dict[str, Any]:
    ...
    return {
        "documentType":
            "aetherlink.wave4-source-acquisition-execution-context-check",
        "schemaVersion": "1.0",
...
def error_document(error: AcquisitionError) -> dict[str, Any]:
    ...
    return {
        "documentType": "aetherlink.wave4-source-acquisition-error",
        "schemaVersion": "1.0",
        "status": status,
