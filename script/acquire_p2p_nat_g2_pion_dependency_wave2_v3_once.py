#!/usr/bin/env python3
"""Acquire the exact G2 Pion dependency wave-two v3 MOD+ZIP set exactly once.
...
EXPECTED_PERMIT_CHECKER_RAW_SHA256 = (
    "7a7b01e761b220f9ad572dec495eb545e30583da019700305e0c0161c18d9261"
)
EXPECTED_PERMIT_STATUS = (
    "wave2_v3_dependency_source_acquisition_authorized_not_consumed"
)
...
EXPECTED_PERMIT_NEXT_ACTION = (
    "execute_bound_dependency_source_wave2_v3_once"
)
EXPECTED_PERMIT_ID = (
...
PERMIT_CHECKER_BOOTSTRAP = execute_module(
    "g2_wave2_permit_checker_bootstrap",
    PERMIT_CHECKER_PATH,
    bootstrap_read(
        PERMIT_CHECKER_PATH,
        EXPECTED_PERMIT_CHECKER_RAW_SHA256,
...
def authority_bindings(
    ...
    bindings = [
        dict(row)
        for row in PERMIT_CHECKER_BOOTSTRAP.preparation_bindings(
            include_permit=True,
        )
    ]
    for row in bindings:
        if row["path"] == PERMIT_CHECKER_PATH:
            row["rawSha256"] = EXPECTED_PERMIT_CHECKER_RAW_SHA256
    seen = {row["path"] for row in bindings}
    for row in terminal_artifact_bindings(terminal_state):
...
def load_authority(
    *,
    ...
    try:
        ...
        runner_binding = tool_bindings.get("wave2_v3_runner")
        if (
...
def validate_readback_terminal(
    ...
    if (
        ...
        != "wave2_v3_independent_readback_complete"
        ...
        or readback.get("compressionRatioPolicy")
        != "non_gating_bounded_telemetry"
        or readback.get("compressionRatioPolicy")
        != acquisition_receipt.get("compressionRatioPolicy")
        or readback.get("archiveCountExceedingHistoricalV2Ratio")
        != acquisition_receipt.get(
            "archiveCountExceedingHistoricalV2Ratio"
        )
        ...
        or readback_manifest.get("compressionRatioPolicy")
        != readback.get("compressionRatioPolicy")
        or readback_manifest.get(
            "archiveCountExceedingHistoricalV2Ratio"
        )
        != readback.get("archiveCountExceedingHistoricalV2Ratio")
...
def validate_claim_and_terminal(
    ...
    if (
        claim.get("claimType")
        != "aetherlink.g2-pion-dependency-wave2-v3-one-use-claim"
    ...
    if classification == "failure":
        ...
        if (
            artifact.get("status")
            != "wave2_v3_acquisition_failed_permit_consumed"
    ...
    if (
        ...
        or artifact.get("compressionRatioPolicy")
        != "non_gating_bounded_telemetry"
        or type(
            artifact.get("archiveCountExceedingHistoricalV2Ratio")
        )
        is not int
        or not (
            0
            <= artifact["archiveCountExceedingHistoricalV2Ratio"]
            <= 15
        )
    ):
        raise COMMON.Wave2Failure("E_TERMINAL_STATE", "preflight")
    ...
    if (
        ...
        or manifest.get("compressionRatioPolicy")
        != artifact.get("compressionRatioPolicy")
        or manifest.get("archiveCountExceedingHistoricalV2Ratio")
        != artifact.get("archiveCountExceedingHistoricalV2Ratio")
...
def preflight() -> dict[str, Any]:
    ...
    try:
        ...
        if classification == "clean":
            ...
            next_action = "run_separate_wave2_v3_independent_readback"
        ...
        return {
            "documentType": (
                "aetherlink.g2-pion-dependency-wave2-v3-runner-preflight"
            ),
            "schemaVersion": "1.0",
            "status": status,
...
def directory_snapshot(info: os.stat_result) -> tuple[int, ...]:
    return (
        info.st_dev,
        info.st_ino,
        info.st_mode,
...
def verify_published_snapshot(
    ...
    inputs: Any,
    directory_guards: Sequence[Mapping[str, Any]],
    artifact_guards: Sequence[COMMON.HeldFile],
    ...
    for guard in directory_guards:
        ...
        named = os.stat(
            str(guard["name"]),
            dir_fd=int(guard["parentFd"]),
            follow_symlinks=False,
        )
        if (
            directory_snapshot(current) != guard["identity"]
            or directory_snapshot(named) != guard["identity"]
        ):
            raise COMMON.Wave2Failure("E_TOCTOU", "publication")
    if (
        legacy.list_names(wave_parent_fd)
        != [COMMON.FINAL_DIRECTORY_NAME]
        or legacy.list_names(published_fd)
        != COMMON.expected_resource_names(items)
        or staging_names()
...
def failure_document(
    ...
    *,
    ...
    return {
        "documentType": (
            "aetherlink.g2-pion-dependency-wave2-v3-acquisition-failure"
        ),
        "schemaVersion": "1.0",
        "status": "wave2_v3_acquisition_failed_permit_consumed",
...
def _execute_once_with_umask() -> dict[str, Any]:
    ...
    root_fd = parent_fd = wave_parent_fd = staging_fd = published_fd = -1
    staging_name: str | None = None
    claim_sha256: str | None = None
    claim_attempted = False
    publication_attempted = False
    held_outputs: list[dict[str, Any]] = []
    artifact_guards: list[COMMON.HeldFile] = []
    directory_guards: list[dict[str, Any]] = []
    ...
    try:
        ...
        artifact_guards.append(
            COMMON.HeldFile(
                ...
                maximum_bytes=64 * 1024,
                expected_sha256=str(claim_sha256),
                owner_only=True,
            )
        )
        ...
        archive_count_exceeding_historical_v2_ratio = sum(
            1
            for row in rows
            if row["compressionTelemetry"][
        ...
        if directory_snapshot(os.fstat(published_fd)) != directory_snapshot(
            os.fstat(staging_fd)
        ):
            raise COMMON.Wave2Failure("E_TOCTOU", "publication")
        with legacy.hard_wall_clock_request_deadline(
        ...
        directory_guards = [
            {
                "fd": parent_fd,
                "parentFd": root_fd,
                "name": COMMON.DEPENDENCY_PARENT.as_posix(),
                "label": str(COMMON.DEPENDENCY_PARENT),
                "identity": directory_snapshot(os.fstat(parent_fd)),
            },
            {
                "fd": wave_parent_fd,
                "parentFd": parent_fd,
                "name": COMMON.WAVE_PARENT_NAME,
                "label": COMMON.WAVE_PARENT_NAME,
                "identity": directory_snapshot(os.fstat(wave_parent_fd)),
            },
            {
                "fd": published_fd,
                "parentFd": wave_parent_fd,
                "name": COMMON.FINAL_DIRECTORY_NAME,
                "label": COMMON.FINAL_DIRECTORY_NAME,
                "identity": directory_snapshot(os.fstat(published_fd)),
            },
        ]
        os.fsync(parent_fd)
        ...
        receipt = {
            "documentType": (
                "aetherlink.g2-pion-dependency-wave2-v3-acquisition-receipt"
            ),
            "schemaVersion": "1.0",
            ...
            **{name: counters[name] for name in COMMON.COUNTER_NAMES},
            ...
            "archiveCountExceedingHistoricalV2Ratio": (
                archive_count_exceeding_historical_v2_ratio
            ),
            ...
            "externalAuthenticationRequired": False,
            "userActionRequired": False,
            "nextAction": "run_separate_wave2_v3_independent_readback",
        }
        receipt_raw = COMMON.canonical_json_bytes(receipt)
        ...
        artifact_guards.append(
            COMMON.HeldFile(
                root_fd,
                COMMON.SUCCESS_RECEIPT_PATH,
                maximum_bytes=COMMON.MAXIMUM_JSON_BYTES,
        ...
        manifest = {
            "documentType": (
                "aetherlink.g2-pion-dependency-wave2-v3-acquisition-manifest"
            ),
            "schemaVersion": "1.0",
            "status": (
                "wave2_v3_acquisition_publication_complete_pending_"
        ...
        return {
            "documentType": (
                "aetherlink.g2-pion-dependency-wave2-v3-runner-result"
            ),
            "schemaVersion": "1.0",
            "status": "acquired_pending_independent_readback",
            **{name: counters[name] for name in COMMON.COUNTER_NAMES},
        ...
        if (
            ...
            try:
                ...
                failure_raw = COMMON.canonical_json_bytes(document)
                failure_sha256 = legacy.write_repo_relative_artifact(
                    root_fd,
                    COMMON.FAILURE_RECEIPT_PATH,
                    failure_raw,
                    COMMON.MAXIMUM_JSON_BYTES,
                )
                failure_guard = COMMON.HeldFile(
                    root_fd,
                    COMMON.FAILURE_RECEIPT_PATH,
                    maximum_bytes=COMMON.MAXIMUM_JSON_BYTES,
                ...
                artifact_guards.append(failure_guard)
                if (
                    staging_names()
                    or path_exists(ROOT / COMMON.FINAL_DIRECTORY_PATH)
                    or path_exists(ROOT / COMMON.SUCCESS_RECEIPT_PATH)
                    ...
                    or (
                        wave_parent_fd >= 0
                        and legacy.list_names(wave_parent_fd)
                    )
                ):
                    raise COMMON.Wave2Failure(
        ...
        for guard in reversed(artifact_guards):
            guard.close()
        for record in held_outputs:
            legacy.close_quietly(record["fd"])
...
def error_document(failure: Any) -> dict[str, Any]:
    ...
    return {
        "documentType": (
            "aetherlink.g2-pion-dependency-wave2-v3-runner-error"
        ),
        "schemaVersion": "1.0",
        "status": (
