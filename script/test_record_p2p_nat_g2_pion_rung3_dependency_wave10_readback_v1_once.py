#!/usr/bin/env python3
"""Tests for the one-use offline Wave10 acquisition readback recorder."""
...
PATH = Path(__file__).with_name(
    "record_p2p_nat_g2_pion_rung3_dependency_wave10_readback_v1_once.py"
)
SPEC = importlib.util.spec_from_file_location("wave10_readback_recorder_tests", PATH)
...
class Wave10ReadbackRecorderTests(unittest.TestCase):
    ...
    def test_05_live_retained_snapshot_verifies_twice_read_only(self):
        ...
        try:
            ...
            self.assertEqual(first["acceptedResourceCount"], 22)
            self.assertEqual(first["authorityFileCount"], 14)
            self.assertEqual(first["selectedTupleCount"], 1)
            self.assertEqual(first["selectedRequestOrdinals"], [21, 22])
            self.assertEqual(first["aggregateAcceptedBytes"], 27_773_526)
            self.assertEqual(first["aggregateZipEntryCount"], 4_872)
            self.assertEqual(
    ...
    def test_05_selector_counts_and_v8_binding_fail_closed(self):
        snapshot = R.FrozenSnapshot()
        permit_path = (
            R.PERMIT.BASE
        ...
        mutations = []
        selected_non_bool = copy.deepcopy(
            original["requestContract"]["resources"]
        )
        selected_non_bool[20]["selectedByGraphAlgorithm"] = 1
        mutations.append(selected_non_bool)
        for value in (0, "false", None):
            unselected_non_bool = copy.deepcopy(
                original["requestContract"]["resources"]
            )
            unselected_non_bool[0]["selectedByGraphAlgorithm"] = value
            mutations.append(unselected_non_bool)
        unselected_missing = copy.deepcopy(
            original["requestContract"]["resources"]
        )
        del unselected_missing[0]["selectedByGraphAlgorithm"]
        mutations.append(unselected_missing)
        none_selected = copy.deepcopy(
            original["requestContract"]["resources"]
        )
        none_selected[20]["selectedByGraphAlgorithm"] = False
        none_selected[21]["selectedByGraphAlgorithm"] = False
        mutations.append(none_selected)
        wrong_pair = copy.deepcopy(original["requestContract"]["resources"])
        wrong_pair[0]["selectedByGraphAlgorithm"] = True
        wrong_pair[1]["selectedByGraphAlgorithm"] = True
        wrong_pair[20]["selectedByGraphAlgorithm"] = False
        wrong_pair[21]["selectedByGraphAlgorithm"] = False
    ...
    def test_10_synthetic_execute_orders_claim_two_passes_receipt_manifest(self):
        ...
        def verify(snapshot):
            ...
            return {"acceptedResourceCount": 22}
    ...
    def test_12_receipt_only_gap_is_terminal_uncertainty(self):
        ...
        with mock.patch.object(
            ...
            R,
            "verify_snapshot",
            return_value={"acceptedResourceCount": 22},
        ), mock.patch.object(R, "atomic_publish", side_effect=publish):
            with self.assertRaises(R.ReadbackError) as caught:
                R.execute(
                    Path("/unused"),
    ...
    def test_21_wave10_uncompressed_limits_are_per_zip_and_across_all(self):
        self.assertEqual(
