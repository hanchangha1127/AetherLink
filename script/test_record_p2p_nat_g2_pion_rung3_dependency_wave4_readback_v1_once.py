#!/usr/bin/env python3
"""Tests for the one-use offline Wave4 acquisition readback recorder."""
...
PATH = Path(__file__).with_name(
    "record_p2p_nat_g2_pion_rung3_dependency_wave4_readback_v1_once.py"
)
SPEC = importlib.util.spec_from_file_location("wave4_readback_recorder_tests", PATH)
...
class Wave4ReadbackRecorderTests(unittest.TestCase):
    ...
    def test_05_live_retained_snapshot_verifies_twice_read_only(self):
        ...
        try:
            ...
            self.assertEqual(first["selectedTupleCount"], 3)
            self.assertEqual(first["aggregateAcceptedBytes"], 24_118_812)
            self.assertEqual(first["aggregateZipEntryCount"], 5_936)
            self.assertEqual(
