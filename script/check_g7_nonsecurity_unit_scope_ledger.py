...
def validate_entries(
    ...
    *,
    ...
    for index, item in enumerate(value):
        ...
        if method_name not in cached_sources[source_path]:
    ...
    *,
    ...
    for identity in swift_identities:
        ...
        candidates = [
            ...
            if re.search(rf"\bfunc\s+{re.escape(method)}\s*\(", source)
            and (
                re.search(rf"\bclass\s+{re.escape(class_name)}\b", source)
                or re.search(rf"\bextension\s+{re.escape(class_name)}\b", source)
            )
        ]
        if len(candidates) != 1:
            raise ScopeLedgerError(
...
def main() -> int:
    ...
    parser.add_argument(
        "--evidence",
        action="store_true",
        help="also bind every eligible identity to current execution evidence",
    )
    parser.add_argument(
        "--allow-unpinned-ledger",
        action="store_true",
    ...
    try:
        _ledger, swift_entries, android_entries = read_and_validate_ledger(
            require_pin=not arguments.allow_unpinned_ledger,
        )
        ...
        summary = scope_summary(
            swift_entries,
            android_entries,
            evidence_validated=arguments.evidence,
        )
