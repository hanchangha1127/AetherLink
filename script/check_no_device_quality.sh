...
run swift test --filter 'AetherLinkLocalizationTests/testRuntimeMemoryInspectorCopyLocalizesAcrossSupportedLanguages|AetherLinkRenderSmokeTests/testRuntimeMemoryInspectorRendersAcrossLanguagesAndAppearances'
	...
	run swift test --filter SQLiteRuntimeDocumentIndexStoreTests
		...
		run swift test --filter RuntimeDocumentCitationGovernanceTests
			...
			run swift test --filter 'LocalRuntimeMessageRouterTests/testChatSendConsumesCurrentDeviceGrantAsBackendOnlyTrustedSourceContext|LocalRuntimeMessageRouterTests/testChatSendTrustedSourceContextFailsClosedWhenAuditPersistenceFails|LocalRuntimeMessageRouterTests/testChatSendTrustedSourceContextRespectsModelWindowWithoutPersistingContext|LocalRuntimeMessageRouterTests/testChatSendRejectsMalformedOrWrongDeviceTrustedSourceGrantsBeforeBackend'
...
echo "Covered protocol reserved RAG/research namespace guard addendum: approved semantic retrieval is active only through retrieval.query; embeddings., unsupported retrieval.* beyond retrieval.query, unsupported index.* beyond index.documents.list, research., citation., and source_control. active messages remain blocked until the remaining indexing, research, citation, trusted-source, source-control, and permission semantics are designed."
...
echo "Covered RuntimeDevServer reserved RAG/research namespace rejection addendum: authenticated RuntimeDevServer relay smoke keeps approved semantic retrieval on retrieval.query and rejects embeddings.create, index.build, research.brief.create, citation.sources.list, and source_control.status with unknown_message_type before separate embedding-generation, indexing, research, citation, source-control, or client trusted-source paths exist."
