...
echo "Covered Android memory.list selected embedding-model isolation addendum: Android RuntimeClientViewModel keeps selected embedding_model_id out of memory.list query payloads even when a memory-indexing model is selected, so current memory search remains deterministic lexical filtering before semantic memory search, embedding-backed ranking, source approval, citation, trusted-source review, permission, or audit semantics exist."
