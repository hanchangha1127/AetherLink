...
run ./gradlew --no-daemon \
  ...
  --tests com.localagentbridge.android.runtime.RuntimeClientViewModelTest.refreshRuntimeMemorySendsTrimmedQueryAndRedactsSearchMetadataFromDeviceStorage \
	  ...
	  --tests com.localagentbridge.android.runtime.RuntimeClientViewModelTest.runtimeDocumentMetadataDropsNonCanonicalContentFingerprintsFromTransientState \
...
echo "Covered Android document catalog summary transient-state bounds addendum: Android RuntimeClientViewModel now coerces index.documents.list summary document_count, chunk_count, extracted_character_count, and quality_counts to nonnegative transient RuntimeUiState values before local persistence, chat context injection, semantic retrieval, source approval, citation, trusted-source review, permission, or audit semantics exist."
...
echo "Covered Android document content-fingerprint transient-state canonicality addendum: Android RuntimeClientViewModel now preserves only exact 16-lowercase-hex content_fingerprint values in transient document state across catalog and retrieval search rows, clearing whitespace-mutated, uppercase, and overlong fingerprints before local persistence, chat context injection, semantic retrieval, source approval, citation, trusted-source review, permission, or audit semantics exist."
echo "Covered Android document MIME transient-state canonicality addendum: Android RuntimeClientViewModel now preserves only exact lowercase type/subtype MIME values up to 128 characters in transient document state across catalog and retrieval search rows, replacing whitespace-mutated, uppercase, parameterized, URL-shaped, and overlong MIME values with application/octet-stream before local persistence, chat context injection, semantic retrieval, source approval, citation, trusted-source review, permission, or audit semantics exist."
echo "Covered Android document quality/chunk-count transient-state consistency addendum: Android RuntimeClientViewModel now derives transient document quality from the nonnegative chunk_count envelope across catalog and retrieval search rows, mapping zero or negative chunks to no_usable_text, one chunk to single_chunk, and two or more chunks to chunked before local persistence, chat context injection, semantic retrieval, source approval, citation, trusted-source review, permission, or audit semantics exist."
echo "Covered Android document id/display-name transient-state bounds addendum: Android RuntimeClientViewModel now keeps transient document ids nonblank, control-free, and within 128 characters with response-local fallbacks, and keeps display names as bounded final path components or untitled-document across catalog and retrieval search rows before local persistence, chat context injection, semantic retrieval, source approval, citation, trusted-source review, permission, or audit semantics exist."
...
echo "Covered Android document response row transient-state cap addendum: Android RuntimeClientViewModel now caps decoded index.documents.list catalog rows to 100 and retrieval.query search rows to 10 before they reach transient RuntimeUiState, matching Android request limits before local persistence, chat context injection, semantic retrieval, source approval, citation, trusted-source review, permission, or audit semantics exist."
