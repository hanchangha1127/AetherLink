...
assert summary["exit_status"] == 0, summary
assert summary["mode"]["dry_run"] is True, summary
assert summary["paths"]["output"] == str(output_path), summary
...
echo "Covered chat.messages.list invalid allowed type rejection addendum: RuntimeDevServer chat.messages.list payloads reject string and fractional limit values before runtime chat store dispatch."
...
echo "Covered memory.upsert invalid allowed type rejection addendum: RuntimeDevServer memory.upsert payloads reject non-string id and non-boolean enabled values before runtime memory store mutation."
