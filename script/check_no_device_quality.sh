...
echo "Covered G2 Pion v4.3.0 rung-two acquisition-authority addendum: status=rung2_source_identity_decision_recorded_acquisition_not_executed; permit=authorized_not_consumed; requestCount=0. The exact official Go proxy URL, upstream commit/tree, decision-pinned raw ZIP SHA-256, SumDB module h1 and go.mod h1, TLS certificate/hostname validation, atomic one-use permit claim, no redirects/proxies/credentials/retries, and archive-only output are fixed. The offline validator independently verifies the public SumDB Ed25519 signed tree, exact record hash, and ordered 25-node RFC 6962 inclusion proof; 20 mutation tests reject schema drift, proof mutation, permission escalation, false completion, and any owner/external identity or user-action requirement. Raw SHA-256 is a reproducibility pin rather than an independent upstream authentication root. sourceAcquisitionExecuted=false; candidateSelected=false; librarySelected=false; dependencyInstallationAllowed=false; compilerInvocationAllowed=false; codeLoadingAllowed=false; socketCreationAllowed=false; runtimeNetworkIoAllowed=false; deviceExecutionAllowed=false; productionDeploymentAllowed=false; gitOperationAllowed=false; externalIdentityProofRequired=false; userActionRequired=false; repositoryOwnerAuthenticationRequired=false. Product endpoint authentication remains mandatory and separate."
