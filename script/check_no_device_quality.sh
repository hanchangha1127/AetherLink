...
echo "Covered V1 G0 decision and assurance addendum: the versioned product, platform, distribution, twelve-cell plus six-variant network matrix, release-blocking direct-P2P thresholds, two-plane fallback, mandatory service-mediated P2P capabilities, absolute revocation closure, TLS signed-lease, pair-epoch, privacy, measurement-contract, protocol/data-flow/threat/risk/observability/release/incident/rollback, and evidence defaults are machine-validated against their exact source records. The unchanged six-record lineage is published at 12c38154, and the repository records that the exact nine-file receipt/intake successor is published at 70350f5e with a separately observed fresh 9/9 no-alternates remote byte readback; this no-device gate does not fetch or reproduce that observation. The tracked content-addressed receipt sidecar still encodes the parent target/checkpoint/hash/time while remaining dormant and non-authorizing; it does not persist or independently reproduce parent acquisition provenance. The tracked content-addressed sparse owner/catalog intake envelope now contains exactly one publication proposal with two evidence-kind references whose supporting-artifact references are null because both selectors are false. The separate content-addressed evidence supporting-artifact profile hash-binds and exactly projects that item-2 selector snapshot into both closed payload kinds; any selector transition requires a new profile, both reserved artifact instances remain absent, and all authentication, verification, acceptance, closure, activation, G0-exit, and G1a flags remain false. G0 remains blocked on owner acceptance, catalog and authorized baseline gates, activation-capable publication acceptance, production namespaces, distribution/key/provider/service-identity/root/privacy/quality/operations ownership, and relay region/capacity/cost approval; G1a, source acquisition, library selection or compilation, sockets, network I/O, production keys, signing, store upload, and deployment remain closed."
