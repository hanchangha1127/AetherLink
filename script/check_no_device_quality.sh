...
run check_python_syntax \
  ...
  script/check_v1_g0_baseline_evidence_readiness.py \
  script/test_v1_g0_baseline_evidence_readiness.py \
  script/check_v1_g0_independent_validation_context.py \
  ...
  script/check_macos_localization.py \
	  ...
	  script/check_p2p_nat_g2_restricted_fork_profile.py \
	  script/test_p2p_nat_g2_restricted_fork_profile.py \
	  script/check_p2p_nat_g2_pion_rung2_acquisition_authority.py \
	  script/test_p2p_nat_g2_pion_rung2_acquisition_authority.py \
	  script/acquire_p2p_nat_g2_pion_source_once.py \
	  script/test_acquire_p2p_nat_g2_pion_source_once.py \
	  script/check_p2p_nat_g2_pion_rung2_acquisition_receipt.py \
	  script/test_p2p_nat_g2_pion_rung2_acquisition_receipt.py \
	  script/prepare_p2p_nat_g2_pion_rung3_review_decision.py \
	  script/test_prepare_p2p_nat_g2_pion_rung3_review_decision.py \
	  script/check_p2p_nat_g2_pion_rung3_review_decision.py \
	  script/test_p2p_nat_g2_pion_rung3_review_decision.py \
	  script/check_p2p_nat_g2_pion_rung3_success_evidence_v3.py \
	  script/test_p2p_nat_g2_pion_rung3_success_evidence_v3.py \
	  script/check_p2p_nat_g2_pion_rung3_semantic_review_decision_v1.py \
	  script/test_p2p_nat_g2_pion_rung3_semantic_review_decision_v1.py \
	  script/check_p2p_nat_g2_pion_rung3_semantic_review_result_v1.py \
	  script/test_p2p_nat_g2_pion_rung3_semantic_review_result_v1.py \
	  ...
	  script/check_p2p_nat_g2_pion_rung3_implementation_dependency_review_decision_v1.py \
	  script/test_p2p_nat_g2_pion_rung3_implementation_dependency_review_decision_v1.py \
	  ...
	  script/test_acquire_p2p_nat_g2_pion_dependency_wave1_once.py \
	  ...
	  script/check_p2p_nat_g2_pion_dependency_source_review_wave1_execution_permit_v1.py \
	  script/test_p2p_nat_g2_pion_dependency_source_review_wave1_execution_permit_v1.py \
	  ...
	  script/test_record_p2p_nat_g2_pion_dependency_source_review_wave1_readback_v1.py \
	  ...
	  script/check_p2p_nat_libnice_acquisition_authority.py \
	  script/test_p2p_nat_libnice_acquisition_authority.py \
	  script/check_p2p_nat_glib_acquisition_authority.py \
	  script/test_p2p_nat_glib_acquisition_authority.py \
	  script/extract_p2p_nat_libnice_source.py \
	  script/extract_p2p_nat_glib_source.py \
	  script/record_p2p_nat_libnice_source_manifest.py \
	  script/record_p2p_nat_glib_source_manifest.py \
	  script/check_p2p_nat_libnice_source_audit.py \
	  script/test_p2p_nat_libnice_source_audit.py \
...
# Their behavior is covered by the unit tests above, but running them directly
# against this personal-project worktree would reintroduce an inactive owner-
# governance gate and block ordinary G1a no-network development.
...
run python3 script/check_release_version_ledger.py
...
run python3 -m unittest script/test_p2p_nat_g2_pion_rung2_acquisition_receipt.py
run python3 script/prepare_p2p_nat_g2_pion_rung3_review_decision.py --check
run python3 -m unittest script/test_prepare_p2p_nat_g2_pion_rung3_review_decision.py
run python3 -c 'import sys, unittest; from script.test_p2p_nat_g2_pion_rung3_review_decision import G2PionRungThreePreparationCheckerTests as Case; names = [name for name in unittest.defaultTestLoader.getTestCaseNames(Case) if not name.startswith("test_26_")]; result = unittest.TextTestRunner(verbosity=2).run(unittest.TestSuite(Case(name) for name in names)); sys.exit(0 if result.wasSuccessful() and result.testsRun == 26 else 1)'
run python3 -I -B -S script/check_p2p_nat_g2_pion_rung3_success_evidence_v3.py
run python3 -I -B -S script/test_p2p_nat_g2_pion_rung3_success_evidence_v3.py
run python3 -I -B -S script/check_p2p_nat_g2_pion_rung3_semantic_review_decision_v1.py
run python3 -I -B -S script/test_p2p_nat_g2_pion_rung3_semantic_review_decision_v1.py
run python3 -I -B -S script/check_p2p_nat_g2_pion_rung3_semantic_review_result_v1.py
run python3 -I -B -S script/test_p2p_nat_g2_pion_rung3_semantic_review_result_v1.py
run python3 -I -B -S script/check_p2p_nat_g2_pion_rung3_patch_dependency_decision_v1.py
run python3 -I -B -S script/test_p2p_nat_g2_pion_rung3_patch_dependency_decision_v1.py
run python3 -I -B -S script/check_p2p_nat_g2_pion_rung3_implementation_dependency_review_decision_v1.py
run python3 -I -B -S script/test_p2p_nat_g2_pion_rung3_implementation_dependency_review_decision_v1.py
run python3 -I -B -S script/test_p2p_nat_g2_pion_rung3_dependency_wave1_decision_v1.py
run python3 -I -B -S script/test_acquire_p2p_nat_g2_pion_dependency_wave1_once.py
run python3 -I -B -S -c 'import runpy, sys, unittest; namespace = runpy.run_path("script/test_p2p_nat_g2_pion_dependency_wave1_execution_permit_v1.py", run_name="wave1_v1_permit_historical_tests"); Case = namespace["DependencyWaveOnePermitCheckerTests"]; excluded = {"test_01_repository_baseline_passes_without_execution", "test_37_main_success_output_requires_no_user_authentication"}; names = [name for name in unittest.defaultTestLoader.getTestCaseNames(Case) if name not in excluded]; result = unittest.TextTestRunner(verbosity=2).run(unittest.TestSuite(Case(name) for name in names)); sys.exit(0 if result.wasSuccessful() and result.testsRun == 36 else 1)'
run python3 -I -B -S script/check_p2p_nat_g2_pion_dependency_wave1_recovery_decision_v1.py
run python3 -I -B -S script/test_p2p_nat_g2_pion_dependency_wave1_recovery_decision_v1.py
run python3 -I -B -S script/test_acquire_p2p_nat_g2_pion_dependency_wave1_v2_once.py
run python3 -I -B -S script/check_p2p_nat_g2_pion_dependency_wave1_execution_permit_v2.py
run python3 -I -B -S script/test_p2p_nat_g2_pion_dependency_wave1_execution_permit_v2.py
run python3 -I -B -S script/test_p2p_nat_g2_pion_dependency_wave1_recovery_decision_v2.py
run python3 -I -B -S script/test_acquire_p2p_nat_g2_pion_dependency_wave1_v3_once.py
run python3 -I -B -S script/test_p2p_nat_g2_pion_dependency_wave1_execution_permit_v3.py
run python3 -I -B -S script/test_p2p_nat_g2_pion_dependency_wave1_success_v3.py
...
run python3 script/check_p2p_nat_libnice_acquisition_authority.py
run python3 -m unittest script/test_p2p_nat_libnice_acquisition_authority.py
run python3 script/check_p2p_nat_glib_acquisition_authority.py
run python3 -m unittest script/test_p2p_nat_glib_acquisition_authority.py
run python3 script/record_p2p_nat_libnice_source_manifest.py --verify
run python3 script/record_p2p_nat_glib_source_manifest.py --verify
run python3 script/check_p2p_nat_libnice_source_audit.py
run python3 -m unittest script/test_p2p_nat_libnice_source_audit.py
...
run python3 script/check_production_g1a_c_vectors.py
run python3 -m unittest script/test_production_g1a_c_vectors.py
run python3 script/check_production_g1a_c_candidate_vectors.py
run python3 -m unittest script/test_production_g1a_c_candidate_vectors.py
...
# The checker proves all 752 unique entries still name real @Test methods and that none remains an active Gradle selector.
...
--tests com.localagentbridge.android.runtime.RuntimeClientViewModelTest.productionCompositionRejectsTrustedStoreWithoutExactPairingStoreOwnership
--tests com.localagentbridge.android.runtime.RuntimeClientViewModelTest.androidTrustedStoreProvidesItsExactPairingStoreForProductionComposition
--tests com.localagentbridge.android.runtime.RuntimeClientViewModelTest.viewModelClearClosesProductionActivationController
...
run swift test --filter LocalRuntimeMessageRouterTests/testCompanionAppModelLateListenerFailureAllowsSamePortRetryAndIgnoresStaleCallback
...
# Suite-subsumed frontend readiness evidence: first-device navigation, exact-payload QR cache,
# compact active-pairing renders, and accessibility-size active-pairing renders.
# Suite-subsumed named evidence is kept visible for contract guards but is not executed twice.
...
run swift test --filter PairingCoordinatorTests
	...
	run swift test --filter 'LocalRuntimeMessageRouterTests/testChatSendUsesBoundedBackendOnlyGeneratedCompactionSummary|LocalRuntimeMessageRouterTests/testChatSendSkipsCompactionPrepassAndCacheWhenCurrentPromptSkillIsUnavailable|LocalRuntimeMessageRouterTests/testChatSendOversizedGeneratedCompactionSummaryFallsBackDeterministically|LocalRuntimeMessageRouterTests/testGeneratedCompactionSummaryStaysAbsentAfterSQLiteReopenAndSearch|LocalRuntimeMessageRouterTests/testChatCancelDuringGeneratedCompactionPrepassCancelsDerivedGeneration|LocalRuntimeMessageRouterTests/testChatCancelBeforeCompactionPrepassRegistrationPreventsDerivedAndPrimaryDispatch|LocalRuntimeMessageRouterTests/testChatCancelFromDifferentConnectionCannotCancelActiveGeneration|LocalRuntimeMessageRouterTests/testDuplicateActiveChatRequestIDIsRejectedWithoutReplacingOriginalContext|LocalRuntimeMessageRouterTests/testDerivedCompactionGenerationIDIsReservedAgainstPrimaryChatCollision|LocalRuntimeMessageRouterTests/testSecondChatCancelCannotReachBackendWhileOwnedCancellationIsInProgress|LocalRuntimeMessageRouterTests/testChatCancelPersistsIntentWhenBackendGenerationHandoffReturnsNotFound|LocalRuntimeMessageRouterTests/testChatCancelWaitsForAtomicPrimaryBackendRegistration'
			...
			# run swift test --filter 'AetherLinkLocalizationTests/testCompactionCalibrationCopyLocalizesAcrossSupportedLanguages|AetherLinkRenderSmokeTests/testCompactionCalibrationSheetRendersAcrossLanguagesAndAppearances'
...
echo "Covered V1 G0 decision and assurance addendum: the historical versioned product, platform, distribution, network, security, privacy, measurement, protocol/data-flow, threat/risk, observability, release, incident, and rollback records are machine-validated against their exact source bytes. Their embedded owner-acceptance, receipt, and blocked_before_g1a state is historical enterprise assurance and does not block current personal-project reads, edits, builds, tests, first-party compilation, or G1a no-network work. Socket or external-network execution, production keys, signing, store upload, and deployment remain separate technical scopes governed by current user direction, not owner identity proof."
echo "Covered V1 G0 dormant proposal-preview addendum: bounded JSON request bytes are snapshotted and strictly parsed before interpretation; blocker, owner, evidence, change-request, source, and optional artifact references are derived and canonicalized from the pinned V3 graph; oversized or duplicate-key requests, invalid dates, non-integer versions, duplicate or derived selectors, caller-buffer mutation, and state promotion fail closed. The public helper returns only dormant candidate bytes and SHA-256 after exact validator round-trip; it performs no file, network, or process I/O, authentication, evidence verification, receipt acceptance, activation, blocker closure, G0 exit, or G1a transition."
echo "Covered V1 G0 baseline-evidence readiness addendum: the exact versioned profile derives five non-derived evidence shapes from effective V3, cross-binds both not-authorized command profiles and all three ordered steps, and compiles only a deterministic prepared-unverified plan with null authority and runner references and false execution, acquisition, catalog, receipt, closure, G0-exit, and G1a states. The pure static compiler deterministically derives the canonical-assurance and 29-source-readback candidate byte pair from supplied snapshots, and the pair validator rejects kind, order, target, profile, lineage, and source drift while returning only the dormant sentinel. All five reserved candidate artifacts remain absent; exact synthetic payloads still return one dormant non-authorizing sentinel, and the pure entry points perform no file, process, socket, network, or clock I/O."
...
echo "Covered V1 G0 closure-contract addendum: the historical ten-blocker, nine-check, fourteen-role, receipt, and evidence graph remains byte-validated for provenance only. It is not an active personal-project approval workflow and cannot require owner authentication, role receipts, a trusted-time service, an external owner-governance ledger, or a separate G1a authority record."
echo "Historical V1 G0 owner-trust-bootstrap v2 integrity: the preserved profile/checker/tests remain available as non-active enterprise-assurance history, but the default no-device gate does not run them and no user authentication is requested."
echo "Current personal single-owner governance: Owner identity authentication is not required for this personal project. Direct user instruction opens repository reads, edits, builds, tests, and G1a no-network implementation while product pairing and secure-session protections remain required."
...
echo "Covered Android adaptive navigation addendum: Android permanent rail actual window-container width breakpoint."
echo "Covered Android chat performance addendum: Android unchanged-message Markdown parse memoization."
echo "Covered Android reduced-motion addendum: Android reduced-motion static streaming progress and immediate chat scrolling."
echo "Covered macOS reduced-motion addendum: the system Reduce Motion preference removes the app's connection-recovery scroll and pairing QR expiry transitions, the deterministic override can force reduction on but cannot disable the OS preference, and both policy and Status/Pairing render regressions are covered."
echo "Covered macOS visual and focus accessibility addendum: increased-contrast status colors and borders, non-color Runtime History selection, native arrow-key session navigation, recovery and Pairing focus handoff, exactly-once QR expiry announcement, decorative-icon suppression, and focused policy/render regressions."
echo "Covered Android large-font chat addendum: Android large-font multilingual Chat render at 200% font size."
...
echo "Covered Android large-font layout addendum: Android large-font multilingual Settings render at 200% font size."
...
echo "Covered shared release version ledger addendum: Android release and macOS package metadata consume one append-only 1.0.0+1 to 1.0.0+3 ledger while development IDs remain unchanged."
echo "Covered macOS self-contained release package-only addendum: release compilation, deduplicated standard Contents/Resources SwiftPM bundle embedding, semantic/build version metadata, strict ad-hoc sealing, and no app launch."
echo "Covered Android release optimization addendum: release-only R8 code shrinking/obfuscation and resource shrinking configuration uses optimized Android defaults and dependency consumer rules; clean offline artifact evidence is recorded separately."
echo "Covered local release artifact archive addendum: Android V1 Release is arm64-v8a-only, independently read APK badging, R8 outputs, and the macOS arm64 app plus matching dSYM enter one canonical exact-input local container with a source snapshot, checksums, no-overwrite publication, and independent full readback; raw tool debug paths and unavailable upstream Android native symbols remain explicit local/open boundaries."
echo "Covered Android locale-aware plural addendum: English/default one-other, Korean/Japanese/Simplified Chinese other-only, and French one-many-other resource categories."
...
echo "Covered macOS runtime connection ownership completion addendum: one main-actor manager owns the local listener, Bonjour advertiser, bootstrap relay, and fingerprint-keyed pair transports. It starts the listener before advertising exact route metadata, stops partial local ownership and advertisement after failed start, replaces prior local ownership in stop-before-start order, refreshes only Bonjour while the current listener remains valid, tears down stale Bonjour and local ownership after asynchronous listener failure, clears the app model's started state so the same port can be retried, ignores superseded listener-generation callbacks, atomically validates and inserts accepted peers under that generation so no connection crosses a concurrent stop/failure snapshot, forwards local and relay disconnect ids to the runtime router, invalidates replaced, retired, stopped, and failed message-callback leases plus relay status generations, waits for admitted message callbacks before external invalidation returns, and performs one idempotent unified shutdown. This is no-device Swift lifecycle and mock evidence, not cross-machine Bonjour discovery, public-network behavior, optical QR, or physical Android proof."
...
echo "Covered G1a-A production secure-session route-binding addendum: independent Swift and Kotlin ALS1 codecs match six typed route authorizations and six 21-field endpoint transcript vectors byte-for-byte, bind pair epoch, capabilities, endpoint identities, P-256 shares, nonces, generation, configuration, keyset, revocation, protocol floor, route kind, and authorization digest, and fail closed on mismatched state, malformed TLV, invalid keys or identities, and 512/1024-byte ceilings. This G1a-A contract remains a socket-free no-network foundation and, by itself, adds no pair-state persistence, key derivation, encrypted record path, connector, STUN/TURN transaction, relay deployment, physical-device proof, or production-readiness claim."
echo "Covered G1a-B production pair-state admission addendum: independent Swift and Kotlin ALS1 type-8 authority and type-9 snapshot codecs match exact shared bytes and digests under 1024/8192-byte ceilings, retain at most 64 session/transcript replay tombstones plus 20 prior transition ID/request-digest entries, reject pair-lifetime transition-ID reuse, and deny every epoch change until signed fresh-pair proof exists. Android DataStore and the macOS owner-only atomic trusted-device store persist canonical state and the replay tombstone before returning an opaque admission permit; generic state bootstrap, malformed or corrupt state, identity mismatch, rollback, revoked state, replay, capacity exhaustion, and durability failure fail closed. Android preserves state through app projection and rejects all legacy-only routes for a stateful target; macOS reloads trust before active or restored pair transport start and rejects missing, ambiguous, corrupt, or stateful legacy starts. Older pre-connector seams are internal and dormant; the completed exact-bound no-network coordinator remains not app- or transport-wired. No active production route/session construction, key derivation, encrypted record path, socket or network execution, STUN/TURN transaction, relay deployment, physical-device proof, or production-readiness claim is created."
echo "Covered G1a-C candidate capability, receipt, and grant readiness addendum: independent Swift and Kotlin consume the unchanged legacy fixture SHA-256 c25c0f4d74b0029f060bcedf31b19ef95c57a0a0e6708a741175c8cedeb611f3 and candidate fixture SHA-256 e6bc666dbf9fded82d5681fdcfdc2c4c9cd5fa197135fc0673569d35656236af; bind object 27 endpoint proof to signed object 23/24 capabilities; verify four fixed-order signed post-commit object 28 receipts on one ledger with adjacent revision and state chains; derive unsigned object 25 evidence and object 26 authorization; and bind object 7 to the exact SHA-256 digest of object 26 rather than generic object 4. The macOS and Android compound stores atomically persist the pair snapshot, endpoint ledger, and chained marker; macOS rereads exact bytes under the same exclusive lock and Android rereads the exact canonical DataStore envelope under one process-wide mutex. Only an APPLIED commit that is still inside the verifier-derived validity window both immediately before persistence and after exact readback may return a live durability token; store-owned trusted clocks reject regression, while idempotent restart and committed retry remain non-authorizing readback. Raw pair-transition and secure-session mutation APIs are not exported to production app adapters, verified wrappers require verifier mint provenance, generic P2P route and admission paths reject P2P_DIRECT, and historical whole-store rollback remains detectable only with an external monotonic head. productionDurabilityClaim=false and durabilityScope=synthetic_contract_readiness_only: no activation, KDF, encrypted records, socket or network execution, physical-device proof, deployment, or production-readiness claim."
echo "Covered G1a-C exact-bound start coordinator addendum: each platform store caches one no-network coordinator that accepts only a verifier-minted candidate binding plus an APPLIED durable compound token. It strict-decodes the current store and recomputes only the latest ledger entry and latest marker, including separate object-4 route and object-26 grant authorization digests, at admit, immediately before start, and immediately after start. Historical readback, AlreadyCommitted output, and caller-supplied time are ineligible. Checked generations, one live start per trusted store, secret-free 64-entry per-pair tombstones, explicit operation-scoped callback reentry, generation-scoped idempotent cleanup before and after late publication, durable-mutation fencing, cancellation, revocation, authority advance, expiry, and late completion fail closed. STARTING fences abort immediately and again after start returns when late publication is possible; ACTIVE fences abort once. Android adds cleanup quarantine until successful retry and cancellation-safe handle/lease handoff; Swift cancellation remains cooperative. This coordinator is not app- or transport-wired and creates no active session, KDF, encrypted record, socket or network operation, physical-device proof, deployment, or production-readiness claim."
echo "Covered G1a-D production secure-session crypto addendum: Swift and Kotlin accept only a verifier-minted exact object-7/object-26 binding, require the matching one-use local P-256 key, derive byte-identical ECDH/HKDF-SHA-256 material, require both role-separated object-29 confirmations, and issue one ordered object-30 AES-256-GCM cipher. The pinned d45fd920e22652d790c742de995d87a8cbfb64bb22aca3b829cbad5b23485448 fixture, independent Python oracle, and both platform suites execute 26 negative classes covering mismatch/reuse, reflection/conflict, replay/gap/future epoch, authentication mutation without receive advance, rekey boundaries, exact record/byte ceilings, expiry, clock regression, invalidation, terminal key wiping, and concurrent unique sequence assignment. This no-network core is not app- or transport-wired and proves no socket, physical device, live route, deployment, or production-readiness claim."
echo "Covered G1a-D authority-bound secure-session lifecycle addendum: Swift and Android retain the verifier-minted exact key-schedule binding inside one store/coordinator graph; a store-owned process-local writer-preferred/FIFO publication gate holds a read permit across start, confirmation, activation, seal, open, and their pre/post lease and live-resource fences. Durable authority writers block new readers, drain in-flight publications, commit, then fence the exact coordinator and synchronously wipe crypto before release. Pure precommit rejection and macOS pre-rename failure preserve the old session. Once an Android DataStore edit is enqueued, cancellation or ambiguous persistence failure fences/wipes the old authority; macOS post-rename directory-sync uncertainty does the same. Terminal crypto failure invalidates the session and closes its lease. Before releasing the read permit, Swift post-fence suppression zeroizes owner-backed confirmation, seal, and open result storage; small-ciphertext plus confirmation/seal/open retained-owner and result-copy regressions are included in the 87/87 complete Swift P2PNAT contract tests. An independent Data snapshot already extracted by a caller is a separate copy and is not retroactively zeroized. This guarantee is single-process and same-store/coordinator-graph only. Bounded no-network app/service caller bridges and the G1b-A Android controller/macOS loopback accepted-raw primitive are present. The upstream Android P2P producer/endpoint stack, macOS CompanionAppModel wiring, and actual socket/network activation remain unwired; an eventual transport must keep seal + channel.send inside the same read-permit closure."
echo "Covered G1a-D production secure-channel transport-composition seam addendum: Android core:transport uses a manager-owned one-use raw-route lease; the composer receives no raw-channel alias or caller-provided scope, and the lease creates ProductionRuntimeSecureChannelAdapter with a manager-owned execution scope. Owned-scope construction failure cancels the scope, and the adapter is registered before handshake suspension. Under stateLock, UNDISPATCHED acquisition linearizes the transition with physical connector entry: cleanup-first prevents connector invocation; after connector entry but before handle return, the residual depends on connector timeout/interruption and any late handle closes when returned. Detached composition uses saturating raw-route timeout addition plus a fixed 15-second handshake budget; the manager timeout IOException is classified as ProductionSessionSecurityRejected. The adapter internal deadline uses one PENDING to COMPLETED/TIMED_OUT CAS plus an UNDISPATCHED watchdog; timeout-winning IOException dominates and suppresses the losing error/cancellation, while completion-winning external or composer CancellationException preserves the exact object. Canonical resume(value, onCancellation) handoff closes only undelivered values: pre-delivery cancellation closes once without retry, while a successfully transferred channel survives later acquisition Job cancellation; there is no permanent caller-Job binding or InternalCoroutinesApi. Production P2P checks exact session, object-7/object-26 binding, route kind, and manager-owned connection generation; route expiry is rechecked immediately before one-use receipt commit, admission-to-commit wall-clock rollback fails closed, and failure cleanup runs in NonCancellable. Even when raw ignores close until it returns, the managed raw wrapper checks open before and after send, fails closed after close, and the regression observes actual late body-byte zeroization. Production relay fails closed without a verifier-derived exact relay route binding. Focused Android evidence is 79/79 (49/49 manager plus 30/30 adapter); root independently reran full core:transport --tests '*' with 10 suites at 163/163 and zero failures, errors, or skips, and app compileDebugKotlin plus compileDebugUnitTestKotlin succeed. An independent iterative audit found and fixed six P3 availability/lifetime races in total; final fresh re-audit reports no P0-P3 finding. Current root-independent full Swift evidence is 2,003 tests with two declared skips, zero failures, in 313.440 seconds. The macOS manager owns the exact one-use attachment, generation cleanup, cancellation/late-result close, raw-handler admission, and terminal mailbox drain before removal or replacement. Terminal teardown synchronously invalidates an available/claimed capability before replacement and runs asynchronous abandon/close outside registry locks, with no plaintext fallback. The bounded Android PairingStore app caller bridge and bounded macOS accepted-session service bridge are now present without opening a socket. A connector entered before handle return still depends on connector timeout/interruption, and actual socket execution/close-interrupt proof remains open. Focused macOS evidence is 39/39 (17/17 composition plus 22/22 secure-channel) and 34/34 (6/6 production-pair-coordinator plus 28/28 manager); the release build passes. The audit-found cancellation/replacement P2 is fixed with a deterministic delayed-abandon regression; final independent re-audit reports no P0-P3 finding. The normal Android graph now owns an empty activation controller and injected fixture manager/ViewModel E2E; macOS now exposes the loopback-only accepted-raw primitive. The Android upstream verifier/candidate/secret producer and actual P2P endpoint stack, macOS CompanionAppModel call site, actual socket execution and close-interrupt behavior, live network, and physical device remain later G1b residuals. This is no live socket, network, physical-device, or production-release evidence."
echo "Covered G1a-D no-network production caller-activation bridge addendum: Android binds route preparation and secure-session material through a renewable thread-safe per-attempt activation slot/source that owns one binding-derived plan at a time and synchronously discards replaced, expired, or ViewModel-cleared pending key material. A generation-bound claimed entry remains slot-owned until atomic beginPairingStoreTransfer: close or replacement before transfer discards the key while transfer-first relinquishes slot ownership; the COMPLETING one-shot rejects sequential and concurrent duplicate completion; and 32-round close/transfer and replacement/transfer races linearize exactly one key owner. Android requires the trusted-runtime owner to return the same exact PairingStore, transfers the transcript-matching ephemeral key once, closes an unconsumed key on preparation or begin failure, and gives only the manager-owned raw-route lease to the composer. macOS fixes one exact TrustedDeviceStore for the service lifetime, derives and exact-compares the accepted route descriptor from the verified binding and durable token, and transfers the raw endpoint through one one-shot claim with no raw-sink alias. macOS closes an untransferred ephemeral key on descriptor, authority-start, or cancellation failure and admits no plaintext fallback. macOS tracks preAttachmentAttempts under an epoch and exact current-generation checks; targeted stop invalidates suspended authority and stopAll rejects a late return without disturbing a fresh generation. Focused caller coverage is 16/16 Android and 9/9 macOS. The normal Android controller graph, injected manager/ViewModel activation, and macOS loopback-only accepted-raw primitive are now present. The Android upstream verifier/candidate/secret producer and actual P2P endpoint stack, macOS CompanionAppModel call site, actual socket execution and close-interrupt behavior, live network, and physical device remain later G1b residuals; repository-owner authentication is never a prerequisite."
echo "Covered G1b-A no-network activation and loopback accepted-raw primitive addendum: the normal Android factory creates one empty AndroidProductionRuntimeActivationController with the exact PairingStore and trusted clock; injected real-fixture manager and ViewModel tests complete the authority-bound handshake and one application record without legacy connector calls or an OS socket. Publication generations are assigned before durable admission, started key and endpoint ownership is revocable during suspended admission, displaced cleanup runs outside controller locks, and deterministic stale-generation, close-during-cleanup, and no-deadlock regressions pass 12/12 with a final independent P0-P3-free audit. macOS LocalPeerServer.startAcceptedRaw requires 127.0.0.1, one bounded authorization, serialized receive ownership, callback-safe stop, and no envelope fallback; RawFrameBodySeamTests pass 20/20 without starting a listener. The Android upstream verifier/candidate/secret producer and actual P2P endpoint stack, macOS CompanionAppModel wiring, actual socket execution and close interruption, live network, physical-device, and production-release proof remain open. No repository-owner authentication, live network/socket execution, device action, or Git operation is required or performed."
echo "Covered G2 Pion v4.3.0 official-source preflight rejection addendum: exact commit 1e8716372f2bb52e45bf2a7172e4fb1004251c46 is rejected as-is before acquisition or compilation because active TCP and proxy/mux/STUN/TURN/mDNS/final-send paths lack one non-bypassable post-resolution pre-I/O destination policy, debug logging includes the remote ICE password, callback queues are unbounded, and graceful shutdown can wait indefinitely. No Pion source was retained, compiled, loaded, or executed; no library is selected and no compile, loopback, controlled-network, external-network, device, production, or Git rung is opened. Repository-owner, GitHub, SSH, GPG, and public-key identity authentication are never prerequisites or future G2 rungs."
echo "Covered historical G2 Pion v4.3.0 restricted-fork rung-one design addendum at_that_checkpoint: status=rung1_profile_complete_candidate_not_selected; result=pion_restricted_fork_profile_ready_for_rung2_decision_only; recordedNextActionAtThatCheckpoint=prepare_versioned_rung2_source_identity_and_acquisition_decision. The hash-pinned portfolio compares unmodified upstream, a wrapper-only gateway, and one minimal AetherLink-maintained restricted fork. Pion and every networking library remained unselected at that checkpoint. Rung one completed only the design, validator, and 17 mutation tests. Schema 1.1 remains a not-yet-implemented and not-runtime-verified design. It requires a separate single-use egress capability after resolution immediately before socket create, bind, connect, TLS handshake, or write, plus fixed-size bounded ingress read/parse/admission before state mutation or payload delivery. It requires authenticated TURN TLS service identity before any credential transmission and a bounded one-use pre-auth path whose atomic promotion occurs only after exact AetherLink endpoint confirmation. Consent loss, path change, candidate restart, capability expiry, verification failure, and session close each atomically revoke both pre-auth and application capabilities before further I/O, state mutation, event, or payload delivery. Exact per-session and process bounds cover current, active, draining, and closing state, and event overflow requires an independent sticky terminal latch. Secret-free diagnostics and a 2,500 ms total close deadline are requirements, not completed implementation or runtime-verified behavior. Non-profile paths are required to fail before I/O, including ICE-TCP, proxy, mux, mDNS, redirects, implicit DNS, automatic port mapping, telemetry, and hidden bootstrap. Android API 26 through 36 arm64-v8a and macOS 14+ arm64 form only a future compile-only matrix, followed by exact toolchain, module graph, SPDX SBOM, license, patch, symbol, and two-build reproducibility evidence. implementationStatus=not_implemented; candidateSelected=false; librarySelected=false; sourceAcquisitionAllowed=false; dependencyInstallationAllowed=false; compilerInvocationAllowed=false; codeLoadingAllowed=false; socketCreationAllowed=false; networkIoAllowed=false; deviceExecutionAllowed=false; productionDeploymentAllowed=false; gitOperationAllowed=false. The actual backend, reliable ordered carrier, and fragmentation/reassembly remain unselected and unimplemented. Only stack-neutral wiring may continue. Repository-owner, GitHub, SSH, GPG, or public-key identity proof is neither a prerequisite nor a future G2 rung; externalIdentityProofRequired=false and userActionRequired=false. Product pairing and endpoint authentication remain mandatory and separate."
echo "Covered production P2P/NAT pre-network approval addendum: immutable review-v1 preserves exactly seven canonically ordered proposed_not_selected recommendations, decision-v1 resolves all seven to their recommended options with explicit_user_instruction, and closed handoff-v3 supersedes handoff-v2. The 16-test mutation suite rejects missing, duplicate-key, unknown, reordered, partial, mismatched, evidence-drifted, weakened, fabricated-measurement, or unauthorized states; Android's live P-256 validator uses API-26-safe BigInteger.valueOf(2L), while the checker reconstructs only that exact prior expression for the closed historical digest and rejects both API regression and all unrelated drift. Handoff-v3 hash-pins Android P2pNatContract.kt, the no-network scan covers SocketFactory.createSocket and CFStreamCreatePairWithSocketToHost, the security-design validator directly verifies canonical handoff closure, and Kotlin and Swift P2P/NAT contract/conformance suites execute in the default gate. controlled-network-spike remains blocked_on_separate_review by networking_library_selection, session_cryptography_library_selection, isolated_harness_design, and socket_destination_and_egress_controls; networkIOAllowed, librarySelectionAuthorized, productionDeploymentAuthorized, and controlledNetworkSpikeSocketExecutionAuthorized stay false. This is policy approval and no-device validation only, not ICE/STUN/TURN implementation, candidate exchange, NAT traversal, physical Android, optical QR, live-network, performance, battery, capacity, deployment, or production-readiness proof."
...
echo "Covered historical production P2P/NAT controlled-spike Phase A crypto and static-policy checkpoint: the shared ALP1 fixture and independent Python oracle verify direct and relay P-256 ECDH, leading-zero secret normalization, transcript-bound HKDF-SHA-256 key separation, role-bound bidirectional confirmation, single-use ephemeral derivation, key-owned one-shot cipher issuance, deterministic directional AES-256-GCM nonce/AAD construction, tamper/replay/provider failure, concurrent sequence safety, and counter exhaustion across Swift CryptoKit and provider-neutral Android JCA. The current security-design validator runs a 57-file SHA-256 preflight that preserves this historical evidence and also pins both exact acquisition and source-audit rejection chains. The static harness remains non-executable; all compiler, socket, runtime network, Phase B, and production gates are closed."
echo "Covered historical production P2P/NAT Phase A offline-source and compile-boundary checkpoint: offline-source-intake-v1 and libjuice-compile-only-contract-v1 preserve the earlier blocked_missing_offline_source and blocked_missing_reviewed_source states and the planned Android minSdk 26 and macOS ABI matrix. They are immutable history, not current source-absence claims. The current successors record exact acquisition followed by source-audit rejection before any compiler, static archiver, native adapter, build wiring, link, load, or nm invocation."
echo "Covered historical production P2P/NAT controlled-spike Phase A progress checkpoint: immutable progress-v1 records the earlier 4 approvals, 2 completed bounded evidence groups, 2 blocked groups, and whole-review blocker. Later versioned progress records supersede its current-status role without rewriting it. The 7-test historical mutation suite still rejects authority, evidence, source-chain, measurement, Phase B, and immutability drift."
echo "Covered production P2P/NAT Phase A exact acquisition receipt addendum: decision-v2 and handoff-v5 authorized only official libjuice v1.7.2 and Android NDK ndk;28.2.13676358 acquisition. The retained libjuice archive SHA-256 is 75159867c4a5a689a6559e11aa0d30c9eba12ce73a4ae3d898b521467e1f635d; the retained NDK r28c archive SHA-256 is 0d4599e8bbf1a1668a0d51a541729b2246360f350018a2081d0b302dbb594f2a. Acquisition and bounded extraction occurred under that exact one-shot authority; source execution, compiler/archive invocation, linking/loading, sockets, runtime or harness network I/O, Phase B, and production permission did not occur."
echo "Covered historical production P2P/NAT libjuice source-audit rejection addendum: the default gate read-only rehashes the retained libjuice archive, byte-equivalent 81-file source tree, NDK r28c archive, installed NDK tools, Apple tools, committed manifest, and local provenance twin. The canonical tree SHA-256 is c17e0d6d3855e9584718584ab644f030939448d0e8f6a8bf5ca9883da719a330. The line-referenced audit rejects libjuice v1.7.2 before compilation for five independent P1 profile failures. review-v2 then opened libnice-0.1.23-glib-c-abi for a separate bounded audit; no compiler or static archiver was invoked at either candidate stage."
echo "Covered production P2P/NAT exact libnice and GLib source-intake addendum: consumed one-shot authorities pin the official libnice 0.1.23 archive SHA-256 618fc4e8de393b719b1641c1d8eec01826d4d39d15ade92679d221c7f5e4e70d and 184-file tree SHA-256 e594b0b2435e10a8df970304ba3dec24ea0353820f1eecb820a810ab56cd276a, plus the official-checksum-matched GLib 2.64.2 archive SHA-256 9a2f21ed8f13b9303399de13a0252b7cbcede593d26971378ec6cb90e87f2277 and 1,961-file tree SHA-256 1c36d535b42d89b62c375b60005dd3c073033ba5bb4928c6825c09a4bc61d3ac. The libnice detached-signature bytes are pinned without a trusted-key OpenPGP verification claim."
echo "Covered production P2P/NAT libnice source-audit rejection addendum: two independent GPT-5.6 Sol static reviews reject libnice 0.1.23 before compilation for four P1 profile failures: non-cryptographic ICE credential and tie-breaker randomness, secret-bearing ICE/TURN/SOCKS/HMAC diagnostics, automatic STUN/TURN alternate-server destination changes before caller authorization, and consent refresh without success-class or source-tuple binding. The minimum remaining source set was identified as libffi 3.7.1, GNU libiconv 1.19, proxy-libintl 0.1 stub-only, and OpenSSL 3.5.7 LTS, but none was acquired because the candidate failed first. decision-v6, handoff-v9, and progress-v8 leave no selected networking candidate and close further acquisition, inspection, source execution, compiler, static archiver, socket, runtime network, harness, Phase B, and production authority. This is no-device static evidence, not signature trust, compile, ABI, runtime ICE/STUN/TURN, NAT traversal, physical Android, or production proof."
...
echo "Covered historical G2 Pion v4.3.0 rung-two acquisition-authority checkpoint: immutable decision/progress-v1/manifest-v2 record status=rung2_source_identity_decision_recorded_acquisition_not_executed; permit=authorized_not_consumed; requestCount=0 before the one-use request. The exact official Go proxy URL, upstream commit/tree, decision-pinned raw ZIP SHA-256, SumDB module h1 and go.mod h1, TLS certificate/hostname validation, atomic one-use permit claim, no redirects/proxies/credentials/retries, and archive-only output are fixed. The offline validator independently verifies the public SumDB Ed25519 signed tree, exact record hash, and ordered 25-node RFC 6962 inclusion proof; 20 mutation tests reject schema drift, proof mutation, permission escalation, false completion, and any owner/external identity or user-action requirement. Raw SHA-256 is a reproducibility pin rather than an independent upstream authentication root. This checkpoint is immutable history and not the current acquisition state."
echo "Covered historical G2 Pion v4.3.0 rung-two acquisition-receipt addendum at_that_checkpoint: status=acquisition_complete_archive_retained_not_extracted; result=exact_archive_acquired_verified_rung3_decision_preparation_only; recordedNextActionAtThatCheckpoint=prepare_versioned_rung3_offline_source_review_decision. Exactly one HTTPS GET completed with requestCount=1 and the one-use permit was consumed. The retained owner-only single-link 293,023-byte ZIP has raw SHA-256 f95ef3ce2e0063c13925f478bf8d188833725b2f0a7ecb1e695dee8ab61ef63c, module h1 h1:X8l4s9zV2HeTKX33nulWAFXAEo5KhIVzOsY62/3t/LM=, and go.mod h1 h1:obAyD+J+Hzs7QA7Y8YXHp5uIn6gb7z87pKedXZkrcFU=. All 129 file entries were boundedly decoded and checked without filesystem extraction. Receipt-v1, progress-v2, and manifest-v3 are append-only acquisition evidence; canonical-document-supersession-v1 and manifest-v4 preserve the prior snapshot, while canonical-document-supersession-v2 and manifest-v5 bind the canonical documents at that checkpoint. The whitespace-normalized semantic guard scoped the G2 roadmap and immediate execution queue and rejected stale rung-two instructions across line breaks. Git lazy fetch was disabled for historical object readback. The 12 mutation tests reject request, hash, extraction, selection, permission, identity-proof, and rung-three execution drift. The archive was retained but not reviewed or executed; no dependency install, compiler, code load, socket, runtime network, device, deployment, or Git write operation occurred; no user action, external identity proof, or repository-owner authentication was required. This is immutable rung-two history and not the current G2 status."
echo "Covered historical G2 Pion v4.3.0 rung-three v3 publication-readback-complete addendum at_that_checkpoint: status=rung3_v3_publication_read_back_complete; result=bounded_v3_exact_candidate_totals_digests_and_ranked_representatives_publication_committed_semantic_review_not_performed; recordedNextActionAtThatCheckpoint=prepare_separate_versioned_rung3_semantic_source_review_decision. The tracked-only canonical success-evidence checker is covered by exactly 17 unit tests under python3 -I -B -S and reads no build/runtime output or archive. Exact bounded inventory: entryCount=129; creatorSystem=0 count=129 and creatorSystem=3 Unix count=0; goSourceFileCount=100; goSourceTotalBytes=1077591; goSourceLogicalLineCount=39064; totalHitCount=4701=recordedRepresentativeCount=144+omittedHitCount=4557. Semantic source review was not performed; rungThreeComplete=false; candidateSelected=false; librarySelected=false. No materialization, dependency install, reviewed-source compile or execution, socket, network, device, deployment, or Git operation occurred. Repository-owner authentication, external identity proof, execution-permit authentication, and user action are not required; product endpoint authentication remains required and separate."
echo "Covered historical G2 Pion v4.3.0 rung-three semantic-review decision v1 addendum at_that_checkpoint: status=rung3_semantic_source_review_v1_decision_recorded_execution_not_started; result=canonical_v3_evidence_bound_full_semantic_review_execution_contract_recorded; recordedNextActionAtThatCheckpoint=execute_rung3_semantic_source_review_v1. The tracked-only decision checker is covered by exactly 15 unit tests under python3 -I -B -S and reads seven exact tracked JSON artifacts, no Markdown, build/runtime output, or archive. It required two passes over all 100 Go source bodies and all 4,701 lexical observations, production/test/example counts 52/44/4, disagreements forced to unresolved, and the one-use zero-hit retained as a missing-required-mechanism gap. At that checkpoint semanticSourceReviewPerformed=false, dependencyClosureComplete=false, rungThreeComplete=false, candidateSelected=false, and librarySelected=false. This immutable decision state is historical and its recorded action has now been executed."
echo "Covered historical G2 Pion v4.3.0 rung-three semantic-review publication-readback v1 addendum at_that_checkpoint: status=rung3_semantic_source_review_v1_publication_read_back_complete_semantic_closure_blocked; result=two_non_attesting_full_coverage_semantic_passes_published_and_independently_read_back_patch_and_dependency_gaps_remain; recordedNextActionAtThatCheckpoint=prepare_versioned_rung3_patch_and_dependency_closure_decision. Two non-attesting GPT-5.6 Sol passes reviewed all 100 Go files and 4,701 lexical observations, producing 29 candidates and 19 exact deduplicated findings with severityCounts=P0:0,P1:11,P2:3,P3:4,none:1 and dispositionCounts=patch_required:7,unresolved:12. The tracked-only independent post-run checker is covered by exactly 25 mutation tests under python3 -I -B -S; it holds all eight file descriptors plus every repository-path directory component through two stable full-set readback passes and a final identity barrier over 255,402 bytes, validates raw/self/cross/artifact bindings, recomputes both candidate digests and 29-to-19 finding aggregation, validates the manifest last, and observes the failure file plus four staging names absent before and after readback without reading build output, the archive, or source bodies. semanticSourceReviewPerformed=true while semanticClosureComplete=false, dependencyClosureComplete=false, rungThreeComplete=false, candidateSelected=false, and librarySelected=false; the checker does not independently reproduce semantic judgments or source-based location bounds, prevent same-UID concurrent mutation, detect mutation after return, or guarantee absence after its final observation. No materialization, dependency install, reviewed-source compilation or execution, socket, network, device, deployment, or Git operation occurred. Repository-owner authentication, external identity proof, execution-permit authentication or document, and user action are not required. This checkpoint is immutable history and its recorded next action has been completed."
echo "Covered historical G2 Pion v4.3.0 rung-three patch-and-dependency decision preparation v1 addendum at_that_checkpoint: status=prepared_options_unselected_dependency_closure_blocked; result=four_structural_recommendations_and_eight_unselected_treatment_units_prepared_all_19_findings_remain_open; recordedNextActionAtThatCheckpoint=prepare_separate_versioned_implementation_or_dependency_review_decision. The preparation binds all 19 canonical findings to seven unselected root patch units and one unselected dependency source/license/security review unit, preserves the three useful existing-control findings, quarantines four checksum-only context tuples, and records the exact 19 root requirements, 44 go.sum records, 23 module/version tuples, 21 source hashes, and 23 go.mod hashes. The security-hardening portfolio compares two options for each of four opportunities and conditionally recommends typed capability ownership, one resource supervisor, typed secret-free diagnostics, and staged fixed-point dependency closure; recommendationsAreSelections=false. The complete 19-file portfolio is pinned by path, byte size, type, and SHA-256. The read-only checker and exactly 28 checker tests run under python3 -I -B -S and reject predecessor, archive path, finding, treatment, option, dependency-seed, authority, selection, closure, schema, symlink, hardlink, missing or unexpected portfolio artifact, reader-facing effect drift, and replace-after-read drift. The suite now re-pins fixture-local raw bytes so selection, authority, closure, nested-schema, and reader-facing contradictions reach their semantic validators. findingsClosedByPreparation=0; rootPatchComplete=false; dependencySourceReviewed=false; dependencyClosureComplete=false; semanticClosureComplete=false; rungThreeComplete=false; candidateSelected=false; librarySelected=false. No implementation plan or patch series was created, and no source modification or extraction, dependency acquisition, package manager, compiler, source load or execution, socket, network, device, deployment, Git write, external authentication, or user action is authorized or required. This checkpoint is immutable history and its recorded next action has been completed."
echo "Covered historical G2 Pion v4.3.0 rung-three implementation-or-dependency review decision v1 addendum at_that_checkpoint: status=dependency_review_selected_acquisition_not_authorized; result=staged_fixed_point_dependency_review_selected_all_19_findings_remain_open; recordedNextActionAtThatCheckpoint=prepare_separate_versioned_bounded_dependency_source_identity_and_acquisition_decision. Exactly one structural option, staged-fixed-point-source-closure, and exactly one treatment unit, dependency_source_license_security_closure_review, are selected for a dependency-review plan; the other seven portfolio options, all seven root patch units, and the other three structural recommendations remain unselected. The exact predecessor decision, semantic triad, retained archive/root metadata, complete 19-file portfolio bundle (19 files, 186,716 bytes, ordered bundle SHA-256 020fa0b627a85844d557323b5106e4179637fe3f14c578fec50e6a3e34e68f56), and 16,155-byte implementation handoff are byte-bound. The read-only checker and exactly 36 baseline, runtime, and mutation checker tests run under python3 -I -B -S; semantic mutations re-pin fixture-local raw bytes and assert the exact selection, authority, finding, closure, contract, sequence, plan, inventory, filesystem, and TOCTOU failure layers. All 19 findings remain open; findingsClosedBySelection=0; rootPatchComplete=false; dependencySourceReviewed=false; dependencyClosureComplete=false; semanticClosureComplete=false; rungThreeComplete=false; candidateSelected=false; librarySelected=false. Source modification/extraction, dependency acquisition, package management, compilation, source load/execution, sockets, network, device, deployment, and Git writes remain unauthorized. Repository-owner identity proof, external authentication, execution-permit authentication or document, and user action are not required. Product endpoint authentication remains a separate later runtime invariant. This checkpoint is immutable history and its recorded next action has been completed."
echo "Covered historical G2 Pion v4.3.0 rung-three dependency wave-one preparation v1 addendum at_that_checkpoint: status=wave1_source_identity_and_request_contract_prepared_acquisition_not_authorized; result=exact_19_root_requirement_source_identities_and_bounded_wave1_request_contract_prepared; recordedNextActionAtThatCheckpoint=prepare_separate_versioned_wave1_execution_permit_after_checker_runner_and_tests. The exact 19-tuple source-intake seed, four quarantined checksum-only tuples, Android/macOS arm64 review profiles, fixed-point graph rules, request/output identities, and finite bounds remain immutable predecessor evidence. Its action was completed by the now-consumed v1 permit."
echo "Covered historical G2 Pion v4.3.0 rung-three dependency wave-one execution permit v1 addendum at_that_checkpoint: status=wave1_dependency_source_acquisition_authorized_not_consumed; result=exact_19_public_proxy_zip_requests_authorized_once_not_executed; recordedNextActionAtThatCheckpoint=execute_bound_dependency_source_wave1_once. The isolated runner still passes 44 tests. The permit suite recorded 38 tests only at the unconsumed checkpoint; the current gate reruns 36 state-independent cases. The permit was consumed exactly once and may not be reused; its direct checker and preflight are intentionally no longer current-state gate commands."
echo "Covered historical G2 Pion v4.3.0 rung-three dependency wave-one recovery decision v1 addendum at_that_checkpoint: status=wave1_v1_failure_read_back_recovery_v2_design_selected_execution_not_authorized; result=v1_ratio_policy_rejected_tuple2_after_two_responses_no_final_set_v2_bounded_telemetry_policy_selected; recordedNextActionAtThatCheckpoint=prepare_separate_v2_runner_checker_tests_and_execution_permit. The retained owner-only v1 claim and failure receipt prove E_ZIP_RATIO during tuple two after two response bodies, one fully validated/staged tuple, zero accepted artifacts, no final set, and no automatic retry. failedTupleId=null remains immutable historical data; ordered counters derive wave1-002-c4e8ffbb48de without inventing the failed entry, sizes, ratio, or raw ZIP hash. Exactly 31 recovery mutation tests protect the bindings, final namespace rescan, counter split, v1 preservation, distinct v2 namespace, absolute streaming limits, non-gating exact-integer compression telemetry, and no-network authority at that checkpoint. V1 deletion, reuse, retry, or staging resume is forbidden. The preparation action has now been completed by the separate v2 runner, checker, tests, and permit. Repository-owner proof, external authentication, private keys, tokens, passwords, signatures, and user action are not required."
echo "Covered historical G2 Pion v4.3.0 rung-three dependency wave-one execution permit v2 addendum at_that_checkpoint: status=wave1_v2_dependency_source_acquisition_authorized_not_consumed; result=exact_19_public_proxy_zip_requests_v2_authorized_once_not_executed; recordedNextActionAtThatCheckpoint=execute_bound_dependency_source_wave1_v2_once. The separate v2 runner and checker/test bytes remain immutable. The permit was consumed exactly once and may not be reused. Its retained owner-only claim and failure receipt prove E_GO_MOD_MISSING during tuple 11 after networkRequestAttemptCount=11, responseBodyCompletedCount=11, validatedAndStagedTupleCount=10, acceptedArtifactCount=0, no final set, and no automatic retry. The failure was caused by conflating the ZIP and separately checksummed .mod resources, not by authentication. Direct v2 preflight is intentionally no longer a current-state gate command. Repository-owner proof, external authentication, private keys, tokens, passwords, signatures, and user action are not required."
echo "Covered historical G2 Pion v4.3.0 rung-three dependency wave-one recovery decision v2 addendum at_that_checkpoint: status=wave1_v2_failure_read_back_recovery_v3_design_selected_execution_not_authorized; result=v2_conflated_zip_and_mod_resources_tuple11_after_eleven_responses_no_final_set_v3_zip_plus_mod_policy_selected; recordedNextActionAtThatCheckpoint=prepare_separate_v3_runner_checker_tests_and_execution_permit. Exactly 39 recovery mutation tests protect both retained terminal generations, the fresh v3 claim/staging/final/receipt/failure/manifest namespace, exact 19 mod-then-zip pairs and 38 bounded sequential public-proxy resources, split counters, optional embedded-go.mod byte parity, non-gating exact-integer ZIP telemetry, absolute mod/ZIP/aggregate/decompression/path/JSON/deadline limits, 41/43 reserved-file path-set semantics, separate independent readback, and no-network authority at that checkpoint. V1/v2 deletion, reuse, retry, or staging resume is forbidden. Its preparation action has now been completed by the separate v3 runner, permit checker, tests, and permit. All 19 findings remain open; candidateSelected=false/librarySelected=false. Repository-owner proof, external authentication, private keys, tokens, passwords, signatures, and user action are not required."
echo "Covered historical predecessor G2 Pion v4.3.0 rung-three dependency wave-one v3 acquisition and fixed-hash readback addendum: historicalPermitStatus=wave1_v3_dependency_source_acquisition_authorized_not_consumed; receiptStatus=acquired_pending_independent_readback; receiptResult=fresh_exact_19_dependency_zip_mod_pairs_acquired_and_hash_verified; readbackStatus=independent_readback_complete; recordedNextActionAtThatCheckpoint=prepare_separate_dependency_source_review_wave. The one-use permit is consumed_success and cannot be retried. Exactly networkRequestAttemptCount=38, responseBodyCompletedCount=38, acceptedArtifactCount=38, retainedResourceCount=38, retainedZipCount=19, retainedModCount=19, and observedRegularFileCount=43 are independently validated. The immutable permit-bound reader retains exactly 34 tests; recovery v2 retains 8 tests; the verification-only v3 suite passes exactly 9 tests and enforces fixedHashEnforcedInsideHeldValidation=true, verificationOnly=true, recordModeExposed=false, networkOperationCount=0, and fileWriteCount=0. Its post-verification decision binds the predecessor three pretty JSON bytes, five canonical generated artifacts, v2 recovery decision/tool/tests, readback receipt SHA-256 63c7db8fce4a1c5c26dba84c22be9ea79afda95afb76506a10457e1ac9e910e0, readback manifest SHA-256 a62e1cc1508a127fa1f5cb4a5009cf7ddeae87ef40172d1c7327c51f8cbc3b96, and orderedSourceSetSha256=2b0176d6d2b800c9a2abd34bf06279403e6f008bd3475ff45970abf11e843246. V1/v2/v3 deletion, reuse, retry, or staging resume remains forbidden. Source extraction/loading/execution, package management, compilation, runtime/product network, device, deployment, and Git writes remain closed. No credentials, private keys, tokens, passwords, signatures, or user action are involved."
echo "Covered historical G2 Pion v4.3.0 dependency source-review wave-one v3 publication and independent-readback checkpoint: status=dependency_source_review_wave1_readback_published_new_tuple_wave_required; result=independent_readback_receipt_published_then_manifest_written_last_new_tuple_wave_required; recordedNextActionAtThatCheckpoint=prepare_separate_versioned_dependency_wave2_identity_and_acquisition_decision. V1 and v2 remain immutable failed-closed attempts (E_HELD_SET and E_ARCHIVE_STRUCTURE) with no partial result. V3 inspected 20 held ZIPs in memory without extraction, source execution, subprocess, or network access and recorded graphSha256=2c94906a07a40737e30ca832c215fa88d2233297c9fb0ea25755488d9a72408b, graphNodeCount=132, graphEdgeCount=1047, moduleNodeCount=35, moduleEdgeCount=86, selectedVersionCount=25, newTupleCount=15, unmappedExternalImportCount=0, unresolvedDeclaredExternalImportCount=0, and route=new_tuple_wave_required. Independent readback reopened no archive members and invoked no runner. The 15 frontier tuples remain acquisitionAuthorized=false. All 19 findings remain open; dependencySourceReviewed=false, graphFixedPointReached=false, dependencyClosureComplete=false, semanticClosureComplete=false, rungThreeComplete=false, candidateSelected=false, and librarySelected=false. Credentials, owner proof, keys, signatures, tokens, passwords, and user action are not required; Git writes remain closed."
echo "Covered historical G2 Pion v4.3.0 dependency wave-two identity preparation at_that_checkpoint: status=wave2_local_checksum_identity_and_30_resource_contract_prepared_future_bytes_unverified_acquisition_not_authorized; result=exact_15_graph_frontier_tuples_30_mod_zip_requests_and_held_h1_expectations_prepared_future_bytes_unverified; recordedNextActionAtThatCheckpoint=prepare_separate_versioned_wave2_checker_runner_tests_and_one_use_execution_permit. The read-only decision checker and 37 offline regression checks preserve all 15 exact module/version vertices, including five selected and ten non-selected version-specific rows; bind every introducing parent .mod declaration; and validate 30 ordered .mod-then-.zip H1 expectations across 20 already-held archives with no conflicting pair. Held go.sum evidence is not a fresh checksum-database proof. This checkpoint is immutable history and its recorded next action has been completed."
echo "Covered current G2 Pion v4.3.0 dependency wave-two one-use acquisition permit: status=wave2_v1_dependency_source_acquisition_authorized_not_consumed; result=exact_15_public_proxy_mod_then_zip_pairs_authorized_once_not_executed; nextAction=execute_bound_dependency_source_wave2_v1_once. The exact 15 tuples and 30 ordered public proxy .mod-then-.zip requests are bound to held H1 expectations, finite byte/archive/deadline limits, an exclusive claim before any request, no retry or resume, manifest-last publication, independent three-pass readback with retained resource and published-evidence descriptors through publication, and exact terminal schemas. Focused evidence passes 37/37 decision, 41/41 permit, 50/50 runner, and 39/39 readback offline regression checks; both exact preflights pass with executionAuthorized=true, permitConsumptionState=authorized_not_consumed, networkUsed=false, fileWriteCount=0, and an empty wave-2-v1 namespace. Future response bytes remain unacquired and unverified until the one-use execution. Only this bounded 30-request public Go proxy source intake is authorized; extraction, source loading/execution, package management, compilation, runtime/product network, device, deployment, and Git writes remain closed. All 19 findings and every dependency, semantic, rung-three, candidate, and library closure remain open. Repository authentication, account login, owner proof, credentials, keys, signatures, tokens, passwords, and user action are not required."
