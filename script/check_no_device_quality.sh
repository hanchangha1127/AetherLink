...
echo "Covered protocol reserved RAG/research namespace guard addendum: embeddings., retrieval., unsupported index.* beyond index.documents.list, research., citation., and source_control. active messages remain blocked by protocol schema hygiene and RuntimeDevServer relay smoke until runtime retrieval, indexing, research, citation, trusted-source, source-control, permission, and audit semantics are designed."
