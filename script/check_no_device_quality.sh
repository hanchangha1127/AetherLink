...
echo "Covered historical production P2P/NAT controlled-spike Phase A crypto and static-policy checkpoint: the shared ALP1 fixture and independent Python oracle verify direct and relay P-256 ECDH, leading-zero secret normalization, transcript-bound HKDF-SHA-256 key separation, role-bound bidirectional confirmation, single-use ephemeral derivation, key-owned one-shot cipher issuance, deterministic directional AES-256-GCM nonce/AAD construction, tamper/replay/provider failure, concurrent sequence safety, and counter exhaustion across Swift CryptoKit and provider-neutral Android JCA. The current security-design validator runs a 44-file SHA-256 preflight that preserves this historical evidence and also pins the later exact acquisition and source-audit rejection chain. The static harness remains non-executable; all compiler, socket, runtime network, Phase B, and production gates are closed."
...
echo "Covered production P2P/NAT libjuice source-audit rejection addendum: the default gate read-only rehashes the retained libjuice archive, byte-equivalent 81-file source tree, NDK r28c archive, installed NDK tools, Apple tools, committed manifest, and local provenance twin. The canonical tree SHA-256 is c17e0d6d3855e9584718584ab644f030939448d0e8f6a8bf5ca9883da719a330. The line-referenced audit rejects libjuice v1.7.2 before compilation for five independent P1 profile failures, including predictable Android/macOS ICE randomness, default-level ICE password logging, unauthenticated Binding error handling, unauthenticated TURN redirect handling, and no numeric-only pre-I/O authorization boundary. No compiler or static archiver was invoked. review-v2 opens libnice-0.1.23-glib-c-abi only as proposed_not_selected; its acquisition, dependencies, compilation, source execution, sockets, runtime networking, Phase B, and production use require separate explicit approval and remain false."
