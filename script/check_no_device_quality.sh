...
run ./gradlew --no-daemon \
  ...
  --tests com.localagentbridge.android.runtime.RuntimeClientViewModelTest.macosCompactRelayQrFixtureParsesAndPreparesRelayRoute \
	  ...
	  --tests com.localagentbridge.android.runtime.RuntimeClientViewModelTest.duplicateCompactRelayQrScanSendsSinglePairingRequestOnActiveRelayConnection \
		  ...
		  --tests com.localagentbridge.android.runtime.RuntimeClientViewModelTest.pairingResultRejectsUnknownMetadataBeforeTrustMutation \
			  ...
			  --tests com.localagentbridge.android.runtime.RuntimeClientViewModelTest.authChallengeRejectsUnknownMetadataBeforeAuthResponseSigning \
  ...
  --tests com.localagentbridge.android.runtime.RuntimeClientViewModelTest.addAttachmentsWithExistingPendingAttachmentsReadsOnlyRemainingSlotsAndShowsLimit \
	  ...
	  --tests com.localagentbridge.android.runtime.RuntimeClientViewModelTest.runtimeDocumentMetadataReplacesNonCanonicalMimeTypesInTransientState \
		  ...
		  --tests com.localagentbridge.android.runtime.RuntimeClientViewModelTest.runtimeDocumentSearchDoesNotSendSelectedEmbeddingModelHint \
  ...
  --tests com.localagentbridge.android.runtime.RuntimeClientViewModelTest.modelPullResultRejectsUnknownMetadataBeforeInstallStateMutation \
...
run swift test --filter 'RuntimeMemoryStoreGeneratedDraftTests|RuntimeLongInactivityMemorySummarizationPolicyTests'
run swift test --filter 'RelayPeerClientTests/testRelayPeerClientRetireKeepsCurrentConnectionAndSuppressesReconnect|RelayPeerClientTests/testRelayPeerConfigurationDefaultControlLineTimeoutAllowsPhysicalQrStartup|RelayPeerClientTests/testRelayPeerClientTimesOutWhenRegistrationLineNeverArrives|RelayPeerClientTests/testRelayPeerClientTimesOutWhenReadyLineNeverArrivesAfterRegistration'
	...
	run swift test --filter 'SQLiteRuntimeChatEventStoreTests/testSQLiteStorePreservesRuntimeCompactionMetadataWithoutIndexingIt|SQLiteRuntimeChatEventStoreTests/testSQLiteStoreImportsLegacyCompactionMetadataWithoutStructuralAccounting|SQLiteRuntimeChatEventStoreTests/testSQLiteStoreRejectsInvalidRuntimeCompactionMetadata'
...
echo "Covered macOS retrieval.query request bounds addendum: authenticated LocalRuntimeMessageRouter rejects retrieval.query query text longer than 1024 characters with invalid_payload before document-index store dispatch, matching the shared schema and runtime document-index query ceiling without semantic retrieval, embeddings, source approval, citation, trusted-source review, permission, audit, Android UI, local persistence, or chat context behavior."
...
echo "Covered Android client capability future Workspace/RAG/source deny-list addendum: Android runtimeClientCapabilities continues to advertise active index.documents.list and retrieval.query while keeping source_anchor.resolve unadvertised until Android UI/client consumption exists, and keeping future embeddings.create, index.build, research.brief.create, citation.sources.list, trusted_source.approve, source_control.status, projects.sessions.list, automation.runs.create, tool.call, tool.result, tool.run, skills.run, mcp.tool.call, web_search.query, python.run, python.exec, permission.request, approval.prompt, audit.events.list, file.read, file.write, file.index, terminal.exec, terminal.kill, network.request, network.open, backend.call, backend.configure, memory.search, route.candidates.exchange, route.diagnostics.report, route.allocation.status, and route.failure.report out of default and diagnostic hello client_capabilities before those workspace, tool, source approval, permission, audit, memory-search, or route-diagnostics semantics exist."
...
echo "Covered protocol reserved RAG/research namespace guard addendum: embeddings., unsupported retrieval.* beyond retrieval.query, unsupported index.* beyond index.documents.list, research., citation., and source_control. active messages remain blocked by protocol schema hygiene and RuntimeDevServer relay smoke until runtime semantic retrieval, indexing, research, citation, trusted-source, source-control, permission, and audit semantics are designed."
echo "Covered protocol reserved source-anchor/trusted-source namespace guard addendum: source_anchor.resolve is the only active read-only source_anchor. resolver message, unsupported source_anchor.* and trusted_source. active messages remain blocked by protocol schema hygiene and RuntimeDevServer relay smoke, and retrieval.query source_anchor_id stays response metadata only."
...
echo "Covered RuntimeDevServer retrieval.query lexical no-device smoke addendum: authenticated RuntimeDevServer relay smoke accepts retrieval.query against a seeded runtime document index with one bounded lexical snippet, source_anchor_id, rank, matched_terms, document metadata, and chunk offsets while rejecting response-only results, embedding_model_id, retrieval_context, source_path, workspace_id, citations, trusted-source, and backend metadata before document-index dispatch."
echo "Covered Android retrieval.query selected embedding-model isolation addendum: Android RuntimeClientViewModel keeps selected embedding_model_id out of retrieval.query payloads even when a memory-indexing model is selected, so current document search remains deterministic lexical retrieval before semantic retrieval, embedding-backed ranking, source approval, citation, trusted-source review, permission, or audit semantics exist."
echo "Covered RuntimeDevServer retrieval.query request bounds no-device smoke addendum: authenticated RuntimeDevServer relay smoke rejects retrieval.query query text longer than 1024 characters with invalid_payload before the seeded document-index retrieval path, matching the router and shared schema request ceiling without semantic retrieval, embeddings, source approval, citation, trusted-source review, permission, audit, Android UI, local persistence, or chat context behavior."
...
echo "Covered Android retrieval lexical metadata decode addendum: Android RetrievalQueryResultItemPayload rejects empty matched_terms arrays, over-maximum matched_terms arrays, empty matched term entries, overlong matched term entries, empty snippets, and overlong snippets during decode, and RuntimeClientViewModel rejects malformed retrieval.query lexical responses before transient source_anchor_id state, source approval, citation, trusted-source review, permission, or audit semantics exist."
echo "Covered Android document retrieval request bounds decode addendum: Android IndexDocumentsListRequestPayload rejects negative and over-maximum limit values during decode, and RetrievalQueryRequestPayload rejects blank or overlong query text, negative or over-maximum limit values, and negative or over-maximum max_snippet_characters values before runtime dispatch, semantic retrieval, source approval, citation, trusted-source review, permission, or audit semantics exist."
...
echo "Covered protocol retrieval.query request bounds sample addendum: protocol schema hygiene now pins retrieval.query request properties to query, limit, and max_snippet_characters and rejects string, fractional, boolean, negative, and over-maximum limit or max_snippet_characters request samples before semantic retrieval, source approval, citation, trusted-source review, permission, or audit semantics exist."
echo "Covered protocol retrieval.query query-length and response result sample addendum: protocol schema hygiene now caps request query text at 1024 characters and rejects oversized query samples while response result samples reject unknown result metadata, missing required rank, empty snippet or matched terms, and string, fractional, boolean, or negative chunk_index, start_character_offset, end_character_offset, and rank values before semantic retrieval, source approval, citation, trusted-source review, permission, or audit semantics exist."
echo "Covered protocol retrieval.query positive-rank wire-shape addendum: protocol schema hygiene now requires response result rank values to be positive integers and rejects zero-rank response samples before semantic retrieval, source approval, citation, trusted-source review, permission, or audit semantics exist."
echo "Covered protocol retrieval.query result ordering and offset sanity addendum: runtime store, SQLite parity, router tests, RuntimeDevServer smoke, and protocol schema hygiene now pin retrieval.query result limits, non-increasing deterministic rank order, and end_character_offset >= start_character_offset before semantic retrieval, source approval, citation, trusted-source review, permission, or audit semantics exist."
echo "Covered protocol index.documents.list request and response sample addendum: protocol schema hygiene now accepts empty or bounded-limit catalog requests, rejects malformed limits, response-only documents or summary fields, and unknown request metadata, and validates catalog response document, summary, and quality-count samples against the safe metadata-only shape before semantic retrieval, source approval, citation, trusted-source review, permission, or audit semantics exist."
echo "Covered protocol index.documents.list quality-count completeness addendum: macOS runtime catalog responses serialize no_usable_text, single_chunk, and chunked quality_counts with zero defaults, while protocol schema hygiene rejects missing nested quality-count keys before semantic retrieval, source approval, citation, trusted-source review, permission, or audit semantics exist."
echo "Covered protocol index.documents.list content-fingerprint wire-shape addendum: protocol schema hygiene now pins catalog document content_fingerprint values to 16 lowercase hex characters and rejects empty, whitespace-mutated, uppercase, short, long, and non-hex response samples before semantic retrieval, source approval, citation, trusted-source review, permission, or audit semantics exist."
echo "Covered Android document content-fingerprint protocol parity addendum: Android ProtocolCodecTest now serializes and decodes catalog and retrieval document content_fingerprint samples as 16 lowercase hex characters, and RuntimeDevServer relay smoke requires catalog and retrieval fingerprints to match the same shape before semantic retrieval, source approval, citation, trusted-source review, permission, or audit semantics exist."
...
echo "Covered Android retrieval lexical metadata transient-state bounds addendum: Android RuntimeClientViewModel now coerces retrieval.query transient rank values to positive integers, keeps source offsets ordered after nonnegative coercion, caps snippets at 480 characters, and keeps matched_terms to 16 distinct trimmed nonblank terms of 64 characters or less before local persistence, chat context injection, semantic retrieval, source approval, citation, trusted-source review, permission, or audit semantics exist."
echo "Covered protocol document quality/chunk-count consistency addendum: protocol schema hygiene now binds indexDocument quality to chunk_count for catalog and retrieval documents and rejects zero-chunk, single-chunk, and multi-chunk mismatch samples before semantic retrieval, source approval, citation, trusted-source review, permission, or audit semantics exist."
echo "Covered protocol document MIME type wire-shape addendum: protocol schema hygiene now pins catalog and retrieval document mime_type values to lowercase type/subtype tokens and rejects whitespace-mutated, uppercase, missing-slash, parameterized, URL-shaped, and overlong MIME samples before semantic retrieval, source approval, citation, trusted-source review, permission, or audit semantics exist."
echo "Covered protocol document metadata string-bounds and retrieval nested-document parity addendum: protocol schema hygiene now caps catalog and retrieval document id and MIME type values at 128 characters, display_name at 256 characters, requires retrieval.query result documents to reuse indexDocument, and rejects malformed nested retrieval document metadata samples before semantic retrieval, source approval, citation, trusted-source review, permission, or audit semantics exist."
echo "Covered protocol retrieval.query matched-terms bounds addendum: protocol schema hygiene now requires non-empty retrieval.query response matched_terms arrays, caps them at 16 terms, caps each matched term at 64 characters, and rejects empty, 17-term, and overlong matched-term response samples before semantic retrieval, source approval, citation, trusted-source review, permission, or audit semantics exist."
echo "Covered protocol retrieval.query snippet bounds addendum: protocol schema hygiene now caps retrieval.query response snippets at 500 characters and rejects overlong snippet response samples before semantic retrieval, source approval, citation, trusted-source review, permission, or audit semantics exist."
echo "Covered protocol document retrieval response array bounds addendum: protocol schema hygiene now caps index.documents.list response documents and retrieval.query response results at 100 items and rejects 101-row response samples before semantic retrieval, source approval, citation, trusted-source review, permission, or audit semantics exist."
...
echo "Covered RuntimeDevServer reserved RAG/research namespace rejection addendum: authenticated RuntimeDevServer relay smoke rejects embeddings.create, index.build, research.brief.create, citation.sources.list, and source_control.status with unknown_message_type before any production embedding, semantic retrieval, indexing, research, citation, source-control, or trusted-source path exists."
...
echo "Covered RuntimeDevServer reserved trusted-source namespace rejection addendum: authenticated RuntimeDevServer relay smoke rejects trusted_source.approve with unknown_message_type before any source approval, citation, trusted-source review, permission, or audit semantics exist."
...
echo "Covered Android models.pull result and chat.cancel acknowledgement closed-payload app-path addendum: RuntimeClientViewModel rejects unknown models.pull result metadata and unknown chat.cancel acknowledgement metadata before model install state mutation, model refresh fanout, streaming cancellation, or device storage mutation, ignores stale model-pull result request ids, and preserves pending/active request state for canonical retry before live-provider behavior, production relay/session/encryption, direct Android backend access, or physical Android proof exists."
...
echo "Covered Android auth.challenge closed-payload app-path addendum: RuntimeClientViewModel rejects unknown auth.challenge response metadata before device identity loading, runtime proof verification, auth.response signing/sending, authenticated session state, route-refresh scheduling, or authenticated refresh fanout, then signs a canonical auth.challenge retry while keeping backend_url canaries out of state/storage before source approval, citation, trusted-source review, permission, or audit semantics exist."
...
echo "Covered Android model pull and chat cancel request bounds decode addendum: Android ModelPullPayload rejects blank model values and ChatCancelPayload rejects blank target_request_id values during decode before runtime model installation dispatch, active generation cancellation, local persistence, live-provider behavior, production relay/session/encryption, direct Android backend access, or physical Android proof exists."
...
echo "Covered production P2P/NAT security design addendum: a hash-pinned portfolio defines authenticated encrypted candidate exchange, paired-identity-bound session establishment, replay and expiry rules, candidate privacy, consent freshness, path validation, downgrade resistance, and encrypted relay fallback. This validates static design artifacts only; it does not claim a concrete P2P connector, STUN/TURN traffic, candidate exchange, hole punching, NAT behavior, public-network reachability, latency or memory measurements, optical QR, or physical Android proof."
