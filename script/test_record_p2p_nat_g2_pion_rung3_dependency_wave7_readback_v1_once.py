#!/usr/bin/env python3
"""Tests for the one-use offline Wave7 acquisition readback recorder."""
...
PATH = Path(__file__).with_name(
    "record_p2p_nat_g2_pion_rung3_dependency_wave7_readback_v1_once.py"
)
SPEC = importlib.util.spec_from_file_location("wave7_readback_recorder_tests", PATH)
...
class Wave7ReadbackRecorderTests(unittest.TestCase):
    ...
    def test_05_live_retained_snapshot_verifies_twice_read_only(self):
        ...
        try:
            ...
            self.assertEqual(first["acceptedResourceCount"], 30)
            self.assertEqual(first["authorityFileCount"], 14)
            self.assertEqual(first["selectedTupleCount"], 0)
            self.assertEqual(first["aggregateAcceptedBytes"], 32_352_251)
            self.assertEqual(first["aggregateZipEntryCount"], 6_469)
            self.assertEqual(
            ...
            self.assertEqual(len(first["resources"]), 30)
        finally:
            snapshot.close()
        self.assertFalse(os.path.lexists(claim))
    ...
    def test_10_synthetic_execute_orders_claim_two_passes_receipt_manifest(self):
        ...
        def verify(snapshot):
            ...
            return {"acceptedResourceCount": 30}
    ...
    def test_12_receipt_only_gap_is_terminal_uncertainty(self):
        ...
        with mock.patch.object(
            ...
            R,
            "verify_snapshot",
            return_value={"acceptedResourceCount": 30},
        ), mock.patch.object(R, "atomic_publish", side_effect=publish):
            with self.assertRaises(R.ReadbackError) as caught:
                R.execute(
                    Path("/unused"),
    ...
    def test_21_wave7_uncompressed_limits_are_per_zip_and_across_all(self):
        self.assertEqual(
