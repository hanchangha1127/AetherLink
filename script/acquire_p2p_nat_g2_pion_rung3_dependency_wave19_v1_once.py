#!/usr/bin/env python3
"""Consume the Wave19 v1 permit once and acquire 4 verified resources."""
...
if not (
    ...
    raise RuntimeError("Wave19 acquisition requires `python3 -I -B -S`")
...
CHECKER_PATH = Path(__file__).with_name(
    "check_p2p_nat_g2_pion_rung3_dependency_wave19_acquisition_v1.py"
)
# Finalized reverse pin to the exact Wave19 acquisition checker bytes. The
...
EXPECTED_CHECKER_RAW = "9fc0b1eae88a029ae3f3c180acdfbb2f296736c8745cebc75248efe3aa2bc435"
EXPECTED_WAVE19_IDENTITY = (
    (
        "golang.org/x/crypto",
        "v0.38.0",
...
EXPECTED_WAVE19_RESOURCE_CONTRACT_SHA256 = (
...
class ImmutablePhaseLedger:
    ...
    def source_acquisition_state(self) -> str:
        ...
        if self.response_committed_count == 4:
...
class ExecutionNamespace:
    ...
    def barrier(self, state: ExecutionState) -> None:
        ...
        if state in {
            ...
            require(
                len(self.resources) == 4 and self.evidence is not None,
                "E_CARDINALITY",
                phase,
            )
...
def _validate_resource_contract(
    ...
    require(
        ...
        and permit["requestContract"]["requestCount"] == 4
        and type(permit["requestContract"]["tupleCount"]) is int
        and permit["requestContract"]["tupleCount"] == 2
        and len(rows) == 4
    ...
    for index, source in enumerate(rows):
        ...
        require(
            ...
            and row["tupleId"] == f"wave19-{tuple_order:03d}-{digest[:12]}"
            and row["kind"] == kind
            and row["method"] == "GET"
    ...
    require(
        len({row["acceptedFileName"] for row in result}) == 4
        and len({row["url"] for row in result}) == 4
        and all(
...
def _attempt(
    ...
    *,
    ...
    try:
        ...
        evidence_payload = {
            "documentType": "aetherlink.wave19-source-acquisition-evidence",
            "schemaVersion": "1.0",
            "attemptId": attempt_id,
            "requestCount": expected_count,
        ...
        receipt = {
            "documentType": "aetherlink.wave19-source-acquisition-receipt",
            "schemaVersion": "1.0",
        ...
        failure = {
            "documentType": "aetherlink.wave19-source-acquisition-failure",
            "schemaVersion": "1.0",
...
def validate_execution_context() -> dict[str, Any]:
    ...
    return {
        "documentType":
            "aetherlink.wave19-source-acquisition-execution-context-check",
        "schemaVersion": "1.0",
...
def error_document(error: AcquisitionError) -> dict[str, Any]:
    ...
    return {
        "documentType": "aetherlink.wave19-source-acquisition-error",
        "schemaVersion": "1.0",
        "status": status,
