#!/usr/bin/env python3
"""Consume the Wave17 v1 permit once and acquire 2 verified resources."""
...
if not (
    ...
    raise RuntimeError("Wave17 acquisition requires `python3 -I -B -S`")
...
CHECKER_PATH = Path(__file__).with_name(
    "check_p2p_nat_g2_pion_rung3_dependency_wave17_acquisition_v1.py"
)
# Finalized reverse pin to the exact Wave17 acquisition checker bytes. The
...
EXPECTED_CHECKER_RAW = "acd6af5f174569c0b3d988d4528cf9b6219c9b5c5b6ba9205db982506b0e7b81"
EXPECTED_WAVE17_IDENTITY = (
    (
        "golang.org/x/tools",
        "v0.33.0",
...
EXPECTED_WAVE17_RESOURCE_CONTRACT_SHA256 = (
...
def _assert_primitive_contract() -> None:
    require(
        ...
        and WAVE4.CHECK.MAX_ZIP_NAME_BYTES
        == CHECK.MAX_ZIP_NAME_BYTES
        and VALIDATION.CHECK.MAX_ZIP_NAME_BYTES
        == CHECK.MAX_ZIP_NAME_BYTES
...
class ImmutablePhaseLedger:
    ...
    def source_acquisition_state(self) -> str:
        ...
        if self.response_committed_count == 2:
...
class ExecutionNamespace:
    ...
    def barrier(self, state: ExecutionState) -> None:
        ...
        if state in {
            ...
            require(
                len(self.resources) == 2 and self.evidence is not None,
                "E_CARDINALITY",
                phase,
            )
...
def _validate_resource_contract(
    ...
    require(
        ...
        and permit["requestContract"]["requestCount"] == 2
        and len(rows) == 2
    ...
    for index, source in enumerate(rows):
        ...
        require(
            ...
            and row["tupleId"] == f"wave17-{tuple_order:03d}-{digest[:12]}"
            and row["kind"] == kind
            and row["method"] == "GET"
    ...
    require(
        len({row["acceptedFileName"] for row in result}) == 2
        and len({row["url"] for row in result}) == 2
        and all(
...
def _attempt(
    ...
    *,
    ...
    try:
        ...
        failure = {
            "documentType": "aetherlink.wave17-source-acquisition-failure",
            "schemaVersion": "1.0",
...
def validate_execution_context() -> dict[str, Any]:
    ...
    return {
        "documentType":
            "aetherlink.wave17-source-acquisition-execution-context-check",
        "schemaVersion": "1.0",
...
def error_document(error: AcquisitionError) -> dict[str, Any]:
    ...
    return {
        "documentType": "aetherlink.wave17-source-acquisition-error",
        "schemaVersion": "1.0",
        "status": status,
