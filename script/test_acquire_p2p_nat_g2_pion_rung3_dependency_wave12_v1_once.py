#!/usr/bin/env python3
"""Adversarial offline tests for the one-use Wave12 acquisition runner."""
...
if not (
    ...
    raise RuntimeError("Wave12 runner tests require `python3 -I -B -S`")
...
def _deny_test_network(*_args, **_kwargs):
    raise AssertionError(
        "offline Wave12 tests must never create network connections"
    )
...
EXPECTED_WAVE12_RESOURCE_CONTRACT_SHA256 = (
    "c8ca9bc4559bea59a5a52fbceaaf068fe82ab9211fa0a888d3918aaa2dec55a2"
)


RUNNER_PATH = Path(__file__).with_name(
    "acquire_p2p_nat_g2_pion_rung3_dependency_wave12_v1_once.py"
)
# The runner carries the finalized Wave12 acquisition-checker reverse pin.
...
def exact_resource_oracle() -> list[dict[str, object]]:
    ...
    for tuple_order, identity in enumerate(
        ...
        tuple_id = f"wave12-{tuple_order:03d}-{digest[:12]}"
...
class Wave12RunnerTests(unittest.TestCase):
    def setUp(self) -> None:
        self.wave4_network_patcher = mock.patch.object(
            runner.WAVE4,
            "direct_fetch",
            side_effect=AssertionError(
                "offline Wave12 tests must never reach the network"
            ),
        )
        ...
        self.https_network_patcher = mock.patch(
            ...
            side_effect=AssertionError(
                "offline Wave12 tests must never create HTTPS connections"
            ),
        )
    ...
    def test_exact_four_tuple_eight_resource_contract_is_closed(
        ...
        self.assertEqual(
            runner.EXPECTED_WAVE12_IDENTITY,
            EXPECTED_WAVE12_IDENTITY,
        )
        ...
        self.assertEqual(
            runner.EXPECTED_WAVE12_RESOURCE_CONTRACT_SHA256,
            EXPECTED_WAVE12_RESOURCE_CONTRACT_SHA256,
        )
        self.assertEqual(
        ...
        self.assertTrue(
            authority[
                "wave12PublicProxy8GetAcquisitionAuthorizedOnce"
            ]
        )
        self.assertTrue(
    ...
    def test_every_false_authority_flag_rejects_true_and_zero_before_claim(
        self,
    ) -> None:
        rows, _ = fixture()
    ...
    def test_lower_ascii_direct_proxy_identity_is_enforced(self) -> None:
        def rewritten_pair(module: str, version: str):
            ...
            for index, kind in enumerate(("mod", "zip")):
                ...
                rows[index]["tupleId"] = f"wave12-001-{digest[:12]}"
    ...
    def test_wave12_direct_fetch_reaches_wave4_and_translates_errors(
        self,
    ) -> None:
        resource = fixture()[0][0]
