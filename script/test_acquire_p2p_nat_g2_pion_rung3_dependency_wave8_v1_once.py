#!/usr/bin/env python3
"""Adversarial offline tests for the one-use Wave8 acquisition runner."""
...
if not (
    ...
    raise RuntimeError("Wave8 runner tests require `python3 -I -B -S`")
...
RUNNER_PATH = Path(__file__).with_name(
    "acquire_p2p_nat_g2_pion_rung3_dependency_wave8_v1_once.py"
)
SPEC = importlib.util.spec_from_file_location(
    "wave8_source_acquirer_v1",
    RUNNER_PATH,
)
...
class Wave8RunnerTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_success_claims_before_28_fetches_and_manifest_is_last(self) -> None:
        rows, bodies = fixture()
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(len(list(accepted.iterdir())), 28)
            self.assertTrue(
                terminal(root, runner.CHECK.CLAIM_PATH).exists()
            )
            self.assertTrue(
    ...
    def test_canonical_preflight_context_composes_without_writes(self) -> None:
        ...
        x_term = [
            ...
            if row["module"] == "golang.org/x/term"
            and row["version"] in {"v0.36.0", "v0.5.0"}
        ]
        self.assertEqual(
            {
                row["expectedH1"]
        ...
        self.assertEqual(
            len({row["acceptedFileName"] for row in x_term}),
            4,
        )
        result = runner.validate_execution_context()
        self.assertTrue(result["validationPassed"])
        self.assertEqual(result["requestCount"], 28)
    ...
    def test_semantic_or_cardinality_mutation_fails_before_claim(self) -> None:
        ...
        mutations = (
            ...
            lambda rows: rows.__setitem__(27, dict(rows[26])),
            lambda rows: rows[27].__setitem__(
                "requestOrdinal",
                29,
            ),
    ...
    def test_lower_ascii_direct_proxy_identity_is_enforced(self) -> None:
        def rewritten_pair(module: str, version: str):
            ...
            for index, kind in enumerate(("mod", "zip")):
                ...
                rows[index]["tupleId"] = f"wave8-001-{digest[:12]}"
    ...
    def test_wave8_direct_fetch_reaches_wave4_and_translates_errors(
        self,
    ) -> None:
        resource = fixture()[0][0]
