#!/usr/bin/env python3
"""Tests for the Wave13 acquisition readback permit package."""
...
def _deny_test_network(*_args, **_kwargs):
    ...
    raise AssertionError(
        "offline Wave13 readback tests must never create network connections"
    )
...
SPEC = importlib.util.spec_from_file_location("wave13_readback_permit_tests", PATH)
...
def _synthetic_package_raw() -> tuple[dict[str, bytes], str, str]:
    ...
    reader_raw = b"synthetic Wave13 readback reader\\n"
...
class Wave13ReadbackPermitTests(unittest.TestCase):
    def tearDown(self) -> None:
        self.assertEqual(NETWORK_ATTEMPTS, [])

    def test_01_frozen_snapshot_constants_are_exact(self):
        self.assertEqual(C.ATTEMPT_ID, "eb05816e0b897ea8c3ad8b7089668e91")
        ...
        acquisition_permit_path = (
            f"{C.BASE}/bounded-dependency-source-acquisition-wave13-"
        ...
        self.assertEqual(payload["aggregateZipEntryCount"], 1_647)
        self.assertEqual(
        ...
        v11 = acquisition_permit["predecessorBindings"][
        ...
        self.assertEqual(
            payload["predecessorBindings"]["combinedFixedPointV11"][
                "contentSha256"
            ],
            C.EXPECTED_V11_CONTENT,
        )
        self.assertFalse(
            any(
    ...
    def test_08_wave12_cardinality_hash_aggregate_and_limit_drift_rejects(self):
        original, _ = _synthetic_permit()
        ...
        def authority_14(value):
            ...
            value["frozenAcquisitionSnapshot"]["acquisitionAuthority"] = [
                ...
                if not row["path"].endswith("/.wave-12-v1.claim")
            ]
        ...
        def bound_rows_13(value):
            ...
            predecessor.pop("wave12NamespaceAnchor")

        def predecessor_v10(value):
        ...
        def wave12_hashes(value):
            value["frozenAcquisitionSnapshot"].update(
                {
                    "frozenFilesCanonicalSha256": (
                        "3d7365fd16905b8fb3c3d8682e8803c5"
                        "487b120b3ba8f49b75d16eba2bc354fd"
                    ),
                    "acceptedResourceHashSetCanonicalSha256": (
                        "38e9b110d8857fe1644a8fc80bb4a584"
                        "da540981d43ee19b62e3e9e845422a2c"
                    ),
                }
            )

        def wave12_limits(value):
            ...
            self.assertEqual(
                {
                    ...
                    for token in ("wave12", "wave-12", "v10")
                },
                expected_counts,
            )
