#!/usr/bin/env python3
"""Consume the Wave15 v1 permit once and acquire 10 verified resources."""
...
if not (
    ...
    raise RuntimeError("Wave15 acquisition requires `python3 -I -B -S`")
...
# Finalized reverse pin to the exact Wave15 acquisition checker bytes. The
...
EXPECTED_CHECKER_RAW = "41030a9cefaf31c08ead2ea6f109225c6a45f95c0d816a24a67a1ace8f2e5fe8"
...
class ImmutablePhaseLedger:
    ...
    def source_acquisition_state(self) -> str:
        ...
        if self.response_committed_count == 10:
...
class ExecutionNamespace:
    ...
    def barrier(self, state: ExecutionState) -> None:
        ...
        if state in {
            ...
            require(
                len(self.resources) == 10 and self.evidence is not None,
                "E_CARDINALITY",
                phase,
            )
...
def _validate_resource_contract(
    ...
    require(
        ...
        and permit["requestContract"]["requestCount"] == 10
        and len(rows) == 10
    ...
    for index, source in enumerate(rows):
        ...
        require(
            ...
            and row["tupleId"] == f"wave15-{tuple_order:03d}-{digest[:12]}"
            and row["kind"] == kind
            and row["method"] == "GET"
    ...
    require(
        len({row["acceptedFileName"] for row in result}) == 10
        and len({row["url"] for row in result}) == 10
        and all(
...
def _attempt(
    ...
    *,
    ...
    try:
        ...
        failure = {
            "documentType": "aetherlink.wave15-source-acquisition-failure",
            "schemaVersion": "1.0",
...
def validate_execution_context() -> dict[str, Any]:
    ...
    return {
        "documentType":
            "aetherlink.wave15-source-acquisition-execution-context-check",
        "schemaVersion": "1.0",
...
def error_document(error: AcquisitionError) -> dict[str, Any]:
    ...
    return {
        "documentType": "aetherlink.wave15-source-acquisition-error",
        "schemaVersion": "1.0",
        "status": status,
