#!/usr/bin/env python3
"""Consume the Wave14 v1 permit once and acquire 8 verified resources."""
...
if not (
    ...
    raise RuntimeError("Wave14 acquisition requires `python3 -I -B -S`")
...
# Finalized reverse pin to the exact Wave14 acquisition checker bytes. The
...
EXPECTED_CHECKER_RAW = "a19f4f9c89f92f694b4de2d54646a9033af3872f4f8fb461e667b31463255b9f"
...
def _validate_resource_contract(
    ...
    for index, source in enumerate(rows):
        ...
        require(
            ...
            and row["tupleId"] == f"wave14-{tuple_order:03d}-{digest[:12]}"
            and row["kind"] == kind
            and row["method"] == "GET"
...
def _attempt(
    ...
    *,
    ...
    try:
        ...
        failure = {
            "documentType": "aetherlink.wave14-source-acquisition-failure",
            "schemaVersion": "1.0",
...
def validate_execution_context() -> dict[str, Any]:
    ...
    return {
        "documentType":
            "aetherlink.wave14-source-acquisition-execution-context-check",
        "schemaVersion": "1.0",
...
def error_document(error: AcquisitionError) -> dict[str, Any]:
    ...
    return {
        "documentType": "aetherlink.wave14-source-acquisition-error",
        "schemaVersion": "1.0",
        "status": status,
