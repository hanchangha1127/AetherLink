#!/usr/bin/env python3
"""Validate the read-only Wave10 identity and acquisition decision.

Run only with ``python3 -I -B -S``.  The checker executes the exact pinned
combined-v8 candidate, then scans the retained 277 inputs twice using only the
...
def require_isolated_interpreter() -> None:
    ...
    if not (
        ...
        raise RuntimeError(
            "Wave10 decision checker requires unoptimized "
            "`python3 -I -B -S`"
        )
...
CHECKER_ID = "g2-pion-ice-v4.3.0-wave10-identity-acquisition-decision-check-v1"
DECISION_ID = (
    "g2-pion-ice-v4.3.0-rung3-bounded-dependency-source-identity-and-"
    "acquisition-decision-wave10-v1"
)
DEPENDENCY_ROOT = "build/offline-source/pion-ice-v4.3.0/dependencies"
NAMESPACE_ANCHOR_PATH = f"{DEPENDENCY_ROOT}/.wave-9-v1.claim"
NAMESPACE_ANCHOR_RAW_SHA256 = (
    "84957554fe937841165f38a2418613e4e740082bea4f55538b21324dfe6d45f4"
)
WAVE10_CLAIM_PATH = f"{DEPENDENCY_ROOT}/.wave-10-v1.claim"
WAVE10_STAGING_PREFIX = ".wave-10-v1-staging-"
WAVE10_ACCEPTED_PATH = f"{DEPENDENCY_ROOT}/wave-10-v1/accepted"
...
EXPECTED_FRONTIER_COUNT = 11
EXPECTED_GRAPH_SELECTED_TUPLE_COUNT = 1
EXPECTED_GO_SUM_ENTRY_COUNT = 97
EXPECTED_PARENT_DECLARATION_COUNT = 15
EXPECTED_GO_MOD_H1_WITNESS_COUNT = 107
EXPECTED_MODULE_ZIP_H1_WITNESS_COUNT = 15
...
EXPECTED_NON_CLAIMS = (
    ...
    "wave10_dependency_source_not_acquired",
...
def portable_name(value: str) -> str:
    require(type(value) is str, "E_WAVE10_NAMESPACE")
    return unicodedata.normalize("NFC", value).casefold()


def validate_wave10_namespace_absent(
    ...
    try:
        ...
        raise DecisionFailure("E_WAVE10_NAMESPACE") from error
    require(
    ...
    for name in dependency_names:
        ...
        require(
            not normalized.startswith(".wave-10-v1")
            and not normalized.startswith("wave-10-v1"),
            "E_WAVE10_NAMESPACE",
        )
    ...
    for name in docs_names:
        ...
        require(
            not normalized.startswith(
                "bounded-dependency-source-acquisition-wave10"
            ),
            "E_WAVE10_NAMESPACE",
        )
    ...
    for name in script_names:
        normalized = portable_name(name)
        if "p2p_nat_g2_pion" in normalized and "wave10" in normalized:
            require(
...
class BootstrapPinnedCodeFile:
    """Immediate-ownership bootstrap pin matching combined-v8 semantics."""
...
def load_v8_checker(
    ...
    module = types.ModuleType("aetherlink_combined_v8_pinned")
    module.__dict__.update(
        {
            "__cached__": None,
            "__file__": str(ROOT / V8_CHECKER_PATH),
            "__loader__": None,
            "__name__": "aetherlink_combined_v8_pinned",
            "__package__": None,
        }
    )
    try:
        ...
        raise DecisionFailure("E_V8_LOAD") from error
    for name in (
        "PinnedCodeFile",
        "SafeHeldInputSet",
        "generate_candidate",
...
def require_closed_identity(scan: Mapping[str, Any]) -> None:
    ...
    require(
        ...
        and scan["archiveCount"] == 139
        and scan["externalModCount"] == 138
...
def validate_v8_candidate(
    candidate: Mapping[str, Any],
) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    require(type(candidate) is dict, "E_V8_CANDIDATE")
    ...
    require(
        ...
        == (
            ...
            "wave4-wave5-wave6-wave7-wave8-wave9-"
    ...
    require(
        ...
        and inputs.get("resourceCount") == 276
        and inputs.get("modCount") == 138
        and inputs.get("zipCount") == 138
        ...
        and inputs.get("uniqueModuleVersionTupleCount") == 138
        and inputs.get("aggregateRawByteSize") == 243_215_320
    ...
    require(
        ...
        == 14
        and verification.get("pinnedV7PredecessorExecuted") is True
        and verification.get("v7TestsBindingScope")
        == "historical_metadata_only_not_live_held"
        and verification.get("v7TestsLiveHeld") is False
        and type(predecessor_verification) is dict
        and predecessor_verification.get("v7TestsBindingScope")
        == "historical_metadata_only_not_live_held"
        and predecessor_verification.get("v7TestsLiveHeld") is False
        ...
        and verification.get("wave9RetainedFdPreManifestBarrierCount")
        == 3
        and verification.get("wave9CompletionAppliesToRetainedSnapshot")
        is True
        and verification.get(
        ...
        and verification.get("hardenedCheckerModuleCount") == 7
        and verification.get("providerFacadeLoadCount") == 7
        ...
        and counters.get("directArchiveOpenCount") == 278
        and counters.get("inheritedArchiveOpenCount") == 1_088
        and counters.get("totalArchiveOpenCount") == 1_366
        and counters.get("archiveOpenCount") == 1_366
        and counters.get("heldSourceInputCount") == 277
        and counters.get("heldTerminalEvidenceCount") == 59
        and counters.get("heldAuxiliaryEvidenceCount") == 3
        and counters.get("heldToolInputCount") == 7
        and counters.get("transitiveDistinctToolPathCount") == 9
        and counters.get("stableReadPassesPerHeldInput") == 2
        and counters.get("directFullSourceReconstructionCount") == 2
        and counters.get("inheritedFullSourceReconstructionCount") == 12
        and counters.get("totalFullSourceReconstructionCount") == 14
        ...
        and len(tool_bindings) == 7
        and type(terminal_bindings) is list
        and len(terminal_bindings) == 59
...
def expected_tool_bindings(
    ...
    require(
        ...
        == TESTS_RAW_SHA256
        and sha256_bytes(package_raw[V8_CHECKER_PATH])
        == V8_CHECKER_RAW_SHA256
        and sha256_bytes(
            package_raw[V8_TESTS_PATH]
        )
        == V8_TESTS_RAW_SHA256,
        "E_TOOL_IDENTITY",
    )
    ...
    *,
    ...
    return {
        "documentType": (
            ...
            "identity-and-acquisition-decision-wave10"
        ),
        "schemaVersion": "1.0",
        ...
        "date": "2026-07-26",
        "status": (
            "wave10_exact_11_frontier_identity_classified_"
            "11_complete_0_blocked_acquisition_ready_not_authorized"
        ),
        "result": (
            f"exact_11_version_vertices_"
            f"{EXPECTED_GRAPH_SELECTED_TUPLE_COUNT}_selected_"
            f"{EXPECTED_FRONTIER_COUNT - EXPECTED_GRAPH_SELECTED_TUPLE_COUNT}"
            "_nonselected_11_complete_h1_pairs_"
        ...
        "heldSourceInputSet": {
            ...
            "sourceBindingsSha256": V8_SOURCE_BINDINGS_SHA256,
        ...
        "sourceAcquisitionPreparation": {
            ...
            "modulePathEncoding":
                "current_wave10_lowercase_ascii_direct_proxy_path",
            "claimPath": WAVE10_CLAIM_PATH,
            "stagingDirectoryPrefix": WAVE10_STAGING_PREFIX,
            "acceptedDirectoryPath": WAVE10_ACCEPTED_PATH,
        ...
        "operationCounters": {
            "combinedV8CandidateInvocationCount": 1,
            "predecessorFullSourceReconstructionCount": 12,
            "directV8FullSourceReconstructionCount": 2,
            "totalFullSourceReconstructionCount": 14,
            "predecessorGraphArchiveOpenCount": 1088,
            "currentV8GraphArchiveOpenCount": 278,
            "totalV8GraphArchiveOpenCount": 1366,
            "identityWitnessScanCount": 2,
            "identityWitnessArchiveOpenCount": 278,
            "overallDecisionExecutionArchiveOpenCount": 1644,
        ...
        "closure": {
            "wave10IdentityResolved": True,
            "wave10AcquisitionReady": True,
            "wave10AcquisitionComplete": False,
        ...
        "nextAction": (
            "prepare_separate_one_use_22_resource_wave10_source_"
...
def validate_semantic_decision(
    ...
    combined = (
        predecessors.get("combinedFixedPointV8")
    ...
    require(
        exact_keys(predecessors, {"combinedFixedPointV8"})
        ...
        and combined["v7TestsLiveHeld"] is False
        and all(
    ...
    require(
        ...
        and held["sourceBindingCount"] == 277
        and held["sourceBindingsSha256"]
        == V8_SOURCE_BINDINGS_SHA256
        and held["archiveCount"] == 139
        and held["externalModCount"] == 138
    ...
    require(
        ...
        == "current_wave10_lowercase_ascii_direct_proxy_path"
        and preparation["claimPath"] == WAVE10_CLAIM_PATH
        and preparation["stagingDirectoryPrefix"]
        == WAVE10_STAGING_PREFIX
        and preparation["acceptedDirectoryPath"]
        == WAVE10_ACCEPTED_PATH
...
def evaluate(
    ...
    *,
    ...
    with ExitStack() as stack:
        ...
        v8_tests_held = stack.enter_context(
            v8.PinnedCodeFile(
                root,
                V8_TESTS_PATH,
                V8_TESTS_RAW_SHA256,
        ...
        package_raw = {
            ...
            V8_CHECKER_PATH: v8_held.raw,
            V8_TESTS_PATH: v8_tests_held.raw,
        }
        held: list[Any] = [
            self_held,
            v8_held,
        ...
        validate_wave10_namespace_absent(
            namespace_anchor_held.parent_fd,
            reader_held.parent_fd,
            self_held.parent_fd,
        )
        identity_barrier(root, held)
        candidate = v8.generate_candidate(root)
        identity_barrier(root, held)
        wave_rows, source_bindings = validate_v8_candidate(candidate)
        v8 = v8.harden_checker_module(v8)
        ...
        require(
            type(runner) is v8.ReadOnlyProviderFacade
            and v7.PinnedCodeFile is v8.PinnedCodeFile
            and v6.PinnedCodeFile is v8.PinnedCodeFile
            and v5.PinnedCodeFile is v8.PinnedCodeFile
            and v4.PinnedCodeFile is v8.PinnedCodeFile
            and v1.PinnedRunnerFile is v8.SafePinnedRunnerFile,
            "E_SAFE_RUNTIME",
        )
        ...
        require(
            ...
            == source_bindings
            and len(full_source_bindings) == 277
            and all(
                type(row) is dict
    ...
    return expected, {
        ...
        "schemaVersion": "1.0",
        "status":
            "validated_11_of_11_acquisition_ready_not_authorized",
...
def error_document(code: str) -> dict[str, Any]:
    return {
        "documentType":
            "aetherlink.wave10-identity-acquisition-decision-error",
        "schemaVersion": "1.0",
