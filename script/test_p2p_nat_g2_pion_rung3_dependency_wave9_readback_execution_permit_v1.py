#!/usr/bin/env python3
"""Tests for the Wave9 acquisition readback permit package."""
...
SPEC = importlib.util.spec_from_file_location("wave9_readback_permit_tests", PATH)
C = importlib.util.module_from_spec(SPEC)
assert SPEC.loader is not None
SPEC.loader.exec_module(C)


class Wave9ReadbackPermitTests(unittest.TestCase):
    def test_01_frozen_snapshot_constants_are_exact(self):
        self.assertEqual(C.ATTEMPT_ID, "df64a4816a083806020580efe953b9a7")
        self.assertEqual(len(C.ACQUISITION_AUTHORITY), 14)
        self.assertEqual(len(C.ACCEPTED_FILES), 20)
        self.assertEqual(
        ...
        self.assertEqual(third_mod["rawSha256"], fourth_mod["rawSha256"])
        payload = C.frozen_snapshot_payload()
        self.assertEqual(payload["frozenFileCount"], 38)
        self.assertEqual(
        ...
        self.assertEqual(payload["aggregateZipEntryCount"], 5_349)
        self.assertEqual(
        ...
        self.assertEqual(
            payload["predecessorBindings"]["combinedFixedPointV7"][
                "contentSha256"
            ],
            C.EXPECTED_V7_CONTENT,
        )
    ...
    def test_03_exact_package_and_package_only_preflight(self):
        ...
        self.assertEqual(summary["acceptedResourceCount"], 20)
